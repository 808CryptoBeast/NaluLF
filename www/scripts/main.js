import{a as yr,b as to,c as fn,d as Ct,e as y,f as Ie,g,h as Fe,i as R,j as z,k as Z,l as te,m as dt,n as be,o as ne,p as he,q as _t,r as O,s as na,v as Me,w as wr}from"./chunk-R7KFE7J7.js";function hn(e){fn.includes(e)||(e="gold"),O.currentTheme=e,fn.forEach(t=>document.body.classList.remove(`theme-${t}`)),document.body.classList.add(`theme-${e}`),te(to,e)}function no(){let e=fn.indexOf(O.currentTheme);hn(fn[(e+1)%fn.length])}function xr(){let e=Z(to);hn(fn.includes(e)?e:"gold")}var kr={landing:"landing-page",dashboard:"dashboard",inspector:"inspector",profile:"dashboard"};function so(e){document.body.classList.remove("modal-open"),Object.values(kr).forEach(i=>document.body.classList.remove(i)),document.body.classList.add(kr[e]||"dashboard");let t=y("landing"),n=y("dashboard"),s=y("profile-page");t&&(t.style.display=e==="landing"?"":"none"),n&&(n.style.display=e==="dashboard"?"":"none"),s&&(s.style.display=e==="profile"?"":"none");let a=e==="landing",o={landingActions:y("navbar-landing-actions"),dashActions:y("navbar-dash-actions"),navConn:y("navbar-conn"),cmdkHint:y("cmdk-hint")};o.landingActions&&(o.landingActions.style.display=a?"":"none"),o.dashActions&&(o.dashActions.style.display=a?"none":""),o.navConn&&(o.navConn.style.display=a?"none":""),o.cmdkHint&&(o.cmdkHint.style.display=a?"none":""),O.currentPage=e,window.scrollTo({top:0,behavior:"smooth"}),window.dispatchEvent(new CustomEvent("naluxrp:pagechange",{detail:{pageId:e}}))}function gn(){so("landing")}function vn(){so("dashboard")}function ao(){so("profile")}function bn(e,t){Ie(".dash-tab").forEach(n=>{n.classList.toggle("active",n===e),n.setAttribute("aria-selected",String(n===e))}),["stream","inspector","network"].forEach(n=>{let s=y(`tab-${n}`);s&&(s.style.display=n===t?"":"none")}),t==="inspector"?(document.body.classList.remove("dashboard"),document.body.classList.add("inspector")):(document.body.classList.remove("inspector"),document.body.classList.add("dashboard")),O.currentTab=t,window.dispatchEvent(new CustomEvent("naluxrp:tabchange",{detail:{tabId:t}}))}var Pt="naluxrp_vault_meta",Rt="naluxrp_vault_data",Bn="naluxrp_session",io="nalulf_used_names",ro="nalulf_used_emails",lo="nalulf_used_domains",oo=15e4,sa="naluxrp_v2",Ne={_key:null,_vault:null,_lockTimer:null,AUTO_LOCK_MS:30*60*1e3,get isUnlocked(){return this._key!==null&&this._vault!==null},get vault(){return this._vault},hasVault(){return!!Z(Pt)&&!!Z(Rt)},async create(e,t,n,s){let a=crypto.getRandomValues(new Uint8Array(32));this._key=await this._deriveKey(e,a);let o=(s||t).toLowerCase().replace(/[^a-z0-9_]/g,"");return this._vault={checksum:sa,identity:{name:t,email:n,domain:o,createdAt:new Date().toISOString()},profile:{},wallets:[],social:{}},te(Pt,JSON.stringify({salt:Array.from(a),iterations:oo,version:sa})),await this._persist(),this._startLockTimer(),this._vault},async unlock(e){let t=be(Z(Pt));if(!t)throw new Error("No account found. Create one first.");this._key=await this._deriveKey(e,new Uint8Array(t.salt));let n;try{let s=be(Z(Rt));if(!s)throw new Error("missing");n=await this._decrypt(s)}catch{throw this._key=null,new Error("Incorrect password. Please try again.")}if((n==null?void 0:n.checksum)!==sa)throw this._key=null,new Error("Account data corrupted. Restore from backup.");return this._vault=n,this._startLockTimer(),this._vault},async update(e){if(!this.isUnlocked)throw new Error("Sign in to continue.");e(this._vault),await this._persist()},lock(){this._key=null,this._vault=null,clearTimeout(this._lockTimer),this._lockTimer=null},resetTimer(){this.isUnlocked&&this._startLockTimer()},async changePassword(e){if(!this.isUnlocked)throw new Error("Sign in first.");let t=crypto.getRandomValues(new Uint8Array(32));this._key=await this._deriveKey(e,t),te(Pt,JSON.stringify({salt:Array.from(t),iterations:oo,version:sa})),await this._persist()},async exportBlob(){if(!this.isUnlocked)throw new Error("Sign in before exporting.");let e={vault:be(Z(Rt)),meta:be(Z(Pt)),exportedAt:new Date().toISOString()},t=URL.createObjectURL(new Blob([JSON.stringify(e,null,2)],{type:"application/json"}));Object.assign(document.createElement("a"),{href:t,download:`naluxrp-backup-${Date.now()}.json`}).click(),URL.revokeObjectURL(t)},async _deriveKey(e,t){let n=new TextEncoder,s=await crypto.subtle.importKey("raw",n.encode(e),"PBKDF2",!1,["deriveKey"]);return crypto.subtle.deriveKey({name:"PBKDF2",salt:t,iterations:oo,hash:"SHA-256"},s,{name:"AES-GCM",length:256},!1,["encrypt","decrypt"])},async _encrypt(e){let t=crypto.getRandomValues(new Uint8Array(12)),n=new TextEncoder,s=await crypto.subtle.encrypt({name:"AES-GCM",iv:t},this._key,n.encode(JSON.stringify(e)));return{iv:Array.from(t),cipher:Array.from(new Uint8Array(s))}},async _decrypt(e){let t=new TextDecoder,n=await crypto.subtle.decrypt({name:"AES-GCM",iv:new Uint8Array(e.iv)},this._key,new Uint8Array(e.cipher).buffer);return JSON.parse(t.decode(n))},async _persist(){!this._key||!this._vault||te(Rt,JSON.stringify(await this._encrypt(this._vault)))},_startLockTimer(){clearTimeout(this._lockTimer),this._lockTimer=setTimeout(()=>{this.lock(),window.dispatchEvent(new CustomEvent("naluxrp:vault-locked"))},this.AUTO_LOCK_MS)}},tt=1;function po(e){var t;Ne.hasVault()?qt(e||"login"):qt("welcome"),(t=y("auth-overlay"))==null||t.classList.add("show")}function Vt(){var e;(e=y("auth-overlay"))==null||e.classList.remove("show"),Gt(),tt=1,yn(1)}function qt(e){var s,a,o,i,r;ra(".auth-view").forEach(l=>l.classList.remove("active")),(s=y(`auth-view-${e}`))==null||s.classList.add("active"),(a=y("auth-overlay"))==null||a.setAttribute("data-view",e);let t=["welcome","forgot","sync","celebrate","syncaware"].includes(e),n=y("auth-tab-row");n&&(n.style.display=t?"none":""),(o=y("tab-login-btn"))==null||o.classList.toggle("active",e==="login"),(i=y("tab-signup-btn"))==null||i.classList.toggle("active",e==="signup"),(r=y("tab-sync-btn"))==null||r.classList.toggle("active",e==="sync"),e==="signup"&&(tt=1,yn(1),Fn()),Gt()}function yn(e){[1,2,3].forEach(s=>{let a=y(`signup-step-${s}`);a&&(a.style.display=s===e?"":"none");let o=y(`signup-dot-${s}`);o&&(o.classList.toggle("active",s===e),o.classList.toggle("done",s<e))});let t=["","Step 1 of 3 \u2014 Identity","Step 2 of 3 \u2014 Security","Step 3 of 3 \u2014 Sync setup"],n=y("signup-step-label");n&&(n.textContent=t[e]||"")}function uo(){var e,t,n,s,a,o;if(Gt(),tt===1){let i=((e=y("inp-signup-name"))==null?void 0:e.value.trim())||"",r=((t=y("inp-signup-email"))==null?void 0:t.value.trim())||"",l=((n=y("inp-signup-domain"))==null?void 0:n.value.trim())||"";if(!i||i.length<3)return Ae("Display name must be at least 3 characters.");if(Pr(i))return Ae(`"${i}" is already in use on this device.`);if(!r||!r.includes("@"))return Ae("Enter a valid email address.");if(Lr(r))return Ae("That email is already registered on this device.");if(l&&Mr(l))return Ae(`@${l} is already taken on this device.`);if(l&&!/^[a-z0-9_]{2,30}$/.test(l))return Ae("Handle: 2-30 lowercase letters, numbers, underscores only.");tt=2,yn(2),Fn(),setTimeout(()=>{var c;return(c=y("inp-signup-pass"))==null?void 0:c.focus()},80)}else if(tt===2){let i=((s=y("inp-signup-pass"))==null?void 0:s.value)||"",r=((a=y("inp-signup-confirm"))==null?void 0:a.value)||"";if(!i||i.length<8)return Ae("Password must be at least 8 characters.");if(!Xr(i))return Ae("Add uppercase, lowercase, and a number.");if(i!==r)return Ae("Passwords do not match.");if(!Cr())return Fn(),Ae("Type the word from the image exactly.");tt=3,yn(3);let l=y("syncaware-name"),c=((o=y("inp-signup-name"))==null?void 0:o.value.trim())||"there";l&&(l.textContent=c.split(" ")[0])}}function Sr(){tt>1&&(tt--,yn(tt),Gt(),tt===1&&setTimeout(()=>{var e;return(e=y("inp-signup-name"))==null?void 0:e.focus()},80))}var $r=["XRPL","LEDGER","VAULT","CRYPTO","BLOCK","TOKEN","CHAIN","WAVE","ATLAS","FORGE","NEXUS","ORBIT","PRISM","DELTA","NOVA","SONIC","PIXEL","GHOST","FLARE","SPARK","TITAN","LUNAR","STORM","PROXY","CIPHER","RELAY","PULSE","SCOUT"],co="";function Fn(){co=$r[Math.floor(Math.random()*$r.length)];let e=document.getElementById("captcha-canvas"),t=y("inp-captcha");if(t&&(t.value=""),!e)return;let n=e.getContext("2d"),s=e.width,a=e.height;n.clearRect(0,0,s,a),n.fillStyle="#080f1e",n.fillRect(0,0,s,a),n.strokeStyle="rgba(0,255,240,.05)",n.lineWidth=1;for(let r=0;r<s;r+=18)n.beginPath(),n.moveTo(r,0),n.lineTo(r,a),n.stroke();for(let r=0;r<a;r+=18)n.beginPath(),n.moveTo(0,r),n.lineTo(s,r),n.stroke();for(let r=0;r<55;r++)n.fillStyle=`rgba(${Math.random()>.5?"0,255,240":"160,180,255"},${(Math.random()*.22+.05).toFixed(2)})`,n.beginPath(),n.arc(Math.random()*s,Math.random()*a,Math.random()*2+.5,0,Math.PI*2),n.fill();for(let r=0;r<4;r++)n.strokeStyle=`rgba(0,255,240,${(Math.random()*.1+.03).toFixed(2)})`,n.lineWidth=1,n.beginPath(),n.moveTo(0,Math.random()*a),n.bezierCurveTo(s*.3,Math.random()*a,s*.7,Math.random()*a,s,Math.random()*a),n.stroke();let o=co.split(""),i=(s-o.length*26)/2+8;o.forEach((r,l)=>{let c=i+l*26+(Math.random()*8-4),d=a/2+8+(Math.random()*10-5);n.save(),n.translate(c,d),n.rotate(Math.random()*.35-.175),n.shadowColor="rgba(0,255,240,.5)",n.shadowBlur=8,n.fillStyle=`rgb(${Math.random()>.4?"180,255,240":"140,200,255"})`,n.font=`bold ${24+Math.random()*4}px 'JetBrains Mono','Courier New',monospace`,n.fillText(r,0,0),n.restore()})}function Tr(){Fn()}function Cr(){var e;return(((e=y("inp-captcha"))==null?void 0:e.value)||"").trim().toUpperCase()===co}function Pr(e){return(be(Z(io))||[]).some(t=>t.toLowerCase()===e.toLowerCase())}function Lr(e){return(be(Z(ro))||[]).some(t=>t.toLowerCase()===e.toLowerCase())}function Mr(e){return(be(Z(lo))||[]).some(t=>t.toLowerCase()===e.toLowerCase())}function Wu(e,t,n){let s=be(Z(io))||[],a=be(Z(ro))||[],o=be(Z(lo))||[];s.includes(e.toLowerCase())||s.push(e.toLowerCase()),a.includes(t.toLowerCase())||a.push(t.toLowerCase()),n&&!o.includes(n.toLowerCase())&&o.push(n.toLowerCase()),te(io,JSON.stringify(s)),te(ro,JSON.stringify(a)),te(lo,JSON.stringify(o))}async function mo(){var s,a,o;if(Dt)return;let e=((s=y("inp-login-email"))==null?void 0:s.value.trim())||"",t=((a=y("inp-login-pass"))==null?void 0:a.value)||"";if(Gt(),!e)return Ae("Enter your email address.");if(!t)return Ae("Enter your password.");let n=y("signin-btn");Dt=!0,On(n,!0,"Signing in\u2026");try{let i=await Ne.unlock(t);O.session={name:i.identity.name,email:i.identity.email,domain:i.identity.domain||""},te(Bn,JSON.stringify(O.session)),Vt(),ia(O.session),vn(),na(),window.dispatchEvent(new CustomEvent("naluxrp:vault-ready",{detail:Ne.vault})),Gu()}catch(i){Ae(i.message),(o=y("auth-modal-inner"))==null||o.classList.add("shake"),setTimeout(()=>{var r;return(r=y("auth-modal-inner"))==null?void 0:r.classList.remove("shake")},500)}finally{Dt=!1,On(n,!1,"Sign In \u2192")}}async function fo(){var i,r,l,c,d;if(Dt)return;let e=((i=y("inp-signup-name"))==null?void 0:i.value.trim())||"",t=((r=y("inp-signup-email"))==null?void 0:r.value.trim())||"",n=((l=y("inp-signup-domain"))==null?void 0:l.value.trim())||e.toLowerCase().replace(/[^a-z0-9_]/g,"_"),s=((c=y("inp-signup-pass"))==null?void 0:c.value)||"",a=((d=y("inp-signup-confirm"))==null?void 0:d.value)||"";if(Gt(),!s||s.length<8)return Ae("Password must be at least 8 characters.");if(!Xr(s))return Ae("Add uppercase, lowercase, and a number.");if(s!==a)return Ae("Passwords do not match.");if(!Cr())return Fn(),tt=2,yn(2),Ae("Type the word from the image exactly.");let o=y("signup-btn");Dt=!0,On(o,!0,"Creating vault\u2026");try{await Ne.create(s,e,t,n),Wu(e,t,n),O.session={name:e,email:t,domain:n},te(Bn,JSON.stringify(O.session)),ia(O.session),qu(e,()=>{Vt(),vn(),na(),window.dispatchEvent(new CustomEvent("naluxrp:vault-ready",{detail:Ne.vault})),setTimeout(Ku,3500)})}catch(u){Ae(u.message),Fn(),tt=2,yn(2)}finally{Dt=!1,On(o,!1,"Create Account \u2192")}}function qu(e,t){qt("celebrate");let n=y("celebrate-name");n&&(n.textContent=e.split(" ")[0]);let s=setTimeout(t,2800),a=y("celebrate-continue-btn");a&&(a.onclick=()=>{clearTimeout(s),t()})}function Ar(){qt("forgot"),ra(".forgot-step").forEach(t=>t.style.display="none");let e=y("forgot-step-options");e&&(e.style.display="")}function Er(){let e=document.createElement("input");e.type="file",e.accept=".json,application/json",e.onchange=async t=>{let n=t.target.files[0];if(n)try{let s=await n.text(),a=JSON.parse(s);if(!(a!=null&&a.vault)||!(a!=null&&a.meta))throw new Error("Invalid backup file.");te(Rt,JSON.stringify(a.vault)),te(Pt,JSON.stringify(a.meta)),ne("Backup restored \u2014 sign in with your original password."),qt("login")}catch(s){_t("Could not read backup: "+s.message)}},e.click()}function Nr(){ra(".forgot-step").forEach(n=>n.style.display="none");let e=y("forgot-step-wipe");e&&(e.style.display="");let t=y("inp-wipe-confirm");t&&(t.value="")}function _r(){var t;if((((t=y("inp-wipe-confirm"))==null?void 0:t.value.trim())||"")!=="DELETE")return Ae("Type DELETE exactly.");dt(Pt),dt(Rt),dt(Bn),Ne.lock(),O.session=null,ne("Account cleared. Create a new one."),Vt(),qt("signup"),window.dispatchEvent(new Event("naluxrp:logout"))}function Rr(){ra(".forgot-step").forEach(t=>t.style.display="none");let e=y("forgot-step-options");e&&(e.style.display=""),Gt()}function Dr(){let e=Z(Rt),t=Z(Pt);if(!e||!t){_t("No vault to export.");return}let n=btoa(JSON.stringify({vault:JSON.parse(e),meta:JSON.parse(t)})),s=document.createElement("div");s.id="sync-code-overlay",s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.92);backdrop-filter:blur(14px);z-index:99999;display:flex;align-items:center;justify-content:center;padding:20px;",s.innerHTML=`<div style="background:#0d1829;border:1.5px solid rgba(0,255,240,.22);border-radius:22px;padding:28px;max-width:500px;width:100%;box-shadow:0 28px 70px rgba(0,0,0,.95);"><div style="font-size:1rem;font-weight:900;margin-bottom:6px;color:#00fff0;">\u{1F4F1} Vault Sync Code</div><p style="font-size:.82rem;color:rgba(255,255,255,.55);margin-bottom:16px;line-height:1.6;">On your new device open NaluLF \u2192 <strong style="color:rgba(255,255,255,.8)">\u{1F4F1} New Device</strong> tab, paste this code and enter your password.</p><textarea readonly id="sync-code-output" style="width:100%;height:110px;background:#060e1a;border:1px solid rgba(255,255,255,.12);border-radius:10px;color:rgba(0,255,240,.85);font-family:monospace;font-size:.7rem;padding:10px;resize:none;box-sizing:border-box;" spellcheck="false">${n}</textarea><div style="display:flex;gap:10px;margin-top:14px;justify-content:flex-end;"><button onclick="document.getElementById('sync-code-overlay').remove()" style="padding:9px 16px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:9px;color:rgba(255,255,255,.65);font-size:.85rem;font-weight:700;cursor:pointer;font-family:inherit;">Close</button><button onclick="navigator.clipboard.writeText(document.getElementById('sync-code-output').value).then(()=>{this.textContent='\u2713 Copied!';setTimeout(()=>this.textContent='Copy Code',2000)})" style="padding:9px 18px;background:linear-gradient(135deg,#00d4ff,#00fff0);border:none;border-radius:9px;color:#000;font-size:.85rem;font-weight:900;cursor:pointer;font-family:inherit;">Copy Code</button></div></div>`,document.body.appendChild(s),s.addEventListener("click",a=>{a.target===s&&s.remove()})}async function ho(){var s,a;if(Dt)return;let e=((s=y("inp-sync-code"))==null?void 0:s.value.trim())||"",t=((a=y("inp-sync-pass"))==null?void 0:a.value)||"";if(Gt(),!e)return Ae("Paste your vault sync code or load a backup file first.");if(!t)return Ae("Enter the password from your original device.");let n=document.querySelector("#auth-view-sync .auth-submit-btn");Dt=!0,On(n,!0,"Importing\u2026");try{let o;try{o=JSON.parse(atob(e))}catch{try{o=JSON.parse(e)}catch{throw new Error("Invalid sync code. Paste the full code from your other device.")}}if(!(o!=null&&o.vault)||!(o!=null&&o.meta))throw new Error("Invalid sync code format \u2014 make sure you copied the entire code.");te(Rt,JSON.stringify(o.vault)),te(Pt,JSON.stringify(o.meta));let i=await Ne.unlock(t);O.session={name:i.identity.name,email:i.identity.email,domain:i.identity.domain||""},te(Bn,JSON.stringify(O.session)),Vt(),ia(O.session),vn(),na(),window.dispatchEvent(new CustomEvent("naluxrp:vault-ready",{detail:Ne.vault})),ne("\u2705 Account imported to this device!")}catch(o){let i=o.message.includes("decrypt")||o.message.includes("Incorrect");Ae(i?"Wrong password \u2014 use the password from your original device.":"Could not import: "+o.message),dt(Rt),dt(Pt)}finally{Dt=!1,On(n,!1,"Import & Sign In \u2192")}}function Ir(){let e=document.createElement("input");e.type="file",e.accept=".json,application/json",e.onchange=async t=>{var s;let n=t.target.files[0];if(n)try{let a=await n.text(),o=JSON.parse(a);if(!(o!=null&&o.vault)||!(o!=null&&o.meta))throw new Error("Invalid backup file format.");let i=y("inp-sync-code");i&&(i.value=btoa(JSON.stringify({vault:o.vault,meta:o.meta})));let r=y("sync-file-feedback");r&&(r.textContent=`\u2713 File loaded: ${n.name}`,r.style.color="#50fa7b"),(s=y("inp-sync-pass"))==null||s.focus()}catch(a){_t("Could not read backup: "+a.message)}},e.click()}function Fr(){Ne.lock(),O.session=null,dt(Bn),gn(),window.dispatchEvent(new Event("naluxrp:logout"))}function Or(){let e=be(Z(Bn));return e!=null&&e.email&&Ne.hasVault()?(O.session=e,ia(e),!0):!1}function Vu(){if(y("vault-lock-banner"))return;let e=document.createElement("div");e.id="vault-lock-banner",e.className="vault-lock-banner",e.innerHTML=`<span class="vlb-icon">\u{1F512}</span><span class="vlb-text">Vault locked for security after 30 min of inactivity.</span><button class="vlb-btn" onclick="openAuth('login')">Unlock \u2192</button><button class="vlb-close" onclick="this.closest('.vault-lock-banner').remove()" title="Dismiss">\u2715</button>`,document.body.prepend(e),requestAnimationFrame(()=>e.classList.add("show"))}function Gu(){let e=y("vault-lock-banner");e&&(e.classList.remove("show"),setTimeout(()=>e.remove(),300))}function Ku(){if(y("backup-reminder-banner")||!Ne.hasVault())return;let e=document.createElement("div");e.id="backup-reminder-banner",e.className="backup-reminder-banner",e.innerHTML=`<span class="brb-icon">\u26A0\uFE0F</span><div class="brb-body"><strong>Back up your vault</strong> \u2014 you'll lose access if browser storage is cleared. <button class="brb-btn" onclick="window.exportVaultBackup?.()">Export Backup</button></div><button class="brb-close" onclick="this.closest('.backup-reminder-banner').remove()">\u2715</button>`;let t=document.getElementById("profile-page");t?t.prepend(e):document.body.prepend(e),requestAnimationFrame(()=>e.classList.add("show"))}function Br(e){var n;let t=((n=y("auth-overlay"))==null?void 0:n.getAttribute("data-view"))||"login";if(e.key==="Enter"&&(t==="login"&&mo(),t==="signup"&&(tt<3?uo():fo()),t==="sync"&&ho()),e.key==="Tab"){let s=y("auth-modal-inner");if(!s)return;let a=Array.from(s.querySelectorAll("button:not([disabled]),input,textarea,select")).filter(r=>r.offsetParent!==null);if(!a.length)return;let o=a[0],i=a[a.length-1];e.shiftKey&&document.activeElement===o&&(e.preventDefault(),i.focus()),!e.shiftKey&&document.activeElement===i&&(e.preventDefault(),o.focus())}}function aa(e){let t=y(e);t==null||t.classList.add("valid"),t==null||t.classList.remove("invalid")}function wn(e){let t=y(e);t==null||t.classList.add("invalid"),t==null||t.classList.remove("valid")}function oa(e){let t=y(e);t==null||t.classList.remove("valid","invalid")}window.validateSignupName=()=>{var s;let e=((s=y("inp-signup-name"))==null?void 0:s.value.trim())||"",t=y("hint-signup-name");if(!e){oa("inp-signup-name"),t&&(t.textContent="");return}if(e.length<3){wn("inp-signup-name"),t&&(t.textContent="At least 3 characters");return}if(Pr(e)){wn("inp-signup-name"),t&&(t.textContent="Already in use on this device");return}aa("inp-signup-name"),t&&(t.textContent="\u2713 Looks good!");let n=y("inp-signup-domain");n&&!n.dataset.manuallyEdited&&(n.value=e.toLowerCase().replace(/[^a-z0-9]/g,"_").replace(/_+/g,"_").replace(/^_|_$/g,""),window.validateSignupDomain())};window.validateSignupEmail=()=>{var n;let e=((n=y("inp-signup-email"))==null?void 0:n.value.trim())||"",t=y("hint-signup-email");if(!e){oa("inp-signup-email"),t&&(t.textContent="");return}if(!e.includes("@")||!e.includes(".")){wn("inp-signup-email"),t&&(t.textContent="Enter a valid email");return}if(Lr(e)){wn("inp-signup-email"),t&&(t.textContent="Already registered on this device");return}aa("inp-signup-email"),t&&(t.textContent="\u2713 Available")};window.validateSignupDomain=()=>{var s;let e=((s=y("inp-signup-domain"))==null?void 0:s.value.trim())||"",t=y("hint-signup-domain"),n=y("inp-signup-domain");if(!e){oa("inp-signup-domain"),t&&(t.textContent="");return}if(!/^[a-z0-9_]{2,30}$/.test(e)){wn("inp-signup-domain"),t&&(t.textContent="2-30 chars: a-z, 0-9, underscore only");return}if(Mr(e)){wn("inp-signup-domain"),t&&(t.textContent="Already taken on this device");return}aa("inp-signup-domain"),t&&(t.textContent=`\u2713 @${e}`),n&&(n.dataset.manuallyEdited="1")};window.validateLoginEmail=()=>{var t;let e=((t=y("inp-login-email"))==null?void 0:t.value.trim())||"";if(!e)return oa("inp-login-email");e.includes("@")?aa("inp-login-email"):wn("inp-login-email")};window.switchSyncMethod=function(e){var i,r;let t=e==="code";(i=document.getElementById("sync-method-code"))==null||i.classList.toggle("sync-method-card--active",t),(r=document.getElementById("sync-method-file"))==null||r.classList.toggle("sync-method-card--active",!t);let n=document.getElementById("sync-code-section"),s=document.getElementById("sync-file-section"),a=document.getElementById("sync-method-code-steps"),o=document.getElementById("sync-method-file-steps");n&&(n.style.display=t?"":"none"),s&&(s.style.display=t?"none":""),a&&(a.style.display=t?"":"none"),o&&(o.style.display=t?"none":"")};window.togglePwVisibility=function(e,t){let n=y(e);if(!n)return;let s=n.type==="password";n.type=s?"text":"password",t.textContent=s?"\u{1F648}":"\u{1F441}"};window.updatePwStrength=function(e){let t=y("pw-strength-fill"),n=y("pw-strength-label");if(!t||!n)return;let s=0;e.length>=8&&s++,/[A-Z]/.test(e)&&s++,/[a-z]/.test(e)&&s++,/[0-9]/.test(e)&&s++,/[^A-Za-z0-9]/.test(e)&&s++;let a=[{w:"0%",bg:"transparent",txt:""},{w:"20%",bg:"#ff5555",txt:"Very weak"},{w:"40%",bg:"#ff8c42",txt:"Weak"},{w:"60%",bg:"#ffb86c",txt:"Fair"},{w:"80%",bg:"#00d4ff",txt:"Good"},{w:"100%",bg:"#50fa7b",txt:"Strong \u2713"}],o=a[s]||a[0];t.style.width=o.w,t.style.background=o.bg,n.textContent=o.txt,n.style.color=o.bg};function Xr(e){return/[A-Z]/.test(e)&&/[a-z]/.test(e)&&/[0-9]/.test(e)}function ia(e){let t=y("user-avatar"),n=y("user-name");t&&(t.textContent=e.name.charAt(0).toUpperCase()),n&&(n.textContent=e.name)}function Ae(e){let t=y("auth-error");t&&(t.textContent=e,t.style.display="")}function Gt(){let e=y("auth-error");e&&(e.textContent="")}function On(e,t,n){e&&(e.disabled=t,e.textContent=n)}var Dt=!1;function ra(e){return Array.from(document.querySelectorAll(e))}["click","keydown","mousemove","touchstart"].forEach(e=>document.addEventListener(e,()=>Ne.resetTimer(),{passive:!0}));window.addEventListener("naluxrp:vault-locked",()=>{O.vaultLocked=!0,Vu()});var Ju=180,Xt=12,Hr=5,zr="naluxrp_compact_mode",Yu=1e5,Qu=18,go=16,Zu=96,Ur=96,em=96,tm=5*6e4,la=72,jr=80,nm=1e4,sm=20,Ss=8,yl=3,wl=.7,wo=.35,ma=12,Kn=10,Jn=2,Un=61e3,Jt="NALU-SPAM-PROOF:",xl="naluxrp_spam_verified",kl="naluxrp_spam_allowlist",$l=new Set(["rPVMhWBsfF9iMXYj3aAzJVkPDTFNSyWdKy","rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B","rrpNnNLKrartuEqfJGpqyDwPj1BBN1ih7","rN7n3473SaZBCG4dFL83w7PB9judJ7qdDo","rEb8TK3gBgk5auZkwc6sHnwrGVJH8DuaLh","rBKPS4oLSaV2KVVuHH8EpQqMGgGefGFQs7","rfk5bwaKCoNU84fTzdqWQowqnNaZorDmiV","rGFuMiw48HdbnrUbkRYDTvT5i9imC5fvv9","rwYHCs2EYBMBvRXFmxDrCUSorPsuqCck7t","rLHzPsX6oXkzU2qL12kHCH8G8cnZv1rBJh","ra5nK24KXen9AHvsdFTKHSANinZseWnPcX","rGWrZyax5eXbi5gs49MRZKkE9eKNL9p4B","rHsMUQFzBb7S6GnQFVgNirqvHRcLpAn5dU","rDsbeomae4FXwgQTJp9Rs64Qg9vDiTCdBv","rMQ98K56yXJbDGv49ZSmW51sLn94Xe1mu1","rKiCet8SdvWxPXnAgYarFUXMh1zCPz432Y","r9mhdcT2K7FdCGDEPqfbMJwVXsXCqEr5bP","r4GDFMLGJUKMjNEycBKPGnRSNXyNVLQLHi","rUA1S9qobBkxLqzdfGEzh5wm5KdLfbf8bx","rHtbQzmN4BDaEBnGSXp3AZaZAuZamNVsME","rhub8VRN55s94qWKDv6jmDy1pUykJzF3wq","razqnFn6FqBaYBdNaGnVzmGaNE6XPRQ9bG","rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh","r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59","rPEPPER7kfTD9w2To4CQk6UCfuHM9c6GDY","rsoLo2S1kiGeCcn6hCUXVrCpGMWLrRrLZz"]),ge={dexPressure:[],nftMints:[],nftBurns:[],autoBridge:[],marketPrice:[],marketVol:[]},Kt=null,vo=0,bt={acct:new Map},ys={window:[],totals:new Map,maxLedgers:12},Ee={window:[],smoothCancelPerMin:null,smoothBurst:null},ae={byAddr:new Map,selectedAddr:null,selectedProof:null,allowList:new Set,verifiedCache:new Map},Wr=!1,qr=!1,Vr=!1,xo=!1,Gr=!1,Kr=!1,Jr=!1,Yr=!1,Qr=!1,Zr=!1,el=!1,tl=!1,nl=!1,sl=!1,al=!1,ol=!1,il=!1,rl=!1,xn=!1,xs=!1,ko={};var $o=new Map;function ca(e,t,n=2){let s=$o.get(e)??-999;return Number(t)-s<n?!1:($o.set(e,Number(t)),!0)}var We={ledgersProcessed:0,startTime:Date.now(),totalTx:0,whaleCount:0,feeSpikes:0,botDetections:0,dexAlerts:0},wt=[],am=40,kn={whaleTxXrp:1e5,feeSpikeMultiple:5,botCvThreshold:.2,dexCancelAlert:.75,clusterMinSize:5},Sl=0,ga="naluxrp_widget_order",Yn="naluxrp_widget_hidden",Xn=null,It=!1,ll=!1,Tl=!0;function Bt(){return Tl&&O.currentPage==="dashboard"&&O.currentTab==="stream"&&!document.hidden}var jn=[],om=30,im=0;function Cl(){hm(),vm(),rf(),df(),km(),Pm(),Ym(),Qm(),$m(),dm(),pm(),kf(),Zm(),nf(),af(),Cf(),Lf(),Af(),Bt()&&ya(),Ff(),Of(),_f(),Bf(),Xf(),Hf(),window.addEventListener("xrpl-connection",e=>{var n,s;let t=!!((n=e==null?void 0:e.detail)!=null&&n.connected);if(t&&Em(),((s=e==null?void 0:e.detail)==null?void 0:s.state)==="connecting"){ba(!0,"Switching networks\u2026");let a=y("tx-mix");a&&(a.innerHTML='<div class="loading-row"><div class="spinner"></div> Accumulating\u2026</div>')}else t&&ba(!1)}),window.addEventListener("xrpl-ledger",e=>{if(!Bt())return;let t=e.detail,n=Number(t.ledgerIndex||0);We.ledgersProcessed++,We.totalTx+=Number(t.txPerLedger||0),["risk-badge","risk-regime","risk-friction","risk-signalcount","landscape-badge","dexp-badge","dexP-badge","ab-badge","nft-badge","whale-badge","ss-badge","health-badge","pattern-badge"].forEach(a=>delete ko[a]),bm(t),Sm(),xm(),Tm(),_m(t.latestLedger),Tf(t.recentTransactions||[],n);let s=Fm(t);for(jn.push({li:n,friction:s.friction,regime:s.regime});jn.length>om;)jn.shift();Bl(),!It&&(ca("breadcrumbs",n,1)&&Gm(s.breadcrumbs),ca("clusters",n,2)&&Km(s.clusters),ca("narratives",n,2)&&Jm(s.narratives),ca("landscape",n,3)&&ef(s),$f(t.txTypes,s.hhi),sf(s.dexPatterns),of(s),cf(s),uf(s),Ef(),n-Sl>=8&&Nf(s,n))}),document.addEventListener("visibilitychange",()=>{Bt()?ya():Ao()})}function Pl(e){Tl=!!e,Bt()?ya():Ao()}function V(e,t){let n=y(e);n&&(n.textContent=String(t))}function ke(e,t,n){return Math.max(t,Math.min(n,e))}function Zt(e){return e!=null&&e.length?e.reduce((t,n)=>t+n,0)/e.length:null}function Mo(e){if(!e||e.length<2)return null;let t=Zt(e),n=Zt(e.map(s=>(s-t)**2));return Math.sqrt(n)}function rm(e,t){return e!=null&&e.length?e.slice(Math.max(0,e.length-t)):[]}function lm(e,t){let n=[];for(let s=0;s<e.length;s++){let a=Math.max(0,s-t+1);n.push(Zt(e.slice(a,s+1)))}return n}function en(e,t=0){return e==null||!Number.isFinite(e)?"\u2014":`${e>=0?"\u2191":"\u2193"}${Math.abs(e).toFixed(t)}%`}function Hn(e,t=2){return e==null||!Number.isFinite(e)?"\u2014":Number(e).toFixed(t)}function Yt(e){if(e==null||!Number.isFinite(e))return"\u2014";let t=Number(e);return t===0?"0 XRP":t>=1e3?`${t.toLocaleString(void 0,{maximumFractionDigits:2})} XRP`:t>=1?`${t.toFixed(4)} XRP`:t>=.01?`${t.toFixed(5)} XRP`:`${t.toFixed(6)} XRP`}function Wn(e,t=1){if(!e)return null;let n=ke(Number(t),0,1),s=String(e).trim();if(s.startsWith("rgba("))return s;if(s.startsWith("rgb(")){let a=s.match(/^rgb\(\s*([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\s*\)$/i);if(!a)return null;let o=ke(Number(a[1]),0,255),i=ke(Number(a[2]),0,255),r=ke(Number(a[3]),0,255);return`rgba(${o},${i},${r},${n})`}if(s[0]==="#"){let a=s.slice(1);if(a.length===3&&(a=a.split("").map(l=>l+l).join("")),a.length!==6)return null;let o=parseInt(a.slice(0,2),16),i=parseInt(a.slice(2,4),16),r=parseInt(a.slice(4,6),16);return[o,i,r].every(Number.isFinite)?`rgba(${o},${i},${r},${n})`:null}return null}function cl(e,t,n=.45){return t==null||!Number.isFinite(t)?e:e==null||!Number.isFinite(e)?t:e*(1-n)+t*n}async function cm(e){if(typeof crypto<"u"&&crypto.subtle){let n=new TextEncoder().encode(e),s=await crypto.subtle.digest("SHA-512",n),a=new Uint8Array(s,0,32);return Array.from(a).map(o=>o.toString(16).padStart(2,"0")).join("")}let t=0x811c9dc5n;for(let n=0;n<e.length;n++)t^=BigInt(e.charCodeAt(n)),t=BigInt.asUintN(32,t*0x01000193n);return"FALLBACK-NON-CRYPTO-"+t.toString(16).padStart(8,"0").repeat(4).slice(0,44)}function dm(){var n,s;if(tl||(tl=!0,document.getElementById("acctPeekOverlay")))return;let e=document.createElement("div");e.id="acctPeekOverlay",e.className="acct-peek-overlay",e.style.display="none",e.innerHTML=`
  ...
  `,document.body.appendChild(e);let t=()=>{e.style.display="none",e.removeAttribute("data-addr"),document.body.classList.remove("modal-open"),xs=!1,yt=!1};e.addEventListener("click",a=>{a.target===e&&t()}),(n=y("acctPeekClose"))==null||n.addEventListener("click",t),(s=y("acctPeekInspect"))==null||s.addEventListener("click",async()=>{let a=e.getAttribute("data-addr");a&&await fm(a),t()})}function pm(){el||(el=!0,document.addEventListener("click",e=>{var s,a;let t=(a=(s=e.target).closest)==null?void 0:a.call(s,"[data-addr]");if(!t)return;let n=t.getAttribute("data-addr");n&&(e.preventDefault(),mm(n))}))}function dl(e){var r,l,c;if(!e||!((r=Ee==null?void 0:Ee.window)!=null&&r.length))return null;let t=0,n=0,s=0;for(let d of Ee.window)s+=Number(d.total||0),t+=Number(((l=d.byActorCreate)==null?void 0:l.get(e))||0),n+=Number(((c=d.byActorCancel)==null?void 0:c.get(e))||0);let a=t+n;if(!a)return null;let o=a?n/a:0,i=s?a/s:null;return{create:t,cancel:n,total:a,cancelRatio:o,share:i}}function um(e){var o;let t=(o=bt==null?void 0:bt.acct)==null?void 0:o.get(e);if(!t||!t.intervals||t.intervals.length<6)return null;let n=Zt(t.intervals),s=Mo(t.intervals);return!n||s==null?null:{cv:s/n,total:t.total||0}}async function mm(e){var i,r,l;if(!Fe(e))return;let t=document.getElementById("acctPeekOverlay");if(!t)return;t.style.display="flex",t.setAttribute("data-addr",e),document.body.classList.add("modal-open"),xs=!0,yt=!0,V("acctPeekAddr",e),V("acctPeekBal","\u2026"),V("acctPeekSeq","\u2026"),V("acctPeekOwner","\u2026"),V("acctPeekFlags","\u2026");let n=y("acctPeekNote"),s=y("acctPeekCtx");n&&(n.textContent="Fetching account_info\u2026"),s&&(s.textContent="Building context\u2026");let a=bt.acct.get(e),o=((i=a==null?void 0:a.ledgers)==null?void 0:i.length)||0;try{if(!O.wsConn||O.wsConn.readyState!==1){n&&(n.textContent="Not connected to XRPL. Connect first.");return}let c=await Me({command:"account_info",account:e,ledger_index:"validated",strict:!0}),d=(r=c==null?void 0:c.result)==null?void 0:r.account_data,u=Number((d==null?void 0:d.Balance)??NaN),p=Number.isFinite(u)?u/1e6:null;if(V("acctPeekBal",p==null?"\u2014":`${p.toLocaleString(void 0,{maximumFractionDigits:6})} XRP`),V("acctPeekSeq",(d==null?void 0:d.Sequence)??"\u2014"),V("acctPeekOwner",(d==null?void 0:d.OwnerCount)??"\u2014"),V("acctPeekFlags",(d==null?void 0:d.Flags)!=null?`0x${Number(d.Flags).toString(16)}`:"\u2014"),n){let m=[];p!=null&&p>=Yu&&m.push("Large balance (whale-sized).");let f=um(e);f&&f.cv<.35&&f.total>10&&m.push(`Bot-like timing signal (CV ${f.cv.toFixed(2)}).`);let v=dl(e);if(v){let w=Math.round(v.cancelRatio*100),k=v.share!=null?Math.round(v.share*100):null;m.push(`DEX activity: ${v.total} offer tx in window.`),m.push(`Creates ${v.create}, cancels ${v.cancel} (${w}% cancels).`),k!=null&&m.push(`~${k}% of window activity.`)}let h=ae.byAddr.get(e);if(h){let w=ks(h.level);m.push(`Spam-defense: score ${(h.score*100).toFixed(0)}% \xB7 level L${h.level} \xB7 bond ${w.toLocaleString()} XRP.`),h.verifiedLedger!=null&&m.push(`Credential verified at ledger #${Number(h.verifiedLedger).toLocaleString()}.`)}o>=6&&m.push(`Shows up often in recent ledgers (${o} times).`),m.length||m.push("No obvious red flags from this quick read."),n.textContent=m.join(" ")}if(s){let m=(l=a==null?void 0:a.ledgers)==null?void 0:l.at(-1),f=dl(e),v=f?`DEX window: ${f.total} (creates ${f.create}, cancels ${f.cancel})`:null,h=wt.filter(x=>x.from===e||x.to===e).slice(0,3),w=h.length?`${h.length} whale tx in session (${h.map(x=>(x.amtXrp>=1e6?(x.amtXrp/1e6).toFixed(1)+"M":(x.amtXrp/1e3).toFixed(0)+"K")+" XRP").join(", ")})`:null,k=ae.byAddr.get(e),b=k?`Spam-defense: L${k.level} \xB7 score ${Math.round(k.score*100)}%`:null;s.textContent=[m?`Last seen around ledger #${Number(m).toLocaleString()}`:"Not in recent window.",v,w,b].filter(Boolean).join(" \xB7 ")}}catch(c){n&&(n.textContent=`Lookup failed: ${String((c==null?void 0:c.message)||c)}`),s&&(s.textContent="\u2014")}}async function fm(e){if(!Fe(e))return;let t=document.querySelector('.dash-tab[data-tab="inspector"]'),n=document.getElementById("tab-inspector"),s=async()=>n&&n.style.display!=="none"?!0:(typeof window.switchTab=="function"&&t?window.switchTab(t,"inspector"):t==null||t.click(),await new Promise(o=>setTimeout(o,80)),n?n.style.display!=="none":!0);for(let o=0;o<6&&!await s();o++)await new Promise(r=>setTimeout(r,80));let a=document.getElementById("inspect-addr");a&&(a.value=e,a.focus()),await new Promise(o=>setTimeout(o,60)),typeof window.runInspect=="function"?window.runInspect():he("Inspector not ready yet.")}function hm(){Ie(".net-btn[data-network]").forEach(e=>{e.addEventListener("click",()=>{let t=e.getAttribute("data-network");Ie(".net-btn").forEach(s=>{s.classList.toggle("active",s===e),s.setAttribute("aria-pressed",String(s===e))}),bt.acct.clear(),ys.window=[],ys.totals=new Map,Ee.window=[],Ee.smoothCancelPerMin=null,Ee.smoothBurst=null,Lt.length=0,ha.clear(),Xe=0,Ft=0,nt=0,bs=0,Ot=!0,va=0,To=!1,ws=null,yt=!1,Qt=0,qn=0,wa=!1;let n=y("ledgerStreamTrack");n&&(n.innerHTML="",n.style.transform="translateX(0px)"),Nl(null,null),lf(),ae.byAddr.clear(),ae.selectedAddr=null,ae.selectedProof=null,Eo(),Fl(),Gn(null),wr(t),ya({force:!0})})})}var fa=[],gm=8,bo=null,So=0,vs=null;function vm(){let e=document.querySelector(".dashboard-metric-grid");if(!e)return;let t=e.closest(".dashboard-metrics")||e.parentElement;t&&t.classList.add("dashboard-sticky-strip"),e.innerHTML=`
  ...
  `}function bm(e){var v,h,w,k;let t=e.ledgerIndex?Number(e.ledgerIndex):null;V("d2-ledger-index",t?t.toLocaleString():"\u2014");let n=((v=e.latestLedger)==null?void 0:v.closeTimeSec)!=null?Number(e.latestLedger.closeTimeSec):null,s=y("d2-ledger-age");if(s&&(n!=null?(s.textContent=`${n<2?n.toFixed(2):n.toFixed(1)}s close`,s.style.color=n<=3?"#50fa7b":n<=6?"#ffb86c":"#ff6e6e"):(s.textContent="Waiting\u2026",s.style.color="")),n!=null&&(fa.push(n),fa.length>gm&&fa.shift(),ym()),So=Date.now(),wm(),e.tps!=null){let b=Number(e.tps),x=y("d2-tps");x&&(x.textContent=b.toFixed(1),x.style.color=b<10?"rgba(255,255,255,.65)":b<40?"#50fa7b":b<80?"#ffb86c":"#ff6e6e");let S=y("d2-tps-trend");if(S){let T=O.tpsHistory||[],$=T.length>2?(T.slice(-10).reduce((M,N)=>M+N,0)/Math.min(T.length,10)).toFixed(1):null,P=b<10?"Low":b<40?"Normal":b<80?"High":"Peak";S.textContent=$?`${P} \xB7 avg ${$}`:P,S.style.color=b<10?"rgba(255,255,255,.55)":b<40?"#50fa7b":b<80?"#ffb86c":"#ff6e6e"}}let a=(((h=e.latestLedger)==null?void 0:h.totalTx)??e.txPerLedger)||0,o=y("d2-tx-per-ledger");o&&(o.textContent=a>0?a.toLocaleString():"\u2014",o.style.color=a<10?"rgba(255,255,255,.65)":a<150?"#50fa7b":a<400?"#ffb86c":"#ff6e6e");let i=y("d2-tx-spread");i&&a>0&&(i.textContent=a<10?"Very light":a<50?"Light":a<150?"Normal":a<400?"High volume":"Very high volume",i.style.color=a<150?"":a<400?"#ffb86c":"#ff6e6e");let r=e.avgFee!=null?Number(e.avgFee):((w=e.latestLedger)==null?void 0:w.avgFee)!=null?Number(e.latestLedger.avgFee):null;if(r!=null){let b=Math.round(r*1e6),x=y("d2-fee-value");x&&(x.textContent=Yt(r),x.style.color=b<=15?"#50fa7b":b<=50?"rgba(255,255,255,.9)":b<=200?"#ffb86c":"#ff6e6e");let S=y("d2-fee-delta");if(S)if(bo!=null){let $=r/bo;$>1.05?(S.textContent="\u2191",S.style.color="#ff6e6e",S.title=`+${(($-1)*100).toFixed(0)}% vs prev ledger`):$<.95?(S.textContent="\u2193",S.style.color="#50fa7b",S.title=`-${((1-$)*100).toFixed(0)}% vs prev ledger`):(S.textContent="\u2192",S.style.color="rgba(255,255,255,.35)",S.title="Stable vs prev ledger")}else S.textContent="";bo=r;let T=y("d2-fee-pressure");if(T){let $=b<=15?"Base fee":b<=50?`${b} drops`:b<=200?`${b} drops \xB7 Elevated`:`${b} drops \xB7 Surge`;T.textContent=$,T.style.color=b<=50?"":b<=200?"#ffb86c":"#ff6e6e"}}let l=e.successRate!=null?Number(e.successRate):((k=e.latestLedger)==null?void 0:k.successRate)!=null?Number(e.latestLedger.successRate):null;if(l!=null){let b=y("d2-success-rate");b&&(b.textContent=`${l.toFixed(1)}%`,b.style.color=l>=90?"#50fa7b":l>=75?"#ffb86c":"#ff6e6e");let x=y("d2-success-note");if(x){let S=(100-l).toFixed(1);x.textContent=l>=90?`${S}% failed \xB7 Normal`:l>=75?`${S}% failed \xB7 Watch`:`${S}% failed \xB7 Alert`,x.style.color=l>=90?"":l>=75?"#ffb86c":"#ff6e6e"}}let c=a>0?Math.min(100,a/500*100):null,d=y("d2-network-capacity"),u=y("d2-capacity-note");c!=null&&(d&&(d.textContent=`${c.toFixed(1)}%`,d.style.color=c<20?"rgba(255,255,255,.65)":c<50?"#50fa7b":c<80?"#ffb86c":"#ff6e6e"),u&&(u.textContent=c<20?"Low usage":c<50?"Moderate":c<80?"Heavy":"Near capacity",u.style.color=c<50?"":c<80?"#ffb86c":"#ff6e6e"));let p=e.txTypes||{},m=Object.entries(p).sort(([,b],[,x])=>x-b);if(m.length){let[b,x]=m[0],S=m.reduce((N,[,_])=>N+_,0)||1,T=(x/S*100).toFixed(0),$=typeof Ct<"u"?Ct:{},P=y("d2-dominant-type");P&&(P.textContent=b,P.style.color=$[b]||"rgba(255,255,255,.9)");let M=y("d2-dominance-score");if(M){let N=m[1];M.textContent=N?`${T}% \xB7 2nd: ${N[0]}`:`${T}% of txs`}}let f=y("stream-loading");f&&(f.style.display="none")}function ym(){let e=y("d2-close-sparkline");if(!e||!e.getContext)return;let t=fa;if(t.length<2)return;let n=e.width,s=e.height,a=e.getContext("2d");a.clearRect(0,0,n,s);let o=Math.max(...t,4),i=Math.floor((n-(t.length-1))/t.length),r=1;t.forEach((l,c)=>{let d=Math.max(3,Math.round(l/o*(s-2))),u=c*(i+r),p=s-d,m=l<=3?"rgba(80,250,123,.80)":l<=6?"rgba(255,184,108,.80)":"rgba(255,110,110,.85)";a.fillStyle=m;let f=Math.min(2,i/2);a.beginPath(),a.moveTo(u+f,p),a.lineTo(u+i-f,p),a.quadraticCurveTo(u+i,p,u+i,p+f),a.lineTo(u+i,s),a.lineTo(u,s),a.lineTo(u,p+f),a.quadraticCurveTo(u,p,u+f,p),a.closePath(),a.fill(),c===t.length-1&&(a.fillStyle="rgba(255,255,255,.18)",a.fillRect(u,p,i,Math.min(3,d)))})}function wm(){if(vs&&cancelAnimationFrame(vs),!Bt())return;let e=0,t=n=>{if(!Bt()){vs=null;return}if(n-e>=120){e=n;let s=y("d2-ledger-age-timer");if(s&&So>0){let a=(Date.now()-So)/1e3;s.textContent=`${a.toFixed(1)}s ago`,s.style.color=a<4?"rgba(255,255,255,.55)":a<7?"#ffb86c":"#ff6e6e",s.style.opacity=a>=7?(.6+.4*Math.sin(Date.now()/300)).toFixed(2):"1"}}vs=requestAnimationFrame(t)};vs=requestAnimationFrame(t)}function xm(){let e=y("tx-mix");if(!e)return;let t=Object.entries(O.txMixAccum||{}).filter(([,s])=>s>0).sort(([,s],[,a])=>a-s).slice(0,10),n=t.reduce((s,[,a])=>s+a,0);n&&(e.innerHTML=t.map(([s,a])=>{let o=(a/n*100).toFixed(1),i=Ct[s]||"#6b7280";return`
      ...
      </div>`}).join(""))}var pt=class{constructor(t,n="#00fff0",s="area"){this.canvasId=t,this.canvas=y(t),this.color=n,this.mode=s}_resolveCanvas(){return this.canvas||(this.canvas=y(this.canvasId)),this.canvas}draw(t){let n=this._resolveCanvas();if(!n||!(t!=null&&t.length))return;let s=n.getContext("2d");if(!s)return;let a=n.width=n.offsetWidth||300,o=n.height=n.offsetHeight||180;if(s.clearRect(0,0,a,o),t.length<2)return;let i=Math.min(...t)*.9,r=Math.max(...t)*1.05||1,l=h=>1-(h-i)/(r-i||1),c={l:10,r:10,t:10,b:12},d=a-c.l-c.r,u=o-c.t-c.b,p=d/(t.length-1),m=t.map((h,w)=>[c.l+w*p,c.t+l(h)*u]),f=Zt(t);if(f!=null){let h=c.t+l(f)*u;s.beginPath(),s.moveTo(c.l,h),s.lineTo(c.l+d,h),s.strokeStyle="rgba(255,255,255,0.16)",s.lineWidth=1,s.stroke()}if(t.length>=Hr+2){let w=lm(t,Hr).map((k,b)=>[c.l+b*p,c.t+l(k)*u]);s.beginPath(),w.forEach(([k,b],x)=>x===0?s.moveTo(k,b):s.lineTo(k,b)),s.strokeStyle="rgba(255,255,255,0.22)",s.lineWidth=1.5,s.stroke()}if(this.mode==="bar"){let h=Math.max(1,p-3);t.forEach((w,k)=>{let b=(1-l(w))*u,x=c.l+k*p-h/2,S=s.createLinearGradient(0,c.t,0,c.t+u);S.addColorStop(0,this.color),S.addColorStop(1,this.color+"33"),s.fillStyle=S,s.fillRect(x,c.t+u-b,h,b)});return}let v=s.createLinearGradient(0,c.t,0,c.t+u);v.addColorStop(0,this.color+"aa"),v.addColorStop(1,this.color+"11"),s.beginPath(),m.forEach(([h,w],k)=>k===0?s.moveTo(h,w):s.lineTo(h,w)),s.lineTo(m.at(-1)[0],c.t+u),s.lineTo(m[0][0],c.t+u),s.closePath(),s.fillStyle=v,s.fill(),s.beginPath(),m.forEach(([h,w],k)=>k===0?s.moveTo(h,w):s.lineTo(h,w)),s.strokeStyle=this.color,s.lineWidth=2,s.lineJoin="round",s.stroke()}},ye={};function km(){ye.tps=new pt("chart-tps","#50fa7b","area"),ye.fee=new pt("chart-fee","#ffb86c","area"),ye.tps2=new pt("chart-tps2","#50fa7b","bar"),ye.fee2=new pt("chart-fee2","#ffb86c","area"),ye.dexPressure=new pt("chart-dex-pressure","#00d4ff","bar"),ye.nftMints=new pt("chart-nft-mints","#bd93f9","bar"),ye.nftBurns=new pt("chart-nft-burns","#ff5555","bar"),ye.autoBridge=new pt("chart-autobridge","#f1fa8c","bar"),ye.marketPrice=new pt("chart-market-price","#50fa7b","area"),ye.marketVol=new pt("chart-market-vol","#8be9fd","bar")}function Mt(e,t=Xt){let n=rm(e,t).filter(r=>Number.isFinite(r));if(!n.length)return{cur:null,avg:null,deltaPct:null,vol:null};let s=n.at(-1),a=Zt(n),o=Mo(n),i=a&&a!==0?(s-a)/a*100:null;return{cur:s,avg:a,deltaPct:i,vol:o}}function $m(){if(Vr)return;Vr=!0;let e=(t,n)=>{let s=y(t);if(!s)return;let a=s.closest(".widget-card"),o=a==null?void 0:a.querySelector(".widget-header");if(!o||document.getElementById(n))return;let i=document.createElement("div");i.className="trend-mini",i.id=n,i.innerHTML=`
    ...
    `,o.appendChild(i)};e("chart-tps","tpsTrendMini"),e("chart-fee","feeTrendMini"),e("chart-tps2","tpsTrendMini2"),e("chart-fee2","feeTrendMini2")}function da(e,t,n,s){let a=document.getElementById(e);if(!a)return;let o=(i,r)=>{let l=a.querySelector(`[data-k="${i}"]`);l&&(l.textContent=r)};if(t.cur==null){o("now","\u2014"),o("avg","\u2014"),o("delta","\u2014"),o("sigma","\u2014");return}o("now",`${Number(t.cur).toFixed(n)}${s}`),o("avg",t.avg!=null?`${Number(t.avg).toFixed(n)}${s}`:"\u2014"),o("delta",t.deltaPct==null?"\u2014":en(t.deltaPct,0)),o("sigma",t.vol==null?"\u2014":`${Number(t.vol).toFixed(Math.max(0,n-1))}${s}`)}function Sm(){var n,s,a,o,i,r,l,c,d,u;(n=ye.tps)==null||n.draw(O.tpsHistory),(s=ye.fee)==null||s.draw(O.feeHistory),(a=ye.tps2)==null||a.draw(O.tpsHistory),(o=ye.fee2)==null||o.draw(O.feeHistory);let e=Mt(O.tpsHistory,Xt),t=Mt(O.feeHistory,Xt);da("tpsTrendMini",e,2,""),da("tpsTrendMini2",e,2,""),da("feeTrendMini",t,0,"d"),da("feeTrendMini2",t,0,"d"),(i=ye.dexPressure)==null||i.draw(ge.dexPressure),(r=ye.nftMints)==null||r.draw(ge.nftMints),(l=ye.nftBurns)==null||l.draw(ge.nftBurns),(c=ye.autoBridge)==null||c.draw(ge.autoBridge),(d=ye.marketPrice)==null||d.draw(ge.marketPrice),(u=ye.marketVol)==null||u.draw(ge.marketVol)}function Tm(){let e=y("ledger-log"),t=y("ledger-log-loading"),n=y("ledger-log-count");if(!e||!(O.ledgerLog||[]).length)return;t&&(t.style.display="none"),n&&(n.textContent=O.ledgerLog.length);let s=y("tab-network");!s||s.style.display==="none"||(e.innerHTML=`<div class="ledger-log-row log-head">
      ...
      </div>`).join(""))}var Lt=[],ha=new Set,Xe=0,pa=null,bs=0,Ft=0,nt=0,Ot=!0,ua=0,yt=!1,va=0,To=!1,ws=null,Qt=0,qn=0,wa=!1,Cm=.18;function Pm(){Lm(),Mm()}function Lm(){let e=y("ledgerStreamParticles");if(e){e.innerHTML="";for(let t=0;t<14;t++){let n=document.createElement("div");n.className="ledger-particle",n.style.left=Math.random()*100+"%",n.style.top=20+Math.random()*60+"%",n.style.animationDuration=6+Math.random()*8+"s",n.style.animationDelay=Math.random()*5+"s",e.appendChild(n)}}}function Mm(){if(pa)return;let e=y("ledgerStreamShell");e&&(e.addEventListener("mouseenter",()=>{yt=!0}),e.addEventListener("mouseleave",()=>{xs||(yt=!1)}));let t=y("ledgerStreamTrack");t&&t.addEventListener("click",s=>{let a=s.target.closest("article.ledger-card");if(!a)return;let o=Number(a.dataset.ledgerIndex);Number.isFinite(o)&&(s.shiftKey?ws===o?(ws=null,xs||(yt=!1),pl()):(ws=o,yt=!0,pl()):Am(o))});let n=s=>{bs||(bs=s);let a=Math.min(.05,(s-bs)/1e3);bs=s;let o=Bt()?y("ledgerStreamTrack"):null;if(o&&Xe>0){if(Ot){let i=o.scrollWidth||0;if(i>100)nt=Math.floor(i/2),Ot=!1,ua=0,Xe>0&&(Qt=nt/Xe);else{ua++,ua>30&&(Ot=!1,ua=0),pa=requestAnimationFrame(n);return}}if(nt>0&&!yt&&wa){let i=Math.min(1,a/Cm);Ft+=(qn-Ft)*i,Math.abs(qn-Ft)<.5&&(Ft=qn);let r=Ft%nt;o.style.transform=`translateX(${-r}px)`}if(!yt&&Math.floor(s/250)!==Math.floor((s-a*1e3)/250)){let i=Date.now(),r=o.querySelectorAll("article.ledger-card[data-arrival-ts]"),l=12e4;r.forEach(c=>{let d=i-Number(c.dataset.arrivalTs||i),u=Math.max(.52,1-d/l*.48);c.style.opacity=u.toFixed(3)})}if(va>0){let i=Date.now()-va>nm;ba(i)}}pa=requestAnimationFrame(n)};pa=requestAnimationFrame(n),window.addEventListener("resize",()=>{Ot=!0})}function Ll(){if(Qt<=0||nt<=0||Xe===0)return Ft;let e=y("ledgerStreamShell"),t=e&&e.offsetWidth||800,n=Qt-14,a=(Xe-1)*Qt+n-t+18;a=Math.max(0,a);let i=Math.floor(Ft/nt)*nt+a;return i<Ft-2&&(i+=nt),i}function ba(e,t){if(e===To)return;To=e;let n=y("ledgerStreamShell");if(!n)return;let s=n.querySelector(".stream-stall-overlay");e?(s||(s=document.createElement("div"),s.className="stream-stall-overlay",n.appendChild(s)),s.innerHTML=`<span class="stream-stall-dot"></span> ${g(t||"Waiting for ledgers\u2026")}`):s==null||s.remove()}function pl(){document.querySelectorAll(".ledger-card").forEach(e=>{let t=Number(e.dataset.ledgerIndex);e.classList.toggle("ledger-card--pinned",t===ws)})}function Am(e){let t=document.querySelector('.dash-tab[data-tab="inspector"]');typeof window.switchTab=="function"&&t?window.switchTab(t,"inspector"):t==null||t.click();let n=document.getElementById("inspect-addr");n&&(n.placeholder=`Ledger #${e.toLocaleString()} \u2014 paste an address`,n.focus())}function Em(){var t;let e=y("ledgerStreamShell");e&&(e.classList.remove("stream-reconnect-flash"),e.offsetWidth,e.classList.add("stream-reconnect-flash"),setTimeout(()=>e.classList.remove("stream-reconnect-flash"),1200),im=((t=jn.at(-1))==null?void 0:t.li)??0,Nm())}function Nm(){let e=document.getElementById("reconnect-banner");if(!e){e=document.createElement("div"),e.id="reconnect-banner",e.className="reconnect-banner";let s=document.querySelector(".dashboard-col-main");s&&s.prepend(e)}e.style.display="",e.innerHTML=`
    ...
    <div class="gateway-item gateway-row">
    ...
    </div>`).join("")}function Km(e){let t=y("d2-cluster-list"),n=y("d2-cluster-persistence");if(t){if(!e.length){t.innerHTML='<div class="gateway-item" style="opacity:.6">Building clusters\u2026 (needs repeated activity)</div>',n&&(n.textContent="\u2014");return}n&&(n.textContent=`${e.length} group${e.length!==1?"s":""}`),t.innerHTML=e.slice(0,6).map((s,a)=>{let i=`hsl(${(a*67+120)%360},70%,60%)`,r=s.members.slice(0,4);return`
      ...
      </div>`}).join("")}}function Jm(e){let t=y("d2-delta-narratives");if(!t)return;if(!e.length){t.innerHTML='<div class="gateway-item" style="opacity:.6">Building baseline \u2014 narratives appear after 1\u20132 ledgers\u2026</div>';return}let n={up:"#50fa7b",down:"#ff5555",new:"#00d4ff",warn:"#ffb86c",ok:"rgba(255,255,255,.85)"};t.innerHTML=e.map(s=>{let a=n[s.sentiment]||"rgba(255,255,255,.85)",o=s.addr&&Fe(s.addr)?`<button class="addr-link narrative-addr" data-addr="${g(s.addr)}">Peek</button>`:"";return`
  ...
  `,t?t.insertAdjacentElement("afterend",s):n?n.insertAdjacentElement("afterend",s):e.prepend(s)}function Zm(){if(Gr)return;Gr=!0;let e=document.querySelector(".dashboard-col-main");if(!e)return;let t=document.createElement("section");t.className="widget-card",t.id="landscape-card",t.setAttribute("aria-label","Landscape brief"),t.innerHTML=`
  ...
  `,e.prepend(t)}function sf(e){var p,m;if(!document.getElementById("dex-pattern-card"))return;let t=e.window.total||0,n=t?Math.round(e.window.cancelRatio*100):0,s=t?Math.round(e.topShare*100):0;V("dexp-badge",t?`${t} offer tx \xB7 ${n}% cancels`:"Quiet");let a=y("dexp-cancel-bar");a&&(a.style.width=`${ke(n,0,100)}%`),V("dexp-cancel-val",t?`${n}%`:"\u2014");let o=y("dexp-topshare-bar");o&&(o.style.width=`${ke(s,0,100)}%`),V("dexp-topshare-val",t?`${s}%`:"\u2014");let i=e.burstPct,r=i==null?0:Math.min(100,Math.abs(i)),l=y("dexp-burst-bar");l&&(l.style.width=`${r}%`),V("dexp-burst-val",i==null?"\u2014":en(i,0)),V("dexp-cpm",e.now.cancelsPerMin==null?"\u2014":e.now.cancelsPerMin.toFixed(1)),V("dexp-hhi",t?e.actorHHI.toFixed(2):"\u2014"),V("dexp-totalwin",t?`${t}`:"\u2014");let c=y("dexp-signals");c&&(c.innerHTML=e.signals.length?e.signals.map(f=>`<span class="sig-pill warn">${g(f)}</span>`).join(""):'<span class="sig-pill ok">No strong DEX signals</span>');let d=y("dexp-cancellers");d&&(d.innerHTML=(p=e.topCanceller)!=null&&p.length?e.topCanceller.slice(0,5).map(f=>`
        ...
        <div class="dex-rowline">
          <button class="addr-link mono cut" data-addr="${g(f.acct)}">${g(z(f.acct))}</button>
          <span class="mono">${f.count}</span>
        </div>`).join(""):'<div style="opacity:.7">\u2014</div>')}function af(){if(Jr)return;Jr=!0;let e=document.querySelector(".dashboard-col-main");if(!e)return;let t=document.createElement("section");t.className="widget-card",t.id="risk-card",t.setAttribute("aria-label","Risk panel"),t.innerHTML=`
    <div class="widget-header">
  ...
  `,t.querySelectorAll(".risk-box-toggle").forEach(n=>{var a;n.addEventListener("click",()=>{let o=n.getAttribute("data-target"),i=document.getElementById(o);if(!i)return;let r=n.getAttribute("aria-expanded")!=="false";n.setAttribute("aria-expanded",r?"false":"true"),i.classList.toggle("risk-collapsed",r),n.querySelector(".risk-box-chevron").textContent=r?"\u25B8":"\u25BE";try{localStorage.setItem("risk_collapsed_"+o,r?"1":"0")}catch{}});let s=n.getAttribute("data-target");try{localStorage.getItem("risk_collapsed_"+s)==="1"&&(n.setAttribute("aria-expanded","false"),(a=document.getElementById(s))==null||a.classList.add("risk-collapsed"),n.querySelector(".risk-box-chevron").textContent="\u25B8")}catch{}}),e.appendChild(t)}function of(e){var r,l,c,d,u,p,m,f,v,h;if(!document.getElementById("risk-card"))return;V("risk-badge",`Risk ${e.friction}/100`),V("risk-regime",e.regime),V("risk-friction",`${e.friction}/100`);let t=[];e.hhi>=.35&&t.push({cls:"warn",t:"High concentration"}),e.breadcrumbs.filter(w=>w.count>=2).length>=3&&t.push({cls:"new",t:"Repeating counterparties"}),(l=(r=e.behavior)==null?void 0:r.bots)!=null&&l.length&&t.push({cls:"warn",t:"Bot-like timing"}),(d=(c=e.dexPatterns)==null?void 0:c.signals)!=null&&d.length&&t.push({cls:"warn",t:"DEX churn signals"}),((u=e.advanced)==null?void 0:u.selfTradeCount)>0&&t.push({cls:"warn",t:`Self-transfer: ${e.advanced.selfTradeCount}`}),(((p=e.advanced)==null?void 0:p.roundnessIdx)??0)>=45&&t.push({cls:"warn",t:`Round-number bias ${e.advanced.roundnessIdx}%`}),((m=e.advanced)==null?void 0:m.pathPays)>0&&t.push({cls:"warn",t:`Path flow: ${e.advanced.pathPays}`}),V("risk-signalcount",`${t.length}`);let n=y("risk-pills");n&&(n.innerHTML=t.length?t.map(w=>`<span class="sig-pill ${w.cls}">${g(w.t)}</span>`).join(""):'<span class="sig-pill ok">No elevated signals</span>');let s=y("risk-bots");if(s){let w=((f=e.behavior)==null?void 0:f.bots)||[];if(w.length){let k={};for(let x of w){let S=x.botType||"Periodic";k[S]||(k[S]=[]),k[S].push(x)}let b=["Market Maker","Arbitrage","Flood / Spam","Periodic"];s.innerHTML=b.filter(x=>k[x]).map(x=>{let S=k[x];return`<div class="bot-type-group">
    ...
    `,e.prepend(t)}}}function lf(){ge.dexPressure=[],ge.nftMints=[],ge.nftBurns=[],ge.autoBridge=[],$o.clear(),Object.keys(ko).forEach(e=>delete ko[e]),_l({offerTotal:null,dexCancelPct:null,dexTopSharePct:null,mints:null,burns:null,pathPays:null,topPathActors:[],topPathPairs:[]})}function zn(e,t,n){for(e.push(Number(t||0));e.length>n;)e.shift()}function cf(e){var n,s,a,o;let t=e==null?void 0:e.advanced;t&&(zn(ge.dexPressure,t.offerTotal,Zu),zn(ge.nftMints,t.mints,Ur),zn(ge.nftBurns,t.burns,Ur),zn(ge.autoBridge,t.pathPays,em),_l(t),(n=ye.dexPressure)==null||n.draw(ge.dexPressure),(s=ye.nftMints)==null||s.draw(ge.nftMints),(a=ye.nftBurns)==null||a.draw(ge.nftBurns),(o=ye.autoBridge)==null||o.draw(ge.autoBridge))}function _l(e){var t,n,s;if(document.getElementById("dex-pressure-card")&&(V("dexP-badge",e.offerTotal==null?"Waiting\u2026":`${e.offerTotal} offer tx`),V("dexP-now",e.offerTotal==null?"\u2014":e.offerTotal),V("dexP-cancel",e.dexCancelPct==null?"\u2014":`${e.dexCancelPct}%`),V("dexP-share",e.dexTopSharePct==null?"\u2014":`${e.dexTopSharePct}%`)),document.getElementById("autobridge-card")){V("ab-badge",e.pathPays==null?"Waiting\u2026":`${e.pathPays} path pays`),V("ab-now",e.pathPays==null?"\u2014":e.pathPays);let a=(t=e.topPathActors)==null?void 0:t[0];V("ab-top-actor",a?z(a.acct):"\u2014"),V("ab-pairs",(n=e.topPathPairs)!=null&&n.length?e.topPathPairs.length:"\u2014");let o=y("ab-top-pairs");o&&(o.innerHTML=(s=e.topPathPairs)!=null&&s.length?e.topPathPairs.map(i=>`
          <div class="dex-rowline">
            <span class="mono cut">${g(z(i.from))}</span>
            <span style="opacity:.7">\u2192</span>
            <span class="mono cut">${g(z(i.to))}</span>
            <span class="mono">${i.count}</span>
          </div>`).join(""):'<div style="opacity:.7">\u2014</div>')}if(document.getElementById("nft-mint-card")){let a=Number(e.mints||0)-Number(e.burns||0);V("nft-badge",e.mints==null&&e.burns==null?"Waiting\u2026":`${e.mints||0} mints \xB7 ${e.burns||0} burns`),V("nft-mints-now",e.mints==null?"\u2014":e.mints),V("nft-burns-now",e.burns==null?"\u2014":e.burns),V("nft-net-now",`${a}`)}}function ya({force:e=!1}={}){if(!Bt()){Ao();return}Kt&&!e||(Kt&&(clearInterval(Kt),Kt=null),ml(),Kt=setInterval(()=>{Bt()&&ml()},tm))}function Ao(){Kt&&(clearInterval(Kt),Kt=null)}async function ml(){var t,n,s,a,o,i,r,l,c,d;let e=++vo;V("mkt-badge","Loading\u2026");try{let p=await fetch("https://api.exchange.coinbase.com/products/XRP-USD/candles?granularity=3600",{cache:"no-store"});if(!p.ok)throw new Error("market history failed");let m=await p.json();if(!Array.isArray(m)||m.length<10)throw new Error("no history");if(e!==vo)return;let f=[...m].sort((x,S)=>Number(x[0])-Number(S[0])),v=f.map(x=>Number(x[4])).filter(Number.isFinite),h=f.map(x=>Number(x[5])).filter(Number.isFinite);ge.marketPrice=v.slice(-la),ge.marketVol=h.slice(-la);let w=ge.marketPrice.at(-1),k=ge.marketPrice.length>24?ge.marketPrice.at(-25):ge.marketPrice.at(0),b=k&&w?(w-k)/k*100:null;V("mkt-badge",w?`$${w.toFixed(4)}`:"\u2014"),V("mkt-price",w?`$${w.toFixed(4)}`:"\u2014"),V("mkt-chg",b==null?"\u2014":en(b,2)),V("mkt-upd",new Date().toLocaleTimeString()),(t=ye.marketPrice)==null||t.draw(ge.marketPrice),(n=ye.marketVol)==null||n.draw(ge.marketVol);return}catch{}try{let u=await fetch("https://api.coinpaprika.com/v1/tickers/xrp-xrp",{cache:"no-store"});if(!u.ok)throw new Error("tick failed");let p=await u.json();if(e!==vo)return;let m=Number((a=(s=p==null?void 0:p.quotes)==null?void 0:s.USD)==null?void 0:a.price),f=Number((i=(o=p==null?void 0:p.quotes)==null?void 0:o.USD)==null?void 0:i.volume_24h),v=Number((l=(r=p==null?void 0:p.quotes)==null?void 0:r.USD)==null?void 0:l.percent_change_24h);Number.isFinite(m)&&(zn(ge.marketPrice,m,la),Number.isFinite(f)&&zn(ge.marketVol,f,la)),V("mkt-badge",Number.isFinite(m)?`$${m.toFixed(4)}`:"\u2014"),V("mkt-price",Number.isFinite(m)?`$${m.toFixed(4)}`:"\u2014"),V("mkt-chg",Number.isFinite(v)?`${v>=0?"\u2191":"\u2193"}${Math.abs(v).toFixed(2)}%`:"\u2014"),V("mkt-upd",new Date().toLocaleTimeString()),(c=ye.marketPrice)==null||c.draw(ge.marketPrice),(d=ye.marketVol)==null||d.draw(ge.marketVol)}catch{V("mkt-badge","Unavailable")}}function df(){var n,s,a,o;if(sl)return;sl=!0;let e=document.querySelector(".dashboard-fullwidth");if(!e){e=document.createElement("div"),e.className="dashboard-fullwidth";let i=document.querySelector(".dashboard-columns");i?i.insertAdjacentElement("afterend",e):(n=document.querySelector(".dashboard-page"))==null||n.appendChild(e)}if(document.getElementById("spam-defense-card"))return;let t=document.createElement("section");t.className="widget-card",t.id="spam-defense-card",t.setAttribute("aria-label","Spam defense"),Eo(),Fl(),t.innerHTML=`
  ...
  `,e.appendChild(t),(s=y("spam-copy-hash"))==null||s.addEventListener("click",()=>{var i;return fl(((i=y("spam-proof-hash"))==null?void 0:i.textContent)||"")}),(a=y("spam-copy-json"))==null||a.addEventListener("click",()=>{var i;return fl(((i=y("spam-proof-json"))==null?void 0:i.textContent)||"")}),(o=y("spam-print-proof"))==null||o.addEventListener("click",xf),_renderBondSim()}window._toggleBondSim=function(){let e=y("spam-sim-panel");if(!e)return;let t=e.style.display!=="none";e.style.display=t?"none":"";let n=y("spam-sim-toggle");n&&(n.style.color=t?"":"#00d4ff")};window._renderBondSim=function(){let e=document.getElementById("sim-base"),t=document.getElementById("sim-growth"),n=document.getElementById("sim-table");if(!n)return;let s=Number((e==null?void 0:e.value)||Kn),a=Number((t==null?void 0:t.value)||Jn),o=ge.marketPrice.at(-1)??null;document.getElementById("sim-base-val")&&(document.getElementById("sim-base-val").textContent=s+" XRP"),document.getElementById("sim-growth-val")&&(document.getElementById("sim-growth-val").textContent=a+"\xD7");let i="";for(let r=0;r<=Ss;r++){let l=Math.round(s*a**r),c=o?"$"+(l*o).toLocaleString(void 0,{maximumFractionDigits:0}):"",d=r<3?"#50fa7b":r<6?"#ffb86c":"#ff5555";i+=`<div style="padding:5px;border-radius:6px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07)">
      ...
      ${c?`<div style="font-size:.62rem;opacity:.45">${c}</div>`:""}
    </div>`}n.innerHTML=i};async function fl(e){try{await navigator.clipboard.writeText(String(e||"")),ne("Copied")}catch{he("Copy failed (browser blocked clipboard).")}}function ks(e){let t=ke(Number(e)||0,0,Ss);return Math.round(Kn*Jn**t)}function Eo(){try{let e=JSON.parse(localStorage.getItem(kl)||"[]");ae.allowList=new Set(e)}catch{ae.allowList=new Set}}function Rl(){try{localStorage.setItem(kl,JSON.stringify([...ae.allowList]))}catch{}}function Dl(e){Fe(e)&&(ae.allowList.add(e),Rl(),ae.byAddr.delete(e),ne(`${z(e)} added to allow-list \u2014 will no longer be flagged`))}window._spamAllowAddr=Dl;function Il(){try{return JSON.parse(localStorage.getItem(xl)||"{}")}catch{return{}}}function pf(e){try{localStorage.setItem(xl,JSON.stringify(e))}catch{}}function Fl(){let e=Il();for(let[t,n]of Object.entries(e))ae.verifiedCache.set(t,n)}window.exportSpamReputation=function(){let e=[...ae.byAddr.entries()].map(([o,i])=>({addr:o,level:i.level,strikes:i.strikes,score:+(i.score||0).toFixed(4),threatType:i.threatType||"Unknown",verified:!!i.verifiedLedger,verifiedLedger:i.verifiedLedger??null,lastSeen:i.lastSeenLedger??null})),t={v:2,network:O.currentNetwork||"xrpl-mainnet",exportedAt:new Date().toISOString(),allowList:[...ae.allowList],suspects:e,policy:{bondBaseXrp:Kn,bondGrowth:Jn,ratchetMax:Ss}},n=new Blob([JSON.stringify(t,null,2)],{type:"application/json"}),s=URL.createObjectURL(n),a=document.createElement("a");a.href=s,a.download=`naluxrp_reputation_${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(s),ne("Reputation list exported")};window.importSpamReputation=function(e){if(!e)return;let t=new FileReader;t.onload=n=>{var s;try{let a=JSON.parse(n.target.result);if(!a.suspects)throw new Error("Invalid format");let o=0;for(let i of a.suspects){if(!Fe(i.addr))continue;let r=ae.byAddr.get(i.addr)||{};ae.byAddr.set(i.addr,{...r,level:i.level??0,strikes:i.strikes??0,score:i.score??0,threatType:i.threatType??"Unknown",verifiedLedger:i.verifiedLedger??null,lastSeenLedger:i.lastSeen??null,scoreHistory:[],signalBreakdown:{}}),o++}if((s=a.allowList)!=null&&s.length){for(let i of a.allowList)Fe(i)&&ae.allowList.add(i);Rl()}ne(`Imported ${o} reputation entries`)}catch(a){he("Import failed: "+a.message)}},t.readAsText(e)};function uf(e){var l,c;if(!document.getElementById("spam-defense-card"))return;let t=gf(e),n=t.reduce((d,u)=>Math.max(d,u.level),0),s=t.filter(d=>d.verified).length,a=ge.marketPrice.at(-1)??null,o=t.reduce((d,u)=>d+ks(u.level),0),i=a?`~$${(o*a).toLocaleString(void 0,{maximumFractionDigits:0})}`:`${o} XRP`;V("spam-count",t.length),V("spam-maxlvl",`L${n}`),V("spam-verified",s),V("spam-allowcount",ae.allowList.size),V("spam-bondusd",i),V("spam-badge",t.length?`${t.length} tracked \xB7 max L${n}`:"Quiet");let r=y("spam-list");if(r&&(t.length?r.innerHTML=t.map(d=>{let u=ks(d.level),p=a?` (~$${(u*a).toFixed(0)})`:"",m=d.level>=6?"#ff5555":d.level>=3?"#ffb86c":"#50fa7b",f=d.score>=.7?"#ff5555":d.score>=.4?"#ffb86c":"#50fa7b",v={"Payment Flooder":"#ff5555","Quote Stuffer":"#ff5555","Wash Trader":"#ff5555","DEX Bot":"#ffb86c","Arb Router":"#00d4ff","Periodic Bot":"#bd93f9"}[d.threatType]||"rgba(255,255,255,.6)",h=d.verified?`<span class="spam-cred-chip">\u2714 L${d.level} verified</span>`:"",w=mf(d.scoreHistory||[]);return`
          <div class="spam-card" data-spam-addr="${g(d.addr)}">
            ...
            <div class="spam-breakdown" id="spam-bd-${g(d.addr.slice(0,10))}" style="display:none">
              ${ff(d.breakdown,d.strikes)}
            </div>
          </div>`}).join(""):r.innerHTML=`<div class="spam-empty">No suspects flagged \u2014 allow-list has ${$l.size} known-good entities.</div>`),Co||hf(),ae.selectedAddr){let d=ae.byAddr.get(ae.selectedAddr);d?No(ae.selectedAddr,d,e.s.ledgerIndex).then(u=>Gn(u)):Gn(null)}((l=document.getElementById("spam-sim-panel"))==null?void 0:l.style.display)!=="none"&&((c=window._renderBondSim)==null||c.call(window))}function mf(e){if(!(e!=null&&e.length))return"";let t=42,n=14,s=1,a=t/Math.max(1,e.length-1),o=e.map((l,c)=>`${(c*a).toFixed(1)},${(n-2-l/s*(n-4)).toFixed(1)}`).join(" "),i=e.at(-1)??0,r=i>=.7?"#ff5555":i>=.4?"#ffb86c":"#50fa7b";return`<svg width="${t}" height="${n}" xmlns="http://www.w3.org/2000/svg" style="overflow:visible">
    ...
    <circle cx="${((e.length-1)*a).toFixed(1)}" cy="${(n-2-i/s*(n-4)).toFixed(1)}"
      r="2" fill="${r}"/>
  </svg>`}function ff(e={},t=0){let n=[{label:"Bot timing",val:e.bot??0,desc:e.botType?`Type: ${e.botType}`:"Low variance in repeated ledger appearances"},{label:"DEX dominance",val:e.dexDom??0,desc:"Share of offer tx window controlled by this address"},{label:"Cancel pattern",val:e.cancelPat??0,desc:"Heavy OfferCancel activity relative to creates"},{label:"Ping-pong loop",val:e.pingPong??0,desc:"Bidirectional repeated payments between two addresses"},{label:"Path routing",val:e.pathRoute??0,desc:"High path-payment count \u2014 common in arb bots"},{label:"Self-transfer",val:e.selfTrade??0,desc:"Payments where sender = receiver"},{label:"Round-number pay",val:e.roundPay??0,desc:"Unusual bias toward exact round-number payment amounts"}].filter(s=>s.val>.001);return n.length?`
    ...
    `)}function gf(e){var o,i,r,l,c;ae.allowList.size||Eo();let t=new Set;(((o=e.behavior)==null?void 0:o.bots)||[]).forEach(d=>t.add(d.acct)),(((i=e.dexPatterns)==null?void 0:i.topActor)||[]).forEach(d=>t.add(d.acct)),(((r=e.dexPatterns)==null?void 0:r.topCanceller)||[]).forEach(d=>t.add(d.acct)),(((l=e.dexPatterns)==null?void 0:l.topMaker)||[]).forEach(d=>t.add(d.acct)),(e.breadcrumbs||[]).forEach(d=>{t.add(d.from),t.add(d.to)}),(e.clusters||[]).forEach(d=>{t.add(d.hub),(d.members||[]).forEach(u=>t.add(u))}),(((c=e.advanced)==null?void 0:c.topPathActors)||[]).forEach(d=>t.add(d.acct));let n=Number(e.s.ledgerIndex||0)||null,s=e.txs||[],a=[];if(n&&n%50===0){let d=ma*24;for(let[u,p]of ae.byAddr)n-(p.lastSeenLedger||0)>d&&ae.byAddr.delete(u)}for(let d of t){if(!Fe(d)||$l.has(d)||ae.allowList.has(d))continue;let{score:u,breakdown:p}=bf(d,e),m=vf(p),f=ae.byAddr.get(d)||{strikes:0,level:0,score:0,verifiedLedger:null,lastSeenLedger:null,scoreHistory:[],threatType:"Unknown",signalBreakdown:{}},v=f.strikes||0;u>=wl?v+=1:u<=wo&&(v=Math.max(0,v-1));let h=f.lastSeenLedger||n,w=n?n-h:0;if(w>ma&&u<wo){let $=Math.floor(w/ma);v=Math.max(0,v-$)}let k=ke(Math.floor(v/yl),0,Ss),b=[...f.scoreHistory||[],u].slice(-10),x=ae.verifiedCache.get(d),S=f.verifiedLedger??(x==null?void 0:x.ledgerIndex)??null;S||(S=yf(d,k,s,n),S&&wf(d,k,n));let T={score:u,strikes:v,level:k,verifiedLedger:S,lastSeenLedger:n,scoreHistory:b,threatType:m,signalBreakdown:p};ae.byAddr.set(d,T),a.push({addr:d,score:u,strikes:v,level:k,verified:!!S,verifiedLedger:S,scoreHistory:b,threatType:m,breakdown:p})}return a.sort((d,u)=>u.level-d.level||u.score-d.score),a.slice(0,sm)}function vf(e){let{bot:t=0,dexDom:n=0,cancelPat:s=0,pingPong:a=0,pathRoute:o=0,selfTrade:i=0,roundPay:r=0}=e,l=n+s;return t>.25&&(e.botType==="Flood / Spam"||r>.05)?"Payment Flooder":t>.2&&l>.15?"DEX Bot":l>.3&&s>.1?"Quote Stuffer":a>.1?"Wash Trader":o>.1?"Arb Router":t>.2?"Periodic Bot":"Multi-Signal"}function bf(e,t){var m,f,v,h,w;let n={bot:0,botType:null,dexDom:0,cancelPat:0,pingPong:0,pathRoute:0,selfTrade:0,roundPay:0},s=(((m=t.behavior)==null?void 0:m.bots)||[]).find(k=>k.acct===e);if(s){let k=ke((.35-s.cv)/.35,0,1);n.bot=+(.4*(.5+.5*k)).toFixed(3),n.botType=s.botType||"Periodic"}let a=t.dexPatterns,o=((v=(f=a==null?void 0:a.topActor)==null?void 0:f[0])==null?void 0:v.acct)===e&&a.topShare||0;o>0&&(n.dexDom=+(.3*ke((o-.2)/.4,0,1)).toFixed(3));let i=((a==null?void 0:a.topCanceller)||[]).find(k=>k.acct===e);i&&(n.cancelPat=+(.2*ke(i.count/20,0,1)).toFixed(3));let r=t.breadcrumbs||[],l=r.find(k=>k.from===e&&k.to===e);l&&(n.selfTrade=+(.2*ke(l.count/20,0,1)).toFixed(3));let c=r[0];if(c&&(c.from===e||c.to===e)){let k=r.find(b=>b.from===c.to&&b.to===c.from);k&&k.count>=3&&c.count>=3&&(n.pingPong=.15)}let d=(((h=t.advanced)==null?void 0:h.topPathActors)||[]).find(k=>k.acct===e);d&&(n.pathRoute=+(.15*ke(d.count/25,0,1)).toFixed(3));let u=(w=t.advanced)==null?void 0:w.roundnessIdx;return u!=null&&u>=45&&(d||s)&&(n.roundPay=+(.1*ke((u-45)/35,0,1)).toFixed(3)),{score:ke(n.bot+n.dexDom+n.cancelPat+n.pingPong+n.pathRoute+n.selfTrade+n.roundPay,0,1),breakdown:n}}function yf(e,t,n,s){if(!(n!=null&&n.length))return null;let a=Un+t;for(let o of n)if((o==null?void 0:o.account)===e){if(o.type==="EscrowCreate"&&o.destination===e){let i=typeof o.amountXrp=="number"?o.amountXrp:null,r=ks(t);if(i!=null&&i>=r&&Po(o.memos).startsWith(Jt))return s}if(o.type==="Payment"&&o.destination===e&&Number(o.destinationTag)===a&&Po(o.memos).startsWith(Jt))return s}return null}async function wf(e,t,n){var a;let s=ae.byAddr.get(e);if(s)try{let o=await No(e,s,n),i=Jt+o.hash;if((((a=O.ledgerLog)==null?void 0:a.flatMap(c=>c.transactions||[]))||[]).some(c=>(c==null?void 0:c.account)!==e?!1:Po(c.memos).startsWith(i))){let c=Il();c[e]={ledgerIndex:n,hash:o.hash,level:t},pf(c),ae.verifiedCache.set(e,{ledgerIndex:n,hash:o.hash}),ne(`\u2714 Credential verified for ${z(e)} at L${t}`)}}catch{}}function Po(e){if(!Array.isArray(e))return"";let t=[];for(let n of e){let s=(n==null?void 0:n.Memo)||(n==null?void 0:n.memo)||null;if(!s)continue;let a=s.MemoData||s.memo_data||"";try{if(typeof a=="string"&&/^[0-9A-Fa-f]+$/.test(a)&&a.length%2===0){let o=Uint8Array.from(a.match(/../g),i=>parseInt(i,16));t.push(new TextDecoder().decode(o))}else typeof a=="string"&&t.push(a)}catch{}}return t.join(" ")}async function No(e,t,n){let s=t.level||0,a=ks(s),o=ge.marketPrice.at(-1)??null,i=o?(a*o).toFixed(2):null,r={hashAlgorithm:"SHA-512Half",ratchetMax:Ss,strikesToLevel:yl,strikeUp:wl,strikeDown:wo,decayLedgers:ma,bondBaseXrp:Kn,bondGrowthFactor:Jn,bondRequiredXrp:a,bondRequiredUsd:i,credTagBase:Un,memoPrefix:Jt,credentialTag:Un+s},l={v:2,hashAlg:"SHA-512Half",network:O.currentNetwork||"xrpl-mainnet",address:e,ledgerIndex:n!=null?Number(n):null,level:s,threatType:t.threatType||"Unknown",score:Number((t.score||0).toFixed(4)),strikes:Number(t.strikes||0),signalBreakdown:t.signalBreakdown||{},credential:{destinationTag:Un+s,memoFormat:Jt+"<SHA-512Half-of-this-proof>",verifiedLedger:t.verifiedLedger??null},policy:r,note:"Generated by NaluXRP Spam Defense POC. Hash uses XRPL-native SHA-512Half (first 256 bits of SHA-512). A gateway or relayer policy can require the indicated bond/credential before providing service to this address.",generatedAt:new Date().toISOString()},c=JSON.stringify(l,Object.keys(l).sort(),0),d=await cm(c);l.credential.memoFormat=Jt+d;let u=JSON.stringify(l,null,2);return{hash:d,canonicalJson:u,level:s,policy:r,threatType:t.threatType}}function xf(){let e=ae.selectedProof;if(!e){he("Generate a proof first using the Proof button.");return}let t=ae.selectedAddr||"\u2014",n=ge.marketPrice.at(-1)??null,s=e.policy.bondRequiredXrp,a=n?` (~$${(s*n).toFixed(0)} USD)`:"",o=e.hash.startsWith("FALLBACK")?'<div style="padding:8px 12px;background:#fff3cd;border:1px solid #f0ad4e;border-radius:4px;margin-bottom:12px">\u26A0 Hash is non-cryptographic (SubtleCrypto unavailable). Do NOT use for enforcement.</div>':"",i=window.open("","_blank","width=760,height=660");i.document.write(`<!DOCTYPE html><html><head>
    ...
    </body></html>`),i.document.close()}var Ol=null;function kf(){if(xo)return;xo=!0;let e=document.querySelector('[aria-label="Pattern detection"]');if(!e){e=document.createElement("section"),e.className="widget-card",e.setAttribute("aria-label","Pattern detection");let t=document.querySelector(".dashboard-col-main");if(!t)return;let n=t.querySelector(".dashboard-metrics");n?n.insertAdjacentElement("afterend",e):t.prepend(e)}e.innerHTML=`
    <div class="widget-header">
    ...
    </div>`}).join("")}function Lf(){if(ol)return;ol=!0;let e=document.querySelector(".dashboard-col-main");if(!e)return;let t=document.createElement("section");t.className="widget-card",t.id="network-health-card",t.setAttribute("aria-label","Network health"),t.innerHTML=`
    <div class="widget-header">
    ...
    <div style="display:flex;align-items:center;gap:16px;padding:8px 0 12px">
      ...
      </div>`}).join(""))}function Af(){if(il)return;il=!0;let e=document.querySelector(".dashboard-col-side");if(!e)return;let t=document.createElement("section");t.className="widget-card",t.id="session-stats-card",t.setAttribute("aria-label","Session stats"),t.innerHTML=`
    <div class="widget-header">
    ...
    </div>`,e.appendChild(t)}function Ef(){if(!document.getElementById("session-stats-card"))return;let e=Math.floor((Date.now()-We.startTime)/6e4);V("ss-ledgers",We.ledgersProcessed.toLocaleString()),V("ss-tx",We.totalTx.toLocaleString()),V("ss-whales",We.whaleCount),V("ss-feespikes",We.feeSpikes),V("ss-bots",We.botDetections),V("ss-dexalerts",We.dexAlerts),V("ss-time",`${e}m`),V("ss-badge",`${We.ledgersProcessed} ledgers`)}function Nf(e,t){var o,i,r,l,c,d,u,p;if(Sl=t,wt.length&&wt[0].ledgerIndex===t){let m=wt[0],f=m.amtXrp>=1e6?`${(m.amtXrp/1e6).toFixed(1)}M`:`${(m.amtXrp/1e3).toFixed(0)}K`;ne(`\u{1F40B} Whale: ${f} XRP  ${z(m.from)} \u2192 ${z(m.to)}`)}let n=Ml(),s=(o=O.feeHistory)==null?void 0:o.at(-1);s&&n&&s>n*kn.feeSpikeMultiple&&(We.feeSpikes++,he(`\u{1F525} Fee spike: ${Yt(s)} (${Math.round(s/n)}\xD7 baseline)`));let a=(((i=e.behavior)==null?void 0:i.bots)||[]).filter(m=>m.cv<kn.botCvThreshold&&m.total>8);a.length&&(We.botDetections=Math.max(We.botDetections,a.length),a[0].botType==="Flood / Spam"&&he(`\u{1F916} Spam bot: ${z(a[0].acct)} (CV ${a[0].cv.toFixed(2)})`)),((l=(r=e.dexPatterns)==null?void 0:r.window)==null?void 0:l.cancelRatio)>=kn.dexCancelAlert&&((d=(c=e.dexPatterns)==null?void 0:c.window)==null?void 0:d.total)>=20&&(We.dexAlerts++,he(`\u{1F9E0} DEX: ${Math.round(e.dexPatterns.window.cancelRatio*100)}% cancel ratio \u2014 possible quote-stuffing`)),((p=(u=e.clusters)==null?void 0:u[0])==null?void 0:p.size)>=kn.clusterMinSize&&ne(`\u{1F578} Cluster: ${e.clusters[0].size} wallets around ${z(e.clusters[0].hub)}`),e.friction>=75&&e.regime==="Manipulated"&&he(`\u26A0\uFE0F Risk score: ${e.friction}/100 \u2014 ${e.regime} regime`),Mf(e.s)}function _f(){var n,s,a,o;if(rl)return;rl=!0;let e=document.createElement("button");e.id="customize-btn",e.className="customize-btn",e.setAttribute("aria-pressed","false"),e.innerHTML="\u2699 Customize",(n=document.querySelector(".dashboard-header"))==null||n.appendChild(e);let t=document.createElement("div");t.id="customize-panel",t.className="customize-panel",t.setAttribute("aria-label","Dashboard customizer"),t.innerHTML=`
...
`).trim();if(!r)throw new Error("AI response was empty.");return r}var zf="https://esm.run/@mlc-ai/web-llm",Io=[{id:"Llama-3.2-3B-Instruct-q4f16_1-MLC",label:"Llama 3.2 3B \u2014 better quality, ~2.3GB download"},{id:"Llama-3.2-1B-Instruct-q4f16_1-MLC",label:"Llama 3.2 1B \u2014 faster/lighter, ~0.9GB download, weaker answers"}],Ts=Io[0].id;function Fo(){return typeof navigator<"u"&&!!navigator.gpu}var Do=null,$n=null,xa=null;async function Uf(){return Do||(Do=await import(zf)),Do}async function jf(e=Ts,t){if(!Fo())throw new Error("Your browser doesn\u2019t support WebGPU, which the local model needs. Try a recent Chrome or Edge on desktop.");if($n&&xa===e)return $n;let n=await Uf();return $n&&xa!==e?(await $n.reload(e),xa=e,$n):($n=await n.CreateMLCEngine(e,{initProgressCallback:t||(()=>{})}),xa=e,$n)}async function zl(e,{system:t,modelId:n=Ts,onProgress:s}={}){var l,c,d,u;let a=await jf(n,s),o=[];t&&o.push({role:"system",content:t}),o.push({role:"user",content:e});let i=await a.chat.completions.create({messages:o}),r=(u=(d=(c=(l=i==null?void 0:i.choices)==null?void 0:l[0])==null?void 0:c.message)==null?void 0:d.content)==null?void 0:u.trim();if(!r)throw new Error("Local model returned an empty response.");return r}var ze={lsfPasswordSpent:65536,lsfRequireDestTag:131072,lsfRequireAuth:262144,lsfDisallowXRP:524288,lsfDisableMaster:1048576,lsfNoFreeze:2097152,lsfGlobalFreeze:4194304,lsfDefaultRipple:8388608,lsfDepositAuth:16777216},Ul={lsfBurnable:1,lsfOnlyXRP:2,lsfTrustLine:4,lsfTransferable:8},Wf=new Set(["SetRegularKey","SignerListSet","AccountSet","AccountDelete","EscrowCreate","PaymentChannelCreate","DepositPreauth"]),tn={blackhole:"#ff5555",exchange:"#00d4ff",newWallet:"#ff79c6",issuer:"#ffb86c",wallet:"#bd93f9",other:"#8be9fd"},jl=.55,qf=.15,Zn=20,Vf=946684800,Gf=new Map([["rPVMhWBsfF9iMXYj3aAzJVkPDTFNSyWdKy",{name:"Bitstamp",type:"exchange"}],["rvYAfWj5gh67oV6fW32ZzP3Aw4Eubs59B",{name:"Bitstamp",type:"exchange"}],["rrpNnNLKrartuEqfJGpqyDwPj1BBN1ih7",{name:"Bitstamp",type:"exchange"}],["rN7n3473SaZBCG4dFL83w7PB9judJ7qdDo",{name:"Binance",type:"exchange"}],["rEb8TK3gBgk5auZkwc6sHnwrGVJH8DuaLh",{name:"Binance",type:"exchange"}],["rBKPS4oLSaV2KVVuHH8EpQqMGgGefGFQs7",{name:"Bitso",type:"exchange"}],["rfk5bwaKCoNU84fTzdqWQowqnNaZorDmiV",{name:"Gate.io",type:"exchange"}],["rGFuMiw48HdbnrUbkRYDTvT5i9imC5fvv9",{name:"Gate.io",type:"exchange"}],["rwYHCs2EYBMBvRXFmxDrCUSorPsuqCck7t",{name:"Kraken",type:"exchange"}],["rLHzPsX6oXkzU2qL12kHCH8G8cnZv1rBJh",{name:"Kraken",type:"exchange"}],["ra5nK24KXen9AHvsdFTKHSANinZseWnPcX",{name:"Uphold",type:"exchange"}],["rGWrZyax5eXbi5gs49MRZKkE9eKNL9p4B",{name:"Bittrex",type:"exchange"}],["rDsbeomae4FXwgQTJp9Rs64Qg9vDiTCdBv",{name:"Coinone",type:"exchange"}],["rHsMUQFzBb7S6GnQFVgNirqvHRcLpAn5dU",{name:"Bithumb",type:"exchange"}],["rMQ98K56yXJbDGv49ZSmW51sLn94Xe1mu1",{name:"Huobi",type:"exchange"}],["rHcFoo6a9qT5NHiVn1THwX3B4QF2VQKWZ",{name:"Huobi",type:"exchange"}],["rKiCet8SdvWxPXnAgYarFUXMh1zCPz432Y",{name:"Coinbase",type:"exchange"}],["rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",{name:"Coinbase",type:"exchange"}],["r9mhdcT2K7FdCGDEPqfbMJwVXsXCqEr5bP",{name:"OKX",type:"exchange"}],["r32U8WFxhqEAVkKcTb1GGRR1VH2oaFdexN",{name:"OKX",type:"exchange"}],["r4GDFMLGJUKMjNEycBKPGnRSNXyNVLQLHi",{name:"Bybit",type:"exchange"}],["rBETszU65yYoFcYdRkiGqFaYmhZpHWC7sj",{name:"Bybit",type:"exchange"}],["rMWUykAmNQDaM9poSes8VLDZDDkEoutilities",{name:"KuCoin",type:"exchange"}],["rUA1S9qobBkxLqzdfGEzh5wm5KdLfbf8bx",{name:"KuCoin",type:"exchange"}],["rHtbQzmN4BDaEBnGSXp3AZaZAuZamNVsME",{name:"MEXC",type:"exchange"}],["rDN1gPWW3XQFXVJFQSiJxPHGZiRLMVSi7K",{name:"MEXC",type:"exchange"}],["rB3gZey7VWHoDokMt3tCiXBSRmaZi5xJi9",{name:"Crypto.com",type:"exchange"}],["rhub8VRN55s94qWKDv6jmDy1pUykJzF3wq",{name:"GateHub",type:"exchange"}],["razqnFn6FqBaYBdNaGnVzmGaNE6XPRQ9bG",{name:"GateHub",type:"exchange"}],["rGQdkxNBQeQC1WTQDQ2F2QoGBZxYcMxBBg",{name:"GateHub",type:"exchange"}],["rpXTzCuXtjiPDFysxq8uNmtZBe9Xo97JbW",{name:"Bitbank",type:"exchange"}],["rsuUjfWxrACCAwGQDsNeZUhpzXf1n1NK5Z",{name:"Bitbank",type:"exchange"}],["r9oxUGJqMfMEhGBxrMJnmNvVh1LKkMv7fz",{name:"Coincheck",type:"exchange"}],["rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",{name:"Genesis (Black Hole)",type:"blackhole"}],["r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59",{name:"Black Hole #2",type:"blackhole"}],["rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",{name:"Genesis Wallet",type:"ripple"}],["rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",{name:"Ripple Labs Ops",type:"ripple"}],["rPEPPER7kfTD9w2To4CQk6UCfuHM9c6GDY",{name:"XAMAN (XUMM)",type:"wallet"}],["rBj4eVRWn6mCELVTNkVFDfGNByE9VFTM3R",{name:"XAMAN",type:"wallet"}],["rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",{name:"XRPL AMM Engine",type:"dex"}],["rsoLo2S1kiGeCcn6hCUXVrCpGMWLrRrLZz",{name:"SOLO Issuer",type:"issuer"}],["rcoreNywaoz2ZCVt2sc3JiEi7G7MpZxZgm",{name:"CORE Token",type:"issuer"}],["rhXo4TcWbLY4GqTSmscMpgZ1KMXFBi9V55",{name:"XRPL DeFi Pool",type:"issuer"}],["rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",{name:"UNL Validator Set",type:"validator"}]]);function ns(e){return Gf.get(e)||null}var Kf=null,st=!1;var es=null,Wl=!1,Bo=!0;function ka(){return Bo&&O.currentPage==="dashboard"&&O.currentTab==="inspector"&&!document.hidden}async function Jf(e){let t=[s=>`https://corsproxy.io/?${encodeURIComponent(s)}`,s=>`https://api.allorigins.win/raw?url=${encodeURIComponent(s)}`],n=async s=>{let a=typeof(AbortSignal==null?void 0:AbortSignal.timeout)=="function"?AbortSignal.timeout(6e3):void 0,o=await fetch(s,{mode:"cors",cache:"no-store",signal:a});if(!o.ok)throw new Error(`HTTP ${o.status}`);return await o.json()};try{return await n(e)}catch{for(let s of t)try{return await n(s(e))}catch{}throw new Error("Price feed unavailable")}}async function Yf(){if(Wl)return es;Wl=!0;try{let e=await Jf("https://api.exchange.coinbase.com/products/XRP-USD/ticker");es=Number((e==null?void 0:e.price)||0)||null}catch{es=null}return es}function sn(e){if(!es||!e)return"";let t=e*es;return t>=1e6?` (~$${(t/1e6).toFixed(2)}M)`:t>=1e3?` (~$${(t/1e3).toFixed(1)}K)`:` (~$${t.toFixed(2)})`}var Ps=null;function Vl(){return Ps||(Ps={input:()=>document.getElementById("inspect-addr"),err:document.getElementById("inspect-err"),result:document.getElementById("inspect-result"),empty:document.getElementById("inspect-empty"),loading:document.getElementById("inspect-loading"),loadMsg:document.getElementById("inspect-loading-msg"),warn:document.getElementById("inspect-warn"),badge:document.getElementById("inspect-addr-badge"),score:document.getElementById("inspect-risk-score"),label:document.getElementById("inspect-risk-label")},Ps)}function Qf(){Ps=null,Vl()}function Gl(){var t,n,s,a;ig(),rg(),lg(),(t=y("inspect-addr"))==null||t.addEventListener("keydown",o=>{o.key==="Enter"&&ts()}),(n=y("inspect-addr"))==null||n.addEventListener("paste",()=>{setTimeout(()=>{var i;let o=(i=y("inspect-addr"))==null?void 0:i.value.trim();o&&Fe(o)&&ts()},60)}),(s=document.getElementById("tab-inspector"))==null||s.addEventListener("click",o=>{var r;let i=o.target.closest(".section-header");i&&((r=i.closest(".inspector-section"))==null||r.classList.toggle("collapsed"))}),(a=document.getElementById("inspector-nav"))==null||a.addEventListener("click",o=>{let i=o.target.closest("[data-jump]");if(!i)return;let r=document.getElementById("section-"+i.dataset.jump);r&&(r.classList.remove("collapsed"),r.scrollIntoView({behavior:"smooth",block:"start"})),ec(i.dataset.jump)});let e=!1;window.addEventListener("scroll",()=>{ka()&&(e||(e=!0,requestAnimationFrame(()=>{cg(),e=!1})))},{passive:!0}),window.runInspect=ts,window.inspectorCopyAddr=pg,window.showInspectorHowTo=ug,window.hideInspectorHowTo=Ho,Qf(),gg(),window.addEventListener("naluxrp:tabchange",o=>{var i;((i=o.detail)==null?void 0:i.tabId)==="inspector"&&ka()&&(jo(),Wo(),Es(),$a())})}function Kl(e){Bo=!!e,Bo||(st=!0)}async function ts(){var a,o,i,r,l,c,d,u,p,m,f,v,h,w;if(!ka())return;let e=Vl(),t=((a=e.input())==null?void 0:a.value.trim())||"";if([e.err,e.result,e.empty,e.warn].forEach(k=>k&&(k.style.display="none")),st=!0,!t){e.empty&&(e.empty.style.display="");return}if(!Fe(t)){e.err&&(e.err.textContent=`\u26A0 Invalid address: ${g(t)}`,e.err.style.display="");return}if(O.connectionState!=="connected"){e.warn&&(e.warn.style.display="");return}Kf=t,st=!1;let n=k=>{e.loading&&(e.loading.style.display="",e.loadMsg&&(e.loadMsg.textContent=k))};n("Fetching account data\u2026");let s=k=>new Promise(b=>setTimeout(b,k));try{n("Fetching account data\u2026");let[k,b,x]=await Promise.all([Me({command:"account_info",account:t,ledger_index:"validated"}),Me({command:"account_offers",account:t,ledger_index:"validated"}),Me({command:"account_nfts",account:t,ledger_index:"validated"}).catch(()=>null)]);if(st)return;let S=((o=k==null?void 0:k.result)==null?void 0:o.account_data)||{},T=((i=b==null?void 0:b.result)==null?void 0:i.offers)||[],$=((r=x==null?void 0:x.result)==null?void 0:r.account_nfts)||[];n("Fetching trustlines\u2026");let P=[],M,N=0;do{N++;let H={command:"account_lines",account:t,ledger_index:"validated",limit:400};M&&(H.marker=M);let Y=await Me(H).catch(()=>null);if(st)return;let me=((l=Y==null?void 0:Y.result)==null?void 0:l.lines)||[];P.push(...me),M=((c=Y==null?void 0:Y.result)==null?void 0:c.marker)||null,N>1&&await s(50)}while(M&&P.length<4e3);n("Fetching account objects\u2026");let _=[],X,I=0;do{I++;let H={command:"account_objects",account:t,ledger_index:"validated",limit:400};X&&(H.marker=X);let Y=await Me(H).catch(()=>null);if(st)return;let me=((d=Y==null?void 0:Y.result)==null?void 0:d.account_objects)||[];_.push(...me),X=((u=Y==null?void 0:Y.result)==null?void 0:u.marker)||null,I>1&&await s(50)}while(X&&_.length<2e3);n("Fetching token supply, AMM data & price\u2026");let D=P.filter(H=>H.currency&&(H.currency.startsWith("03")||H.currency.length===40)),[F,,...W]=await Promise.all([Me({command:"gateway_balances",account:t,ledger_index:"validated"}).catch(()=>null),Yf(),...D.slice(0,5).map(H=>Me({command:"amm_info",asset:{currency:"XRP"},asset2:{currency:H.currency,issuer:H.account},ledger_index:"validated"}).catch(()=>null))]);if(st)return;let E=(F==null?void 0:F.result)||null,U=new Map;D.slice(0,5).forEach((H,Y)=>{var me,De;(De=(me=W[Y])==null?void 0:me.result)!=null&&De.amm&&U.set(H.currency,W[Y].result.amm)});let Q=400,ie=250,ce=window._inspectMaxTx||5e3,de=Math.ceil(ce/Q),se=[],fe=new Set,K=H=>{var Y,me;for(let De of H||[]){let Be=((Y=De.tx_json)==null?void 0:Y.hash)||((me=De.tx)==null?void 0:me.hash)||De.hash||null;Be&&fe.has(Be)||(Be&&fe.add(Be),se.push(De))}},xe;for(let H=1;H<=de&&se.length<ce;H++){if(st)return;n(`Fetching transactions \u2014 page ${H} (${se.length.toLocaleString()} so far)\u2026`);let Y={command:"account_tx",account:t,limit:Q,ledger_index_min:-1,ledger_index_max:-1,forward:!1};xe&&(Y.marker=xe);let me=await Me(Y).catch(()=>null);if(st)return;if(K((p=me==null?void 0:me.result)==null?void 0:p.transactions),xe=((m=me==null?void 0:me.result)==null?void 0:m.marker)||null,!xe)break;H<de&&se.length<ce&&await s(ie)}if(se.length<ce){if(st)return;n("Fetching oldest transactions (anchoring history start)\u2026");let H=await Me({command:"account_tx",account:t,limit:Q,ledger_index_min:-1,ledger_index_max:-1,forward:!0}).catch(()=>null);if(st)return;K((f=H==null?void 0:H.result)==null?void 0:f.transactions),await s(ie)}e.loading&&(e.loading.style.display="none");let B=Zf(se).sort((H,Y)=>(H.tx.date??0)-(Y.tx.date??0)),J=946684800,re=null,pe=null;if(B.length>0){let H=B[0].tx;H!=null&&H.date&&(pe=(H.date+J)*1e3,re=Math.floor((Date.now()-pe)/864e5))}let Se=new Map;for(let{tx:H}of B){if(H.TransactionType!=="OfferCreate"||!H.TakerPays||!H.TakerGets)continue;let Y=De=>typeof De=="string"?"XRP":`${De.currency}+${De.issuer||""}`,me=[Y(H.TakerPays),Y(H.TakerGets)].sort().join("\u2194");Se.set(me,(Se.get(me)||0)+1)}let ve=null;if(Se.size>0){let H=[...Se.entries()].sort((jt,Zs)=>Zs[1]-jt[1])[0][0],[Y,me]=H.split("\u2194"),De=jt=>jt==="XRP"?{currency:"XRP"}:{currency:jt.split("+")[0],issuer:jt.split("+")[1]},Be=await Me({command:"book_offers",taker_pays:De(Y),taker_gets:De(me),limit:20,ledger_index:"validated"}).catch(()=>null);(h=(v=Be==null?void 0:Be.result)==null?void 0:v.offers)!=null&&h.length&&(ve={pair:H,offers:Be.result.offers})}let rt=[...new Set(B.filter(({tx:H})=>H.TransactionType==="Payment"&&H.Account===t&&H.Destination).map(({tx:H})=>H.Destination))].slice(0,6),lt=new Map;for(let H of rt){if(st)return;let Y=await Me({command:"account_info",account:H,ledger_index:"validated"}).catch(()=>null),me=(w=Y==null?void 0:Y.result)==null?void 0:w.account_data;me&&lt.set(H,{sequence:me.Sequence||0,balance:Number(me.Balance||0)/1e6}),await s(80)}eh(t,S,P,T,$,_,B,{gatewayBalances:E,ammInfoMap:U,destAgeMap:lt,walletAgeDays:re,walletCreatedTs:pe,liveOrderBook:ve}),e.result&&(e.result.style.display="",qo());let Ce=e.score?Number(e.score.textContent):null;$g(t,isNaN(Ce)?null:Ce);let ue=window._lastAllFindings||[];Sg(t,isNaN(Ce)?null:Ce,ue),Eg(t,ue),zo(t)&&Cg(t,isNaN(Ce)?null:Ce),Mg()}catch(k){if(st)return;e.loading&&(e.loading.style.display="none"),e.err&&(e.err.textContent=`Error: ${g(k.message)}`,e.err.style.display="")}}function Zf(e){return e.map(t=>{let n=t.tx_json||t.tx||t.transaction||{},s=t.metadata||t.meta||{};return n.date==null&&t.date!=null&&(n.date=t.date),!n.hash&&t.hash&&(n.hash=t.hash),{tx:n,meta:s}})}function eh(e,t,n,s,a,o,i,r={}){let{gatewayBalances:l=null,ammInfoMap:c=new Map,destAgeMap:d=new Map,walletAgeDays:u=null,walletCreatedTs:p=null,liveOrderBook:m=null}=r,f=Number(t.Balance||0)/1e6,v=Number(t.OwnerCount||0),h=10+v*2,w=Number(t.Flags||0),k=t.Sequence??"\u2014",b=o.filter(ve=>ve.LedgerEntryType==="SignerList"),x=o.filter(ve=>ve.LedgerEntryType==="Escrow"),S=o.filter(ve=>ve.LedgerEntryType==="PayChannel"),T=o.filter(ve=>ve.LedgerEntryType==="DepositPreauth"),$=o.filter(ve=>ve.LedgerEntryType==="Check"),P=oh(t,w,b,i),M=ih(t,w,b,i,S,x),N=rh(a,i,e),_=lh(i,e,n),X=hh(t,n,w,i),I=gh(n,i,o,c),D=ch(i),F=fh(i,e),W=dh(i,e),E=ph(i,e),U=uh(i),Q=mh(i,e),ie=th(i,e,d),ce=vh(i),de=bh(i,e),se=yh(i,e),fe=nh(i,e,n,l),K=wh(i,e),xe=xh(i,e),B=kh(o,i,e),J=$h(o),re=Sh(m,e),pe=Ch(P,M,N,_,D,F,W,E,U,Q,ce);Rh(e,t,f,h,v,k,pe,u,p),Dh(P,t,w,b,T),Ih(M,S,x,$),jh(ie,f,K),Fh(N,a),Bh(_),Ph(D),Lh(F),Mh(W),Ah(E),Eh(U),Nh(Q),_h(D,W,E,U,Q),Xh(X,n),Wh(fe,n),qh(ce),Vh(de),Gh(se),Hh(I,n),Kh(K),Jh(xe),Yh(B),Qh(J),Zh(re),eg(pe,P,M,N,_,D,F,W,E,U,Q,ce,K,xe),zh(n),Uh(i,e),ql(i),_g(i,e,ie,K),window._lastTxList=i;let Se=y("inspect-report-body");Se&&(og(Se,e,t,f,pe,P,M,N,_,D,F,X,I,ie,fe,i,W,E,U,Q,{feeAnalysis:ce,destTagAnalysis:de,pathDepthAnalysis:se,gatewayBalances:l,inboundFlowAnalysis:K,memoAnalysis:xe,escrowDepthAnalysis:B,checkAnalysis:J,liveBookAnalysis:re,walletAgeDays:u,walletCreatedTs:p}),ql(i,"inspect-report-activity-chart"),Ag(pe,window._lastAllFindings||[],u,i.length),window._lastInspectResult={addr:e,riskScore:pe,walletAgeDays:u,txCount:i.length,findings:window._lastAllFindings||[],timestamp:new Date().toISOString()},ac(),oc(e))}function th(e,t,n=new Map){let s=new Map,a=[];for(let{tx:p,meta:m}of e){if(p.TransactionType!=="Payment"||p.Account!==t)continue;let f=p.Destination;if(!f)continue;let v=0,h=null,w=p.Amount;typeof w=="string"?v=Number(w)/1e6:w!=null&&w.value&&(h={value:Number(w.value),currency:Sn(w.currency),issuer:w.issuer});let k=Array.isArray(p.Paths)&&p.Paths.length>0,b=p.SendMax!=null,x=k||b,S=k?p.Paths.reduce((M,N)=>Math.max(M,(N||[]).length+1),1):x?2:1,T=At(p),$={dest:f,amtXrp:v,amtToken:h,ts:T,isPathPay:x,hopCount:S,hash:p.hash||p.Hash||"",ledger:p.ledger_index||p.LedgerIndex||0,destTag:p.DestinationTag};a.push($),s.has(f)||s.set(f,{addr:f,totalXrp:0,txCount:0,firstSeen:T,lastSeen:T,entity:ns(f)||null,pathCount:0,maxHops:1,tokens:new Map});let P=s.get(f);if(P.totalXrp+=v,P.txCount++,P.lastSeen=Math.max(P.lastSeen,T),P.firstSeen=Math.min(P.firstSeen,T),x&&(P.pathCount++,P.maxHops=Math.max(P.maxHops,S)),h){let M=`${h.currency}.${z(h.issuer||"")}`;P.tokens.set(M,(P.tokens.get(M)||0)+h.value)}}let o=[...s.values()].sort((p,m)=>m.totalXrp-p.totalXrp||m.txCount-p.txCount).slice(0,10).map(p=>({...p,tokens:[...p.tokens.entries()].map(([m,f])=>({k:m,v:f}))})),i=o.reduce((p,m)=>p+m.totalXrp,0),r=a.filter(p=>p.isPathPay).length,l=o.filter(p=>{let m=n.get(p.addr);return m&&m.sequence<10&&p.totalXrp>10}),c=o.filter(p=>{var m;return((m=p.entity)==null?void 0:m.type)==="exchange"}),d=o.filter(p=>{var m;return((m=p.entity)==null?void 0:m.type)==="blackhole"});return{timeline:[...a].filter(p=>p.amtXrp>.01||p.amtToken).sort((p,m)=>p.ts-m.ts).slice(0,30),destinations:o,totalOut:i,totalPathPay:r,uniqueDests:s.size,exchangeDests:c,blackHoleDests:d,newWalletDests:l}}function nh(e,t,n,s=null){var T,$;let a=null;if(s!=null&&s.obligations){let P=Object.values(s.obligations);P.length===1?a=Number(P[0])||null:P.length>1&&(a=null)}let o=[],i=new Map,r=new Map,l=new Set,c=new Set(n.filter(P=>Number(P.balance)<0).map(P=>Sn(P.currency)));for(let{tx:P,meta:M}of e)if(P.Account===t&&P.TransactionType==="Payment"){(($=(T=M==null?void 0:M.AffectedNodes)==null?void 0:T.some)==null?void 0:$.call(T,X=>{var I,D,F;return((I=X.CreatedNode)==null?void 0:I.LedgerEntryType)==="AccountRoot"&&((F=(D=X.CreatedNode)==null?void 0:D.NewFields)==null?void 0:F.Account)===P.Destination}))&&P.Destination&&l.add(P.Destination);let _=P.Amount;if(typeof _=="object"&&(_!=null&&_.value)&&(_!=null&&_.currency)){let X=Sn(_.currency);if(c.has(X)){let I=Number(_.value),D=P.Destination;i.has(D)||(i.set(D,0),r.set(D,At(P))),i.set(D,i.get(D)+I)}}}let d=[...i.entries()].sort((P,M)=>M[1]-P[1]),u=[];if(d.length>=3){let P=new Map;for(let[M,N]of d){if(N<=0)continue;let _=Math.pow(10,Math.floor(Math.log10(N))),I=(Math.round(N/_/.1)*.1*_).toPrecision(2);P.has(I)||P.set(I,[]),P.get(I).push({addr:M,amt:N})}for(let[,M]of P.entries())if(M.length>=3){let N=M.reduce((_,X)=>_+X.amt,0)/M.length;u.push({approxAmt:N,accounts:M}),o.push({sev:"warn",label:`${M.length} accounts each received ~${R(N,0)} tokens`,detail:"Highly similar token amounts suggest coordinated wallets, pre-arranged airdrop clusters, or sybil accounts."})}}let p=[...r.values()].sort();if(p.length>=5){let P=p[p.length-1]-p[0];P<3600&&p.length>=10&&o.push({sev:"warn",label:`${p.length} accounts funded within ${Math.ceil(P/60)} minutes`,detail:"Rapid token distribution to many wallets in a narrow time window. Matches pre-sale airdrop or coordinated distribution for wash trading."})}l.size>0&&o.push({sev:l.size>10?"warn":"info",label:`${l.size} account(s) created by this address`,detail:"This issuer funded the activation of these accounts. They may be controlled by the same entity."});let m=n.filter(P=>Number(P.balance)<0),f=a,v=m.reduce((P,M)=>P+Math.abs(Number(M.balance)),0),h=f??v,w=f==null&&m.length>0,k=w?` (based on ${m.length} visible trustlines \u2014 actual total supply may be higher if there are more holders)`:"",b=m.map(P=>({addr:P.account,balance:Math.abs(Number(P.balance)),currency:Sn(P.currency)})).sort((P,M)=>M.balance-P.balance).slice(0,10),x=f??v,S=x>0&&(f!=null||m.length>=50);if(b.length>=2&&S){let P=b[0].balance/x*100,M=f!=null?"":" (of visible sample)";P>50?o.push({sev:"critical",label:`Top holder controls ${P.toFixed(0)}%${M} of supply`,detail:`${z(b[0].addr)} holds ${R(b[0].balance,0)} of ${R(x,0)} total${k}. `+(f!=null?"Extreme dump risk \u2014 one wallet could sell everything.":"Check gateway_balances or a block explorer to confirm the full supply picture.")}):P>25&&o.push({sev:"warn",label:`Top holder controls ${P.toFixed(0)}%${M} of supply`,detail:`Large single-holder concentration${k}. Monitor for coordinated sell events.`});let _=b.slice(0,5).reduce((X,I)=>X+I.balance,0)/x*100;_>75&&o.push({sev:"warn",label:`Top 5 holders own ${_.toFixed(0)}%${M} of supply`,detail:`Supply heavily concentrated in a few wallets${k}. This pattern is common in pre-launch setups or tokens with limited real distribution.`})}else m.length>0&&!S&&o.push({sev:"info",label:`${m.length} trustline holder(s) visible \u2014 supply data limited`,detail:`Only ${m.length} trustlines returned by account_lines. The true holder count and total supply cannot be determined from this data alone. Use a block explorer (XRPScan, Bithomp) for a complete holder distribution.`});return o.length===0&&h===0&&o.push({sev:"info",label:"No token issuance detected",detail:"This account does not appear to be an active token issuer."}),{signals:o,totalIssued:h,holderCount:m.length,topHolders:b,mirrorGroups:u,createdAccts:[...l],distributions:d.slice(0,10),isSampleOnly:w}}var sh=new Set(["rrrrrrrrrrrrrrrrrrrrrhoLvTp","rrrrrrrrrrrrrrrrrrrrBZbvji","rrrrrrrrrrrrrrrrrNAMEtxvNvQ","rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh","r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59"]);function ah(e){return!!e&&sh.has(e)}function Jl(e,t,n=[],s=[]){let a=!!(t&ze.lsfDisableMaster),o=(e==null?void 0:e.RegularKey)||"",i=Array.isArray(n)&&n.length>0,r=ah(o);return!!(a&&r&&!i)}function Yl(e,t,n=[]){let s=!!(t&ze.lsfDefaultRipple),a=!!(t&ze.lsfRequireAuth),o=!!(t&ze.lsfGlobalFreeze),i=!!(t&ze.lsfNoFreeze),r=n.filter(({tx:c})=>c.TransactionType==="TrustSet").length,l=n.filter(({tx:c})=>{var d;return c.TransactionType==="Payment"&&typeof c.Amount=="object"&&((d=c.Amount)==null?void 0:d.currency)}).length;return s||a||o||i||r>=3||l>=5}function oh(e,t,n,s){let a=[],o=100,i=!!(t&ze.lsfDisableMaster),r=!!e.RegularKey,l=n.length>0,c=Jl(e,t,n,s),d=Yl(e,t,s);if(i&&!r&&!l?(a.push({sev:"critical",label:"Master key disabled \u2014 no fallback",detail:"Account cannot sign transactions. Funds are inaccessible."}),o-=40):c?(a.push({sev:"info",label:"Intentional blackhole pattern detected",detail:`Master key is disabled and regular key ${e.RegularKey} is a known blackhole address. This usually indicates the account was intentionally locked, not compromised.`}),d&&a.push({sev:"warn",label:"Blackholed issuer caution",detail:"This account appears issuer-like and intentionally blackholed. Sending issued tokens back here may make them unrecoverable or effectively burn them."})):i&&a.push({sev:"info",label:"Master key disabled",detail:"Signing via regular key or multisig only."}),r){let p=s.find(({tx:f})=>f.TransactionType==="SetRegularKey"),m=p&&Date.now()/1e3-At(p.tx)<86400*30;c?a.push({sev:"info",label:"Regular key points to blackhole address",detail:e.RegularKey}):m?(a.push({sev:"warn",label:"Regular key set recently",detail:`Key: ${e.RegularKey} \u2014 changed within 30 days. Verify you intended this.`}),o-=15):a.push({sev:"info",label:"Regular key active",detail:e.RegularKey})}n.forEach(p=>{let m=p.SignerEntries||[],f=p.SignerQuorum||1;a.push({sev:"info",label:`Multisig: ${m.length} signers, quorum ${f}`,detail:m.map(v=>{var h;return z(((h=v.SignerEntry)==null?void 0:h.Account)||"")}).join(", ")})}),t&ze.lsfGlobalFreeze&&(a.push({sev:"warn",label:"Global Freeze active",detail:"This issuer has frozen all token balances."}),o-=10),t&ze.lsfDepositAuth&&a.push({sev:"ok",label:"Deposit Authorization enabled",detail:"Only pre-authorized senders can deposit."}),t&ze.lsfDefaultRipple&&a.push({sev:"info",label:"Default Ripple enabled",detail:"Balances can ripple through this account (issuer behaviour)."});let u=s.filter(({tx:p})=>p.TransactionType==="AccountDelete");return u.length&&(a.push({sev:"warn",label:`${u.length} AccountDelete attempt(s)`,detail:"Account deletion was attempted."}),o-=5),a.length===0&&a.push({sev:"ok",label:"No security issues detected",detail:"Master key active, no regular key or signer list overrides, no risky flags set."}),{findings:a,score:Math.max(0,o)}}function ih(e,t,n,s,a,o){let i=[],r="low",l=!!(t&ze.lsfDisableMaster),c=Jl(e,t,n,s),d=Yl(e,t,s);c?(i.push({sev:"info",label:"Intentional blackhole detected",detail:`Master key is disabled and regular key ${e.RegularKey} is a known blackhole address. This is typical for a permanently locked issuer/account, not a classic drain setup.`}),d&&i.push({sev:"warn",label:"Caution: sending assets back may burn them",detail:"Because this account appears to be an intentionally blackholed issuer, sending issued tokens back to it may strand or effectively burn those tokens."})):l&&e.RegularKey&&(i.push({sev:"critical",label:"Classic drain setup detected",detail:`Master key disabled. Regular key ${e.RegularKey} controls the account. If this key was set by an attacker, funds are at risk.`}),r="critical");let u=s.filter(({tx:f})=>f.TransactionType==="SetRegularKey"&&f.Account!==e.Account);!c&&u.length&&(i.push({sev:"critical",label:"Regular key set by external account",detail:`${u.length} key change(s) where sender \u2260 account owner. This is unusual.`}),r="critical");let p=s.filter(({tx:f})=>["SetRegularKey","SignerListSet"].includes(f.TransactionType));if(!c&&p.length>0){let f=At(p[0].tx),v=s.filter(({tx:h})=>{if(h.TransactionType!=="Payment"||h.Account!==e.Account)return!1;let w=At(h);return w>f&&w<f+3600*48});if(v.length>0){let h=v.reduce((w,{tx:k})=>{let b=k.Amount;return typeof b=="string"?w+Number(b)/1e6:w},0);h>10&&(i.push({sev:"critical",label:`${R(h,2)} XRP sent within 48h of auth change`,detail:`${v.length} payment(s) shortly after key/signer modification. Pattern matches drain attack.`}),r="critical")}}if(a.length){let f=a.reduce((v,h)=>v+Number(h.Amount||0)/1e6,0);i.push({sev:"warn",label:`${a.length} open payment channel(s) \u2014 ${R(f,2)} XRP locked`,detail:`Destination(s): ${a.map(v=>z(v.Destination)).join(", ")}`}),r==="low"&&(r="medium")}if(o.length){let f=o.reduce((v,h)=>v+Number(h.Amount||0)/1e6,0);i.push({sev:"info",label:`${o.length} open escrow(s) \u2014 ${R(f,2)} XRP escrowed`,detail:`Escrow(s): ${o.map(v=>v.Destination?z(v.Destination):"self-escrow").join(", ")}`})}let m=s.filter(({tx:f})=>f.TransactionType==="DepositPreauth"&&f.Authorize);return m.length>5&&(i.push({sev:"warn",label:`${m.length} DepositPreauth grants issued`,detail:"Account pre-authorized many senders. Review if all are trusted."}),r==="low"&&(r="medium")),i.length===0&&i.push({sev:"ok",label:"No drain patterns detected",detail:"Auth structure looks intact."}),{signals:i,riskLevel:r}}function rh(e,t,n){let s=[],a=new Map(e.map(m=>[m.NFTokenID,m])),i=t.filter(({tx:m})=>m.TransactionType==="NFTokenCreateOffer"&&m.Account===n).filter(({tx:m})=>{let f=m.Amount;return f?typeof f=="string"?Number(f)<1e6:!1:!0});i.length&&s.push({sev:"critical",label:`${i.length} NFT offer(s) created for \u22641 XRP`,detail:"You created sell offers at near-zero price. This is a common NFT drain vector \u2014 attackers trick victims into listing their NFTs for free."});let r=t.filter(({tx:m})=>m.TransactionType==="NFTokenAcceptOffer"&&m.Account===n);r.length&&s.push({sev:"warn",label:`${r.length} NFT offer(s) accepted`,detail:"Review these transactions to confirm they were intentional sales/purchases."});let l=t.filter(({tx:m})=>m.TransactionType==="NFTokenBurn"&&m.Account===n);l.length&&s.push({sev:"warn",label:`${l.length} NFT(s) burned`,detail:"Burned NFTs cannot be recovered. Confirm these were intentional."});let c=t.filter(({tx:m})=>m.TransactionType==="NFTokenMint"&&m.Account===n),d=t.filter(({tx:m})=>m.TransactionType==="NFTokenCreateOffer"||m.TransactionType==="NFTokenAcceptOffer");c.length>0&&d.length>0&&s.push({sev:"info",label:`${c.length} NFT(s) minted \xB7 ${d.length} transfer event(s)`,detail:"This account has minting activity."});let u=e.filter(m=>m.TransferFee&&m.TransferFee>0);if(u.length){let m=u.reduce((f,v)=>f+v.TransferFee,0)/u.length;s.push({sev:"info",label:`${u.length} NFT(s) carry transfer fees (avg ${(m/1e3).toFixed(1)}%)`,detail:"These NFTs pay royalties on each transfer."})}let p=e.filter(m=>!m.URI);return p.length>2&&s.push({sev:"warn",label:`${p.length} NFT(s) have no URI / metadata`,detail:"NFTs without a URI may be spam or placeholder tokens used in scams."}),s.length===0&&e.length===0?s.push({sev:"ok",label:"No NFT activity detected",detail:"This account holds no NFTs."}):s.length===0&&s.push({sev:"ok",label:`${e.length} NFT(s) held \u2014 no risk signals`,detail:"NFT posture looks normal."}),{flags:s,nftCount:e.length,mintCount:c.length}}function lh(e,t,n){let s=[],a=0,o=e.filter(({tx:b})=>b.TransactionType==="OfferCreate"),i=e.filter(({tx:b})=>b.TransactionType==="OfferCancel"),r=o.filter(({meta:b})=>{var x,S;return(S=(x=b==null?void 0:b.AffectedNodes)==null?void 0:x.some)==null?void 0:S.call(x,T=>{var $;return(($=T.DeletedNode)==null?void 0:$.LedgerEntryType)==="Offer"})}),l=e.filter(({tx:b})=>b.TransactionType==="Payment"),c=o.length>0?i.length/o.length:0;o.length>=Zn&&c>jl&&(s.push({sev:"warn",label:`High cancel ratio: ${(c*100).toFixed(1)}%`,detail:`${i.length} cancels vs ${o.length} creates. Threshold: ${(jl*100).toFixed(0)}%. May indicate layering / spoofing.`}),a+=25);let d=new Set(l.filter(({tx:b})=>b.Account===t&&b.Destination).map(({tx:b})=>b.Destination)),u=new Set(l.filter(({tx:b})=>b.Destination===t&&b.Account).map(({tx:b})=>b.Account)),p=[...d].filter(b=>u.has(b));if(p.length>0&&l.length>=Zn){let b=p.length/d.size;b>qf&&(s.push({sev:"warn",label:`${p.length} round-trip counterpart(s) detected`,detail:`${(b*100).toFixed(1)}% of payment recipients also sent back to this account. Possible wash-trade cycle.`}),a+=20)}let m=new Map;o.forEach(({tx:b})=>{if(!b.TakerPays||!b.TakerGets)return;let x=T=>typeof T=="string"?"XRP":`${T.currency}.${z(T.issuer||"")}`,S=[x(b.TakerPays),x(b.TakerGets)].sort().join("\u2194");m.set(S,(m.get(S)||0)+1)});let f=[...m.entries()].sort((b,x)=>x[1]-b[1])[0];if(f&&o.length>=Zn){let b=f[1]/o.length;b>.7&&(s.push({sev:"warn",label:`${(b*100).toFixed(0)}% of offers on single pair: ${f[0]}`,detail:`${f[1]} of ${o.length} offers on one pair. High concentration is a wash trading signal.`}),a+=20)}let v=o.length>0?r.length/o.length:0;if(o.length>=Zn&&v<.05&&(s.push({sev:"warn",label:`Very low fill rate: ${(v*100).toFixed(1)}%`,detail:`Only ${r.length} of ${o.length} offers filled. Placing orders never intended to execute.`}),a+=15),o.length>=5){let b=o.map(({tx:S})=>At(S)).sort(),x=1;for(let S=0;S<b.length;S++){let T=1;for(let $=S+1;$<b.length&&b[$]-b[S]<=30;$++)T++;x=Math.max(x,T)}x>=8&&(s.push({sev:"warn",label:`Burst activity: ${x} offers within 30 seconds`,detail:"Rapid automated trading pattern detected."}),a+=10)}let h=l.filter(({tx:b})=>b.Account===t&&b.Destination===t);if(h.length>0&&(s.push({sev:"critical",label:`${h.length} self-trade(s): sender = receiver`,detail:"Payments where origin and destination are the same address. Classic wash-trading indicator \u2014 creates artificial volume with zero economic transfer."}),a+=30),o.length>=10){let b=o.map(({tx:P})=>{let M=P.TakerGets;return typeof M=="string"?Number(M)/1e6:null}).filter(P=>P!=null),x=b.sort((P,M)=>M-P)[Math.floor(b.length*.05)]||0,S=o.filter(({tx:P})=>{let M=P.TakerGets;return typeof M=="string"&&Number(M)/1e6>=x}),T=S.filter(({meta:P})=>{var M,N;return!((N=(M=P==null?void 0:P.AffectedNodes)==null?void 0:M.some)!=null&&N.call(M,_=>{var X;return((X=_.DeletedNode)==null?void 0:X.LedgerEntryType)==="Offer"}))}),$=S.length>=5?T.length/S.length:0;$>=.95?(s.push({sev:"critical",label:`Spoofing pattern: ${($*100).toFixed(0)}% of large orders cancelled`,detail:`${T.length} of ${S.length} top-5% size orders were cancelled without execution. \u226595% cancel rate on large orders strongly implies fake order book depth (spoofing).`}),a+=30):$>=.8&&(s.push({sev:"warn",label:`Elevated large-order cancel rate: ${($*100).toFixed(0)}%`,detail:`${T.length}/${S.length} large orders cancelled. Watch for spoofing behaviour.`}),a+=15)}if(o.length>=Zn){let b=o.map(({tx:x})=>{let S=x.TakerGets;return typeof S=="string"?Number(S)/1e6:S!=null&&S.value?Number(S.value):null}).filter(x=>x!=null&&x>0);if(b.length>=Zn){let x=b.reduce(($,P)=>$+P,0)/b.length,S=Math.sqrt(b.reduce(($,P)=>$+(P-x)**2,0)/b.length),T=x>0?S/x:null;T!==null&&T<.05?(s.push({sev:"critical",label:`Robotic trade uniformity (CV ${T.toFixed(3)})`,detail:`${(T*100).toFixed(1)}% coefficient of variation across ${b.length} offer sizes. Near-identical sizes indicate bot-generated fake volume (natural markets show CV \u2265 0.5).`}),a+=25):T!==null&&T<.2&&(s.push({sev:"warn",label:`Unusually uniform trade sizes (CV ${T.toFixed(3)})`,detail:`Only ${(T*100).toFixed(1)}% variation in offer sizes \u2014 suspiciously low for organic activity.`}),a+=10)}}let w=[...o,...l].map(({tx:b})=>{let x=b.TakerGets||b.Amount;return typeof x=="string"?Number(x)/1e6:x!=null&&x.value?Number(x.value):null}).filter(b=>b!=null&&b>0&&Number.isFinite(b));if(w.length>=10){let b=[100,1e3,1e4,1e5],x=w.filter(T=>b.some($=>Math.abs(T%$)<1e-6&&T/$>=1)).length,S=x/w.length;S>.45&&(s.push({sev:"warn",label:`Round-number bias: ${(S*100).toFixed(0)}% of amounts at exact multiples`,detail:`${x}/${w.length} trade / payment amounts are exact multiples of 100, 1,000, or 10,000. Statistical excess of round numbers is a signature of bot-generated activity.`}),a+=12)}if(o.length>=5){let b=o.map(({tx:S})=>At(S)).sort((S,T)=>S-T),x=0;for(let S=0;S<b.length;S++){let T=1;for(let $=S+1;$<b.length&&b[$]-b[S]<=3600;$++)T++;T>x&&(x=T)}if(x>100)s.push({sev:"critical",label:`Hourly burst: ${x} offers within 60 minutes`,detail:">100 OfferCreate txs in a single hour is a strong bot-pump indicator, especially in typically illiquid token markets."}),a+=20;else if(x>=8){let S=b,T=1;for(let $=0;$<S.length;$++){let P=1;for(let M=$+1;M<S.length&&S[M]-S[$]<=30;M++)P++;P>T&&(T=P)}T>=8&&(s.push({sev:"warn",label:`Rapid burst: ${T} offers within 30 seconds`,detail:"Automated trading pattern \u2014 bursts at this speed exceed human capability."}),a+=10)}}let k=a===0?"clean":a<25?"low-risk":a<50?"suspicious":"high-risk";return s.length===0&&s.push({sev:"ok",label:"No wash trading signals",detail:`${o.length} offers \xB7 ${i.length} cancels \xB7 ${h.length} self-trades \u2014 patterns look normal.`}),{signals:s,score:a,verdict:k,stats:{creates:o.length,cancels:i.length,fills:r.length,payments:l.length,roundTrip:p.length,selfTrades:h.length}}}function ch(e){let t=[];for(let{tx:c}of e){let d=[c.Amount,c.TakerGets,c.TakerPays,c.SendMax,c.DeliverMin];for(let u of d){let p=typeof u=="string"?Number(u)/1e6:u!=null&&u.value?Number(u.value):null;p!=null&&p>0&&Number.isFinite(p)&&t.push(p)}}if(t.length<50)return{signals:[{sev:"info",label:"Insufficient data for Benford's Law",detail:`Need \u226550 monetary amounts, found ${t.length}.`}],chiSq:null,verdict:"insufficient",digitBreakdown:[],sampleSize:t.length};let n=[0,.301,.176,.125,.097,.079,.067,.058,.051,.046],s=new Array(10).fill(0);for(let c of t){let d=c.toFixed(6).replace(/^0+\.?0*/,""),u=parseInt(d[0],10);u>=1&&u<=9&&s[u]++}let a=t.length,o=0,i=[];for(let c=1;c<=9;c++){let d=s[c]/a,u=n[c];o+=a*Math.pow(d-u,2)/u,i.push({digit:c,obs:(d*100).toFixed(1),exp:(u*100).toFixed(1),delta:((d-u)*100).toFixed(1)})}let r=[],l;return o>20.09?(l="high-deviation",r.push({sev:"warn",label:`Benford's Law: significant deviation (\u03C7\xB2=${o.toFixed(1)})`,detail:`First-digit distribution deviates significantly from natural patterns (p<0.01, n=${a}). This is a statistical signature of fabricated or algorithmically generated transaction amounts.`})):o>15.51?(l="moderate-deviation",r.push({sev:"info",label:`Benford's Law: moderate deviation (\u03C7\xB2=${o.toFixed(1)})`,detail:`Some deviation from expected natural distribution (p<0.05, n=${a}). Worth monitoring alongside other signals.`})):(l="normal",r.push({sev:"ok",label:`Benford's Law: normal distribution (\u03C7\xB2=${o.toFixed(1)})`,detail:`First-digit distribution is consistent with organic transaction patterns (n=${a}).`})),{signals:r,chiSq:o,verdict:l,digitBreakdown:i,sampleSize:a}}function dh(e,t){let s=[],a=[];for(let{tx:h}of e){let w=[h.Amount,h.TakerGets,h.TakerPays];for(let k of w){let b=typeof k=="string"?Number(k)/1e6:k!=null&&k.value?Number(k.value):null;b&&b>0&&Number.isFinite(b)&&a.push(b)}}let o=a.length>=30?(()=>{let h=new Array(12).fill(0);for(let w of a){let k=Math.min(11,Math.max(0,Math.floor(Math.log10(w+1)*2)));h[k]++}return Ls(h)})():null,i={};for(let{tx:h}of e){let w=h.Account===t?h.Destination:h.Account;w&&w!==t&&(i[w]=(i[w]||0)+1)}let r=Object.values(i),l=r.length>=3?Ls(r):null,c=new Array(24).fill(0),d=!1;for(let{tx:h}of e)if(h.date){let k=new Date((h.date+946684800)*1e3).getUTCHours();c[k]++,d=!0}let u=d&&e.length>=30?Ls(c):null,p={};for(let{tx:h}of e)p[h.TransactionType]=(p[h.TransactionType]||0)+1;let m=Ls(Object.values(p)),f="normal",v=0;if(o!==null&&(o<1.5?(f="low-entropy",v+=18,s.push({sev:"warn",label:`Amount entropy critically low (H=${o.toFixed(2)} bits)`,detail:"Transaction amounts are highly repetitive. A bot or scripted actor tends to reuse the same values. Organic wallets show entropy \u22652.4 bits across amount magnitudes."})):o<2.2?(v+=8,s.push({sev:"info",label:`Amount entropy below natural range (H=${o.toFixed(2)} bits)`,detail:"Some amount repetition detected. Could indicate automated activity mixed with organic transactions."})):o>4.5?(v+=10,s.push({sev:"info",label:`Amount entropy abnormally high (H=${o.toFixed(2)} bits)`,detail:"Transaction amounts are maximally varied \u2014 more than organic activity typically shows. This can indicate amounts were artificially randomized to evade Benford detection."})):s.push({sev:"ok",label:`Amount entropy normal (H=${o.toFixed(2)} bits)`,detail:"Transaction amount diversity is consistent with organic financial activity."})),l!==null&&(l<1&&r.length<4?(v+=14,s.push({sev:"warn",label:`Counterparty entropy very low (H=${l.toFixed(2)} bits)`,detail:"This wallet transacts with very few unique addresses and with high repetition \u2014 a structural signature of round-trip wash trading rings."})):l<2?(v+=5,s.push({sev:"info",label:`Counterparty entropy low (H=${l.toFixed(2)} bits)`,detail:"Most interactions are concentrated among a small set of counterparties."})):s.push({sev:"ok",label:`Counterparty diversity healthy (H=${l.toFixed(2)} bits)`,detail:"Counterparty distribution reflects diverse interaction patterns."})),u!==null){let h=Math.log2(24),w=u/h;w<.45?(v+=10,s.push({sev:"warn",label:`Time-of-day entropy low (H=${u.toFixed(2)} bits, ${(w*100).toFixed(0)}% of max)`,detail:"Transactions cluster heavily in a few hours of the day. Bots typically run at fixed UTC hours; organic users spread activity across the day."})):s.push({sev:"ok",label:`Time-of-day distribution natural (H=${u.toFixed(2)} bits)`,detail:"Transaction timing is distributed across hours in a pattern consistent with human activity."})}return s.length||s.push({sev:"info",label:"Insufficient data for entropy analysis",detail:`Need \u226530 transactions. Found ${e.length}.`}),v>=18?f="anomalous":v>=8&&(f="elevated"),{signals:s,verdict:f,riskPenalty:v,amountEntropy:o,counterpartyEntropy:l,timeEntropy:u,typeEntropy:m,uniqueCounterparties:r.length,sampleSize:e.length}}function Ls(e){let t=e.reduce((n,s)=>n+s,0);return t?-e.reduce((n,s)=>{if(!s)return n;let a=s/t;return n+a*Math.log2(a)},0):0}function ph(e,t){let s=[],a={};for(let{tx:M}of e){let N=M.Account===t?M.Destination:M.Account;N&&N!==t&&(a[N]=(a[N]||0)+1)}let o=Object.values(a).sort((M,N)=>N-M);if(o.length<8)return{signals:[{sev:"info",label:`Insufficient counterparties for Zipf's Law (need \u22658, found ${o.length})`,detail:"Zipf analysis becomes meaningful with a broader counterparty network."}],verdict:"insufficient",zipfExponent:null,riskPenalty:0,freqTable:[],uniqueCounterparties:o.length};let i=o.length,r=0,l=0,c=0,d=0,u=o.map((M,N)=>({rank:N+1,freq:M,lx:Math.log(N+1),ly:Math.log(M)}));for(let{lx:M,ly:N}of u)r+=M,l+=N,c+=M*N,d+=M*M;let p=i*d-r*r,m=p!==0?(i*c-r*l)/p:null,f=m!==null?Math.abs(m):null,v=l/i,h=0,w=0,k=(l-m*r)/i;for(let{lx:M,ly:N}of u)h+=Math.pow(N-v,2),w+=Math.pow(N-(m*M+k),2);let b=h>0?1-w/h:0,x="normal",S=0;f!==null&&(b<.55?(S+=12,s.push({sev:"warn",label:`Counterparty distribution doesn't follow Zipf's Law (R\xB2=${b.toFixed(2)})`,detail:`Natural networks follow a power-law rank-frequency relationship. This wallet's counterparty network has poor Zipf fit (R\xB2=${b.toFixed(2)}), suggesting artificial or script-driven interaction structure.`}),x="anomalous"):f<.4?(S+=15,s.push({sev:"warn",label:`Zipf exponent too flat (s=${f.toFixed(2)}, expected 0.8\u20131.3)`,detail:"A very flat Zipf exponent means counterparties are used with surprisingly equal frequency. In organic networks, you transact far more often with a few key addresses. Flat distribution is consistent with round-trip wash trading rings."}),x="anomalous"):f>2.2?(S+=10,s.push({sev:"warn",label:`Zipf exponent hyper-concentrated (s=${f.toFixed(2)}, expected 0.8\u20131.3)`,detail:"Extreme concentration on one or two counterparties with steep dropoff. While not unusual for simple wallets, combined with other signals this suggests coordinated narrow-ring activity."}),x="elevated"):s.push({sev:"ok",label:`Counterparty network follows Zipf's Law (s=${f.toFixed(2)}, R\xB2=${b.toFixed(2)})`,detail:"The rank-frequency distribution of counterparties follows the expected natural power-law pattern. This is consistent with organic wallet activity."}));let T={};for(let{tx:M}of e){let N=typeof M.Amount=="string"?Math.round(Number(M.Amount)/1e4)*10:null;N&&N>0&&(T[N]=(T[N]||0)+1)}let $=Object.values(T).sort((M,N)=>N-M),P=$.length?$[0]/$.reduce((M,N)=>M+N,0):0;return P>.45&&(S+=8,s.push({sev:"warn",label:`Single amount dominates ${(P*100).toFixed(0)}% of transactions`,detail:"One transaction amount value accounts for nearly half of all payments. Round-number dominance is a hallmark of scripted or wash-trading activity."})),s.length||s.push({sev:"info",label:"Zipf analysis: no anomalies detected",detail:"Counterparty distribution consistent with natural activity."}),{signals:s,verdict:x,riskPenalty:S,zipfExponent:f,rSquared:b,freqTable:o.slice(0,12),uniqueCounterparties:o.length}}function uh(e){let n=[];if(e.length<20)return{signals:[{sev:"info",label:`Insufficient transactions for time series analysis (need \u226520, found ${e.length})`,detail:"Time series analysis requires a longer transaction history."}],verdict:"insufficient",riskPenalty:0,intervalCV:null,autocorrelation:null,burstScore:null,periodicityScore:null};let a=e.filter(({tx:b})=>b.date!=null).map(({tx:b})=>(b.date+946684800)*1e3).sort((b,x)=>b-x);if(a.length<20)return{signals:[{sev:"info",label:"No timestamp data available",detail:"Time series requires date-stamped transactions."}],verdict:"insufficient",riskPenalty:0,intervalCV:null,autocorrelation:null,burstScore:null,periodicityScore:null};let o=[];for(let b=1;b<a.length;b++){let x=(a[b]-a[b-1])/1e3;x>0&&x<86400*30&&o.push(x)}let i=null;if(o.length>=5){let b=o.reduce((S,T)=>S+T,0)/o.length,x=Math.sqrt(o.reduce((S,T)=>S+Math.pow(T-b,2),0)/o.length);i=b>0?x/b:null}let r={};for(let b of a){let x=new Date(b).toISOString().slice(0,10);r[x]=(r[x]||0)+1}let l=Object.values(r),c=0;if(l.length>=4){let b=l.reduce((T,$)=>T+$,0)/l.length,x=Math.sqrt(l.reduce((T,$)=>T+Math.pow($-b,2),0)/l.length),S=Math.max(...l);c=x>0?(S-b)/x:0}let d=null;if(l.length>=6){let b=l.reduce((T,$)=>T+$,0)/l.length,x=l.map(T=>T-b),S=x.reduce((T,$)=>T+$*$,0);S>0&&(d=x.slice(0,-1).reduce(($,P,M)=>$+P*x[M+1],0)/S)}let u=new Array(7).fill(0);for(let b of a)u[new Date(b).getUTCDay()]++;let p=Ls(u),m=Math.log2(7),f=0;if(o.length>=10){let b=[...o].sort((T,$)=>T-$),x=b[Math.floor(b.length/2)];f=o.filter(T=>Math.abs(T-x)/x<.2).length/o.length}let v="normal",h=0;i!==null&&(i<.25?(h+=20,v="bot-pattern",n.push({sev:"warn",label:`Transaction intervals mechanically regular (CV=${i.toFixed(2)})`,detail:"The time gaps between transactions are too regular for human behavior (CV < 0.25). Organic wallets show irregular timing (CV 0.8\u20133.0). This pattern is a strong bot signature."})):i<.5?(h+=8,n.push({sev:"info",label:`Transaction timing somewhat regular (CV=${i.toFixed(2)})`,detail:"Interval regularity is below typical human variance. Could indicate scheduled automation."})):n.push({sev:"ok",label:`Transaction timing is irregular (CV=${i.toFixed(2)})`,detail:"Inter-transaction intervals show natural human-like variance."})),f>.55&&(h+=12,n.push({sev:"warn",label:`Strong periodicity detected (${(f*100).toFixed(0)}% of intervals near median)`,detail:"More than half of transaction intervals cluster around the same duration. This mechanical repetition is consistent with an automated script executing on a fixed schedule."})),c>3.5&&n.push({sev:"info",label:`Activity burst detected (peak day z-score=${c.toFixed(1)})`,detail:"One or more days had extreme transaction volume compared to baseline. Could indicate a coordinated pump event or account recovery sweep."}),d!==null&&d>.6&&(h+=6,n.push({sev:"info",label:`High day-to-day volume autocorrelation (\u03C1=${d.toFixed(2)})`,detail:"Transaction volume is strongly self-correlated \u2014 today's activity predicts tomorrow's. This is consistent with an automated routine that maintains a constant pace."}));let w=p/m;w<.7&&a.length>30&&(h+=6,n.push({sev:"info",label:`Day-of-week distribution concentrated (${(w*100).toFixed(0)}% of max entropy)`,detail:"Transactions cluster heavily on specific days. Automated systems often run every day (maximally flat) or skip weekends \u2014 both deviate from natural human patterns."})),n.length||n.push({sev:"ok",label:"No temporal anomalies detected",detail:"Transaction timing patterns are consistent with organic human activity."}),h>=20?v="bot-pattern":h>=8&&(v="elevated");let k=a.length>=2?Math.round((a[a.length-1]-a[0])/864e5):null;return{signals:n,verdict:v,riskPenalty:h,intervalCV:i,autocorrelation:d,burstScore:c,periodicityScore:f,dowEntropy:p,dowBins:u,dailyVolume:Object.entries(r).slice(-30),activeSpanDays:k,totalTimestamped:a.length}}function mh(e,t){let s=[];if(e.length<20)return{signals:[{sev:"info",label:`Insufficient data for Granger causality analysis (need \u226520, found ${e.length})`,detail:"Granger causality requires enough temporal observations to test lead-lag relationships."}],verdict:"insufficient",riskPenalty:0,offerCancelCausality:null,inflowOutflowCausality:null};let o=12*3600*1e3,i=D=>D.date?Math.floor((D.date+946684800)*1e3/o):null,r={},l=D=>{r[D]||(r[D]={offerCreate:0,offerCancel:0,inflow:0,outflow:0,nftList:0,nftAccept:0})};for(let{tx:D,meta:F}of e){let W=i(D);if(W===null)continue;l(W);let E=r[W],U=D.TransactionType;U==="OfferCreate"?E.offerCreate++:U==="OfferCancel"&&E.offerCancel++,U==="NFTokenCreateOffer"?E.nftList++:U==="NFTokenAcceptOffer"&&E.nftAccept++;let Q=(F==null?void 0:F.delivered_amount)||D.Amount,ie=typeof Q=="string"?Number(Q)/1e6:0;ie>0&&(D.Destination===t?E.inflow+=ie:D.Account===t&&(E.outflow+=ie))}let c=Object.keys(r).map(Number).sort((D,F)=>D-F);if(c.length<6)return{signals:[{sev:"info",label:"Insufficient temporal windows for Granger test",detail:"Need activity spread across multiple time windows."}],verdict:"insufficient",riskPenalty:0,offerCancelCausality:null,inflowOutflowCausality:null};let d=D=>c.map(F=>r[F][D]||0),u=(D,F,W=4)=>{let E=D.length,U=D.reduce((fe,K)=>fe+K,0)/E,Q=F.reduce((fe,K)=>fe+K,0)/E,ie=D.map(fe=>fe-U),ce=F.map(fe=>fe-Q),de=Math.sqrt(ie.reduce((fe,K)=>fe+K*K,0)/E),se=Math.sqrt(ce.reduce((fe,K)=>fe+K*K,0)/E);return!de||!se?Array(W+1).fill(0):Array.from({length:W+1},(fe,K)=>{let xe=0,B=0;for(let J=0;J+K<E;J++)xe+=ie[J]*ce[J+K],B++;return B>0?xe/(B*de*se):0})},p="normal",m=0,f=d("offerCreate"),v=d("offerCancel"),h=u(f,v),w=h.indexOf(Math.max(...h)),k=Math.max(...h),b={ccf:h,maxCorr:k,maxLag:w};k>.55&&w<=2?(m+=18,p="causal-signal",s.push({sev:"warn",label:`OfferCreate \u2192 OfferCancel Granger signal (\u03C1=${k.toFixed(2)}, lag=${w} window${w===1?"":"s"})`,detail:`Offer creation strongly predicts subsequent cancellation at lag ${w} (${w*12}h). This causal pattern is the mechanical signature of wash trading: create offers to inflate visible book activity, then cancel them. A leading correlation this strong at such short lag is unlikely in organic market-making.`})):k>.35?(m+=6,s.push({sev:"info",label:`Mild offer-cancel lead relationship (\u03C1=${k.toFixed(2)}, lag=${w})`,detail:"Some temporal link between creating and cancelling offers. Worth monitoring alongside other signals."})):s.push({sev:"ok",label:"No Granger signal between offer creation and cancellation",detail:"Offer creation and cancellation timing appear independent \u2014 no evidence of systematic cancel-to-create cycles."});let x=d("inflow"),S=d("outflow"),T=u(x,S),$=T.indexOf(Math.max(...T)),P=Math.max(...T),M={ccf:T,maxCorr:P,maxLag:$};P>.65&&$===0?(m+=12,s.push({sev:"warn",label:`Inflow and outflow move in perfect lockstep (\u03C1=${P.toFixed(2)} at lag 0)`,detail:"Funds entering and leaving the wallet in the same time window with high correlation at zero lag is consistent with pass-through or round-trip self-trading: money comes in and immediately goes back out."})):P>.55&&$<=1?(m+=8,s.push({sev:"info",label:`Inflow leads outflow (\u03C1=${P.toFixed(2)}, lag=${$})`,detail:"Incoming funds reliably precede outgoing funds at short lag. Could indicate legitimate management, but in conjunction with other signals suggests fund cycling."})):s.push({sev:"ok",label:"No suspicious inflow\u2192outflow Granger pattern",detail:"Inflow and outflow timing are not predictably linked, consistent with independent organic transaction activity."});let N=d("nftList"),_=d("nftAccept"),X=N.reduce((D,F)=>D+F,0),I=_.reduce((D,F)=>D+F,0);if(X>3&&I>3){let D=u(N,_),F=Math.max(...D),W=D.indexOf(F);F>.6&&W<=1&&(m+=8,s.push({sev:"warn",label:`NFT listing causes rapid acceptance (\u03C1=${F.toFixed(2)}, lag=${W})`,detail:"NFT sell offer creation is closely followed by acceptance. Combined with the NFT trap detection module, this timing pattern can indicate coordinated offer traps with a controlled accepting address."}))}return s.some(D=>D.sev==="warn"||D.sev==="critical")||s.length||s.push({sev:"ok",label:"No Granger causality anomalies detected",detail:"Temporal relationships between transaction types show no suspicious lead-lag patterns."}),m>=18?p="causal-signal":m>=8&&(p="elevated"),{signals:s,verdict:p,riskPenalty:m,offerCancelCausality:b,inflowOutflowCausality:M,windowCount:c.length}}function fh(e,t){let n=new Map;for(let{tx:o}of e){let i=[o.TakerGets,o.Amount];for(let r of i){if(!r||typeof r!="object")continue;let l=r.currency,c=Number(r.value||0),d=o.Account;if(!l||!d||c<=0||!Number.isFinite(c))continue;n.has(l)||n.set(l,{senders:new Set,vol:0,trades:0});let u=n.get(l);u.senders.add(d),u.vol+=c,u.trades++}}let s=[],a=[];for(let[o,i]of n.entries()){if(i.trades<8)continue;let r=i.senders.size;a.push({currency:o,uniqueActors:r,vol:i.vol,trades:i.trades}),r<5?s.push({sev:"critical",label:`${o}: ${r} wallet(s) driving all volume`,detail:`${i.trades} trades totalling ${i.vol.toFixed(2)} ${o} from only ${r} address(es). Fewer than 5 unique actors generating most volume is a wash trading red flag.`}):r<10&&s.push({sev:"warn",label:`${o}: low actor diversity (${r} wallets, ${i.trades} trades)`,detail:`Volume concentrated among only ${r} addresses. Organic markets typically have broader participation.`})}return a.length?s.length||s.push({sev:"ok",label:"Volume concentration normal",detail:`${a.length} token(s) analysed \u2014 all have \u226510 unique trading participants.`}):s.push({sev:"info",label:"No IOU/token volume data",detail:"No token-denominated transactions found in history (XRP-only activity)."}),{signals:s,concentrations:a}}function hh(e,t,n,s){let a=[],o=!!(n&ze.lsfDefaultRipple)||t.some(p=>p.account===e.Account),i=t.filter(p=>Number(p.balance)<0),r=i.reduce((p,m)=>p+Math.abs(Number(m.balance)),0);i.length>0&&a.push({sev:"info",label:`Token issuer: ${i.length} outstanding currency lines`,detail:`Total outstanding: ${R(r,2)} across ${i.length} holder(s).`});let l=t.filter(p=>p.freeze),c=t.filter(p=>p.freeze_peer);l.length&&a.push({sev:"warn",label:`${l.length} trustline(s) frozen by this account`,detail:"This account has frozen specific trustlines."}),c.length&&a.push({sev:"critical",label:`${c.length} of your trustline(s) frozen by issuer`,detail:`Frozen currencies: ${c.map(p=>p.currency).join(", ")}. You cannot transfer these tokens.`}),n&ze.lsfGlobalFreeze&&a.push({sev:"critical",label:"Global Freeze \u2014 all token transfers suspended",detail:"No holders can transfer tokens issued by this account."}),n&ze.lsfNoFreeze&&a.push({sev:"ok",label:"NoFreeze flag set \u2014 issuer cannot freeze balances",detail:"Token holders are protected against future freeze actions."});let d=Number(e.Balance||0)/1e6,u=10+Number(e.OwnerCount||0)*2;return i.length>0&&d<u+1&&a.push({sev:"warn",label:"Issuer balance near reserve \u2014 possible black hole",detail:"Issuer with outstanding tokens has almost no XRP above reserve. Tokens may be stranded."}),a.length===0&&a.push({sev:"ok",label:"No token issuer flags",detail:"This account does not appear to be a token issuer."}),{signals:a,isIssuer:o,obligationCount:i.length}}function gh(e,t,n,s=new Map){var m,f,v;let a=[],o=[],i=e.filter(h=>h.currency&&(h.currency.startsWith("03")||h.currency.length===40)),r=t.filter(({tx:h})=>h.TransactionType==="AMMDeposit"),l=t.filter(({tx:h})=>h.TransactionType==="AMMWithdraw"),c=t.filter(({tx:h})=>h.TransactionType==="AMMCreate"),d=t.filter(({tx:h})=>h.TransactionType==="AMMVote"),u=t.filter(({tx:h})=>h.TransactionType==="AMMBid");if(i.forEach(h=>{let w=Number(h.balance),k=Number(h.limit);o.push({currency:h.currency,issuer:h.account,balance:w,limit:k})}),o.length){a.push({sev:"info",label:`${o.length} LP token position(s)`,detail:`Active liquidity provider in ${o.length} AMM pool(s).`});for(let h of o){let w=s.get(h.currency);if(w){if(h.tvl=w.amount?Number(w.amount)/1e6:null,h.tvl2=(m=w.amount2)!=null&&m.value?Number(w.amount2.value):null,h.feeRate=w.trading_fee!=null?w.trading_fee/1e3:null,h.lpSupply=(f=w.lp_token)!=null&&f.value?Number(w.lp_token.value):null,h.lpSupply&&h.balance){let k=Math.abs(h.balance)/h.lpSupply*100;h.ownerPct=k,k>50&&a.push({sev:"warn",label:`Dominant AMM position: ${k.toFixed(0)}% of pool`,detail:`This account controls ${k.toFixed(0)}% of the LP token supply for pool ${z(h.currency)}. Withdrawing all at once would severely impact pool liquidity and anyone currently trading in it.`})}h.tvl!=null&&a.push({sev:"info",label:`Pool TVL: ${R(h.tvl,2)} XRP${h.tvl2?` + ${R(h.tvl2,2)} tokens`:""} \xB7 Fee: ${((v=h.feeRate)==null?void 0:v.toFixed(2))??"?"}%`,detail:`Actual pool context from amm_info. Your LP position represents ${h.ownerPct!=null?h.ownerPct.toFixed(1)+"% of the pool.":"an unknown share of the pool."}`})}}}return c.length&&a.push({sev:"info",label:`Created ${c.length} AMM pool(s)`,detail:"This account bootstrapped one or more liquidity pools."}),(r.length||l.length)&&a.push({sev:"info",label:`${r.length} deposit(s) \xB7 ${l.length} withdrawal(s)`,detail:"LP activity history."}),d.length&&a.push({sev:"info",label:`${d.length} AMM fee vote(s)`,detail:"This account has voted on AMM trading fee parameters."}),u.length&&a.push({sev:"info",label:`${u.length} continuous auction bid(s)`,detail:"Bid for the AMM auction slot (reduced fee trading window)."}),o.filter(h=>Math.abs(h.balance)>1e3).length&&a.push({sev:"warn",label:"Large LP positions \u2014 impermanent loss risk",detail:"Significant liquidity positions carry exposure to price divergence between pool assets."}),a.length===0&&a.push({sev:"ok",label:"No AMM positions",detail:"This account is not a liquidity provider."}),{signals:a,positions:o,deposits:r.length,withdrawals:l.length}}function vh(e){let t=[],s=e.filter(({tx:p})=>p.Fee&&Number(p.Fee)>0);if(s.length<10)return{signals:[],verdict:"insufficient",riskPenalty:0,avgFeeMultiplier:null,spikeCount:0,topFeeHashes:[]};let a=s.map(({tx:p})=>Number(p.Fee)/12),o=a.reduce((p,m)=>p+m,0)/a.length,r=s.filter(({tx:p})=>Number(p.Fee)/12>100).length,l=r/s.length,c=0,d="normal",u=[...s].sort((p,m)=>Number(m.tx.Fee)-Number(p.tx.Fee)).slice(0,5).map(({tx:p})=>({hash:p.hash,mult:(Number(p.Fee)/12).toFixed(0),fee:Number(p.Fee)}));return l>.15&&r>=5?(c=10,d="elevated",t.push({sev:"warn",label:`Fee spike pattern: ${r} txs paid >100x base fee (${(l*100).toFixed(0)}% of history)`,detail:`Average fee multiplier: ${o.toFixed(0)}x. In XRPL, bots often pay elevated fees to guarantee same-ledger execution as a counterparty \u2014 a technique used in coordinated wash trading and sandwich attacks. Organic users rarely pay more than 2\u20135x the base fee. Top hashes: ${u.slice(0,3).map(p=>z(p.hash)).join(", ")}.`,hashes:u.map(p=>p.hash)})):o>20?(c=4,t.push({sev:"info",label:`Elevated average fee (${o.toFixed(0)}x base fee)`,detail:"This wallet consistently pays above-average fees. Could indicate priority execution requirements or automated trading."})):t.push({sev:"ok",label:`Fee levels normal (avg ${o.toFixed(1)}x base fee)`,detail:"Transaction fees are within typical organic ranges."}),{signals:t,verdict:d,riskPenalty:c,avgFeeMultiplier:+o.toFixed(2),spikeCount:r,topFeeHashes:u}}function bh(e,t){let n=[],s=new Map;for(let{tx:r}of e){if(r.TransactionType!=="Payment"||r.Account!==t)continue;let l=r.Destination,c=r.DestinationTag;l&&(s.has(l)||s.set(l,new Set),c!=null&&s.get(l).add(c))}let a=[...s.entries()].filter(([r])=>{let l=ns(r);return(l==null?void 0:l.type)==="exchange"}),o=0,i=[];for(let[r,l]of s.entries()){let c=ns(r),d=(c==null?void 0:c.name)||z(r),u=l.size,p=e.filter(({tx:m})=>m.Account===t&&m.Destination===r).length;i.push({dest:r,name:d,uniqueTags:u,txCount:p,tags:[...l].slice(0,10)}),u===1&&p>=5&&(c==null?void 0:c.type)==="exchange"?n.push({sev:"info",label:`${d}: ${p} payments all using tag ${[...l][0]}`,detail:`Single destination tag used across all ${p} payments to ${d}. This is the normal pattern for one person funding their own exchange account.`}):u>10&&(c==null?void 0:c.type)==="exchange"?(o=Math.max(o,8),n.push({sev:"warn",label:`${d}: ${u} different destination tags used`,detail:`${p} payments to ${d} used ${u} different tags \u2014 each tag typically identifies a different customer account. Funding many exchange sub-accounts can indicate either a service (legitimate) or coordinated deposit layering where funds are spread across many exchange wallets to avoid detection.`})):u===0&&p>=2&&(c==null?void 0:c.type)==="exchange"&&n.push({sev:"warn",label:`${d}: ${p} payments with no destination tag`,detail:"Payments to exchange addresses without a destination tag may not be credited. Most exchanges require a tag to identify which customer account receives the funds."})}return n.length===0&&s.size>0?n.push({sev:"ok",label:"Destination tag patterns normal",detail:"Payment routing tags are consistent with regular outbound payments."}):s.size===0&&n.push({sev:"info",label:"No outbound payments to analyse for destination tags",detail:"No outbound Payment transactions found in history."}),{signals:n,riskPenalty:o,tagProfiles:i}}function yh(e,t){var c,d;let n=[],s=e.filter(({tx:u})=>u.TransactionType==="Payment"&&u.Account===t&&(Array.isArray(u.Paths)&&u.Paths.length>0||u.SendMax!=null));if(s.length===0)return{signals:[],riskPenalty:0,roundTripCount:0,deepHopCount:0,selfRoutedCount:0,noData:!0};let a=s.length<5?` (small sample: ${s.length} path payments found \u2014 patterns may not be statistically significant)`:"",o=s.filter(({tx:u})=>{let p=typeof u.Amount=="string",m=typeof u.SendMax=="string";return p&&m}),i=s.filter(({tx:u})=>Array.isArray(u.Paths)?u.Paths.some(p=>Array.isArray(p)&&p.length>=3):!1),r=s.filter(({tx:u})=>u.Destination===t),l=0;return o.length>=1&&(l+=12,n.push({sev:"warn",label:`${o.length} XRP\u2192IOU\u2192XRP round-trip path payments`,detail:`Sending XRP and receiving XRP via intermediate token pairs means the payment routes through the DEX and creates trading volume without changing economic position. ${o.length} occurrences suggests this is deliberate. This is the classic cross-currency wash-trading arb pattern on XRPL. Example hash: ${(d=(c=o[0])==null?void 0:c.tx)!=null&&d.hash?z(o[0].tx.hash):"N/A"}.`,hashes:o.slice(0,5).map(({tx:u})=>u.hash).filter(Boolean)})),i.length>=1&&(l+=6,n.push({sev:"info",label:`${i.length} path payments with \u22653 intermediate hops`,detail:"Deep routing chains (3+ hops) can indicate: legitimate arbitrage, liquidity optimization, or deliberate obfuscation of fund origin. Check each transaction for the intermediate issuers in the path."})),r.length>0&&(l+=15,n.push({sev:"critical",label:`${r.length} path payment(s) where sender = destination`,detail:`Money sent to your own address via a multi-hop path creates DEX trading volume with no net change in balance. This is a direct wash-trading technique: the path through the order book generates artificial volume on every intermediate pair. Hashes: ${r.slice(0,3).map(({tx:u})=>z(u.hash||"")).join(", ")}.`,hashes:r.slice(0,5).map(({tx:u})=>u.hash).filter(Boolean)})),n.length===0&&n.push({sev:"ok",label:`${s.length} path payment(s) \u2014 no suspicious routing patterns${a}`,detail:"No circular routing (XRP\u2192IOU\u2192XRP), self-routing, or unusual deep hop chains detected."}),{signals:n,riskPenalty:l,roundTripCount:o.length,deepHopCount:i.length,selfRoutedCount:r.length}}function wh(e,t){var u;let n=new Map,s=[];for(let{tx:p,meta:m}of e){if(p.TransactionType!=="Payment"||p.Destination!==t)continue;let f=p.Account;if(!f||f===t)continue;let v=0,h=null,w=(m==null?void 0:m.delivered_amount)||p.Amount;typeof w=="string"?v=Number(w)/1e6:w!=null&&w.value&&(h={value:Number(w.value),currency:Sn(w.currency),issuer:w.issuer});let k=At(p);s.push({src:f,amtXrp:v,amtToken:h,ts:k,hash:p.hash||"",destTag:p.DestinationTag}),n.has(f)||n.set(f,{addr:f,totalXrp:0,txCount:0,firstSeen:k,lastSeen:k,entity:ns(f)||null});let b=n.get(f);b.totalXrp+=v,b.txCount++,b.lastSeen=Math.max(b.lastSeen,k),b.firstSeen=Math.min(b.firstSeen,k)}let a=[...n.values()].sort((p,m)=>m.totalXrp-p.totalXrp||m.txCount-p.txCount).slice(0,10),o=a.reduce((p,m)=>p+m.totalXrp,0),i=a.filter(p=>{var m;return((m=p.entity)==null?void 0:m.type)==="exchange"}),r={};for(let p of s){if(p.amtXrp<=0)continue;let m=Math.round(p.amtXrp/10)*10;r[m]=(r[m]||0)+1}let l=Object.entries(r).sort((p,m)=>m[1]-p[1])[0],c=l&&l[1]>=5&&l[1]/s.length>.4,d=[];if(i.length){let p=[...new Set(i.map(m=>m.entity.name))].join(", ");d.push({sev:"info",label:`Funding from ${i.length} known exchange(s): ${p}`,detail:`${R(i.reduce((m,f)=>m+f.totalXrp,0),2)} XRP received from exchange withdrawals \u2014 typical for a personal trading wallet.`})}if(c&&d.push({sev:"warn",label:`Structured inbound pattern: ${l[1]} payments near ~${l[0]} XRP`,detail:`Over 40% of inbound payments cluster around the same amount (~${l[0]} XRP). Structured deposits can indicate layering \u2014 deliberately splitting large amounts into smaller equal transfers to avoid detection.`}),n.size===1&&s.length>=5){let p=a[0];d.push({sev:"info",label:`Single funding source: all ${s.length} inbound payments from one address`,detail:`${((u=p.entity)==null?void 0:u.name)||z(p.addr)} is the sole funding source. This is normal for a personal wallet but notable for a wallet claiming broad community usage.`})}return!d.length&&s.length>0&&d.push({sev:"ok",label:`${s.length} inbound payment(s) from ${n.size} source(s)`,detail:`Total received: ${R(o,2)} XRP. No unusual inbound patterns.`}),s.length===0&&d.push({sev:"info",label:"No inbound payments found in analysed history",detail:"Wallet may be funded via DEX activity or in ledgers outside the analysed range."}),{signals:d,topSources:a,totalIn:o,uniqueSources:n.size,timeline:s.slice(-20).reverse(),exchangeSrcs:i,structuredFlag:!!c}}function xh(e,t){var l,c;let n=[],s=[],a=[/airdrop/i,/claim.*reward/i,/free.*xrp/i,/verify.*wallet/i,/support.*team/i,/urgent/i,/suspended/i,/confirm.*seed/i,/your.*account.*hold/i,/unlock/i];for(let{tx:d}of e)if((l=d.Memos)!=null&&l.length)for(let u of d.Memos){let p=((c=u.Memo)==null?void 0:c.MemoData)||"";if(!p)continue;let m="";try{m=decodeURIComponent(p.replace(/../g,f=>"%"+f))}catch{m=p}if(!m||m===p)try{let f="";for(let v=0;v<p.length;v+=2){let h=parseInt(p.slice(v,v+2),16);h>=32&&h<127&&(f+=String.fromCharCode(h))}f.length>4&&(m=f)}catch{}s.push({tx:d.hash||"",type:d.TransactionType,sender:d.Account,text:m.slice(0,200),raw:p})}if(s.length===0)return{signals:[],allMemos:[],scamMemos:[],repeatedMemos:[]};let o=s.filter(d=>a.some(u=>u.test(d.text)));o.length&&n.push({sev:"critical",label:`${o.length} memo(s) match known scam patterns`,detail:`Memos containing phrases like "airdrop", "claim reward", "verify wallet", or "urgent" are used in social engineering attacks. These payments were likely sent to trick the recipient into taking action. Examples: ${o.slice(0,2).map(d=>'"'+d.text.slice(0,40)+'"').join(", ")}`});let i={};for(let d of s){let u=d.text.slice(0,50).trim().toLowerCase();u.length>3&&(i[u]=(i[u]||0)+1)}let r=Object.entries(i).filter(([,d])=>d>=3).sort((d,u)=>u[1]-d[1]);return r.length&&n.push({sev:"warn",label:`${r.length} memo text(s) repeated \u22653 times`,detail:`Identical memo text across multiple transactions suggests scripted or automated activity. Most repeated: "${r[0][0]}" (${r[0][1]}\xD7)`}),n.length||n.push({sev:"ok",label:`${s.length} memo(s) found \u2014 no suspicious patterns`,detail:"Memo content looks normal."}),{signals:n,allMemos:s,scamMemos:o,repeatedMemos:r.slice(0,5)}}function kh(e,t,n){let s=e.filter(u=>u.LedgerEntryType==="Escrow");if(!s.length)return{signals:[],escrows:[],hasThirdParty:!1};let a=[],o=Math.floor(Date.now()/1e3),i=946684800,r=s.map(u=>{let p=u.Account||null,m=u.Destination||null,f=Number(u.Amount||0)/1e6,v=u.FinishAfter?u.FinishAfter+i:null,h=u.CancelAfter?u.CancelAfter+i:null,w=p&&p!==n&&m===n,k=p===n&&m===n,b=v?Math.ceil((v-o)/86400):null;return{creator:p,dest:m,amtXrp:f,finishAfter:v,cancelAfter:h,isThirdParty:w,isSelfEscrow:k,daysToFinish:b,conditional:!!u.Condition}}),l=r.filter(u=>u.isThirdParty),c=r.reduce((u,p)=>u+p.amtXrp,0);l.length&&a.push({sev:"warn",label:`${l.length} escrow(s) created by external account(s) \u2014 funds locked to this address`,detail:`${R(l.reduce((u,p)=>u+p.amtXrp,0),2)} XRP in escrows that an outside party controls. The creator sets the conditions. Escrows created by attackers just before a drain attempt have been observed in some compromise patterns \u2014 verify who created these.`});let d=r.filter(u=>u.daysToFinish!=null&&u.daysToFinish>=0&&u.daysToFinish<=7);return d.length&&a.push({sev:"info",label:`${d.length} escrow(s) mature within 7 days`,detail:`${R(d.reduce((u,p)=>u+p.amtXrp,0),2)} XRP will become claimable soon. If these are third-party escrows, the creator can claim funds once the condition is met.`}),a.length||a.push({sev:"ok",label:`${s.length} self-escrow(s) \u2014 ${R(c,2)} XRP locked`,detail:"All escrows appear to be self-controlled time-locks. No third-party escrow risk."}),{signals:a,escrows:r,hasThirdParty:l.length>0,totalLocked:c}}function $h(e){let t=e.filter(l=>l.LedgerEntryType==="Check");if(!t.length)return{signals:[],checks:[]};let n=[],s=946684800,a=Math.floor(Date.now()/1e3),o=t.map(l=>{let c=typeof l.SendMax=="string"?Number(l.SendMax)/1e6:null,d=typeof l.SendMax=="object"?l.SendMax:null,u=l.Expiration?l.Expiration+s:null,p=(l.ledger_index,null),m=u&&u<a;return{sender:l.Account,dest:l.Destination,amtXrp:c,amtToken:d,expiry:u,expired:m,id:l.index||""}}),i=o.filter(l=>l.amtXrp&&l.amtXrp>100),r=o.filter(l=>l.expired);return i.length&&n.push({sev:"info",label:`${i.length} large uncashed check(s) \u2014 ${R(i.reduce((l,c)=>l+(c.amtXrp||0),0),2)} XRP pending`,detail:"Open checks can be cashed by the recipient at any time before expiry. Large uncashed checks represent a future outflow commitment."}),r.length&&n.push({sev:"info",label:`${r.length} expired check(s) \u2014 should be cancelled to reclaim reserve`,detail:"Expired checks still occupy owner reserve slots (2 XRP each). Cancelling them returns the reserved XRP."}),n.length||n.push({sev:"ok",label:`${t.length} check(s) found \u2014 no unusual patterns`,detail:"Check amounts are within normal range."}),{signals:n,checks:o}}function Sh(e,t){var m,f,v;if(!e||!((m=e.offers)!=null&&m.length))return{signals:[],hasData:!1};let{pair:n,offers:s}=e,a=[],o=new Map;for(let h of s){let w=h.Account,k=typeof h.TakerGets=="string"?Number(h.TakerGets)/1e6:Number(((f=h.TakerGets)==null?void 0:f.value)||0);o.set(w,(o.get(w)||0)+k)}let i=[...o.values()].reduce((h,w)=>h+w,0),r=o.get(t)||0,l=i>0?r/i:0,d=[...s].sort((h,w)=>{var x,S;let k=typeof h.TakerGets=="string"?Number(h.TakerGets)/1e6:Number(((x=h.TakerGets)==null?void 0:x.value)||0);return(typeof w.TakerGets=="string"?Number(w.TakerGets)/1e6:Number(((S=w.TakerGets)==null?void 0:S.value)||0))-k})[0],u=typeof(d==null?void 0:d.TakerGets)=="string"?Number(d.TakerGets)/1e6:Number(((v=d==null?void 0:d.TakerGets)==null?void 0:v.value)||0),p=i>0?u/i:0;return p>.4&&(d==null?void 0:d.Account)===t?a.push({sev:"critical",label:`Active wall order: this wallet controls ${(p*100).toFixed(0)}% of current book depth`,detail:`A single order from this address represents ${(p*100).toFixed(0)}% of the visible order book depth on pair ${n}. Large orders placed to make a market look deeper than it is \u2014 without intent to fill \u2014 is spoofing. This order is live right now.`}):p>.4&&a.push({sev:"warn",label:`Wall order present: ${(p*100).toFixed(0)}% of book depth in one order`,detail:`A single address controls ${(p*100).toFixed(0)}% of the current order book for pair ${n}. Wall orders dominate book depth and can be removed instantly \u2014 they create false liquidity signals.`}),l>.25&&a.push({sev:"info",label:`This wallet controls ${(l*100).toFixed(0)}% of current order book depth`,detail:`${R(r,2)} of ${R(i,2)} total book volume on pair ${n}.`}),a.length||a.push({sev:"ok",label:`Live order book looks normal (${s.length} orders, pair: ${n.split("\u2194").map(h=>h.split("+")[0]).join("\u2194")})`,detail:"No wall orders or unusual depth concentration detected in the current order book."}),{signals:a,hasData:!0,pair:n,offerCount:s.length,ourShare:l,wallShare:p}}function Th(e,t,n,s,a,o,i,r,l,c,d,u,p,m){var v,h;return[{label:"Security",pts:Math.round((100-t.score)*.4),max:40,color:"#ff5555",icon:"\u{1F510}"},{label:"Drain Risk",pts:{low:0,medium:10,high:25,critical:35}[n.riskLevel]||0,max:35,color:"#ff5555",icon:"\u26A0\uFE0F"},{label:"Wash Trading",pts:Math.min(15,Math.round((a.score||0)*.15)),max:15,color:"#ffb86c",icon:"\u{1F4CA}"},{label:"NFT Risk",pts:Math.min(15,s.flags.filter(w=>w.sev==="critical").length*8+s.flags.filter(w=>w.sev==="warn").length*3),max:15,color:"#bd93f9",icon:"\u{1F3A8}"},{label:"Benford's",pts:(o==null?void 0:o.chiSq)>20.09?10:(o==null?void 0:o.chiSq)>15.51?5:0,max:10,color:"#f1fa8c",icon:"\u{1F4D0}"},{label:"Forensic Suite",pts:Math.min(20,Math.min(8,Math.round(((r==null?void 0:r.riskPenalty)||0)*.35))+Math.min(8,Math.round(((l==null?void 0:l.riskPenalty)||0)*.4))+Math.min(8,Math.round(((c==null?void 0:c.riskPenalty)||0)*.35))+Math.min(8,Math.round(((d==null?void 0:d.riskPenalty)||0)*.35))),max:20,color:"#00d4ff",icon:"\u{1F9EC}"},{label:"Vol Conc",pts:Math.min(10,(((v=i==null?void 0:i.signals)==null?void 0:v.filter(w=>w.sev==="critical").length)||0)*6+(((h=i==null?void 0:i.signals)==null?void 0:h.filter(w=>w.sev==="warn").length)||0)*3),max:10,color:"#ffb86c",icon:"\u{1FAE7}"},{label:"Fee Spikes",pts:Math.min(5,(u==null?void 0:u.riskPenalty)||0),max:5,color:"#ffb86c",icon:"\u{1F4B8}"}].filter(w=>w.pts>0)}function Ch(e,t,n,s,a,o,i,r,l,c,d=null){let u=0;u+=Math.round((100-e.score)*.4),u+={low:0,medium:10,high:25,critical:35}[t.riskLevel]||0;let m=n.flags.filter(v=>v.sev==="critical").length,f=n.flags.filter(v=>v.sev==="warn").length;if(u+=Math.min(15,m*8+f*3),u+=Math.min(15,Math.round(s.score*.15)),(a==null?void 0:a.chiSq)!=null&&(a.chiSq>20.09?u+=10:a.chiSq>15.51&&(u+=5)),o!=null&&o.signals){let v=o.signals.filter(w=>w.sev==="critical").length,h=o.signals.filter(w=>w.sev==="warn").length;u+=Math.min(10,v*6+h*3)}return i!=null&&i.riskPenalty&&(u+=Math.min(8,Math.round(i.riskPenalty*.35))),r!=null&&r.riskPenalty&&(u+=Math.min(8,Math.round(r.riskPenalty*.4))),l!=null&&l.riskPenalty&&(u+=Math.min(8,Math.round(l.riskPenalty*.35))),c!=null&&c.riskPenalty&&(u+=Math.min(8,Math.round(c.riskPenalty*.35))),d!=null&&d.riskPenalty&&(u+=Math.min(5,d.riskPenalty)),Math.min(100,u)}function Ph(e){var h;let t=document.getElementById("inspect-benfords-body");if(!t)return;let n={critical:"sev-critical",warn:"sev-warn",info:"sev-info",ok:"sev-ok"},s=e.signals.map(w=>`
  ...
  `;t.innerHTML=s+o+a+v}function Lh(e){var o;let t=document.getElementById("inspect-volconc-body");if(!t)return;let n={critical:"sev-critical",warn:"sev-warn",info:"sev-info",ok:"sev-ok"},s=e.signals.map(i=>`
    ...
    </table>`:"";t.innerHTML=s+a}function Sa(e,t,n){let s=document.getElementById(e);if(!s)return;let a={critical:"sev-critical",warn:"sev-warn",info:"sev-info",ok:"sev-ok"},o=t.signals.map(i=>`
    <div class="finding finding--${i.sev}">
    ...
    </div>`).join("");s.innerHTML=o+(n||"")}function Mh(e){let t=[["Sample size",e.sampleSize],["Amount entropy",e.amountEntropy!=null?e.amountEntropy.toFixed(2)+" bits":"\u2014",e.amountEntropy!=null&&e.amountEntropy<2?"risk-text-high":""],["Counterparty entropy",e.counterpartyEntropy!=null?e.counterpartyEntropy.toFixed(2)+" bits":"\u2014"],["Time-of-day entropy",e.timeEntropy!=null?e.timeEntropy.toFixed(2)+" bits":"\u2014"],["Unique counterparties",e.uniqueCounterparties],["Verdict",e.verdict,e.verdict==="anomalous"?"risk-text-high":e.verdict==="elevated"?"risk-text-med":""]];Sa("inspect-entropy-body",e,'<div class="wash-stat-row" style="margin-top:10px"><span>Metric</span><span class="mono" style="opacity:.45">Value</span></div>'+t.map(([n,s,a])=>`<div class="wash-stat-row"><span>${n}</span><span class="mono ${a||""}">${s}</span></div>`).join(""))}function Ah(e){var s;let t=[["Unique counterparties",e.uniqueCounterparties],["Zipf exponent (s)",e.zipfExponent!=null?e.zipfExponent.toFixed(3):"\u2014",e.zipfExponent!=null&&(e.zipfExponent<.4||e.zipfExponent>2.2)?"risk-text-high":""],["Fit quality (R\xB2)",e.rSquared!=null?e.rSquared.toFixed(3):"\u2014",e.rSquared!=null&&e.rSquared<.55?"risk-text-high":""],["Natural range","s \u2248 0.8\u20131.3, R\xB2 > 0.55"],["Verdict",e.verdict,e.verdict==="anomalous"?"risk-text-high":e.verdict==="elevated"?"risk-text-med":""]],n=((s=e.freqTable)==null?void 0:s.slice(0,10).map((a,o)=>{let i=e.freqTable[0]||1,r=(a/i*100).toFixed(0),l=e.freqTable[0]?(e.freqTable[0]/Math.pow(o+1,e.zipfExponent||1)).toFixed(1):0;return`<div class="wash-stat-row">
    ...
    </div>`).join("")}function Dh(e,t,n,s,a){let o=y("inspect-security-body");if(!o)return;let i=Object.entries(ze).filter(([,r])=>n&r).map(([r])=>r.replace("lsf",""));o.innerHTML=`
    <div class="audit-items">
      ${e.findings.map(r=>Tn(r)).join("")}
    </div>
    ${i.length?`
  ...
  `,Ta("badge-security",e.findings)}function Ih(e,t,n,s){let a=y("inspect-drain-body");if(!a)return;let o={low:"#50fa7b",medium:"#ffb86c",high:"#ff8c42",critical:"#ff5555"},i={low:"\u2713",medium:"\u26A0",high:"\u26A0",critical:"\u26D4"};a.innerHTML=`
  ...
  `,dg("badge-drain",e.riskLevel)}function Fh(e,t){let n=y("inspect-nft-body");n&&(n.innerHTML=`
    <div class="audit-items">
      ${e.flags.map(s=>Tn(s)).join("")}
    </div>
    ${t.length?`
    <div class="nft-grid">
      ${t.slice(0,12).map(s=>Oh(s)).join("")}
    </div>
    ${t.length>12?`<div class="nft-more">+${t.length-12} more NFTs</div>`:""}
    `:""}
  `,Ta("badge-nft",e.flags))}function Oh(e){let t=Number(e.Flags||0),n=!!(t&Ul.lsfTransferable),s=!!(t&Ul.lsfBurnable),a=e.NFTokenTaxon||0,o=e.TransferFee?`${(e.TransferFee/1e3).toFixed(1)}%`:"0%";return`
    <div class="nft-card">
    ...
    <div class="wash-stats">
      ${Cs("Offer Creates",e.stats.creates)}
      ${Cs("Offer Cancels",e.stats.cancels)}
      ${Cs("Filled Offers",e.stats.fills)}
      ${Cs("Payments",e.stats.payments)}
      ${Cs("Round-trip Counterparties",e.stats.roundTrip)}
    </div>
    <div class="audit-items">
      ${e.signals.map(a=>Tn(a)).join("")}
    </div>
  `;let s=y("badge-wash");if(s){let a=e.verdict==="clean"||e.verdict==="low-risk"?"ok":e.verdict==="suspicious"?"warn":"crit";s.textContent=e.verdict.replace("-"," "),s.className="section-badge section-badge--"+a}}function Cs(e,t){return`<div class="wash-stat"><span class="wash-stat-label">${g(e)}</span><span class="wash-stat-val">${t}</span></div>`}function Xh(e,t){let n=y("inspect-issuer-body");if(!n)return;let s=t.filter(a=>a.currency&&(a.currency.length===3||a.currency.length===40));n.innerHTML=`
    <div class="audit-items">
      ${e.signals.map(a=>Tn(a)).join("")}
    </div>
    ${s.length?`
    <div class="trustline-list">
      ${s.slice(0,10).map(a=>`
        <div class="trustline-row">
          <span class="trustline-currency">${g(Sn(a.currency))}</span>
  ...
  `,Ta("badge-issuer",e.signals)}function Hh(e,t){let n=y("inspect-amm-body");n&&(n.innerHTML=`
    <div class="audit-items">
      ${e.signals.map(s=>Tn(s)).join("")}
    </div>
    ${e.positions.length?`
  ...
  `,Ta("badge-amm",e.signals))}function zh(e){let t=y("trust-count-badge");t&&(t.textContent=e.length);let n=y("inspect-trust-body");n&&(n.innerHTML=e.length?e.map(s=>{let a=s.freeze?'<span class="trustline-frozen">Frozen</span>':"",o=s.freeze_peer?'<span class="trustline-frozen trustline-frozen--peer">Issuer Frozen</span>':"",i=s.no_ripple?'<span class="trustline-norip">NoRipple</span>':"";return`
          <div class="trustline-row">
            <span class="trustline-currency">${g(Sn(s.currency))}</span>
          ...
          </div>`}).join(""):'<div class="inspect-empty-note">No transactions found.</div>',e.length>s&&(n.innerHTML+=`<div class="tx-more">Showing ${s} of ${e.length} transactions</div>`)}function Ql(e,t,n){let s=Number((n==null?void 0:n.totalIn)||0),a=Number((t==null?void 0:t.totalOut)||0),o=Number(e||0);if(s<=0&&a<=0)return'<div class="inspect-empty-note">No inbound or outbound XRP flow found in the analysed transaction history.</div>';let i=new Set(((t==null?void 0:t.blackHoleDests)||[]).map(x=>x.addr)),r=new Set(((t==null?void 0:t.exchangeDests)||[]).map(x=>x.addr)),l=new Set(((t==null?void 0:t.newWalletDests)||[]).map(x=>x.addr)),c=0,d=0,u=0,p=0;for(let x of(t==null?void 0:t.destinations)||[])i.has(x.addr)?c+=x.totalXrp:r.has(x.addr)?d+=x.totalXrp:l.has(x.addr)?u+=x.totalXrp:p+=x.totalXrp;let m=Math.max(s,a,o,1),f=x=>(x>0?Math.max(1.5,x/m*100):0).toFixed(1),v=[{label:"Black hole",xrp:c,color:tn.blackhole},{label:"Exchange",xrp:d,color:tn.exchange},{label:"New wallet",xrp:u,color:tn.newWallet},{label:"Other",xrp:p,color:tn.other}].filter(x=>x.xrp>0),h=a>0?Math.max(1.5,a/m*100):0,w=v.map(x=>`<div style="width:${(a>0?x.xrp/a*100:0).toFixed(1)}%;background:${x.color};height:100%" title="${g(x.label)}: ${R(x.xrp,2)} XRP"></div>`).join(""),k=(x,S,T,$,P)=>`
  ...
  `,a){let c=e.blackHoleDests.length||e.exchangeDests.length>2;a.textContent=`${e.uniqueDests} dests`,a.className=`section-badge section-badge--${c?"crit":e.uniqueDests>0?"warn":"ok"}`}}function Zl(e,t){if(!e||!t)return"";let n=new Date(e*1e3).toLocaleDateString(),s=new Date(t*1e3).toLocaleDateString();return n===s?n:`${n} \u2013 ${s}`}function Wh(e,t){let n=y("inspect-issuer-connections-body");if(!n)return;let s=e.totalIssued,a=y("badge-issuer-connections");if(n.innerHTML=`
    <div class="audit-items">
      ${e.signals.map(o=>Tn(o)).join("")}
    </div>
  ...
  `,a){let o=e.signals.some(i=>i.sev==="critical")?"crit":e.signals.some(i=>i.sev==="warn")?"warn":e.signals.some(i=>i.sev==="info")?"neutral":"ok";a.className=`section-badge section-badge--${o}`,a.textContent=s>0?`${e.holderCount} holders`:"No issuance"}}function qh(e){var d;let t=document.getElementById("inspect-fee-analysis-body");if(!t||!e)return;let n={critical:"sev-critical",warn:"sev-warn",info:"sev-info",ok:"sev-ok"},s=(e.signals||[]).map(u=>`
    ...
    </div>`;t.innerHTML=l+s+a+o;let c=document.getElementById("badge-fee-analysis");c&&(c.textContent=r?"Elevated":"Normal",c.className=`section-badge section-badge--${r?"warn":"ok"}`)}function Vh(e){var d,u;let t=document.getElementById("inspect-desttag-body");if(!t||!e)return;let n={critical:"sev-critical",warn:"sev-warn",info:"sev-info",ok:"sev-ok"},s=(e.signals||[]).map(p=>`
    ...
    ${e.selfRoutedCount?`<div class="wash-stat-row"><span>Self-routed payments</span><span class="mono risk-text-high">${e.selfRoutedCount}</span></div>`:""}`;if(n.innerHTML=o+i,s){let l=e.signals.some(d=>d.sev==="critical"),c=e.signals.some(d=>d.sev==="warn");s.textContent=l?"Critical":c?"Check":"Normal",s.className=`section-badge section-badge--${l?"crit":c?"warn":"ok"}`}}function Kh(e){var r,l,c;let t=y("inspect-inbound-body");if(!t)return;let n={critical:"sev-critical",warn:"sev-warn",info:"sev-info",ok:"sev-ok"},s=(e.signals||[]).map(d=>`
    ...
    <div class="wash-stat-row">
      ...
      <span class="mono">${R(c.amtXrp,2)} XRP${sn(c.amtXrp)}</span>
    ...
    <div class="wash-stat-row">
      ...
      <span class="mono">${l.amtXrp!=null?R(l.amtXrp,2)+" XRP"+sn(l.amtXrp):(((c=l.amtToken)==null?void 0:c.value)||"?")+" "+((d=l.amtToken)==null?void 0:d.currency)}</span>
      <span style="font-size:.72rem;${l.expired?"color:#ff5555":"opacity:.55"}">${l.expired?"Expired":"Open"}</span>
    </div>`}).join("");n.innerHTML=a+`<div style="margin-top:10px">${o}</div>`;let i=y("badge-checks");i&&(i.textContent=`${e.checks.length} check${e.checks.length!==1?"s":""}`,i.className="section-badge section-badge--neutral")}function Zh(e){let t=y("section-livebook"),n=y("inspect-livebook-body");if(t&&(t.style.display=""),!n)return;if(!(e!=null&&e.hasData)){n.innerHTML='<div class="inspect-empty-note">No live order book activity found for this wallet.</div>';let r=y("badge-livebook");r&&(r.textContent="None",r.className="section-badge section-badge--neutral");return}let s={critical:"sev-critical",warn:"sev-warn",info:"sev-info",ok:"sev-ok"},a=(e.signals||[]).map(r=>`
    ...
    ${e.wallShare>.3?`<div class="wash-stat-row"><span>Largest single order share</span><span class="mono ${e.wallShare>.4?"risk-text-high":"risk-text-med"}">${(e.wallShare*100).toFixed(1)}%</span></div>`:""}`;n.innerHTML=a+o;let i=y("badge-livebook");if(i){let r=(e.signals||[]).some(c=>c.sev==="critical"),l=(e.signals||[]).some(c=>c.sev==="warn");i.textContent=r?"Wall order":l?"Check":"Normal",i.className=`section-badge section-badge--${r?"crit":l?"warn":"ok"}`}}function eg(e,...t){let n=y("inspect-risk-breakdown");if(!n)return;let s=Th(e,...t);if(!s.length){n.innerHTML="";return}let a=Math.max(1,s.reduce((o,i)=>o+i.pts,0));n.innerHTML=`
    ...
    </div>`}function Tn({sev:e,label:t,detail:n}){return`
    ...
    <div class="report-wrap">
        ...
        <div id="ai-explanation-body">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:8px">
            ...
            <select id="ai-local-model" class="xrpl-input" style="width:auto;padding:4px 8px;font-size:.7rem">
              ${Io.map(A=>`<option value="${A.id}" ${A.id===Ts?"selected":""}>${g(A.label)}</option>`).join("")}
            </select>
      ...
      <div class="report-section">
        ...
        <div style="margin-bottom:16px">${Ng(f,e)}</div>
        ${Ql(n,p,P)}
      </div>
      ...
      <div class="report-section">
        ...
        <div class="report-stats-grid">${lt}</div>
      </div>
      ...
      <div class="report-section">
        ...
        <div class="report-findings">${rt}</div>
      </div>`:`
      <div class="report-section">
  ...
  `}function og(e,...t){e.innerHTML=ag(...t)}function ig(){let e=document.getElementById("tab-inspector");e&&(e.querySelector("[data-inspector-v2]")||(e.innerHTML=`
    <style>
  ...
  `))}function rg(){if(document.getElementById("inspector-nav"))return;let e=document.createElement("nav");e.id="inspector-nav",e.setAttribute("aria-label","Inspector navigation"),e.innerHTML=`
  ...
  `;let t=document.getElementById("tab-inspector");t&&t.appendChild(e)}function lg(){if(document.getElementById("inspector-howto"))return;let e=document.createElement("div");e.id="inspector-howto",e.className="howto-overlay",e.style.display="none",e.innerHTML=`
  ...
  `).join(""))}function bg(){let e=document.getElementById("isd-cap-grid");e&&(e.innerHTML=hg.map(t=>`
  ...
  `).join(""))}function jo(){let e=document.getElementById("isd-wallets-section"),t=document.getElementById("isd-wallet-list");if(!e||!t)return;let n=be(Z(mg))||[];if(!n.length){e.style.display="none";return}e.style.display="",t.innerHTML=n.map(s=>{let a=s.color||"#50fa7b",o=s.emoji||"\u{1F48E}",i=s.label||"Wallet",r=s.address||"",l=r?r.slice(0,8)+"\u2026"+r.slice(-6):"\u2014",c=s.testnet?'<span class="isd-wallet-testnet">TESTNET</span>':"";return`
    ...
    `}).join("")}function Wo(){let e=document.getElementById("isd-recent-section"),t=document.getElementById("isd-recent-list");if(!e||!t)return;let n=Ca();if(!n.length){e.style.display="none";return}e.style.display="",t.innerHTML=n.map((s,a)=>{let o=s.addr?s.addr.slice(0,10)+"\u2026"+s.addr.slice(-8):"\u2014",i=s.riskScore!=null?`<span class="isd-risk-pill isd-risk-pill--${sc(s.riskScore)}">${s.riskScore}</span>`:"";return`
...
`),a=new Blob([s],{type:"text/csv;charset=utf-8;"}),o=URL.createObjectURL(a),i=document.createElement("a"),r=((c=(l=document.getElementById("inspect-addr-badge"))==null?void 0:l.dataset)==null?void 0:c.fullAddr)||"wallet";i.href=o,i.download=`naluxrp_${r.slice(0,10)}_${new Date().toISOString().slice(0,10)}.csv`,document.body.appendChild(i),i.click(),document.body.removeChild(i),URL.revokeObjectURL(o)};function kg(e,t){let s=Ca().find(o=>o.addr===e);if(!s||s.riskScore==null||t==null)return null;let a=t-s.riskScore;return{prev:s.riskScore,curr:t,diff:a,ts:s.ts}}function $g(e,t){let n=kg(e,t);if(!n||Math.abs(n.diff)<2)return;let s=document.getElementById("inspect-risk-score");if(!s)return;let a=n.diff>0?`<span style="color:#ff5555;font-size:.7rem;font-weight:700"> \u2191${n.diff}</span>`:`<span style="color:#50fa7b;font-size:.7rem;font-weight:700"> \u2193${Math.abs(n.diff)}</span>`,o=Pa(n.ts);s.insertAdjacentHTML("afterend",`<span class="risk-score-diff" title="Changed from ${n.prev} \u2192 ${n.curr} since ${o}">${a} vs ${o}</span>`)}window._inspectMaxTx||(window._inspectMaxTx=5e3);window.inspectorLoadAddr=function(e){let t=y("inspect-addr");t&&(t.value=e),ts()};window.inspectWalletAddr=function(e){var n,s;window.inspectorLoadAddr(e);let t=document.querySelector('[data-tab="inspector"]');t&&((n=window.switchTab)==null||n.call(window,t,"inspector")),(s=window.showDashboard)==null||s.call(window)};window.printInspectorReport=function(){var s,a;let e=document.getElementById("inspect-report-body");if(!e)return;let t=((a=(s=document.getElementById("inspect-addr-badge"))==null?void 0:s.dataset)==null?void 0:a.fullAddr)||"wallet",n=window.open("","_blank","width=900,height=700");n.document.write(`<!DOCTYPE html><html><head>
    ...
    ${e.innerHTML}
  </body></html>`),n.document.close()};window.exportInspectorReport=function(){var n;let e=document.getElementById("inspect-report-body");if(!e)return;let t=e.innerText||e.textContent||"";(n=navigator.clipboard)==null||n.writeText(t).then(()=>{let s=document.getElementById("report-export-btn");s&&(s.textContent="\u2713 Copied!",setTimeout(()=>{s.textContent="\u{1F4CB} Copy Report"},2e3))}).catch(()=>{let s=document.createRange();s.selectNodeContents(e);let a=window.getSelection();a.removeAllRanges(),a.addRange(s)})};window.inspectorClearHistory=function(){dt(Uo);let e=document.getElementById("isd-recent-section");e&&(e.style.display="none")};window.inspectorClearWatchlist=function(){dt(Ms),Es()};function Ca(){return be(Z(Uo))||[]}function Sg(e,t,n=[]){let s=Ca();s=s.filter(o=>o.addr!==e);let a=n.filter(o=>o.sev==="critical"||o.sev==="warn").map(o=>o.module+":"+o.headline.slice(0,40)).sort().join("|");s.unshift({addr:e,riskScore:t,ts:Date.now(),fingerprint:a}),s=s.slice(0,12),te(Uo,JSON.stringify(s))}function As(){return be(Z(Ms))||[]}function Tg(e,t){let n=As().filter(s=>s.addr!==e);n.unshift({addr:e,label:t||z(e),addedTs:Date.now(),lastScore:null,lastTs:null}),te(Ms,JSON.stringify(n.slice(0,50)))}function nc(e){te(Ms,JSON.stringify(As().filter(t=>t.addr!==e)))}function zo(e){return As().some(t=>t.addr===e)}function Cg(e,t){let n=As().map(s=>s.addr===e?{...s,lastScore:t,lastTs:Date.now()}:s);te(Ms,JSON.stringify(n))}function Pa(e){let t=Date.now()-e;return t<6e4?"just now":t<36e5?Math.floor(t/6e4)+"m ago":t<864e5?Math.floor(t/36e5)+"h ago":Math.floor(t/864e5)+"d ago"}function sc(e){return Xo(e).replace("risk-","")}var nn=!1;function ac(){nn=localStorage.getItem(tc)==="true",qo()}function qo(){let e=document.getElementById("inspect-result");if(!e)return;e.classList.toggle("mode-advanced",nn),e.classList.toggle("mode-simple",!nn);let t=document.getElementById("analyst-mode-btn");t&&(t.textContent=nn?"\u2697 Advanced":"\u{1F441} Simple",t.title=nn?"Switch to Simple view":"Switch to Advanced (analyst) view")}window.toggleAnalystMode=function(){nn=!nn,localStorage.setItem(tc,nn),qo()};var Pg=new Set(["section-overview","section-report"]),Lg=new Set(["section-security","section-drain","section-fundflow","section-inbound"]);function Mg(){let e=document.getElementById("inspect-result");if(!e)return;let t=[...e.querySelectorAll(".inspector-section")],n={crit:0,warn:1,neutral:2,ok:3,"":4},s=t.filter(i=>!Pg.has(i.id)&&!Lg.has(i.id));s.sort((i,r)=>{let l=c=>{let d=c.querySelector(".section-badge");if(!d)return"";let u=d.className;return u.includes("crit")?"crit":u.includes("warn")?"warn":u.includes("neutral")?"neutral":u.includes("ok")?"ok":""};return(n[l(i)]??4)-(n[l(r)]??4)});let a=document.getElementById("section-inbound")||document.getElementById("section-drain");if(!a)return;let o=a;for(let i of s)o.after(i),o=i}function Ag(e,t,n,s){let a=document.getElementById("quick-verdict-body");if(!a)return;let o=t.filter(u=>u.sev==="critical"),i=t.filter(u=>u.sev==="warn"),r=e<20?"#50fa7b":e<45?"#ffb86c":e<70?"#ff8c42":"#ff5555",l=e<20?"Low Risk":e<45?"Moderate":e<70?"High Risk":"Critical",c="",d="";if(o.length===0&&i.length===0)c=`No elevated signals found across ${s.toLocaleString()} transactions${n!=null?` and ${n} days of history`:""}.`,d="This wallet appears to operate within normal parameters.";else{let u=o.slice(0,2).map(m=>m.headline).join("; "),p=i.slice(0,2).map(m=>m.headline).join("; ");c=o.length?`${o.length} critical issue${o.length>1?"s":""}: ${u}.`:`${i.length} warning${i.length>1?"s":""}: ${p}.`,d=o.length?"Review the highlighted sections below. Scroll to the Report for full recommendations.":"Review the flagged sections below for context before drawing conclusions."}a.innerHTML=`
    ...
    </div>`}function Es(){let e=document.getElementById("isd-watchlist-section"),t=document.getElementById("isd-watchlist-list");if(!e||!t)return;let n=As();if(!n.length){e.style.display="none";return}e.style.display="",t.innerHTML=n.map(s=>{let a=s.addr.slice(0,8)+"\u2026"+s.addr.slice(-6),o=s.lastScore!=null?`<span class="isd-risk-pill isd-risk-pill--${sc(s.lastScore)}">${s.lastScore}</span>`:"",i=s.lastTs?Pa(s.lastTs):"never checked";return`
    ...
    </div>`}function ic(e,t){let n=new Map;for(let{tx:s,meta:a}of e){let o=s.Account===t,i=s.Destination===t;if(!o&&!i)continue;let r=o?s.Destination:s.Account;if(!r||r===t)continue;let l=At(s);n.has(r)||n.set(r,{cnt:0,xrpOut:0,xrpIn:0,entity:ns(r),firstSeen:l,lastSeen:l});let c=n.get(r);c.cnt++,l&&(c.firstSeen=c.firstSeen?Math.min(c.firstSeen,l):l,c.lastSeen=Math.max(c.lastSeen,l));let d=(a==null?void 0:a.delivered_amount)||s.Amount,u=typeof d=="string"?Number(d)/1e6:0;o?c.xrpOut+=u:c.xrpIn+=u}return n}function Ng(e,t,n=15){let s=ic(e,t),a=[...s.entries()].sort((r,l)=>l[1].xrpOut+l[1].xrpIn-(r[1].xrpOut+r[1].xrpIn)||l[1].cnt-r[1].cnt).slice(0,n);if(!a.length)return'<div class="inspect-empty-note">No counterparty interactions found.</div>';let o=Math.max(...a.map(([,r])=>r.xrpOut+r.xrpIn),1),i=a.map(([r,l],c)=>{var w;let d=l.xrpOut+l.xrpIn,u=Math.max(1.5,d/o*100),p=tn[(w=l.entity)==null?void 0:w.type]||tn.other,m=d>0?l.xrpOut/d:.5,f=m>.65?"\u2192 out":m<.35?"\u2190 in":"\u21C4 both",v=l.entity?`<span style="font-size:.64rem;color:${p};border:1px solid ${p};border-radius:999px;padding:1px 7px;margin-left:6px">${g(l.entity.name)}</span>`:"",h=Zl(l.firstSeen,l.lastSeen);return`
    ...
    ${i}`}function _g(e,t,n,s,a="inspect-network-map"){let o=document.getElementById(a);if(!o)return;let r=[...ic(e,t).entries()].sort(($,P)=>P[1].xrpOut+P[1].xrpIn-($[1].xrpOut+$[1].xrpIn)||P[1].cnt-$[1].cnt).slice(0,20);if(r.length<2){o.style.display="",o.innerHTML=`<div class="inspect-empty-note">${r.length===0?"No counterparty interactions found.":"Only one counterparty found \u2014 not enough to chart a network map."}</div>`;return}o.style.display="";let l=560,c=340,d=l/2,u=c/2,p=95,m=7,f=155,v=13,h=r[0][1].xrpOut+r[0][1].xrpIn||1,w=r[0][1].cnt||1,k=[{id:t,x:d,y:u,r:13,main:!0,label:z(t),color:"#00d4ff",xrpOut:0,xrpIn:0,cnt:0}];r.forEach(([$,P],M)=>{let N=M<m?p:f,_=M<m?m:v,I=(M<m?M:M-m)/_*2*Math.PI-Math.PI/2,D=P.xrpOut+P.xrpIn,F=Math.max(5,Math.min(14,4+D/h*10)),W=P.entity,E=tn[W==null?void 0:W.type]||tn.other,U=D>0?P.xrpOut/D:.5,Q=U>.65?"out":U<.35?"in":"both";k.push({id:$,x:d+N*Math.cos(I),y:u+N*Math.sin(I),r:F,color:E,label:(W==null?void 0:W.name)||z($),ent:W,xrpOut:P.xrpOut,xrpIn:P.xrpIn,cnt:P.cnt,dir:Q,vol:D,ring:N})});let b=`<defs>
...
Write a clear, plain-English explanation of what this account's activity suggests, for someone who isn't a blockchain expert. Synthesize the findings above into a coherent read of the account rather than restating them one by one \u2014 call out which ones reinforce each other and which are weak signals on their own. Be direct about how concerning (or not) this looks, and note any real uncertainty rather than overstating confidence. Do not repeat the address or risk score back verbatim; the reader can already see those above your response.`}window.generateAiExplanation=async function(){let e=document.getElementById("ai-explanation-body");if(!e)return;if(!Xl()){e.innerHTML=`<p style="font-size:.82rem;color:#ffb86c;line-height:1.6">
    ...
    </p>`;return}let t=((s=document.getElementById("ai-local-model"))==null?void 0:s.value)||Ts,n=lc(e);if(n){e.innerHTML=`<div style="font-size:.82rem;color:rgba(255,255,255,.5)" id="ai-local-progress">
    Loading local model \u2014 first run downloads it (a browser-cached one-time cost; later runs are instant)\u2026
      ...
      <button class="settings-btn" style="margin-top:8px" onclick="generateLocalAiExplanation()">Try again</button>`}}};window.exportInspectorJSON=function(){var o,i;let e=window._lastInspectResult;if(!e){alert("Run an inspection first.");return}let t=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),n=URL.createObjectURL(t),s=document.createElement("a"),a=((i=(o=document.getElementById("inspect-addr-badge"))==null?void 0:o.dataset)==null?void 0:i.fullAddr)||"wallet";s.href=n,s.download=`naluxrp_${a.slice(0,10)}_${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(n)};var Rg=6e4,Dg=1e4,Ig=12e4,Fg=8e3,Og=12e4,Bg=300,wc="nalulf_net_baseline_v2",Xg=80,Hg=5,Rs="/api/v1/validatorregistry",xc="/api/v1/validator";var zg=5*60*1e3,kc=10*60*1e3,cc=1e4,$c=8e3,Jo=new Map,Yo=new Map;var zt={quorumTight:{w:3,label:"Quorum within 3 validators of failure threshold"},nUnlActive:{w:2,label:"Negative UNL active \u2014 validators currently being ignored"},amendVeto:{w:1,label:"Amendment veto clustering \u2014 protocol governance dispute"},feeSpike:{w:2,label:"Open ledger fee 10\xD7 minimum \u2014 DDoS / spam attack likely"},burnAnomaly:{w:2,label:"XRP burn rate z-score > 3\u03C3 \u2014 resource exhaustion pattern"},peerSaturate:{w:2,label:"Inbound peers > 80% of connections \u2014 Eclipse Attack risk"},eclipseRisk:{w:3,label:"Peer count < 6 \u2014 node highly vulnerable to isolation"},dexSpike:{w:1,label:"DEX volume > 3\xD7 AMM baseline \u2014 unusual event-driven flow"},reserveSpike:{w:1,label:"New account rate > 3\xD7 baseline \u2014 possible bot creation"},slowConverge:{w:2,label:"Consensus convergence > 6s \u2014 network agreement degraded"},lowProposers:{w:2,label:"Proposer count below quorum \u2014 validator participation low"},queuePressure:{w:2,label:"TX queue > 80% full \u2014 fee surge imminent"},spamLedger:{w:1,label:"Ledger > 2\xD7 expected size \u2014 ledger spam in progress"},ioStressed:{w:1,label:"Node IO latency > 5ms \u2014 storage or network I/O stress"},peerChurn:{w:1,label:"Elevated peer disconnect rate \u2014 DDoS or instability"},staleLedger:{w:3,label:"Ledger age > 10s \u2014 validation appears stalled"}},Sc={MultiSign:{purpose:"Multi-signature authorization",intro:"rippled 0.31",desc:"Lets multiple keys jointly authorize one transaction. Essential for institutional custody and hardware-wallet setups.",impact:"New transaction: SignerListSet. All signers submit their signature; the last one broadcasts."},MultiSignReserve:{purpose:"Cheaper signer-list reserve",intro:"rippled 1.2",desc:"Cuts the owner reserve for SignerList objects from 5 owner-items (10 XRP) down to 1 owner-item (2 XRP).",impact:"Existing SignerLists do not gain the reduction automatically; delete and re-create the list."},DepositAuth:{purpose:"Block unsolicited incoming payments",intro:"rippled 0.90",desc:"An account can set lsfDepositAuth so it only receives payments explicitly pre-authorized via DepositPreauth.",impact:"Senders to un-authorized accounts receive tecNO_PERMISSION."},DeletableAccounts:{purpose:"Permanent account deletion + reserve reclaim",intro:"rippled 1.4",desc:"An account with no objects and sequence \u2265 (current ledger \u2212 256) can permanently delete itself and recover the base reserve.",impact:"New transaction: AccountDelete. Sends all XRP minus fees to a destination."},NegativeUNL:{purpose:"Fault-tolerant consensus during outages",intro:"rippled 1.6",desc:"When validators are persistently offline, the network may add them to the Negative UNL so they do not count against quorum.",impact:"Enables the network to survive planned outages of up to 20% of trusted validators."},Checks:{purpose:"Deferred, cancellable payments",intro:"rippled 1.0",desc:"The sender creates a Check; the recipient can later cash it up to the authorized amount or let it expire. The sender can cancel at any time.",impact:"New transactions: CheckCreate, CheckCash, CheckCancel. Each open Check uses one owner-reserve slot."},AMM:{purpose:"Native Automated Market Maker DEX",intro:"rippled 1.12",desc:"Adds a Constant-Product (x*y=k) AMM directly in the ledger. Anyone can deposit two assets to earn LP tokens and a share of swap fees.",impact:"New transactions: AMMCreate, AMMDeposit, AMMWithdraw, AMMVote, AMMBid, AMMDelete."},XChainBridge:{purpose:"Cross-chain bridge protocol",intro:"rippled 2.0",desc:"Lets assets move between XRPL Mainnet, sidechains, and EVM chains via a locking/minting bridge secured by Witness servers.",impact:"New ledger objects: Bridge, XChainOwnedCreateAccountClaimID, etc. Requires Witness infrastructure."},Clawback:{purpose:"Token-issuer recovery",intro:"rippled 1.12",desc:"Issuers who set lsfAllowTrustLineClawback before issuing tokens can claw back tokens from any holder.",impact:"Must be enabled on a fresh account before any trust lines are created."},NonFungibleTokensV1:{purpose:"Native NFT support",intro:"rippled 1.9",desc:"Adds NFTokenMint, NFTokenBurn, and offer-based transfer mechanics for non-fungible tokens stored in NFTokenPage objects.",impact:"Each NFTokenPage holds up to 32 tokens and costs one owner-reserve slot. Royalties up to 50%."},NonFungibleTokensV1_1:{purpose:"NFT V1 corrections",intro:"rippled 1.10",desc:"Fixes pagination bugs, transfer-fee edge cases, and minting with the URI field that were present in V1.",impact:"Breaking fix for some V1 edge cases. Wallets built for V1 should test V1_1 compatibility."},PayChan:{purpose:"Payment channels for streaming micropayments",intro:"rippled 0.33",desc:"Sender deposits XRP into a channel, then issues signed claims off-ledger. Recipient submits the highest claim at any time to settle on-ledger.",impact:"New transactions: PaymentChannelCreate, PaymentChannelFund, PaymentChannelClaim."},Escrow:{purpose:"Time-locked and condition-based XRP transfers",intro:"rippled 0.60",desc:"Lock XRP until a future time OR a cryptographic fulfillment (PREIMAGE-SHA-256) is revealed, enabling vesting schedules and atomic swaps.",impact:"New transactions: EscrowCreate, EscrowFinish, EscrowCancel. Locked XRP counts against reserves."},DisallowIncoming:{purpose:"Block unsolicited ledger-object creation",intro:"rippled 1.10",desc:"New account flags let you individually block incoming Trust Lines, Check objects, NFToken offers, and Payment Channels.",impact:"Four new AccountSet flags; existing incoming objects are unaffected."},ExpandedSignerList:{purpose:"Larger multi-sig signer lists",intro:"rippled 1.9.1",desc:"Increases the maximum signers per SignerList from 8 to 32, enabling more complex institutional multi-sig and DAO governance.",impact:"Reserve cost scales with signer count. Requires MultiSignReserve to be cost-effective."},OwnerPaysFee:{purpose:"Correct fee payer in PayChan",intro:"rippled 0.33",desc:"Fixes a spec inconsistency where the channel owner correctly pays the transaction fee when closing or expiring channels.",impact:"Purely a fee-accounting fix; no user-visible behavior changes."},fixMasterKeyAsRegularKey:{purpose:"Master-key mis-use bug fix",intro:"rippled 0.90",desc:"Prevents accounts from setting their master key as their regular key \u2014 an operation that could create an unusable account state.",impact:"No application changes needed; existing accounts are not affected."},TrustSetAuth:{purpose:"Authorized trust lines",intro:"rippled 0.30",desc:"Issuers can require explicit authorization before anyone can hold their token \u2014 a prerequisite for regulatory-grade stablecoins.",impact:"New flow: issuer sends TrustSet with tfSetfAuth to approve each holder."}},dc=new Set(["xrpscan.com","xrpl.aesthetes.art","xrpkuwait.com","xrpgoat.com","xrp.vet","xrp.unic.ac.cy","xrp-validator.interledger.org","xpmarket.com","verum.eminence.im","validator.xrpl.robertswarthout.com","validator.xrpl-labs.com","validator.poli.usp.br","validator.gatehub.net","validator.aspired.nz","v2.xrpl-commons.org","tequ.dev","squidrouter.com","ripple.ittc.ku.edu","ripple.com","peersyst.cloud","onxrp.com","katczynski.net","jon-nilsen.no","ekiserrepe.es","cabbit.tech","bithomp.com","aureusox.com","arrington-xrp-capital.blockdaemon.com","anodos.finance","bitso.com","ripple.kenan-flagler.unc.edu","ripplevalidator.uwaterloo.ca","shadow.haas.berkeley.edu","www.bitrue.com","xrp-col.anu.edu.au"]),Ug=new Set(["nHBu3iuq1SQ9Z686pYwWYVKpScSMDWfpUJHdNEQRxn5XyETui7Db","nHDwBbubxJswoweWQKEgWLNRJv2hNRCTR4GGApJmbtCcbtYNSpdB","nHU2FpRbPrvVcyQQpkqrAUDJDTRHZpjij8DpKeSC481PYY9ikYkb","nHUbmg8QNzEGjHzgnt99e9YE2scU3DZGH7FsF6MCcK5eiPt3AtaH","nHUxBD1UPb383SdWgJx62GGQ7W2WKvgpUtUXLjiGGRRcPb3nbSXd","nHUP6rfQfgzg6tKga3k9ziEvtjwn1PB32gcr5dLcamqzmitszYv2","nHBT58yHyDdPdJ6gzaBMT7gqwTMpj5ERji1s9SvfKKtfoZUS89WX","nHBxsUzx3Bbf6J4yJ3fLQ3VizPtdREVwTJ6mdqkuDTjLcVbggVbk","nHUeUNSn3zce2xQZWNghQvd9WRH6FWEnCBKYVJu2vAizMxnXegfJ","nHUCAdca6VoWWYVdBH1bwCUQggEX2e5acQSqxM3DwyuhsFknxmh3","nHDDiwQBqXhEL1CFoRHdMXD33x9K7rpYJfniXxL7kFavpPd21EGe","nHBipbbREjNEiCs4hpy3K2489dRf27MPnxdivTTWKSd8ZUhfRvn8","nHDDe5uAdiv6RA59MA1oM4JLDtVSYKNShgjEqq1KsdJXZiR47CQT","nHBbiP5ua5dUqCTz5i5vd3ia9jg3KJthohDjgKxnc7LxtmnauW7Z","nHBQ3CT3EWYZ4uzbnL3k6TRf9bBPhWRFVcK1F5NjtwCBksMEt5yy","nHU16DF2kq7TmbR1Y5z8yKNXiLEf3oHT19HVpVXv7unFLfxa17nT","nHUC23NnutZyYeQxQbAbPUpKoVGj5aisBxf2zzcZzJ43fcw5rc9z","nHUgdMvuchx7AWG4ATMQdNNuMryo1SFoNptLCEVt2Dn7wEc625mF","nHUhQVE93dajM3srxubsEj1mK1gzRwJob14QSrJefY1FdLs6r7WJ","nHUU8xBczYzW6kZ6Ei9DsggzTJXRFkN3wE3FP5H4SLYzhbodeYcG","nHUGqooyfGqFkyH6uskbaEi6y2MjXjdA7QdbmyZ6p9etL5isRKLT","nHDp1ZXxEn7eo5YaUtiagaxSLwXudnKZDx68C96p4tdVLGLLLUFn","nHBcLEB4S6moQGrhMjJo1jbp58WL5psHY9EMDWNAtdqykUYiA1rF","nHUAECq1v1cKwn3NsYVyD7v6BNbfqyXmNVSF3e4XCVxPgBHRWkvv","nHUif4sukXu9pJGyyBaeVMwmE8L1fJ5KJj4X4ksgTKhgjG6k96s2","nHUVJR7SeT3nn6JPTz46JHqYRf7vX2if1sdTxnceywmSBWa167pt","nHUEYz4TtTv7yebjhY3aDib5KYPHnKjnY5mPYK5y4QuKdocwS5tD","nHUN5n2S3nQ8bzKm7bqeFMiQeDijh1LMgEocyNyQbb4mREazVdZ5","nHUgchANqM3giYSSvY5HsafFW6qxmG5jJ3CvPiv7n8gjNQuNm8Uz","nHDUqGoM7KR1pgbdYBRgKpGKdFLhpnMzVbECs8RE73RGZm3Va6MJ","nHUVxTi8XfXjaaJppw7mLSrYDRpkDpf8H9ypzgVKxfSXShcWwAoK","nHBveTxA1NaBj5AayRAU91f6YopuFWt9rmxfGaEh77a32Q6ZzzHc"]),jg=new Set(["validator.pftperry.com","rip973.com","preaware.org","postfiat.org","pftmeech.xyz","pft.xbtseal.com","pft.wizbubba.xyz","pft.permanentupperclass.com","pft.g.money","pft.akirax.xyz","jollydinger.com","auri0x.io","app.w.ai"]);function Tc(e,t){let n=(t||"").toLowerCase().replace(/^www\./,"");return Ug.has(e)||jg.has(n)?{chain:"test",isUnl:!1}:dc.has(n)||dc.has("www."+n)?{chain:"main",isUnl:!0}:{chain:"main",isUnl:!1}}var oi={"xrpscan.com":{label:"XRP Scan",domain:"xrpscan.com",chain:"main",category:"unl"},"xrpl.aesthetes.art":{label:"Aesthetes",domain:"xrpl.aesthetes.art",chain:"main",category:"unl"},"xrpkuwait.com":{label:"XRP Kuwait",domain:"xrpkuwait.com",chain:"main",category:"unl"},"xrpgoat.com":{label:"XRP Goat",domain:"xrpgoat.com",chain:"main",category:"unl"},"xrp.vet":{label:"XRP Vet",domain:"xrp.vet",chain:"main",category:"unl"},"xrp.unic.ac.cy":{label:"Univ. of Nicosia",domain:"xrp.unic.ac.cy",chain:"main",category:"unl"},"xrp-validator.interledger.org":{label:"Interledger",domain:"xrp-validator.interledger.org",chain:"main",category:"unl"},"xpmarket.com":{label:"XPMarket",domain:"xpmarket.com",chain:"main",category:"unl"},"verum.eminence.im":{label:"Eminence",domain:"verum.eminence.im",chain:"main",category:"unl"},"validator.xrpl.robertswarthout.com":{label:"R. Swarthout",domain:"validator.xrpl.robertswarthout.com",chain:"main",category:"unl"},"validator.xrpl-labs.com":{label:"XRPL Labs",domain:"validator.xrpl-labs.com",chain:"main",category:"unl"},"validator.poli.usp.br":{label:"USP",domain:"validator.poli.usp.br",chain:"main",category:"unl"},"validator.gatehub.net":{label:"Gatehub",domain:"validator.gatehub.net",chain:"main",category:"unl"},"validator.aspired.nz":{label:"Aspired NZ",domain:"validator.aspired.nz",chain:"main",category:"unl"},"v2.xrpl-commons.org":{label:"XRPL Commons",domain:"v2.xrpl-commons.org",chain:"main",category:"unl"},"tequ.dev":{label:"Tequ",domain:"tequ.dev",chain:"main",category:"unl"},"squidrouter.com":{label:"Squid Router",domain:"squidrouter.com",chain:"main",category:"unl"},"ripple.ittc.ku.edu":{label:"Univ. of Kansas",domain:"ripple.ittc.ku.edu",chain:"main",category:"unl"},"ripple.com":{label:"Ripple",domain:"ripple.com",chain:"main",category:"unl"},"peersyst.cloud":{label:"Peersyst",domain:"peersyst.cloud",chain:"main",category:"unl"},"onxrp.com":{label:"OnXRP",domain:"onxrp.com",chain:"main",category:"unl"},"katczynski.net":{label:"Katczynski",domain:"katczynski.net",chain:"main",category:"unl"},"jon-nilsen.no":{label:"Jon Nilsen",domain:"jon-nilsen.no",chain:"main",category:"unl"},"ekiserrepe.es":{label:"Ekiserrepe",domain:"ekiserrepe.es",chain:"main",category:"unl"},"cabbit.tech":{label:"Cabbit",domain:"cabbit.tech",chain:"main",category:"unl"},"bithomp.com":{label:"Bithomp",domain:"bithomp.com",chain:"main",category:"unl"},"aureusox.com":{label:"Aureus Ox",domain:"aureusox.com",chain:"main",category:"unl"},"arrington-xrp-capital.blockdaemon.com":{label:"Arrington / Blockdaemon",domain:"arrington-xrp-capital.blockdaemon.com",chain:"main",category:"unl"},"anodos.finance":{label:"Anodos Finance",domain:"anodos.finance",chain:"main",category:"unl"},"bitso.com":{label:"Bitso",domain:"bitso.com",chain:"main",category:"unl"},"ripple.kenan-flagler.unc.edu":{label:"UNC Kenan-Flagler",domain:"ripple.kenan-flagler.unc.edu",chain:"main",category:"unl"},"ripplevalidator.uwaterloo.ca":{label:"Univ. of Waterloo",domain:"ripplevalidator.uwaterloo.ca",chain:"main",category:"unl"},"shadow.haas.berkeley.edu":{label:"UC Berkeley Haas",domain:"shadow.haas.berkeley.edu",chain:"main",category:"unl"},"www.bitrue.com":{label:"Bitrue",domain:"www.bitrue.com",chain:"main",category:"unl"},"xrp-col.anu.edu.au":{label:"ANU",domain:"xrp-col.anu.edu.au",chain:"main",category:"unl"},"xrpval.rawsec.de":{label:"Rawsec",domain:"xrpval.rawsec.de",chain:"main",category:"other"},"xrplvl.carbonvibe.com":{label:"Carbon Vibe",domain:"xrplvl.carbonvibe.com",chain:"main",category:"other"},"xrplvalidator.alloy.ee":{label:"Alloy",domain:"xrplvalidator.alloy.ee",chain:"main",category:"other"},"xrpl.uni.lu":{label:"Univ. of Luxembourg",domain:"xrpl.uni.lu",chain:"main",category:"other"},"xrpl.to":{label:"XRPL.to",domain:"xrpl.to",chain:"main",category:"other"},"xrpl.su":{label:"XRPL.su",domain:"xrpl.su",chain:"main",category:"other"},"xrpl.sbivc.co.jp":{label:"SBI VC Trade",domain:"xrpl.sbivc.co.jp",chain:"main",category:"other"},"xrpl-verification.flare.network":{label:"Flare Network",domain:"xrpl-verification.flare.network",chain:"main",category:"other"},"xrpl-validator.7rev.dev":{label:"7Rev",domain:"xrpl-validator.7rev.dev",chain:"main",category:"other"},"xrp.teacopula.com":{label:"Teacopula",domain:"xrp.teacopula.com",chain:"main",category:"other"},"xrp.moneymindedapes.com":{label:"MoneyMindedApes",domain:"xrp.moneymindedapes.com",chain:"main",category:"other"},"xrp.hazza-systems.de":{label:"Hazza Systems",domain:"xrp.hazza-systems.de",chain:"main",category:"other"},"xrp.cs.uoregon.edu":{label:"Univ. of Oregon",domain:"xrp.cs.uoregon.edu",chain:"main",category:"other"},"xrp.bpsqn.com":{label:"BPSQN",domain:"xrp.bpsqn.com",chain:"main",category:"other"},"xrp-validator.grapedrop.xyz":{label:"Grapedrop",domain:"xrp-validator.grapedrop.xyz",chain:"main",category:"other"},"xaodao.io":{label:"XaoDAO",domain:"xaodao.io",chain:"main",category:"other"},"vl.xrpsalute.com":{label:"XRP Salute",domain:"vl.xrpsalute.com",chain:"main",category:"other"},"validator.xrpl.app":{label:"XRPL App",domain:"validator.xrpl.app",chain:"main",category:"other"},"validator.ukcbt.org":{label:"UKCBT",domain:"validator.ukcbt.org",chain:"main",category:"other"},"validator.sugarxrpl.com":{label:"SugarXRPL",domain:"validator.sugarxrpl.com",chain:"main",category:"other"},"validator.boscaern.digital":{label:"Boscaern",domain:"validator.boscaern.digital",chain:"main",category:"other"},"trimaera.tech":{label:"Trimaera",domain:"trimaera.tech",chain:"main",category:"other"},"textrp.io":{label:"TextRP",domain:"textrp.io",chain:"main",category:"other"},"tesbert.com":{label:"Tesbert",domain:"tesbert.com",chain:"main",category:"other"},"tachyon-xrpl-validator.github.io":{label:"Tachyon",domain:"tachyon-xrpl-validator.github.io",chain:"main",category:"other"},"solonation.io":{label:"SoloNation",domain:"solonation.io",chain:"main",category:"other"},"smokydrip.com":{label:"SmokyDrip",domain:"smokydrip.com",chain:"main",category:"other"},"rippled-validator.us":{label:"rippled-validator.us",domain:"rippled-validator.us",chain:"main",category:"other"},"rippleat.snt.uni.lu":{label:"Univ. Luxembourg (SNT)",domain:"rippleat.snt.uni.lu",chain:"main",category:"other"},"ripple.uni.lu":{label:"Univ. Luxembourg",domain:"ripple.uni.lu",chain:"main",category:"other"},"ripple.j2b.com":{label:"J2B",domain:"ripple.j2b.com",chain:"main",category:"other"},"rich-list.info":{label:"Rich List",domain:"rich-list.info",chain:"main",category:"other"},"proptoexchange.com":{label:"ProPtoExchange",domain:"proptoexchange.com",chain:"main",category:"other"},"printscierge.com":{label:"Printscierge",domain:"printscierge.com",chain:"main",category:"other"},"opulencex.io":{label:"OpulenceX",domain:"opulencex.io",chain:"main",category:"other"},"onledger.net":{label:"OnLedger",domain:"onledger.net",chain:"main",category:"other"},"oclost.art":{label:"Oclost",domain:"oclost.art",chain:"main",category:"other"},"managednetwork.us":{label:"ManagedNetwork",domain:"managednetwork.us",chain:"main",category:"other"},"joshuahamsa.com":{label:"Joshua Hamsa",domain:"joshuahamsa.com",chain:"main",category:"other"},"grimmsxrpflow.jwscott.net":{label:"Grimm XRP Flow",domain:"grimmsxrpflow.jwscott.net",chain:"main",category:"other"},"getlol.xyz":{label:"GetLol",domain:"getlol.xyz",chain:"main",category:"other"},"gen3labs.xyz":{label:"Gen3 Labs",domain:"gen3labs.xyz",chain:"main",category:"other"},"garveyvalid.com":{label:"Garvey",domain:"garveyvalid.com",chain:"main",category:"other"},"easynpl.kr":{label:"EasyNPL",domain:"easynpl.kr",chain:"main",category:"other"},"diseb.ewi.tudelft.nl":{label:"TU Delft",domain:"diseb.ewi.tudelft.nl",chain:"main",category:"other"},"datamossa.com":{label:"DataMossa",domain:"datamossa.com",chain:"main",category:"other"},"crypto.unibe.ch":{label:"Univ. of Bern",domain:"crypto.unibe.ch",chain:"main",category:"other"},"commonprefix.com":{label:"Common Prefix",domain:"commonprefix.com",chain:"main",category:"other"},"catalyze-research.com":{label:"Catalyze Research",domain:"catalyze-research.com",chain:"main",category:"other"},"catalog.org":{label:"Catalog",domain:"catalog.org",chain:"main",category:"other"},"blockchain.korea.ac.kr":{label:"Korea Univ.",domain:"blockchain.korea.ac.kr",chain:"main",category:"other"},"astatiumprotocol.com":{label:"Astatium Protocol",domain:"astatiumprotocol.com",chain:"main",category:"other"},"ladykxrpl.mywire.org":{label:"LadyK XRPL",domain:"LadyKXRPL.mywire.org",chain:"main",category:"other"},"eelap-p1201-xrp.abudhabi.nyu.edu":{label:"NYU Abu Dhabi",domain:"EELAP-P1201-XRP.ABUDHABI.NYU.EDU",chain:"main",category:"other"},"589.clouds.hspeed.ch":{label:"HSpeed",domain:"589.clouds.hspeed.ch",chain:"main",category:"other"},"validator.pftperry.com":{label:"PFT Perry",domain:"validator.pftperry.com",chain:"test",category:"other"},"rip973.com":{label:"rip973",domain:"rip973.com",chain:"test",category:"other"},"preaware.org":{label:"Preaware",domain:"preaware.org",chain:"test",category:"other"},"postfiat.org":{label:"PostFiat",domain:"postfiat.org",chain:"test",category:"other"},"pftmeech.xyz":{label:"PFT Meech",domain:"pftmeech.xyz",chain:"test",category:"other"},"pft.xbtseal.com":{label:"PFT XBT Seal",domain:"pft.xbtseal.com",chain:"test",category:"other"},"pft.wizbubba.xyz":{label:"PFT Wizbubba",domain:"pft.wizbubba.xyz",chain:"test",category:"other"},"pft.permanentupperclass.com":{label:"PFT Perm Upper Class",domain:"pft.permanentupperclass.com",chain:"test",category:"other"},"pft.g.money":{label:"PFT G.Money",domain:"pft.g.money",chain:"test",category:"other"},"pft.akirax.xyz":{label:"PFT Akirax",domain:"pft.akirax.xyz",chain:"test",category:"other"},"jollydinger.com":{label:"Jollydinger",domain:"jollydinger.com",chain:"test",category:"other"},"auri0x.io":{label:"Auri0x",domain:"auri0x.io",chain:"test",category:"other"},"app.w.ai":{label:"W.ai",domain:"app.w.ai",chain:"test",category:"other"}},_a={nHB8QMKGt9VB4Vg71VszjBVQnDW3v3QudM4436zXRZgiuUBBSWJe:{lat:37.77,lng:-122.42,city:"San Francisco",org:"Ripple"},nHUon2tpyJEHHYGmxqNd3h3oGNQwNyX8PNS3aHe3bNpCrNXZlHo:{lat:37.77,lng:-122.41,city:"San Francisco",org:"Ripple"},nHUpwrafS45zmi6eT72XS5ijpkW5JwfL5mLdPhEibrqUvtRcMAjU:{lat:37.78,lng:-122.4,city:"San Francisco",org:"Ripple"},nHUkp7WhouVMobBUKGrV5FNqjsdD9zKP5jpGnnLfQXCMNe4dkDqo:{lat:37.76,lng:-122.43,city:"San Francisco",org:"Ripple"},nHUryiyDqEtyWVtFG24AAhaYjMf9FRLietZGBWYwUTojmugMsx3o:{lat:37.79,lng:-122.38,city:"San Francisco",org:"Ripple"},nHUpcmNsxAw47yt2ADDoNoQrzLyTJPgnyq16u6Qx2kRPA17oUNHz:{lat:37.8,lng:-122.39,city:"San Francisco",org:"Ripple"},nHUnhRJK3csknycNK5SXRFi8jvDp3sKoWvS9wKWLq1ATBBGgPBjp:{lat:37.75,lng:-122.44,city:"San Francisco",org:"Ripple"},nHUq9tJvk5QTDkwurB7EzbzkZ2uuoHjS3GKjP6pZiU3DJGnobNYK:{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"AWS (Coil)"},nHUvcCcmoH1FJMMC6NtF9KKA4LpCWhjsxk2reCQidsp5AHQ7QY9H:{lat:49.45,lng:11.08,city:"Nuremberg",org:"Hetzner (Gatehub)"},nHDH7bQJpVfDhVSqdui3Z8GPvKEBQpo6AKHcnXe21zoD4nABA6xj:{lat:52.37,lng:4.9,city:"Amsterdam",org:"GCP (XRPL Labs)"},nHUED59jjpQ5QbNtesAbB6Es3uUPv3c9Ri5MNNgfMv5t5Lhb5ndW:{lat:19.43,lng:-99.13,city:"Mexico City",org:"AWS (Bitso)"},nHBidG3pZK11zqjeVos6hFxTDPGYuqfRFZ5gu9b7tQFdB8nPZujG:{lat:35.69,lng:139.69,city:"Tokyo",org:"NTT (Digital Garage)"},nHDB2PAPYqF86j9j3c6w1F1ZqwvQfiWcFShZ9Pokg9q4ohNDSkAz:{lat:47.61,lng:-122.33,city:"Seattle, WA",org:"Azure (Arrington)"},nHUdphn3LXa31w5sLd39MQdPEKQNrNYL3DQFByijVXiNQ3G6BYBZ:{lat:1.35,lng:103.82,city:"Singapore",org:"AWS (Tokenize)"},nHUFCyRCrUjvtZmKiLeF8ReopzKuSkVzdl1VsMCqm75aqyohLYEg:{lat:48.86,lng:2.35,city:"Paris",org:"OVH (XRPL Commons)"},nHULqGBkJtWeNFjhTzYeAsHA3qKKS7HoBh8CV3BAGTGMZuepEhWC:{lat:40.71,lng:-74.01,city:"New York",org:"Equinix (Blockchain LLC)"},nHBdXSF6YHAHSZUk7rvox6jwbvvyqBnsWGcewBtq8x1XuH6KXKXr:{lat:37.79,lng:-122.4,city:"San Francisco",org:"Cloudflare (XRP Scan)"}},pc={"xrpscan.com":{lat:51.51,lng:-.13,city:"London",org:"XRPScan"},"xrpl.aesthetes.art":{lat:52.37,lng:4.9,city:"Amsterdam",org:"Aesthetes"},"xrpkuwait.com":{lat:29.37,lng:47.98,city:"Kuwait City",org:"XRP Kuwait"},"xrpgoat.com":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"XRP Goat"},"xrp.vet":{lat:48.86,lng:2.35,city:"Paris",org:"XRP Vet"},"xrp.unic.ac.cy":{lat:35.17,lng:33.37,city:"Nicosia",org:"Univ. of Nicosia"},"xrp-validator.interledger.org":{lat:40.71,lng:-74.01,city:"New York",org:"Interledger Foundation"},"xpmarket.com":{lat:52.37,lng:4.9,city:"Amsterdam",org:"XPMarket"},"verum.eminence.im":{lat:51.51,lng:-.13,city:"London",org:"Eminence"},"validator.xrpl.robertswarthout.com":{lat:52.37,lng:4.9,city:"Amsterdam",org:"R. Swarthout"},"validator.xrpl-labs.com":{lat:52.37,lng:4.9,city:"Amsterdam",org:"XRPL Labs"},"validator.xrpl.app":{lat:52.37,lng:4.9,city:"Amsterdam",org:"XRPL App"},"validator.poli.usp.br":{lat:-23.55,lng:-46.63,city:"S\xE3o Paulo",org:"USP"},"validator.gatehub.net":{lat:46.05,lng:14.51,city:"Ljubljana",org:"Gatehub"},"validator.aspired.nz":{lat:-36.86,lng:174.76,city:"Auckland",org:"Aspired NZ"},"v2.xrpl-commons.org":{lat:48.86,lng:2.35,city:"Paris",org:"XRPL Commons"},"tequ.dev":{lat:60.17,lng:24.94,city:"Helsinki",org:"Tequ"},"squidrouter.com":{lat:37.77,lng:-122.42,city:"San Francisco",org:"Squid Router"},"ripple.ittc.ku.edu":{lat:38.97,lng:-95.24,city:"Lawrence, KS",org:"Univ. of Kansas"},"ripple.com":{lat:37.77,lng:-122.42,city:"San Francisco",org:"Ripple"},"peersyst.cloud":{lat:41.39,lng:2.15,city:"Barcelona",org:"Peersyst"},"onxrp.com":{lat:51.51,lng:-.13,city:"London",org:"OnXRP"},"katczynski.net":{lat:52.23,lng:21.01,city:"Warsaw",org:"Katczynski"},"jon-nilsen.no":{lat:59.91,lng:10.75,city:"Oslo",org:"Jon Nilsen"},"ekiserrepe.es":{lat:40.42,lng:-3.7,city:"Madrid",org:"Ekiserrepe"},"cabbit.tech":{lat:51.51,lng:-.13,city:"London",org:"Cabbit"},"bithomp.com":{lat:59.33,lng:18.07,city:"Stockholm",org:"Bithomp"},"aureusox.com":{lat:40.71,lng:-74.01,city:"New York",org:"Aureus Ox"},"arrington-xrp-capital.blockdaemon.com":{lat:40.71,lng:-74.01,city:"New York",org:"Blockdaemon"},"anodos.finance":{lat:37.98,lng:23.73,city:"Athens",org:"Anodos Finance"},"bitso.com":{lat:19.43,lng:-99.13,city:"Mexico City",org:"Bitso"},"ripple.kenan-flagler.unc.edu":{lat:35.9,lng:-79.05,city:"Chapel Hill, NC",org:"UNC Kenan-Flagler"},"ripplevalidator.uwaterloo.ca":{lat:43.47,lng:-80.54,city:"Waterloo, ON",org:"Univ. of Waterloo"},"shadow.haas.berkeley.edu":{lat:37.87,lng:-122.26,city:"Berkeley, CA",org:"UC Berkeley Haas"},"www.bitrue.com":{lat:1.35,lng:103.82,city:"Singapore",org:"Bitrue"},"xrp-col.anu.edu.au":{lat:-35.28,lng:149.13,city:"Canberra",org:"ANU"},"xrpval.rawsec.de":{lat:51.17,lng:10.45,city:"Germany",org:"Rawsec"},"xrplvl.carbonvibe.com":{lat:51.51,lng:-.13,city:"London",org:"Carbon Vibe"},"xrplvalidator.alloy.ee":{lat:59.44,lng:24.75,city:"Tallinn",org:"Alloy"},"xrpl.uni.lu":{lat:49.61,lng:6.13,city:"Luxembourg",org:"Univ. of Luxembourg"},"xrpl.to":{lat:48.86,lng:2.35,city:"Paris",org:"XRPL.to"},"xrpl.su":{lat:55.75,lng:37.62,city:"Moscow",org:"XRPL.su"},"xrpl.sbivc.co.jp":{lat:35.69,lng:139.69,city:"Tokyo",org:"SBI VC Trade"},"xrpl-verification.flare.network":{lat:51.51,lng:-.13,city:"London",org:"Flare Network"},"xrpl-validator.7rev.dev":{lat:52.52,lng:13.4,city:"Berlin",org:"7Rev"},"xrp.teacopula.com":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"Teacopula"},"xrp.moneymindedapes.com":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"MoneyMindedApes"},"xrp.hazza-systems.de":{lat:52.52,lng:13.4,city:"Berlin",org:"Hazza Systems"},"xrp.cs.uoregon.edu":{lat:44.05,lng:-123.08,city:"Eugene, OR",org:"Univ. of Oregon"},"xrp.bpsqn.com":{lat:37.77,lng:-122.42,city:"San Francisco",org:"BPSQN"},"xrp-validator.grapedrop.xyz":{lat:52.37,lng:4.9,city:"Amsterdam",org:"Grapedrop"},"xaodao.io":{lat:1.35,lng:103.82,city:"Singapore",org:"XaoDAO"},"vl.xrpsalute.com":{lat:40.71,lng:-74.01,city:"New York",org:"XRP Salute"},"validator.ukcbt.org":{lat:51.51,lng:-.13,city:"London",org:"UKCBT"},"validator.sugarxrpl.com":{lat:37.77,lng:-122.42,city:"San Francisco",org:"SugarXRPL"},"validator.boscaern.digital":{lat:53.33,lng:-6.25,city:"Dublin",org:"Boscaern"},"trimaera.tech":{lat:48.86,lng:2.35,city:"Paris",org:"Trimaera"},"textrp.io":{lat:1.35,lng:103.82,city:"Singapore",org:"TextRP"},"tesbert.com":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"Tesbert"},"tachyon-xrpl-validator.github.io":{lat:37.77,lng:-122.42,city:"San Francisco",org:"Tachyon"},"solonation.io":{lat:1.35,lng:103.82,city:"Singapore",org:"SoloNation"},"smokydrip.com":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"SmokyDrip"},"rippled-validator.us":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"rippled-validator.us"},"rippleat.snt.uni.lu":{lat:49.61,lng:6.13,city:"Luxembourg",org:"Univ. of Luxembourg"},"ripple.uni.lu":{lat:49.61,lng:6.13,city:"Luxembourg",org:"Univ. of Luxembourg"},"ripple.j2b.com":{lat:37.77,lng:-122.42,city:"San Francisco",org:"J2B"},"rich-list.info":{lat:51.51,lng:-.13,city:"London",org:"Rich List"},"proptoexchange.com":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"ProPtoExchange"},"printscierge.com":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"Printscierge"},"opulencex.io":{lat:1.35,lng:103.82,city:"Singapore",org:"OpulenceX"},"onledger.net":{lat:52.37,lng:4.9,city:"Amsterdam",org:"OnLedger"},"oclost.art":{lat:48.86,lng:2.35,city:"Paris",org:"Oclost"},"managednetwork.us":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"ManagedNetwork"},"joshuahamsa.com":{lat:37.77,lng:-122.42,city:"San Francisco",org:"Joshua Hamsa"},"grimmsxrpflow.jwscott.net":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"Grimm XRP Flow"},"getlol.xyz":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"GetLol"},"gen3labs.xyz":{lat:37.77,lng:-122.42,city:"San Francisco",org:"Gen3 Labs"},"garveyvalid.com":{lat:40.71,lng:-74.01,city:"New York",org:"Garvey"},"easynpl.kr":{lat:37.57,lng:126.98,city:"Seoul",org:"EasyNPL"},"diseb.ewi.tudelft.nl":{lat:52,lng:4.36,city:"Delft",org:"TU Delft"},"datamossa.com":{lat:48.86,lng:2.35,city:"Paris",org:"DataMossa"},"crypto.unibe.ch":{lat:46.95,lng:7.45,city:"Bern",org:"Univ. of Bern"},"commonprefix.com":{lat:37.98,lng:23.73,city:"Athens",org:"Common Prefix"},"catalyze-research.com":{lat:51.51,lng:-.13,city:"London",org:"Catalyze Research"},"catalog.org":{lat:37.77,lng:-122.42,city:"San Francisco",org:"Catalog"},"blockchain.korea.ac.kr":{lat:37.57,lng:126.98,city:"Seoul",org:"Korea Univ."},"astatiumprotocol.com":{lat:1.35,lng:103.82,city:"Singapore",org:"Astatium Protocol"},"ladykxrpl.mywire.org":{lat:39.04,lng:-77.49,city:"Ashburn, VA",org:"LadyK XRPL"},"eelap-p1201-xrp.abudhabi.nyu.edu":{lat:24.47,lng:54.37,city:"Abu Dhabi",org:"NYU Abu Dhabi"},"589.clouds.hspeed.ch":{lat:47.38,lng:8.54,city:"Zurich",org:"HSpeed"},"www.payonline.financial":{lat:55.75,lng:37.62,city:"Moscow",org:"PayOnline"}};function ii(e){if(!e)return null;let t=e.toLowerCase().replace(/^www\./,"");return pc[t]??pc["www."+t]??null}var Wg=[{lat:37.34,lng:-121.89,label:"s1.ripple.com",city:"San Jose, CA",org:"Ripple"},{lat:37.34,lng:-121.87,label:"s2.ripple.com",city:"San Jose, CA",org:"Ripple"},{lat:52.37,lng:4.91,label:"xrplcluster.com",city:"Amsterdam",org:"Cluster"},{lat:52.36,lng:4.89,label:"xrpl.ws",city:"Amsterdam",org:"Cluster"}],_e={nunl:{hex:"#ff5555",glow:"rgba(255,85,85,.4)",label:"Negative UNL"},both:{hex:"#50fa7b",glow:"rgba(80,250,123,.4)",label:"UNL + dUNL"},unl:{hex:"#00fff0",glow:"rgba(0,255,240,.35)",label:"UNL"},dunl:{hex:"#bd93f9",glow:"rgba(189,147,249,.4)",label:"dUNL only"},other:{hex:"#ffb86c",glow:"rgba(255,184,108,.4)",label:"Other"},pub:{hex:"#50fa7b",glow:"rgba(80,250,123,.4)",label:"Public Node"}},Qo="https://api.xrpscan.com/api/v1/validatorregistry",$e=new Map,an={},Ea=0,Ln=!1,on="fallback";function Cc(){if(!Ln){$e.clear();for(let[e,t]of Object.entries(oi)){let n=t.domain??e,s=Tc("",n),a=ii(n)??null;$e.set(e,{key:e,label:t.label,domain:n,domainVerified:!1,provider:t.provider??(a==null?void 0:a.org)??null,lists:t.lists??[],category:s.isUnl?"unl":"other",chain:s.chain,unl:s.isUnl,dunl:!1,geo:a,meta:{},agreement:{"1h":null,"24h":null,"30d":null}})}on="fallback",ri({ok:!1,source:"fallback",error:"using hardcoded data \u2014 mount /api/v1 router"})}}var Zo=0,qg=5*60*1e3;function uc(e,t){var s,a;let n=Array.isArray(e)?e:e.validators??e.data??[];if(!n.length)throw new Error("Empty validators array");$e.clear();for(let o of n){let i=o.key??o.validation_public_key??o.master_key??o.signing_key??null;if(!i)continue;let r=o.domain??null,l=r?r.toLowerCase().replace(/^www\./,""):null,c=o.unl===!0||o.unl===1||o.unl==="true",d=Tc(i,l),u=c||d.isUnl,p=d.chain,m=u?"unl":"other",f=Object.values(oi).find(b=>b.domain&&b.domain.toLowerCase().replace(/^www\./,"")===l),v=(f==null?void 0:f.label)??o.label??o.account_name??o.name??l??null??`${i.slice(0,8)}\u2026${i.slice(-6)}`,h=_a[i]??ii(l)??null,k=(((s=o.geo)==null?void 0:s.lat)!=null&&((a=o.geo)==null?void 0:a.lng)!=null?o.geo:null)??h;$e.set(i,{key:i,label:v,domain:r??null,domainVerified:!!(o.domainVerified??o.domain_verified),provider:o.provider??(k==null?void 0:k.org)??(f==null?void 0:f.provider)??null,lists:Array.isArray(o.lists)?o.lists:[],category:m,chain:p,unl:u,dunl:!1,geo:k,version:o.version??o.build_version??null,baseFee:o.base_fee_xrp??o.base_fee??null,ownerReserve:o.reserve_inc_xrp??o.owner_reserve??null,meta:o.meta??{},agreement:o.agreement??{"1h":Vo(o.agreement_1h??o.agr_1h),"24h":Vo(o.agreement_24h??o.agr_24h),"30d":Vo(o.agreement_30d??o.agr_30d)}})}an=e.lists??{},Ea=Date.now(),Ln=!0,on=t,Zo=0,ri({ok:!0,count:$e.size,lists:an,source:t})}function Vo(e){if(!e)return null;let t=Number(e.total??e.ledgers??0),n=Number(e.missed??0),s=t-n,a=t>0?(s/t*100).toFixed(1):null;return{total:t,missed:n,hit:s,score:a?`${a}%`:null,scoreRaw:e.score??null}}async function Aa(e=!1){let t=Date.now();if(!e&&Ln&&t-Ea<zg)return!0;if(!e&&!Ln&&t-Zo<qg)return!1;let n=typeof Rs=="string"&&Rs.startsWith("/api/"),s=location.protocol==="file:"||location.hostname==="127.0.0.1"||location.hostname==="localhost";if(!(n&&s))try{let a=new AbortController,o=setTimeout(()=>a.abort(),cc),i=await fetch(Rs,{signal:a.signal,headers:{Accept:"application/json"}});if(clearTimeout(o),!i.ok)throw new Error(`HTTP ${i.status}`);let r=await i.json();return uc(r,"proxy"),!0}catch(a){console.warn("[registry] tier 1 (proxy) failed:",a.message)}try{let a=new AbortController,o=setTimeout(()=>a.abort(),cc),i=await fetch(Qo,{signal:a.signal});if(clearTimeout(o),!i.ok)throw new Error(`HTTP ${i.status}`);let r=await i.text(),l=JSON.parse(r);return uc(l,"xrpscan"),!0}catch(a){console.warn("[registry] tier 2 (xrpscan direct) failed:",a.message),(a.message.includes("Failed to fetch")||a.message.includes("CORS"))&&console.warn("[registry] CORS blocked \u2014 mount the backend proxy route (see validatorregistry.js)")}return console.warn("[registry] all sources failed \u2014 showing",Object.keys(oi).length,"hardcoded validators"),Zo=Date.now(),Ln||Cc(),ri({ok:!1,source:"fallback",error:"all sources failed \u2014 mount /api/v1 router"}),!1}window.debugRegistry=async function(){console.group("\u{1F50D} Registry debug"),console.log("Current source:",on),console.log("Validators loaded:",$e.size),console.log("Registry ok:",Ln),console.log("Last fetch:",Ea?new Date(Ea).toLocaleTimeString():"never"),console.log(""),console.log("Testing proxy \u2192",Rs);try{let e=await fetch(Rs),t=await e.text();console.log("  Status:",e.status,"\u2014 body preview:",t.slice(0,300))}catch(e){console.warn("  Failed:",e.message)}console.log(""),console.log("Testing XRPScan direct \u2192",Qo);try{let e=await fetch(Qo),t=await e.text();console.log("  Status:",e.status,"\u2014 body preview:",t.slice(0,300))}catch(e){console.warn("  Failed:",e.message)}return console.groupEnd(),{source:on,size:$e.size,ok:Ln}};function Ye(e){return $e.get(e)??{key:e,label:`${e.slice(0,10)}\u2026${e.slice(-6)}`,domain:null,provider:null,lists:[],category:"other",geo:_a[e]??null,meta:{}}}function rn(e){var n;if(!e)return null;if(((n=e.geo)==null?void 0:n.lat)!=null)return e.geo;let t=_a[e.key];return t||ii(e.domain)}function Pc(){return[...$e.values()].filter(e=>(e.category==="unl"||e.category==="both")&&e.chain!=="test").map(e=>e.key)}function Vg(){return[...$e.values()].filter(e=>(e.category==="dunl"||e.category==="both")&&e.chain!=="test").map(e=>e.key)}function Gg(){return[...$e.values()].filter(e=>e.category==="other"&&e.chain!=="test").map(e=>e.key)}function Kg(){return[...$e.values()].filter(e=>e.chain==="test").map(e=>e.key)}function ri({ok:e,count:t,lists:n,source:s,error:a}){let o=y("m1-registry-badge");if(o)if(e){let i=[...$e.values()].filter(c=>c.category==="unl").length,r=[...$e.values()].filter(c=>c.chain==="test").length,l=t-r;o.textContent=`${l} mainnet \xB7 ${i} UNL \xB7 ${r} testnet \xB7 live`,o.className="registry-badge registry-badge--ok"}else o.textContent=s==="fallback"?`Fallback data \xB7 ${$e.size} known \xB7 ${a??"endpoint unreachable"}`:"Refreshing\u2026",o.className="registry-badge registry-badge--warn"}var Na=null,mc=!1,Go=!1,fc=0,hc=0,gc=0,Ko=0,Lc=!0,Ve=null,Pn=null,Fs=null,Et=null,Oe={},La=null,ei={},qe=null,Ht=[],Ma=null,ti={},Ns="unl",at={fees:[],burnDrops:[],dexOffers:[],ammSwaps:[],newAccounts:[],converge:[],proposers:[],peerCounts:[],peerDiscon:[]};function Mc(){var t,n;if(mc)return;mc=!0,kv(),Cc(),Aa(!0),yv(),wv(),window.addEventListener("xrpl-connected",()=>{_s(),os()&&(Ds({force:!0}),Is({force:!1}))}),window.addEventListener("xrpl-disconnected",()=>{ni(),ai(null)}),window.addEventListener("xrpl-connection",s=>{var a;((a=s==null?void 0:s.detail)==null?void 0:a.state)==="connecting"&&(ni(),ai(null,"Switching networks\u2026"))}),window.addEventListener("xrpl-ledger",s=>{gv(s.detail),os()&&vv(s.detail)}),(t=y("btn-network-refresh"))==null||t.addEventListener("click",()=>{Ds({force:!0}),Aa(!0),Is({force:!0})}),(n=document.querySelector('.dash-tab[data-tab="network"]'))==null||n.addEventListener("click",()=>{_s(),Ds({force:!0}),Aa(!1),Is({force:!1})});let e=y("tab-network");e&&new MutationObserver(_s).observe(e,{attributes:!0,attributeFilter:["style","class"]}),document.addEventListener("visibilitychange",_s)}function os(){let e=y("tab-network"),t=e?e.style.display!=="none":!1;return Lc&&O.currentPage==="dashboard"&&O.currentTab==="network"&&t&&!document.hidden}function _s(){os()?Jg():ni()}function Ac(e){Lc=!!e,_s()}function Jg(){Na||(Ds({force:!1}),Na=setInterval(()=>{os()&&Ds({force:!1})},Rg))}function ni(){clearInterval(Na),Na=null}async function Ds({force:e=!1}={}){var n;if(!os()&&!e)return;let t=Date.now();if(!(!e&&(t-fc<Dg||Go||t<hc))){Go=!0,fc=t,Oe={},vc(!0);try{await Promise.allSettled([Aa(!1),Yg(),Qg(),Zg(),ev(),tv()]),nv(),iv(),rv(),lv(),fv(),ai({info:Ve,fee:Pn,vals:Fs}),$v()}catch(s){String((s==null?void 0:s.message)??"").toLowerCase().includes("too much load")&&(hc=Date.now()+Ig,(n=he)==null||n("Rate-limited \u2014 backing off 2 min."))}finally{vc(!1),Go=!1}}}function vc(e){var t;(t=y("btn-network-refresh"))==null||t.classList.toggle("spinning",e)}async function Yg(){var n,s,a;let e=await Me({command:"server_info"});if(Ve=((n=e==null?void 0:e.result)==null?void 0:n.info)??null,!Ve)return;Je("converge",Number(((s=Ve.last_close)==null?void 0:s.converge_time_s)??0)),Je("proposers",Number(((a=Ve.last_close)==null?void 0:a.proposers)??0)),Je("peerCounts",Number(Ve.peers??0));let t=Number(Ve.peer_disconnects_resources??0);La!==null&&t>La&&Je("peerDiscon",t-La),La=t}async function Qg(){var t;let e=await Me({command:"fee"});Pn=(e==null?void 0:e.result)??null,((t=Pn==null?void 0:Pn.drops)==null?void 0:t.open_ledger_fee)!=null&&Je("fees",Number(Pn.drops.open_ledger_fee))}async function Zg(){try{let e=await Me({command:"validators"});Fs=(e==null?void 0:e.result)??null}catch{Fs=null}}async function ev(){var e;try{let t=await Me({command:"peers"});Et=Array.isArray((e=t==null?void 0:t.result)==null?void 0:e.peers)?t.result.peers:null}catch{Et=null}}async function tv(){var e;try{let t=await Me({command:"feature"});(e=t==null?void 0:t.result)!=null&&e.features&&(cv(t.result.features),Nc(t.result.features))}catch{}}function nv(){var M,N,_,X,I,D;let e=Ve,t=Fs,n=(t==null?void 0:t.trusted_validator_keys)??[],s=Pc(),a=Vg(),o=n.length?n:s,i=a,r=Number((t==null?void 0:t.validation_quorum)??(e==null?void 0:e.validation_quorum)??0),l=o.length,c=l-r,d=l>0?r/l*100:0,u=o.filter(F=>$e.has(F)).length,p=l>0?Math.round(u/l*100):0,m=Number(((M=e==null?void 0:e.last_close)==null?void 0:M.converge_time_s)??0),f=Number(((N=e==null?void 0:e.last_close)==null?void 0:N.proposers)??0),v=r>0?Math.round(f/r*100):0,h=Array.isArray(e==null?void 0:e.negative_unl)?e.negative_unl:[],w=!!Fs;c>=0&&c<=3&&(Oe.quorumTight=!0),h.length>0&&(Oe.nUnlActive=!0),m>6&&(Oe.slowConverge=!0),f>0&&r>0&&f<r&&(Oe.lowProposers=!0),G("m1-active",w?l:"\u2014"),G("m1-quorum",r>0?r:"\u2014"),G("m1-margin",w?c>=0?c:`\u2212${Math.abs(c)}`:"\u2014"),G("m1-overlap",w?`${p}%`:"\u2014"),G("m1-known",w?`${u} / ${l} identified`:"\u2014 (validators cmd unavailable)"),G("m1-proposers",((_=e==null?void 0:e.last_close)==null?void 0:_.proposers)!=null?f:"\u2014"),G("m1-particip",r>0&&((X=e==null?void 0:e.last_close)==null?void 0:X.proposers)!=null?`${v}%`:"\u2014"),G("m1-converge",((I=e==null?void 0:e.last_close)==null?void 0:I.converge_time_s)!=null?`${m.toFixed(2)}s`:"\u2014"),G("m1-converge-avg",Os("converge")>0?`avg ${Os("converge").toFixed(2)}s`:"\u2014"),ut("m1-qbar",d,d>90?"bar-danger":d>80?"bar-warn":"bar-ok"),ut("m1-obar",p,p<40?"bar-danger":p<70?"bar-warn":"bar-ok"),ut("m1-pbar",v,v<80?"bar-danger":v<95?"bar-warn":"bar-ok"),ut("m1-cvgbar",Math.min(100,m/10*100),m>6?"bar-danger":m>4?"bar-warn":"bar-ok"),ov({active:l,quorum:r,margin:c,nUnl:h,valsAvail:w});let k=(D=t==null?void 0:t.publisher_lists)==null?void 0:D[0],b=(an==null?void 0:an.ripple)??(an==null?void 0:an.xrplf)??null,x=(k==null?void 0:k.uri)??(b==null?void 0:b.uri)??"\u2014",S=(k==null?void 0:k.seq)??(b==null?void 0:b.seq)??"\u2014";if(G("m1-vl-uri",x),G("m1-vl-seq",S),k!=null&&k.expiration||b!=null&&b.expiration){let F=y("m1-vl-expiry");if(F){let W=(k==null?void 0:k.expiration)??(b==null?void 0:b.expiration),E=Math.floor((new Date(W)-Date.now())/864e5);F.textContent=E>0?`Expires ${E}d`:"\u26A0 EXPIRED",F.className=`expiry-pill ${E>30?"pill-ok":E>7?"pill-warn":"pill-bad"}`,F.style.display=""}}let T=y("m1-nunl-list");if(G("m1-nunl-count",h.length||"0"),T)if(!h.length)T.innerHTML='<div class="nunl-empty">\u2713 No validators on Negative UNL</div>';else{let F={};T.innerHTML=h.map(E=>{let U=Ye(E);return U.provider&&(F[U.provider]=(F[U.provider]||0)+1),`<div class="nunl-entry">
        ...
        </div>`}).join("");let W=Object.entries(F).sort((E,U)=>U[1]-E[1])[0];(W==null?void 0:W[1])>1&&(T.innerHTML+=`<div class="nunl-alert">\u26A0 ${W[1]} offline validators share <b>${g(W[0])}</b> \u2014 likely provider outage</div>`)}Ec(o,i,r,h,w);let $=Number((Ve==null?void 0:Ve.peers)??0);G("wm-stat-val",w?`${l} validators`:`${$e.size} in registry`),G("wm-stat-nunl",`${h.length} on nUNL`),G("wm-stat-peers",`${Et?Et.length:$} peers`);let P=[...$e.keys()];pv(o,i,h,Et,!w)}function Ec(e,t,n,s,a){let o=y("m1-val-grid");if(!o)return;let i=new Set(e),r=new Set(t),l=new Set(s),c=Pc(),d=Kg(),u=Gg(),p=e.length?e:c,m=[...$e.keys()],f=m.filter(I=>{var D;return(((D=$e.get(I))==null?void 0:D.chain)??"main")==="main"}),v=d,h=u,w=p.filter(I=>!!Ye(I).domain),k=p.filter(I=>!Ye(I).domain),b=h.filter(I=>!!Ye(I).domain),x=h.filter(I=>!Ye(I).domain),S=m.filter(I=>!!rn(Ye(I))),T=[{id:"unl",label:"UNL",count:p.length,dot:"unl"},{id:"others",label:"Others",count:h.length,dot:"other"},{id:"all",label:"All",count:f.length,dot:null},{id:"test",label:"Testnet",count:v.length,dot:null}],$={unl:"#00fff0",dunl:"#bd93f9",both:"#50fa7b",nunl:"#ff5555",other:"#ffb86c"},P='<div class="vg-tabs" role="tablist">'+T.map(I=>`<button class="vg-tab ${Ns===I.id?"vg-tab--active":""}" data-vgtab="${I.id}">
        ${I.dot?`<span class="vg-tab-dot" style="background:${$[I.dot]}"></span>`:""}
        ${g(I.label)}
        <span class="vg-tab-count">${I.count}</span>
      </button>`).join("")+"</div>",M=[],N="";if(Ns==="unl"){let I=p.filter(E=>l.has(E)),D=w.filter(E=>!l.has(E)),F=k.filter(E=>!l.has(E));I.length&&M.push({title:`\u26A0 Negative UNL (${I.length})`,keys:I,cls:"vgs-warn"}),M.push({title:`Named UNL validators \xB7 ${D.length}`,keys:D}),F.length&&M.push({title:`Key-only UNL validators \xB7 ${F.length}`,keys:F,collapsed:!0}),p.length||M.push({title:"No UNL data",keys:[],notice:"Registry not loaded yet"});let W=a?"live rippled":`registry (${on})`;N=`${p.length} trusted \xB7 quorum ${n} \xB7 ${s.length} on nUNL \xB7 source: ${W}`}else if(Ns==="others")M.push({title:`Named mainnet (non-UNL) \xB7 ${b.length}`,keys:b}),x.length&&M.push({title:`Key-only mainnet \xB7 ${x.length}`,keys:x,collapsed:!0}),N=`${h.length} mainnet validators not on UNL \xB7 source: ${on}`;else if(Ns==="test"){let I=v.filter(F=>!!Ye(F).domain),D=v.filter(F=>!Ye(F).domain);I.length&&M.push({title:`Named testnet \xB7 ${I.length}`,keys:I}),D.length&&M.push({title:`Key-only testnet \xB7 ${D.length}`,keys:D,collapsed:!0}),N=`${v.length} testnet validators`}else M.push({title:`\u2B50 UNL \xB7 ${p.length}`,keys:p}),M.push({title:`Other mainnet \xB7 ${h.length}`,keys:h,collapsed:!0}),v.length&&M.push({title:`Testnet \xB7 ${v.length}`,keys:v,collapsed:!0}),N=`${f.length} mainnet \xB7 ${v.length} testnet \xB7 ${S.length} geo-located \xB7 source: ${on}`;function _(I,D){var Se;let F=Ye(I),W=l.has(I),E=i.has(I),U=r.has(I),Q=F.chain==="test",ie,ce,de;W?(ie="vp-nunl",ce="nUNL",de=$.nunl):E&&U?(ie="vp-both",ce="UNL+dUNL",de=$.both):E?(ie="vp-unl",ce="UNL",de=$.unl):U?(ie="vp-dunl",ce="dUNL",de=$.dunl):(ie="vp-other",ce="Other",de=$.other);let se=rn(F),fe=!!se,K=se?`\u{1F4CD} ${se.city??""}`:"",xe=F.domain?F.domain.replace(/^www\./,"").replace(/^validator\./,"").replace(/^xrp\./,""):null,B=`${I.slice(0,8)}\u2026${I.slice(-6)}`,J=E&&D!=null?`<span class="vg-num">${D+1}</span>`:"",re=(Se=F.agreement)==null?void 0:Se["24h"],pe=re!=null&&re.score?`<span class="vg-agr" style="color:${parseFloat(re.score)>=99?"#50fa7b":parseFloat(re.score)>=95?"#ffb86c":"#ff5555"}" title="24h agreement">${re.score}</span>`:"";return`<div class="vpill ${ie} ${fe?"vp-locatable":""}"
      ...
      </div>`}),X||(X='<div class="nunl-empty">No validators in this view.</div>'),o.innerHTML=P+X,G("m1-val-summary",N),o.querySelectorAll(".vg-tab").forEach(I=>{I.addEventListener("click",D=>{D.stopPropagation();let F=I.getAttribute("data-vgtab");F&&(Ns=F,Ec(e,t,n,s,a))})})}async function sv(e){let t=Jo.get(e);if(t&&Date.now()-t.cachedAt<kc)return t.data;try{let n=new AbortController,s=setTimeout(()=>n.abort(),$c),a=await fetch(`${xc}/${encodeURIComponent(e)}`,{signal:n.signal});if(clearTimeout(s),!a.ok)throw new Error(`HTTP ${a.status}`);let o=await a.json();return Jo.set(e,{data:o,cachedAt:Date.now()}),o}catch{return null}}async function av(e){let t=Yo.get(e);if(t&&Date.now()-t.cachedAt<kc)return t.data;try{let n=new AbortController,s=setTimeout(()=>n.abort(),$c),a=await fetch(`${xc}/${encodeURIComponent(e)}/reports`,{signal:n.signal});if(clearTimeout(s),!a.ok)throw new Error(`HTTP ${a.status}`);let o=await a.json(),i=o.reports??o??[];return Yo.set(e,{data:i,cachedAt:Date.now()}),i}catch{return[]}}window.focusValidator=async function(e){let t=Ye(e),n=rn(t);document.querySelectorAll(".vpill").forEach(o=>o.classList.remove("vp-active"));let s=document.querySelector(`.vpill[data-key="${CSS.escape(e)}"]`);if(s&&(s.classList.add("vp-active"),s.scrollIntoView({behavior:"smooth",block:"nearest"})),si(e,t,null),sv(e).then(o=>{o&&si(e,t,o)}),!n){let o=y("world-map-container");if(o){let i=o.querySelector(".wm-no-geo");i&&i.remove();let r=document.createElement("div");r.className="wm-no-geo",r.textContent=`\u{1F4CD} ${t.label} \u2014 geographic location unknown`,o.appendChild(r),setTimeout(()=>r.remove(),3500)}return}let a=y("world-map-container");a&&a.scrollIntoView({behavior:"smooth",block:"nearest"}),qe&&(qe.flyTo([n.lat,n.lng],6,{duration:1.2}),setTimeout(()=>{let o=ti[e];o&&o.openPopup()},1300))};function si(e,t,n){var M,N,_,X,I;let s=y("amend-modal-overlay"),a=y("amend-modal-body");if(!s||!a)return;let o=n??{},i=t.label??o.label??o.account_name??e.slice(0,16)+"\u2026",r=t.domain??o.domain??null,l=t.chain??o.chain??"main",c=t.category??"other",d=_e[c]??_e.other,u=rn(t),p=t.unl||c==="unl"||c==="both",m=t.dunl||c==="dunl"||c==="both",f=t.domainVerified||o.domain_verified?"\u2713 Verified":r?"Unverified":"\u2014",v=t.agreement??o.agreement??{},h=!n&&!((M=t.agreement)!=null&&M["24h"]);function w(D,F){if(!F||!F.total)return`<div class="adm-mi"><span class="adm-mk">${D}</span><span class="adm-mv" style="opacity:.5">\u2014</span></div>`;let W=parseFloat(F.score)||0,E=W>=99?"color:#50fa7b":W>=95?"color:#ffb86c":"color:#ff5555";return`<div class="adm-mi">
    ...
    </div>`}function iv(){var F,W,E;let e=Ve,t=(e==null?void 0:e.server_state)??"unknown",n=(e==null?void 0:e.build_version)??"\u2014",s=Number((e==null?void 0:e.uptime)??0),a=e==null?void 0:e.network_id,o=Number((e==null?void 0:e.io_latency_ms)??0),i=String((e==null?void 0:e.jq_trans_overflow)??"0"),r=Number((e==null?void 0:e.peer_disconnects_resources)??0),l=Number((e==null?void 0:e.load_factor)??1),c=Number((e==null?void 0:e.load_factor_net)??1),d=Number((e==null?void 0:e.load_factor_server)??(e==null?void 0:e.load_factor_local)??1),u=Number(((F=e==null?void 0:e.validated_ledger)==null?void 0:F.age)??0),p=(W=e==null?void 0:e.validated_ledger)==null?void 0:W.seq,m=(e==null?void 0:e.complete_ledgers)??"",f=(e==null?void 0:e.state_accounting)??null;u>10&&(Oe.staleLedger=!0),o>5&&(Oe.ioStressed=!0),Os("peerDiscon")>5&&(Oe.peerChurn=!0);let v=y("m2-state");if(v){v.textContent=t;let U=["full","proposing","validating"].includes(t),Q=["syncing","tracking","connected"].includes(t);v.className=`state-pill state-${U?"ok":Q?"warn":"bad"}`}G("m2-version",n),G("m2-uptime",Tv(s)),G("m2-netid",a===0?"0 (Mainnet)":a===1?"1 (Testnet)":a??"\u2014"),G("m2-ledger-seq",p!=null?Number(p).toLocaleString():"\u2014"),G("m2-ledger-age",u>0?`${u}s`:"< 1s"),G("m2-io-ms",o>0?`${o}ms`:"< 1ms"),G("m2-jq",i==="0"?"0 (clean)":`\u26A0 ${i}`),G("m2-discon",r.toLocaleString());let h=y("m2-ledger-age");h&&(h.className=`kv-v ${u>10?"text-danger":u>5?"text-warn":""}`);let w=Math.min(100,(l-1)/49*100);ut("m2-load-bar",w,l>5?"bar-danger":l>2?"bar-warn":"bar-ok"),G("m2-load-total",`${l.toFixed(2)}\xD7`),G("m2-load-net",`${c.toFixed(2)}\xD7`),G("m2-load-local",`${d.toFixed(2)}\xD7`);let k=d>c*1.5?"Local node stressed":c>d*1.5?"Network-wide stress":l>1.2?"Distributed":"Normal";G("m2-load-src",k);let b=O.tpsHistory.length?O.tpsHistory[O.tpsHistory.length-1]:null;G("m2-tps",b!=null?b.toFixed(1):"\u2014"),G("m2-txcount",((E=O.ledgerLog[0])==null?void 0:E.txCount)??"\u2014");let x=m==="entire ledger"||m.startsWith("32570"),S=x?100:Sv(m);if(G("m2-ledger-range",m||"\u2014"),G("m2-hist-type",x?"Full History Node":"Pruned / Partial"),G("m2-hist-score",`${S}%`),ut("m2-hist-bar",S,S<30?"bar-danger":S<70?"bar-warn":"bar-ok"),f){let U=y("m2-state-acct");if(U){let Q=["full","syncing","tracking","connected","disconnected"],ie=0,ce={};Q.forEach(de=>{var se;ce[de]=Number(((se=f[de])==null?void 0:se.duration_us)??0),ie+=ce[de]}),U.innerHTML=ie>0?Q.map(de=>{let se=Math.round(ce[de]/ie*100);return se?`<div class="sa-row"><span class="sa-lbl">${de}</span>
          <div class="bar-track sa-bar"><div class="bar-fill ${de==="full"?"bar-ok":de==="syncing"?"bar-warn":"bar-danger"}" style="width:${se}%"></div></div>
          <span class="sa-pct">${se}%</span></div>`:""}).join(""):'<span class="dim">No data</span>'}}let T=(e==null?void 0:e.peers)!=null?Number(e.peers):null,$=T!=null,P=0,M=0,N=!1;Et&&(Et.forEach(U=>{U.inbound===!0?P++:M++}),N=!0);let _=N&&P+M>0?Math.round(P/(P+M)*100):0,X=N?Et.length:T??0,I=X<6?"HIGH":X<15?"MEDIUM":"LOW";X<6&&(Oe.eclipseRisk=!0),_>80&&(Oe.peerSaturate=!0),G("m2-peers",$?X:"\u2014"),G("m2-inbound",N?P:$?"\u2014 (cmd restricted)":"\u2014"),G("m2-outbound",N?M:$?"\u2014 (cmd restricted)":"\u2014"),G("m2-ib-pct",N?`${_}%`:"\u2014"),ut("m2-peer-bar",Math.min(100,X/21*100),X>18?"bar-danger":X>15?"bar-warn":"bar-ok"),ut("m2-ib-bar",_,_>80?"bar-danger":_>60?"bar-warn":"bar-ok");let D=y("m2-eclipse");D&&(D.textContent=I,D.className=`risk-badge risk-${I.toLowerCase()}`)}function rv(){var M,N,_,X;let e=Pn,t=Number(((M=e==null?void 0:e.drops)==null?void 0:M.minimum_fee)??10),n=Number(((N=e==null?void 0:e.drops)==null?void 0:N.open_ledger_fee)??10),s=Number(((_=e==null?void 0:e.drops)==null?void 0:_.median_fee)??10),a=Number(((X=e==null?void 0:e.drops)==null?void 0:X.base_fee)??10),o=Number((e==null?void 0:e.current_ledger_size)??0),i=Number((e==null?void 0:e.expected_ledger_size)??1),r=Number((e==null?void 0:e.current_queue_size)??0),l=Number((e==null?void 0:e.max_queue_size)??1),c=l>0?Math.round(r/l*100):0,d=i>0?o/i:1,u=Os("fees"),p=u>0?Math.round((n-u)/u*100):0,m=Math.min(100,Math.round(Math.log2(Math.max(1,n/10))*14));n>t*10&&(Oe.feeSpike=!0),c>80&&(Oe.queuePressure=!0),d>2&&(Oe.spamLedger=!0);let f=at.burnDrops.slice(-10),v=f.length?f.reduce((I,D)=>I+D,0)/f.length:0,h=xv(at.burnDrops),w=Os("burnDrops"),k=h>0&&at.burnDrops.length>5?((v-w)/h).toFixed(2):"0.00",b=Math.min(100,Math.abs(Number(k))*20);Math.abs(Number(k))>3&&(Oe.burnAnomaly=!0);let x=n>5e3?"Severe":n>500?"High":n>100?"Elevated":n>20?"Normal":"Minimal",T={Severe:"#ff5555",High:"#ff9955",Elevated:"#ffb86c",Normal:"#50fa7b",Minimal:"#6272a4"}[x]??"#50fa7b",$=y("m3-pressure");$&&($.textContent=x,$.className=`pressure-badge p-${x.toLowerCase()}`),G("m3-open",`${n} drops`),G("m3-med",`${s} drops`),G("m3-base",`${a} drops`),G("m3-devpct",`${p>0?"+":""}${p}% vs baseline`),G("m3-spam",`${m} / 100`),G("m3-qsize",`${r} / ${l}`),G("m3-qpct",`${c}%`),G("m3-szratio",`${d.toFixed(2)}\xD7`),G("m3-curledger",`${o} txs`),G("m3-expledger",`${i} expected`),G("m3-burn",v>0?`${(v/1e6).toFixed(4)} XRP / ledger`:"\u2014"),G("m3-burnz",`z = ${k}`),ut("m3-spam-bar",m,m>70?"bar-danger":m>40?"bar-warn":"bar-ok"),ut("m3-q-bar",c,c>80?"bar-danger":c>50?"bar-warn":"bar-ok"),ut("m3-sz-bar",Math.min(100,d*50),d>2?"bar-danger":d>1.5?"bar-warn":"bar-ok"),ut("m3-burn-bar",b,b>60?"bar-danger":b>30?"bar-warn":"bar-ok");let P=y("m3-congestion-summary");if(P){let I=[];d>2&&I.push({icon:"\u{1F6A8}",txt:`Ledger ${d.toFixed(1)}\xD7 normal size \u2014 possible spam burst`,col:"#ff5555"}),c>80&&I.push({icon:"\u26A0",txt:`TX queue ${c}% full \u2014 fee surge imminent`,col:"#ffb86c"}),m>70&&I.push({icon:"\u26A0",txt:`Spam index ${m}/100 \u2014 elevated DDoS risk`,col:"#ffb86c"}),Math.abs(Number(k))>3&&I.push({icon:"\u{1F4C9}",txt:`Burn rate z=${k} \u2014 resource exhaustion pattern`,col:"#ff9955"}),p>200&&I.push({icon:"\u{1F4B8}",txt:`Open fee ${p}% above baseline \u2014 network stress`,col:"#ffb86c"}),P.innerHTML=I.length?I.map(D=>`<div class="cong-line"><span>${D.icon}</span><span style="color:${D.col}">${g(D.txt)}</span></div>`).join(""):'<div class="cong-clear"><span style="color:#50fa7b">\u2713</span> No congestion signals \u2014 network traffic is normal</div>'}}function lv(){var u,p;let e={},t=0;for(let m of $e.values()){if(m.chain==="test")continue;let f=m.version??"unknown";e[f]=(e[f]??0)+1,t++}let n={},s=0;if(Array.isArray(Et))for(let m of Et){let f=(m.version??"unknown").replace(/rippled-/i,"").split(" ")[0];n[f]=(n[f]??0)+1,s++}let a=y("m4-version-dist");if(a&&t>0){let m=Object.entries(e).sort((w,k)=>k[1]-w[1]),f=((u=m[0])==null?void 0:u[0])??"?",v=t>0?Math.round((((p=m[0])==null?void 0:p[1])??0)/t*100):0,h=["#00fff0","#50fa7b","#ffb86c","#bd93f9","#ff5555","#8be9fd"];a.innerHTML=m.slice(0,6).map(([w,k],b)=>{let x=Math.round(k/t*100),S=h[b%h.length],T=w.includes("beta")||w.includes("rc")||w.includes("RC");return`<div class="vd-row">
    ...
    <div class="adm-footer">
    ...
    </div>`,u.style.display="flex",u.addEventListener("click",m=>{m.target===u&&window.closeAmendModal()},{once:!0}))};window.closeAmendModal=function(){let e=y("amend-modal-overlay");e&&(e.style.display="none")};function dv(e){if(window.L){e();return}if(!document.querySelector("#leaflet-css")){let n=document.createElement("link");n.id="leaflet-css",n.rel="stylesheet",n.href="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.css",document.head.appendChild(n)}let t=document.createElement("script");t.src="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.js",t.onload=e,document.head.appendChild(t)}function pv(e,t,n,s,a=!1){y("world-map-container")&&dv(()=>uv(e,t,n,s,a))}function uv(e,t,n,s,a=!1){let o=y("world-map-container");if(!o)return;let i=new Set(n||[]),r=new Set(e||[]),l=new Set(t||[]),c=O.currentNetwork||"xrpl-mainnet";if(qe&&Ma!==c&&(qe.remove(),qe=null,Ht=[],Ma=null),!qe){o.innerHTML="";let x=document.createElement("div");x.id="wm-leaflet",x.style.cssText="width:100%;height:440px;",o.appendChild(x),qe=L.map("wm-leaflet",{center:[25,5],zoom:2,minZoom:1,maxZoom:12,zoomControl:!0,attributionControl:!0,worldCopyJump:!0}),L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{attribution:'&copy; <a href="https://carto.com">CARTO</a> &copy; <a href="https://openstreetmap.org">OSM</a>',subdomains:"abcd",maxZoom:19}).addTo(qe),Ma=c}Ht.forEach(x=>{try{x.remove()}catch{}}),Ht=[];function d(x){return i.has(x)?"nunl":r.has(x)&&l.has(x)?"both":r.has(x)?"unl":l.has(x)?"dunl":"other"}ti={};let u={};for(let[x,S]of $e){let T=rn(S);if(!(T!=null&&T.lat)||!(T!=null&&T.lng))continue;let $=`${(Math.round(T.lat*2)/2).toFixed(1)},${(Math.round(T.lng*2)/2).toFixed(1)}`;u[$]||(u[$]={lat:T.lat,lng:T.lng,keys:[],city:T.city,org:T.org}),u[$].keys.push(x)}if(Object.values(u).forEach(x=>{let S=x.keys.length,T=x.keys.map(d),$=["nunl","both","unl","dunl","other"].find(X=>T.includes(X))||"other",P=_e[$],M=S>5?13:S>2?10:7,N=L.divIcon({html:`<div class="wm-lmarker wm-lmarker-val ${$==="nunl"?"wm-lmarker-nunl":""} ${a?"wm-lmarker-ref":""}"
      ...
      </div>`,className:"",iconSize:[(M+8)*2,(M+8)*2],iconAnchor:[M+8,M+8]}),_=L.marker([x.lat,x.lng],{icon:N}).bindPopup(mv(x.keys,x.city,x.org,i,r,l,a),{maxWidth:380,className:"wm-popup-wrap"}).addTo(qe);x.keys.forEach(X=>{ti[X]=_}),Ht.push(_)}),a){let x=L.control({position:"bottomleft"});x.onAdd=()=>{let S=L.DomUtil.create("div","wm-ref-banner");return S.innerHTML="\u{1F4E1} Reference positions \xB7 live validator list unavailable from endpoint",S},x.addTo(qe),Ht.push(x)}let p=[...$e.keys()].filter(x=>{var T;return((T=$e.get(x).geo)==null?void 0:T.lat)!=null||_a[x]}).length,m=$e.size-p;if(m>0){let x=L.control({position:"bottomright"});x.onAdd=()=>{let S=L.DomUtil.create("div","wm-unknown-ctrl");return S.innerHTML=`+ ${m} validators \xB7 location unknown`,S},x.addTo(qe),Ht.push(x)}Wg.forEach(x=>{let S=L.divIcon({html:`<div class="wm-lmarker wm-lmarker-pub">
        <div class="wm-lring"></div>
    ...
    </div>`}).join(""),c=[...new Set(e.map(d=>{var p;let u=Ye(d);return u.provider??((p=rn(u))==null?void 0:p.org)??null}).filter(Boolean))];return`<div class="wm-popup-inner">
    ...
    </div>`}).join("")}function gv(e){var t,n,s,a,o,i,r,l,c,d;e&&(Je("burnDrops",(e.avgFee||0)*1e6*(e.txPerLedger??0)),Je("dexOffers",((t=e.txTypes)==null?void 0:t.OfferCreate)??0),Je("ammSwaps",(((n=e.txTypes)==null?void 0:n.AMMDeposit)??0)+(((s=e.txTypes)==null?void 0:s.AMMWithdraw)??0)+(((a=e.txTypes)==null?void 0:a.AMMBid)??0)),Je("newAccounts",((o=e.txTypes)==null?void 0:o.AccountSet)??0),Je("txPayment",((i=e.txTypes)==null?void 0:i.Payment)??0),Je("txNFT",(((r=e.txTypes)==null?void 0:r.NFTokenMint)??0)+(((l=e.txTypes)==null?void 0:l.NFTokenBurn)??0)+(((c=e.txTypes)==null?void 0:c.NFTokenCreateOffer)??0)+(((d=e.txTypes)==null?void 0:d.NFTokenAcceptOffer)??0)),Je("txLedger",e.txPerLedger??0),Je("tps",e.tps??(e.txPerLedger??0)/3.5),Je("closeTime",e.closeTime??3.5))}function vv(e){let t=O.tpsHistory.length?O.tpsHistory[O.tpsHistory.length-1]:null;G("m2-tps",t!=null?t.toFixed(1):"\u2014"),G("m2-txcount",e.txPerLedger??"\u2014"),e.successRate!=null&&G("m2-success",`${e.successRate.toFixed(0)}%`)}async function Is({force:e=!1}={}){if(!os()&&!e)return;let t=Date.now();if(!e&&t-gc<Og)return;gc=t;let n=y("latency-list");if(!n)return;let s=yr[O.currentNetwork]??[],a=++Ko;n.innerHTML=s.map((i,r)=>`
  ...
  `,document.getElementById("net-panel-styles")||document.head.appendChild(s)}function Je(e,t){at[e]||(at[e]=[]),at[e].push(Number(t)),at[e].length>Xg&&at[e].shift()}function Os(e){let t=at[e]??[];return t.length?t.reduce((n,s)=>n+s,0)/t.length:0}function xv(e){if(e.length<2)return 0;let t=e.reduce((n,s)=>n+s,0)/e.length;return Math.sqrt(e.reduce((n,s)=>n+(s-t)**2,0)/e.length)}function kv(){try{let e=localStorage.getItem(wc);if(!e)return;let t=JSON.parse(e);Object.keys(at).forEach(n=>{Array.isArray(t[n])&&(at[n]=t[n])})}catch{}}function $v(){try{localStorage.setItem(wc,JSON.stringify(at))}catch{}}function G(e,t){let n=y(e);n&&(n.textContent=t??"\u2014")}function ut(e,t,n){let s=y(e);s&&(s.style.width=`${Math.min(100,Math.max(0,Number(t)||0))}%`,s.className=`bar-fill ${n??""}`)}function Sv(e){if(!e)return 0;let t=e.match(/(\d+)-(\d+)/);if(!t)return 10;let n=Number(t[2])-Number(t[1]);return n>1e7?95:n>1e6?70:n>1e5?40:15}function Tv(e){if(!e)return"\u2014";let t=Math.floor(e/86400),n=Math.floor(e%86400/3600),s=Math.floor(e%3600/60);return t>0?`${t}d ${n}h`:n>0?`${n}h ${s}m`:`${s}m`}function Cv(e){return new Promise(t=>setTimeout(t,e))}var Pv="https://xrplcluster.com/",Lv="https://s2.ripple.com:51234/";async function Ra(e,t){var a;try{if(((a=O.wsConn)==null?void 0:a.readyState)===1){let o=await Me({command:e,...t});if((o==null?void 0:o.status)==="error")throw new Error(o.error_message||o.error||"XRPL RPC error");return(o==null?void 0:o.result)||null}}catch{}let n=JSON.stringify({method:e,params:[t]}),s=async o=>{let i=await fetch(o,{method:"POST",headers:{"Content-Type":"application/json"},body:n,mode:"cors"});if(!i.ok)throw new Error(`HTTP ${i.status}`);return(await i.json()).result};try{return await s(Pv)}catch{return await s(Lv)}}function rs(e){return typeof e=="string"?{value:Number(e)/1e6,isXrp:!0,currency:"XRP"}:e&&typeof e=="object"?{value:Number(e.value||0),isXrp:!1,currency:e.currency,issuer:e.issuer}:{value:0,isXrp:!0,currency:"XRP"}}function Mv(e){let t=String(e||"").trim().toUpperCase();if(t==="XRP"||/^[A-Z0-9?!@#$%^&*(){}[\]|]{3}$/i.test(t)||/^[0-9A-F]{40}$/.test(t))return t;let n=new TextEncoder().encode(t).slice(0,20),s=new Uint8Array(20);return s.set(n),Array.from(s).map(a=>a.toString(16).padStart(2,"0")).join("").toUpperCase()}var ln={lsfRequireAuth:262144,lsfDisableMaster:1048576,lsfNoFreeze:2097152,lsfGlobalFreeze:4194304,lsfDefaultRipple:8388608,lsfAllowTrustLineClawback:2147483648};async function Av(e,t){var n;try{let s=await Ra("amm_info",{asset:{currency:"XRP"},asset2:{currency:e,issuer:t}}),a=s==null?void 0:s.amm;if(!a)return{exists:!1};let o=rs(a.amount).value,i=rs(a.amount2).value,r=rs(a.lp_token||a.lp_token_balance).value,l=((n=a.lp_token||a.lp_token_balance)==null?void 0:n.currency)||null;return{exists:!0,account:a.account,xrpReserve:o,tokenReserve:i,lpSupply:r,lpCurrency:l,tradingFeePct:Number(a.trading_fee||0)/1e3,auctionSlot:a.auction_slot?{account:a.auction_slot.account,discountedFeePct:Number(a.auction_slot.discounted_fee||0)/1e3,priceXrp:rs(a.auction_slot.price).value,expiration:a.auction_slot.expiration||null}:null,voteSlots:Array.isArray(a.vote_slots)?a.vote_slots.map(c=>({account:c.account,tradingFeePct:Number(c.trading_fee||0)/1e3,voteWeight:Number(c.vote_weight||0)/1e3})):[]}}catch{return{exists:!1,error:!0}}}async function Ev(e){try{let t=await Ra("account_info",{account:e,ledger_index:"validated"}),n=t==null?void 0:t.account_data;if(!n)return{exists:!1};let s=Number(n.Flags||0),a=o=>(s&o)===o;return{exists:!0,requireAuth:a(ln.lsfRequireAuth),globalFreeze:a(ln.lsfGlobalFreeze),noFreeze:a(ln.lsfNoFreeze),freezeCapable:!a(ln.lsfNoFreeze),defaultRipple:a(ln.lsfDefaultRipple),clawbackEnabled:a(ln.lsfAllowTrustLineClawback),blackholed:a(ln.lsfDisableMaster)&&!n.RegularKey,masterDisabled:a(ln.lsfDisableMaster),hasRegularKey:!!n.RegularKey}}catch{return{exists:!1,error:!0}}}async function Dc(e,t,n=8){let s=[],a;for(let o=0;o<n;o++){let i=await Ra("account_lines",{account:e,ledger_index:"validated",limit:400,...a?{marker:a}:{}});for(let r of(i==null?void 0:i.lines)||[]){if(r.currency!==t)continue;let l=-Number(r.balance||0);l>0&&s.push({address:r.account,balance:l})}if(a=i==null?void 0:i.marker,!a)break}return s}function Ic(e){let t=[...e].sort((a,o)=>o.balance-a.balance),n=t.reduce((a,o)=>a+o.balance,0),s=a=>n>0?t.slice(0,a).reduce((o,i)=>o+i.balance,0)/n*100:0;return{holderCount:t.length,totalSampled:n,top1Pct:s(1),top5Pct:s(5),top10Pct:s(10),topHolders:t.slice(0,10)}}async function Nv(e,t){try{let n=await Dc(e,t);return n.length?{exists:!0,...Ic(n),sampleOnly:!0}:{exists:!1}}catch{return{exists:!1,error:!0}}}async function _v(e,t){if(!e||!t)return{exists:!1};try{let n=await Dc(e,t);return n.length?{exists:!0,...Ic(n)}:{exists:!1}}catch{return{exists:!1,error:!0}}}var Rv=[1,2,5,10,25];async function _c(e,t,n){let o=await Ra("book_offers",{taker_gets:n==="buy"?{currency:e,issuer:t}:{currency:"XRP"},taker_pays:n==="buy"?{currency:"XRP"}:{currency:e,issuer:t},limit:200,ledger_index:"validated"});return((o==null?void 0:o.offers)||[]).map(r=>{let l=rs(r.taker_gets_funded??r.TakerGets).value,c=rs(r.taker_pays_funded??r.TakerPays).value;return l<=0||c<=0?null:{gets:l,pays:c,quality:c/l}}).filter(Boolean).sort((r,l)=>r.quality-l.quality)}function Rc(e){if(!e.length)return{bestPrice:null,levels:[]};let t=e[0].quality,n=Rv.map(s=>{let a=t*(1+s/100),o=0,i=0,r=!0;for(let l of e){if(l.quality>a){r=!1;break}o+=l.pays,i+=l.gets}return{pct:s,xrpAtOffer:o,tokenAtOffer:i,bookExhausted:r}});return{bestPrice:t,levels:n}}async function Dv(e,t){try{let[n,s]=await Promise.all([_c(e,t,"buy"),_c(e,t,"sell")]);if(!n.length&&!s.length)return{exists:!1};let a=Rc(s.map(l=>({...l,gets:l.pays,pays:l.gets}))),o=Rc(n),i=a.bestPrice?1/a.bestPrice:null,r=o.bestPrice&&i?(o.bestPrice-i)/i*100:null;return{exists:!0,buySideOfferCount:n.length,sellSideOfferCount:s.length,buyPriceXrp:o.bestPrice,sellPriceXrp:i,spreadPct:r,buyDepth:o.levels,sellDepth:a.levels}}catch{return{exists:!1,error:!0}}}var li={liquidity:31.25,distribution:18.75,marketQuality:18.75,lpStability:18.75,issuerRisk:12.5};function Fc(e){return Math.max(0,Math.min(100,e))}function is(e,t,n){return n===t?0:Fc((e-t)/(n-t)*100)}function Iv({amm:e,orderBook:t,holders:n,lp:s,issuer:a}){var p;let o={},i=e!=null&&e.exists?e.xrpReserve*2:0,r=t!=null&&t.exists&&((p=t.buyDepth.find(m=>m.pct===2))==null?void 0:p.xrpAtOffer)||0;o.liquidity={score:Math.round((is(i,0,1e5)+is(r,0,2e4))/2),inputs:{ammTvlXrp:i,depthTo2PctXrp:r,ammExists:!!(e!=null&&e.exists),orderBookExists:!!(t!=null&&t.exists)}},o.distribution={score:n!=null&&n.exists?Math.round(is(n.top10Pct,90,20)):0,inputs:{top1Pct:(n==null?void 0:n.top1Pct)??null,top5Pct:(n==null?void 0:n.top5Pct)??null,top10Pct:(n==null?void 0:n.top10Pct)??null,holderCount:(n==null?void 0:n.holderCount)??0,sampleOnly:(n==null?void 0:n.sampleOnly)??!0}};let l=t!=null&&t.exists?Math.abs(t.spreadPct??100):100;o.marketQuality={score:t!=null&&t.exists?Math.round(is(l,15,0)):0,inputs:{spreadPct:(t==null?void 0:t.spreadPct)??null,buySideOfferCount:(t==null?void 0:t.buySideOfferCount)??0,sellSideOfferCount:(t==null?void 0:t.sellSideOfferCount)??0}},o.lpStability={score:s!=null&&s.exists?Math.round((is(s.top1Pct,100,20)+is(s.holderCount,1,25))/2):0,inputs:{top1Pct:(s==null?void 0:s.top1Pct)??null,top5Pct:(s==null?void 0:s.top5Pct)??null,holderCount:(s==null?void 0:s.holderCount)??0}};let c=100,d={clawbackEnabled:!1,globalFreeze:!1,freezeCapable:!1,requireAuth:!1,blackholed:!1};a!=null&&a.exists?(d.clawbackEnabled=a.clawbackEnabled,d.globalFreeze=a.globalFreeze,d.freezeCapable=a.freezeCapable,d.requireAuth=a.requireAuth,d.blackholed=a.blackholed,a.clawbackEnabled&&(c-=30),a.globalFreeze&&(c-=25),a.freezeCapable&&(c-=10),a.requireAuth&&(c-=10),a.blackholed&&(c+=15)):c=0,o.issuerRisk={score:Fc(Math.round(c)),inputs:d};let u=Object.entries(li).reduce((m,[f,v])=>m+o[f].score*v/100,0);return{overall:Math.round(u),subScores:o,weights:li,dimensionsLive:Object.keys(li),dimensionsDeferred:["networkActivity","treasuryHealth","transparency"]}}async function Oc(e){let t=Mv(e.currency||e.symbol),n=e.issuer;if(!n)throw new Error("This token has no issuer address \u2014 Project Intelligence only applies to issued tokens, not XRP itself.");let[s,a,o,i]=await Promise.all([Av(t,n),Ev(n),Nv(n,t),Dv(t,n)]),r=s.exists&&s.account&&s.lpCurrency?await _v(s.account,s.lpCurrency):{exists:!1},l=Iv({amm:s,orderBook:i,holders:o,lp:r,issuer:a});return{token:{symbol:e.symbol,name:e.name,issuer:n,currency:t},amm:s,issuerRisk:a,holders:o,lp:r,orderBook:i,strength:l,fetchedAt:Date.now()}}var Zc="nalulf_wallets",ed="nalulf_profile",td="nalulf_social",Us="naluxrp_active_wallet",dn="nalulf_avatar_img",Nn="nalulf_banner_img",hi="nalulf_activity_log",Ci="nalulf_balhist_",Fv="nalulf_addr_book",Ov="https://xrplcluster.com/",Bv="https://s2.ripple.com:51234/",Xv=[e=>`https://corsproxy.io/?${encodeURIComponent(e)}`,e=>`https://api.allorigins.win/raw?url=${encodeURIComponent(e)}`],gi="claude-opus-5",Hv=[{id:"claude-opus-5",label:"Claude Opus 5 \u2014 most capable, highest cost"},{id:"claude-sonnet-5",label:"Claude Sonnet 5 \u2014 strong, cheaper"},{id:"claude-haiku-4-5",label:"Claude Haiku 4.5 \u2014 fastest, cheapest"}],zv="https://api.xrpscan.com/api/v1/tokens",Uv="https://api.coingecko.com/api/v3/coins/markets",jv="https://bithomp.com/api/v2/tokens",Wv="https://api.xrpl.to/api/tokens",qv=!1,Ba=10,Xa=2,Vv=["\u{1F30A}","\u{1F40B}","\u{1F409}","\u{1F98B}","\u{1F981}","\u{1F43A}","\u{1F98A}","\u{1F43B}","\u{1F43C}","\u{1F985}","\u{1F42C}","\u{1F988}","\u{1F419}","\u{1F991}","\u{1F9FF}","\u{1F33A}","\u{1F338}","\u{1F340}","\u26A1","\u{1F525}","\u{1F48E}","\u{1F319}","\u2B50","\u{1F3AF}","\u{1F9E0}","\u{1F52E}","\u{1F6F8}","\u{1F5FA}","\u{1F3D4}","\u{1F3AD}","\u{1F3DB}"],Gv=["\u{1F48E}","\u{1F3E6}","\u{1F510}","\u{1F511}","\u{1F4B0}","\u{1F30A}","\u26A1","\u{1F680}","\u{1F319}","\u2B50","\u{1F3F4}\u200D\u2620\uFE0F","\u{1F3AF}","\u{1F9E0}","\u{1F52E}"],Kv=["#50fa7b","#00d4ff","#ffb86c","#bd93f9","#ff79c6","#f1fa8c","#ff5555","#00fff0","#ff6b6b","#a78bfa"],js=["banner-ocean","banner-neon","banner-gold","banner-cosmic","banner-sunset","banner-aurora"],ps=[{id:"discord",label:"Discord",icon:"\u{1F4AC}",prefix:"https://discord.com/users/"},{id:"twitter",label:"X / Twitter",icon:"\u{1D54F}",prefix:"https://x.com/"},{id:"linkedin",label:"LinkedIn",icon:"in",prefix:"https://linkedin.com/in/"},{id:"github",label:"GitHub",icon:"\u2325",prefix:"https://github.com/"},{id:"telegram",label:"Telegram",icon:"\u2708",prefix:"https://t.me/"},{id:"facebook",label:"Facebook",icon:"f",prefix:"https://facebook.com/"},{id:"tiktok",label:"TikTok",icon:"\u266A",prefix:"https://tiktok.com/@"}],Jv={tecNO_DST:"Destination account does not exist \u2014 fund it with 10 XRP first.",tecINSUF_RESERVE_LINE:"Insufficient reserve to add another trustline.",tecINSUF_RESERVE_OFFER:"Insufficient reserve to place a DEX order.",tecUNFUNDED_PAYMENT:"Insufficient balance (including reserve).",tecDST_TAG_NEEDED:"This destination requires a Destination Tag.",tecNO_PERMISSION:"Account has DepositAuth enabled \u2014 destination must preauthorize.",temBAD_AMOUNT:"Invalid amount.",temBAD_CURRENCY:"Invalid currency code.",temBAD_ISSUER:"Invalid issuer address.",tefPAST_SEQ:"Sequence number already used \u2014 please retry.",terQUEUED:"Transaction queued \u2014 will be included in a future ledger."},Yv="https://cdn.jsdelivr.net/npm/xrpl@4.2.5/build/xrpl-latest-min.js",Ha=21e4,Da=null;function ci(e){return btoa(String.fromCharCode(...e))}function di(e){let t=atob(e),n=new Uint8Array(t.length);for(let s=0;s<t.length;s+=1)n[s]=t.charCodeAt(s);return n}async function ja(){var e,t;return(e=window.xrpl)!=null&&e.Wallet?!0:(Da||(Da=new Promise((n,s)=>{let a=document.querySelector('script[data-xrpl-lib="1"]');if(a){a.addEventListener("load",()=>n(!0),{once:!0}),a.addEventListener("error",()=>s(new Error("Failed to load xrpl.js")),{once:!0});return}let o=document.createElement("script");o.src=Yv,o.async=!0,o.defer=!0,o.dataset.xrplLib="1",o.onload=()=>n(!0),o.onerror=()=>s(new Error("Failed to load xrpl.js")),document.head.appendChild(o)}).finally(()=>{var n;(n=window.xrpl)!=null&&n.Wallet||(Da=null)})),await Da,!!((t=window.xrpl)!=null&&t.Wallet))}async function nd(e,t,n=Ha){let s=new TextEncoder,a=await crypto.subtle.importKey("raw",s.encode(e),{name:"PBKDF2"},!1,["deriveKey"]);return crypto.subtle.deriveKey({name:"PBKDF2",hash:"SHA-256",salt:t,iterations:n},a,{name:"AES-GCM",length:256},!1,["encrypt","decrypt"])}async function sd(e,t){let n=new TextEncoder,s=crypto.getRandomValues(new Uint8Array(16)),a=crypto.getRandomValues(new Uint8Array(12)),o=await nd(t,s,Ha),i=await crypto.subtle.encrypt({name:"AES-GCM",iv:a},o,n.encode(e));return{v:1,kdf:"PBKDF2-SHA256",iter:Ha,alg:"AES-GCM-256",salt:ci(s),iv:ci(a),ct:ci(new Uint8Array(i))}}async function Qv(e,t){if(!(e!=null&&e.ct)||!(e!=null&&e.salt)||!(e!=null&&e.iv))throw new Error("Wallet seed blob is invalid.");let n=await nd(t,di(e.salt),e.iter||Ha),s=await crypto.subtle.decrypt({name:"AES-GCM",iv:di(e.iv)},n,di(e.ct));return new TextDecoder().decode(s)}async function Zv(e,t){let n=(t||"").trim();if(n)return n;if(e!=null&&e.encSeed){let a=prompt("Enter your wallet password to decrypt and sign this transaction:");if(!a)throw new Error("Wallet password is required to sign.");try{return await Qv(e.encSeed,a)}catch{throw new Error("Could not decrypt wallet seed. Check your wallet password and try again.")}}let s=prompt("Enter the wallet seed to sign this transaction (used once, not stored):");if(!s)throw new Error("Seed phrase is required to sign transactions.");return s.trim()}var le={displayName:"",handle:"",bio:"",location:"",website:"",avatar:"\u{1F30A}",banner:"banner-ocean",joinedDate:new Date().toISOString(),domain:""},oe=[],je={},it=null,Ke={},Pi={},Li={},ad={},Mi={},Xs={},od={},Te={loading:!0,data:null,error:""},mt={loading:!0,items:[],error:""},ft={loading:!0,pools:[],error:""},kt={loading:!0,pools:[],error:""},Re={loading:!1,pool:null,error:""},C={pair:"BITSTAMP:XRPUSD",interval:"15",chartType:"candles",stats:null,loading:!0,error:"",comparePair:"",indicators:{sma20:!0,ema20:!1,wma20:!1,bb20:!1,vwap:!1,ichimoku:!1,macd:!1,rsi:!1,atr:!1,adx:!1,aroon:!1,cci:!1,williamsr:!1,mfi:!1,obv:!1,adline:!1,cmf:!1,stoch:!1,uo:!1,stdev:!1,donchian:!1,keltner:!1,supertrend:!1,pivots:!1,sar:!1,vortex:!1,elderRay:!1},tokenFocusKey:"",drawingTool:"none",drawings:[],alerts:[],indicatorMenuOpen:!1,moreMenuOpen:!1,indicatorQuery:"",indicatorSettings:{},settingsOpenFor:null,threeEnabled:!0,selectedIndicator:"sma20",selectedEducationTab:"indicator",educationCollapsed:!1,educationHint:"",chartMeta:{tokenKey:"XRP|",symbol:"XRP",source:"Coinbase + XRPL live",last:null,high:null,low:null,mode:"pair"}},j={loading:!0,tokens:[],filtered:[],trending:[],error:"",query:"",total:0,lastSyncAt:0,filters:{type:"all",minCap:0,minVol:0,hasDex:!1},selectedTokenKey:"",listLimit:240},xt={loading:!0,items:[],error:""},Qe={loading:!1,error:"",data:null,tokenKey:"",expandedSubScore:""},Bc=new Map,Ia=null,Fa=null,Xc=!1,Hc=0,id=0,Hs=null,zc={xrplto:0},ee={chart:null,volumeSeries:null,activeSeries:null,compareSeries:null,indicatorSeries:[],indicatorPriceLines:[],priceLines:[],resizeObserver:null,chartType:"",configKey:"",legendEl:null,renderLegend:null,ichimokuData:null,indicatorLegendItems:[],alertPriceLines:[]},Ue={renderer:null,scene:null,camera:null,points:null,raf:0,host:null,resizeHandler:null},zs=new Map,Mn=0,pi="",eb="naluxrp_hist_v1_",rd=60,ld=25,tb=200,nb=e=>new Promise(t=>setTimeout(t,e)),vi="naluxrp_seed_backed_up",cd="naluxrp_token_watchlist",bi="naluxrp_chart_layout",dd="naluxrp_selected_token",Ai="naluxrp_chart_3d",pd="naluxrp_price_alerts";function za(){return O.currentPage==="profile"&&!document.hidden}var sb=[{symbol:"XRP",name:"XRP Ledger Native",marketCap:"$124.3B",source:"CoinGecko"},{symbol:"RLUSD",name:"Ripple USD",marketCap:"$312.0M",source:"Static sample"},{symbol:"SOLOGENIC",name:"Sologenic",marketCap:"$96.4M",source:"Static sample"}],_n=[{id:"BITSTAMP:XRPUSD",label:"XRP / USD (Coinbase + XRPL)",source:"coinbase",ticker:"xrpusd",symbol:"XRP",coingeckoId:"ripple"},{id:"BINANCE:XRPUSDT",label:"XRP / USD (Coinbase mirror)",source:"coinbase",ticker:"XRPUSDT",symbol:"XRP",coingeckoId:"ripple"},{id:"BINANCE:ETHUSDT",label:"ETH / USD (Coinbase)",source:"coinbase",ticker:"ETHUSDT",symbol:"ETH",coingeckoId:"ethereum"},{id:"BINANCE:BTCUSDT",label:"BTC / USD (Coinbase)",source:"coinbase",ticker:"BTCUSDT",symbol:"BTC",coingeckoId:"bitcoin"},{id:"BINANCE:SOLUSDT",label:"SOL / USD (Coinbase)",source:"coinbase",ticker:"SOLUSDT",symbol:"SOL",coingeckoId:"solana"}],ud=[{value:"1",label:"1m"},{value:"3",label:"3m"},{value:"5",label:"5m"},{value:"15",label:"15m"},{value:"30",label:"30m"},{value:"60",label:"1h"},{value:"120",label:"2h"},{value:"240",label:"4h"},{value:"D",label:"1D"},{value:"W",label:"1W"},{value:"M",label:"1M"}];var md=[{key:"none",label:"Cursor"},{key:"hline",label:"Horizontal Line"}],ab={trend:["sma20","ema20","wma20","ichimoku","adx","aroon","sar","supertrend","vortex","elderRay","macd"],momentum:["rsi","stoch","cci","williamsr","mfi","uo"],volume:["obv","adline","cmf","vwap"],volatility:["bb20","keltner","atr","donchian","stdev"],advanced:["pivots"]},Ut={sma20:{name:"SMA 20",what:"Simple average of closing prices over 20 periods.",purpose:"Baseline trend smoothing.",apply:"Use slope and price relation for trend confirmation.",mistake:"Assuming one crossover equals full trend reversal.",bias:"Check higher timeframe trend first."},ema20:{name:"EMA 20",what:"Weighted moving average that reacts faster.",purpose:"Track momentum shifts early.",apply:"Use with structure breaks for continuation entries.",mistake:"Overtrading every touch.",bias:"Wait for confirmation candle close."},wma20:{name:"WMA 20",what:"Linear weighted average favoring recent closes.",purpose:"Balance noise and reactivity.",apply:"Useful for dynamic pullback zones.",mistake:"Treating it as support in chop.",bias:"Confirm with volatility context."},ichimoku:{name:"Ichimoku Cloud",what:"Multi-line trend, momentum, and support/resistance framework.",purpose:"One-glance regime detection.",apply:"Favor trades aligned with cloud direction and conversion/base line confluence.",mistake:"Ignoring lagging span context.",bias:"Only take signals in clear trend phases."},macd:{name:"MACD",what:"Difference between fast and slow EMAs with signal line.",purpose:"Momentum and trend acceleration.",apply:"Use histogram contraction/expansion and line cross with structure.",mistake:"Late entries from isolated crosses.",bias:"Match cross direction with market structure."},rsi:{name:"RSI",what:"Relative strength oscillator from 0-100.",purpose:"Momentum strength and exhaustion.",apply:"40-80 bull range, 20-60 bear range is often more useful than 30/70 alone.",mistake:"Shorting every overbought reading in uptrends.",bias:"Use RSI with trend filters and divergence context."},stoch:{name:"Stochastic",what:"Close location relative to recent range.",purpose:"Short-term momentum turns.",apply:"Best in ranges or pullbacks within trend.",mistake:"Treating every cross as signal.",bias:"Require structure or support/resistance confluence."},cci:{name:"CCI",what:"Deviation from statistical mean.",purpose:"Identify cyclical overextensions.",apply:"Look for trend-aligned re-entry after reset.",mistake:"Using fixed thresholds in all regimes.",bias:"Adapt thresholds to volatility."},williamsr:{name:"Williams %R",what:"Inverse stochastic oscillator.",purpose:"Range extremes and momentum snapbacks.",apply:"Use with market regime filter.",mistake:"Fading trends blindly.",bias:"Avoid countertrend trades without invalidation levels."},mfi:{name:"MFI",what:"Volume-weighted RSI style oscillator.",purpose:"Money flow pressure.",apply:"Combine with volume spikes for conviction.",mistake:"Ignoring thin liquidity distortions.",bias:"Cross-check on multiple venues when possible."},uo:{name:"Ultimate Oscillator",what:"Weighted momentum across multiple windows.",purpose:"Reduce single-window false signals.",apply:"Divergences can be strong with structure breaks.",mistake:"Using without trend filter.",bias:"Require two independent confirmations."},obv:{name:"On Balance Volume",what:"Cumulative signed volume.",purpose:"Volume pressure trend.",apply:"Look for OBV breaks before price breaks.",mistake:"Trusting OBV in sparse data periods.",bias:"Check liquidity quality first."},adline:{name:"Accumulation/Distribution",what:"Volume flow using close location in candle.",purpose:"Detect stealth accumulation/distribution.",apply:"Use for divergence against price trend.",mistake:"Ignoring wick distortions.",bias:"Validate with average volume regime."},cmf:{name:"Chaikin Money Flow",what:"Normalized accumulation/distribution over window.",purpose:"Money flow bias.",apply:"Sustained above/below zero is more meaningful than single crosses.",mistake:"Reacting to one-bar flips.",bias:"Use persistence thresholds."},vwap:{name:"VWAP",what:"Volume weighted average price.",purpose:"Institutional execution benchmark.",apply:"Use as intraday mean reversion or trend continuation anchor.",mistake:"Ignoring session resets.",bias:"Define session context explicitly."},bb20:{name:"Bollinger Bands",what:"Moving average with standard deviation envelopes.",purpose:"Volatility expansion/contraction.",apply:"Squeezes can precede breakouts; walks indicate trend.",mistake:"Assuming every upper-band touch is sell.",bias:"Pair with trend and volume confirmation."},keltner:{name:"Keltner Channel",what:"EMA center with ATR-based envelopes.",purpose:"Trend-aware volatility channel.",apply:"Break/hold beyond band can mark trend strength.",mistake:"Using fixed ATR multiplier everywhere.",bias:"Tune per asset volatility."},atr:{name:"ATR",what:"Average true range.",purpose:"Position sizing and stop calibration.",apply:"Use ATR multiples for stops/targets.",mistake:"Using fixed pip stops in all regimes.",bias:"Normalize risk by volatility."},donchian:{name:"Donchian Channels",what:"Highest high / lowest low bands.",purpose:"Breakout systems.",apply:"Use channel breaks with trend filter.",mistake:"Ignoring false breakout environment.",bias:"Wait for close confirmation."},stdev:{name:"Standard Deviation",what:"Dispersion of price from mean.",purpose:"Volatility regime shifts.",apply:"Expand risk controls during high dispersion.",mistake:"Mistaking volatility for direction.",bias:"Separate volatility from trend."},adx:{name:"ADX",what:"Trend strength metric independent of direction.",purpose:"Regime filter.",apply:"Use +DI/-DI with ADX slope.",mistake:"Trading direction off ADX alone.",bias:"Combine with directional structure."},aroon:{name:"Aroon",what:"Time since highs/lows.",purpose:"Trend emergence detection.",apply:"Aroon up/down crosses near extremes can flag regime shifts.",mistake:"Using in high-noise micro ranges.",bias:"Require multi-candle confirmation."},pivots:{name:"Pivot Points",what:"Session-based support/resistance levels.",purpose:"Map likely reaction zones.",apply:"Use confluence with order flow and trend.",mistake:"Treating pivots as guaranteed reversal levels.",bias:"Plan invalidation before entry."},sar:{name:"Parabolic SAR",what:"Trailing stop indicator with acceleration factor.",purpose:"Trend trailing and stop logic.",apply:"Works best in persistent trends.",mistake:"Using in sideways chop.",bias:"Filter with ADX or structure."},supertrend:{name:"Supertrend",what:"ATR-based trend-following overlay.",purpose:"Trend direction and trailing stop.",apply:"Follow flips when volatility supports continuation.",mistake:"Chasing every flip in range.",bias:"Use higher timeframe confirmation."},vortex:{name:"Vortex",what:"Positive/negative trend movement lines.",purpose:"Trend turning points.",apply:"Crosses with expansion can mark trend shifts.",mistake:"Ignoring low-liquidity noise.",bias:"Confirm with volume and structure."},elderRay:{name:"Elder Ray",what:"Bull/Bear power versus EMA baseline.",purpose:"Pressure around trend mean.",apply:"Look for divergence and trend continuation.",mistake:"Using without baseline trend direction.",bias:"Anchor decisions to trend context."}},Ei={sma20:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Color",type:"color",default:"#f1c40f"}],ema20:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Color",type:"color",default:"#ffb86c"}],wma20:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Color",type:"color",default:"#bd93f9"}],vwap:[{id:"color",label:"Color",type:"color",default:"#80ffea"}],bb20:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Band Color",type:"color",default:"#ff79c6"}],ichimoku:[{id:"tenkanLen",label:"Tenkan Length",type:"number",default:9},{id:"kijunLen",label:"Kijun Length",type:"number",default:26},{id:"senkouBLen",label:"Senkou B Length",type:"number",default:52},{id:"tenkanColor",label:"Tenkan Color",type:"color",default:"#ffde59"},{id:"kijunColor",label:"Kijun Color",type:"color",default:"#6ecbff"},{id:"bullColor",label:"Cloud Bull Color",type:"color",default:"#46ffa0"},{id:"bearColor",label:"Cloud Bear Color",type:"color",default:"#ff7878"}],donchian:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Color",type:"color",default:"#9cfb8c"}],keltner:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Color",type:"color",default:"#7ee7ff"}],supertrend:[{id:"color",label:"Color",type:"color",default:"#8bffde"}],sar:[{id:"color",label:"Color",type:"color",default:"#ffaf7a"}],elderRay:[{id:"bullColor",label:"Bull Color",type:"color",default:"#5fff9d"},{id:"bearColor",label:"Bear Color",type:"color",default:"#ff9d9d"}],rsi:[{id:"length",label:"Length",type:"number",default:14},{id:"color",label:"Color",type:"color",default:"#a6ff4d"}],atr:[{id:"length",label:"Length",type:"number",default:14},{id:"color",label:"Color",type:"color",default:"#ffb86c"}],stdev:[{id:"color",label:"Color",type:"color",default:"#b2a3ff"}],stoch:[{id:"length",label:"Length",type:"number",default:14},{id:"kColor",label:"%K Color",type:"color",default:"#9ee8ff"},{id:"dColor",label:"%D Color",type:"color",default:"#ffd86b"}],macd:[{id:"fastLen",label:"Fast Length",type:"number",default:12},{id:"slowLen",label:"Slow Length",type:"number",default:26},{id:"signalLen",label:"Signal Length",type:"number",default:9},{id:"lineColor",label:"MACD Color",type:"color",default:"#8fd9ff"},{id:"signalColor",label:"Signal Color",type:"color",default:"#ffcf8e"}],obv:[{id:"color",label:"Color",type:"color",default:"#8cf9ff"}],adline:[{id:"color",label:"Color",type:"color",default:"#ffb7ff"}],cmf:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Color",type:"color",default:"#f8ff87"}],williamsr:[{id:"length",label:"Length",type:"number",default:14},{id:"color",label:"Color",type:"color",default:"#ff9adf"}],cci:[{id:"length",label:"Length",type:"number",default:20},{id:"color",label:"Color",type:"color",default:"#b8ff8e"}],mfi:[{id:"length",label:"Length",type:"number",default:14},{id:"color",label:"Color",type:"color",default:"#7bffd2"}],uo:[{id:"color",label:"Color",type:"color",default:"#ffd36f"}],adx:[{id:"length",label:"Length",type:"number",default:14},{id:"adxColor",label:"ADX Color",type:"color",default:"#9fd8ff"},{id:"plusColor",label:"+DI Color",type:"color",default:"#73ffc0"},{id:"minusColor",label:"-DI Color",type:"color",default:"#ff9797"}],aroon:[{id:"length",label:"Length",type:"number",default:14},{id:"upColor",label:"Up Color",type:"color",default:"#6cffb0"},{id:"downColor",label:"Down Color",type:"color",default:"#ff8f8f"}],vortex:[{id:"length",label:"Length",type:"number",default:14},{id:"plusColor",label:"VI+ Color",type:"color",default:"#d6a8ff"},{id:"minusColor",label:"VI- Color",type:"color",default:"#ffb0f3"}]},ob={sma20:{creator:"Early quantitative analysts (1900s tape reading era)",era:"Formalized in the early 20th century",math:"Arithmetic mean of the last N closes.",context:"Designed to smooth noisy tape data for trend direction visibility.",regime:"Best in directional trends, weaker in mean-reverting chop."},ema20:{creator:"Modern technical analysts adapting exponential smoothing",era:"Popularized in 1960s-1980s",math:"Recursive weighted mean with alpha = 2/(N+1).",context:"Improves responsiveness versus SMA while preserving trend structure.",regime:"Useful for pullback entries in trending environments."},ichimoku:{creator:"Goichi Hosoda",era:"Developed pre-WW2, published 1969",math:"Median-price lines (9/26/52) plus shifted cloud projections.",context:"Built as a full market regime system: trend, momentum, support/resistance in one frame.",regime:"Most reliable when cloud slope and price acceptance align."},macd:{creator:"Gerald Appel",era:"Late 1970s",math:"MACD = EMA(12)-EMA(26), signal=EMA(9) of MACD, histogram=spread.",context:"Tracks trend acceleration/deceleration, not just direction.",regime:"Strong in trend transitions, noisy in low-volatility ranges."},rsi:{creator:"J. Welles Wilder Jr.",era:"1978",math:"RSI = 100 - 100/(1+RS), RS = avg gain / avg loss.",context:"Measures internal momentum pressure rather than price level alone.",regime:"Range shifts (bull/bear RSI zones) matter more than static 30/70."},adx:{creator:"J. Welles Wilder Jr.",era:"1978",math:"Smoothed directional movement (+DI/-DI) transformed into trend-strength index.",context:"Separates trend strength from trend direction.",regime:"Filter trades: momentum systems improve when ADX slope rises."},aroon:{creator:"Tushar Chande",era:"1995",math:"Time since recent high/low scaled to 0-100.",context:"Focuses on trend freshness instead of pure magnitude.",regime:"Good at identifying emergent trend phases and late-trend fatigue."},cci:{creator:"Donald Lambert",era:"1980",math:"Deviation of typical price from moving average normalized by mean deviation.",context:"Originally commodity cycle tool for identifying statistical extremes.",regime:"Works better with volatility-aware thresholds than fixed +/-100."},williamsr:{creator:"Larry Williams",era:"1970s",math:"Position of close within rolling high-low range, scaled negative.",context:"Fast oscillator for short-horizon exhaustion and reversion timing.",regime:"Most effective in bounded ranges; trend filters prevent fade traps."},mfi:{creator:"Gene Quong and Avrum Soudack",era:"1989",math:"RSI-style transform using typical price * volume money flow.",context:"Adds participation/volume dimension to momentum analysis.",regime:"Useful where volume quality is high; weaker on fragmented liquidity."},obv:{creator:"Joseph Granville",era:"1963",math:"Cumulative signed volume based on close direction.",context:"Detects accumulation/distribution before obvious price moves.",regime:"Best when confirmed with structure breaks and volume regime shifts."},vwap:{creator:"Institutional execution desks",era:"1980s electronic execution era",math:"Cumulative price*volume divided by cumulative volume.",context:"Execution benchmark and intraday fair-value reference.",regime:"Most meaningful intraday and around session anchor resets."},bb20:{creator:"John Bollinger",era:"1980s",math:"SMA +/- k * standard deviation.",context:"Captures volatility contraction/expansion around a mean.",regime:"Band walks imply trend persistence; squeezes imply potential expansion."},atr:{creator:"J. Welles Wilder Jr.",era:"1978",math:"Smoothed average of true range components.",context:"Volatility unit for risk sizing and adaptive stops.",regime:"Risk engine input rather than directional signal."},donchian:{creator:"Richard Donchian",era:"1940s-1950s",math:"Rolling highest-high and lowest-low channels.",context:"Classic breakout trend-following framework.",regime:"Performs in sustained directional moves, whipsaws in compression."},supertrend:{creator:"Olivier Seban",era:"2009",math:"ATR envelope with trend-state switching logic.",context:"Simplifies trend-following and stop-trailing into one overlay.",regime:"Good in clean trends; combine with structure/ADX in chop."},vortex:{creator:"Etienne Botes and Douglas Siepman",era:"2010",math:"Normalized positive/negative movement vectors over rolling true range.",context:"Detects trend emergence and directional dominance shifts.",regime:"Improves when paired with volatility and liquidity filters."}},ib={hline:"Horizontal levels represent reaction zones. Respect zone width and liquidity sweeps."},rb=[{label:"XRP/USD (Bitstamp)",asset:{currency:"XRP"},asset2:{currency:"USD",issuer:"rhub8VRN55s94qWKDv6jmDy1pUykJzF3wq"}},{label:"XRP/EUR (Bitstamp)",asset:{currency:"XRP"},asset2:{currency:"EUR",issuer:"rhub8VRN55s94qWKDv6jmDy1pUykJzF3wq"}},{label:"XRP/USDC (Gatehub)",asset:{currency:"XRP"},asset2:{currency:"USDC",issuer:"rKveEyR1SrkWbJX214xcfH43ZsoGMb3PEv"}}],An=null,cn={},ls="",ht=1,we={algo:"ed25519",label:"",emoji:"\u{1F48E}",color:"#50fa7b",seed:"",address:"",passphrase:""},En=new Set,Uc=60,lb={wallet_created:"\u{1F48E}",wallet_removed:"\u{1F5D1}",social_connected:"\u{1F517}",social_removed:"\u2715",profile_saved:"\u270F\uFE0F",trustline_added:"\u{1F517}",sent:"\u2B06",received:"\u2B07",vault_created:"\u{1F510}",backup_exported:"\u{1F4C2}",theme_changed:"\u{1F3A8}",wallet_imported:"\u{1F511}",watch_added:"\u{1F441}"};function fd(){yi(),t0(),mb(),C.threeEnabled=Z(Ai)!=="0",cb(),ja().catch(()=>{he("Could not preload xrpl.js. Wallet generation/signing will require network access when used.")}),q(),Nt(),l0(),pb(),za()&&cs({silent:!0}),window.addEventListener("naluxrp:pagechange",e=>{var t;((t=e==null?void 0:e.detail)==null?void 0:t.pageId)==="profile"?cs({silent:!0}):Ue.raf&&Ks()}),document.addEventListener("click",e=>{var s;let t=document.querySelector(".xpd-indicator-menu-wrap");t&&!t.contains(e.target)&&C.indicatorMenuOpen&&(C.indicatorMenuOpen=!1,q());let n=document.querySelector(".xpd-more-menu-wrap");if(n&&!n.contains(e.target)&&C.moreMenuOpen&&(C.moreMenuOpen=!1,q()),C.settingsOpenFor){let a=(s=document.querySelector(".xpd-indicator-settings"))==null?void 0:s.closest(".xpd-indicator-chip-wrap");a&&!a.contains(e.target)&&(C.settingsOpenFor=null,q())}}),window.addEventListener("naluxrp:vault-ready",()=>{yi(),q(),Nt(),Wy(),cs({silent:!0,force:!0})}),window.addEventListener("naluxrp:vault-locked",()=>{q(),cs({silent:!0,force:!0})}),document.addEventListener("visibilitychange",()=>{if(za()&&!document.hidden&&oe.length){let e=oe.filter(t=>{let n=Ke[t.address];return!n||Date.now()-n.fetchedAt>3e5});e.length&&Promise.all(e.map(t=>Rn(t.address))).then(()=>{vt(),Nt()})}})}function cb(){let e=new URLSearchParams(window.location.search),t=e.get("pair")||"",n=e.get("tf")||"",s=e.get("token")||Z(dd)||"",a=e.get("ind")||"";if(t&&_n.some(o=>o.id===t)&&(C.pair=t),n&&ud.some(o=>o.value===n)&&(C.interval=n),a){let o=new Set(a.split(",").map(i=>i.trim()).filter(Boolean));Object.keys(C.indicators).forEach(i=>{C.indicators[i]=o.has(i)})}s&&(C.tokenFocusKey=s,j.selectedTokenKey=s)}function St(){let e=new URLSearchParams(window.location.search);e.set("pair",C.pair||"BITSTAMP:XRPUSD"),e.set("tf",C.interval||"15"),C.tokenFocusKey?e.set("token",C.tokenFocusKey):e.delete("token");let t=Object.entries(C.indicators).filter(([,s])=>!!s).map(([s])=>s).join(",");t?e.set("ind",t):e.delete("ind");let n=`${window.location.pathname}?${e.toString()}${window.location.hash||""}`;window.history.replaceState(null,"",n),C.tokenFocusKey&&te(dd,C.tokenFocusKey)}function db(){let e=document.getElementById("xpd-chart-section");e&&e.scrollIntoView({behavior:"smooth",block:"start"})}function pb(){Xc||(Xc=!0,window.addEventListener("xrpl-ledger",()=>{let e=Date.now();e-Hc<5e3||(Hc=e,O.currentPage==="profile"&&document.querySelector("#profile-page .profile-wrap .xrpl-profile-dashboard")&&Ed().then(()=>{if(Date.now()-id>15e3)return;let n=document.activeElement;n&&/^(INPUT|TEXTAREA|SELECT)$/.test(n.tagName)||C.moreMenuOpen||C.indicatorMenuOpen||(zs.clear(),q())}).catch(()=>{}))}))}function Ni(e){let t=y(`profile-tab-${e}`);t&&t.scrollIntoView({behavior:"smooth",block:"start"})}function ub(e,t,n){e.innerHTML=`<div class="tab-error-card">
    ...
    </div>`}function q(){var w,k;let e=document.querySelector("#profile-page .profile-wrap");if(!e)return;let t=document.getElementById("xpd-tv-widget"),n=!!(t&&ee.chart);t&&t.remove();let s=document.getElementById("xpd-chart-atmosphere"),a=!!(s&&Ue.renderer);s&&s.remove();let o=Tt(),i=(o==null?void 0:o.address)||"",r=hb(),l=localStorage.getItem(dn),c=oe.some(b=>!b.watchOnly),d=Z(vi)==="1",u=(w=Ke[i])==null?void 0:w.xrp,p=Object.values(Ke).reduce((b,x)=>b+((x==null?void 0:x.xrp)||0),0),m=(k=Te.data)!=null&&k.priceUsd?p*Te.data.priceUsd:0,f=_n.map(b=>`<option value="${g(b.id)}" ${C.pair===b.id?"selected":""}>${g(b.label)}</option>`).join(""),v=_n.map(b=>`<option value="${g(b.id)}" ${C.comparePair===b.id?"selected":""}>${g(b.label)}</option>`).join(""),h=ud.map(b=>`<option value="${g(b.value)}" ${C.interval===b.value?"selected":""}>${g(b.label)}</option>`).join("");if(e.innerHTML=`
    ...
    <div class="xpd-indicator-menu-wrap">
      ...
      ${C.indicatorMenuOpen?`
        <div class="xpd-indicator-menu" role="menu" aria-label="Indicator menu">
          ...
          ${t.map(n=>{let s=(ab[n.key]||[]).filter(a=>{var i;return e?(((i=Ut[a])==null?void 0:i.name)||a).toLowerCase().includes(e):!0});return s.length?`
    ...
    ${(()=>{let f=qa(),v=(C.alerts||[]).filter(h=>h.tokenKey===f);return v.length?`
      <div class="xpd-indicator-row">
        ${v.map(h=>`
    ...
    <p class="xpd-note">Professional chart controls: wheel zoom, hold-and-drag pan, horizontal price-line marking, and token-focused context ribbons for faster execution decisions.</p>`}function bb(){let e=C.selectedIndicator||"sma20",t=Ut[e]||Ut.sma20,n=ob[e]||null,s=C.educationCollapsed;return`
    ...
    <div class="xpd-token-strip">
      ${sb.map(n=>`<div class="xpd-token-pill" title="${g(`${n.symbol} ${n.name}`)}"><span class="sym xpd-pill-text">${n.symbol}</span><span class="xpd-pill-text">${g(n.name)}</span><span class="cap xpd-pill-text">${n.marketCap}</span></div>`).join("")}
    </div>
    <p class="xpd-note">Top XRPL token cards are static examples when direct DEX market-cap feeds are unavailable.</p>`}function wb(e){return e?mt.loading?'<div class="xpd-loading">Loading account NFTs...</div>':mt.error?`<div class="xpd-error">${g(mt.error)}</div>`:mt.items.length?`<div class="xpd-nft-grid">${mt.items.map(xb).join("")}</div>`:'<div class="xpd-empty">No NFTs found for this wallet.</div>':'<div class="xpd-empty">Select or create a wallet to load NFTs.</div>'}function xb(e){let t=`${e.id.slice(0,12)}...${e.id.slice(-10)}`;return`<article class="xpd-nft-card">
  ...
  </article>`}function kb(e){let t=_b(e),n=Rb(),s=Db();return`<div class="xpd-amm-columns">${t}${n}${s}</div>`}function Ri(){return(be(Z(cd))||[]).filter(Boolean)}function yd(e){let t=[...new Set((e||[]).filter(Boolean))];te(cd,JSON.stringify(t))}function $b(e,t){return t.has(e)?t.get(e):j.tokens.find(s=>s.symbol===e)||null}function Sb(){let e=j.query||"",t=Ri(),n=j.filtered,s=Math.max(120,Number(j.listLimit||240)),a=n.slice(0,s),o=j.trending.slice(0,14),i=new Map(j.tokens.map(c=>[He(c),c])),r=i.get(j.selectedTokenKey)||null,l=j.filters||{type:"all",minCap:0,minVol:0,hasDex:!1};return`
    <div class="xpd-token-grid">
      <div class="xpd-token-col">
        ...
        <div class="xpd-token-list">${a.map(c=>{let d=He(c),u=Bs(d),p=String(e||"").trim();return`
      ...
      <div class="xpd-token-col">
        ${r?`<div class="xpd-token-detail-card">
          ...
          <div class="xpd-row-actions">
            <button class="xpd-mini-btn" onclick="openTokenOnChart(decodeURIComponent('${Bs(He(r))}'))">Switch Main Chart</button>
            ${r.issuer?`<button class="xpd-mini-btn" onclick="window.open('https://xrpscan.com/account/${g(r.issuer)}','_blank')">View Issuer</button>`:""}
            ${r.issuer?`<button class="xpd-mini-btn xpd-mini-btn--accent" onclick="openProjectIntel(decodeURIComponent('${Bs(He(r))}'))">\u{1F52C} Project Strength</button>`:""}
          </div>
        </div>`:""}
        <h3>Watchlist</h3>
        <div class="xpd-watchlist">${t.length?t.map(c=>{let d=$b(c,i),u=d?`${d.symbol} \xB7 ${d.name}`:c,p=(d==null?void 0:d.price)!=null?`$${R(d.price,6)}`:"\u2014",m=Bs(c);return`<div class="xpd-token-row xpd-token-row--clickable" onclick="openTokenOnChart(decodeURIComponent('${m}'))"><span>${g(u)}</span><div class="xpd-token-actions"><span>${p}</span><button class="xpd-mini-btn" onclick="event.stopPropagation(); openTokenOnChart(decodeURIComponent('${m}'))">Chart</button><button class="xpd-mini-btn" onclick="event.stopPropagation(); removeTokenFromWatchlist(decodeURIComponent('${m}'))">Remove</button></div></div>`}).join(""):'<div class="xpd-empty">No watchlist tokens yet.</div>'}</div>
        <h3>Trending</h3>
        <div class="xpd-watchlist">${o.length?o.map(c=>`<div class="xpd-token-row xpd-token-row--clickable" onclick="openTokenOnChart(decodeURIComponent('${Bs(He(c))}'))"><span>${g(c.symbol)} \xB7 ${g(c.name)}</span><span>${c.volume24h!=null?`$${Ge(c.volume24h)} vol`:c.marketCap!=null?`$${Ge(c.marketCap)} mcap`:"\u2014"}</span></div>`).join(""):'<div class="xpd-empty">No trending data.</div>'}</div>
      </div>
    </div>`}function wd(){let e=document.getElementById("xpd-lookup-currency");if(!e){he("Token Discovery hasn't loaded yet \u2014 give it a moment and try again.");return}e.scrollIntoView({behavior:"smooth",block:"center"}),setTimeout(()=>e.focus(),350)}async function Di(e){let t=String(e||"").trim();if(!t)return;let n=t.includes("|")?j.tokens.find(s=>He(s)===t):j.tokens.find(s=>String(s.symbol||"").toUpperCase()===t.toUpperCase());if(!(n!=null&&n.issuer)){he("Project Intelligence needs a token with a known issuer.");return}Qe={loading:!0,error:"",data:null,tokenKey:He(n),expandedSubScore:""},q(),setTimeout(()=>{var s;return(s=document.getElementById("xpd-project-intel-section"))==null?void 0:s.scrollIntoView({behavior:"smooth",block:"start"})},30);try{Qe={loading:!1,error:"",data:await Oc(n),tokenKey:He(n),expandedSubScore:""}}catch(s){Qe={loading:!1,error:(s==null?void 0:s.message)||"Could not load project intelligence.",data:null,tokenKey:He(n),expandedSubScore:""}}q()}function xd(e){Qe.expandedSubScore=Qe.expandedSubScore===e?"":e,q()}function kd(){Qe={loading:!1,error:"",data:null,tokenKey:"",expandedSubScore:""},q()}var Tb={liquidity:{label:"Liquidity",icon:"\u{1F4A7}"},distribution:{label:"Distribution",icon:"\u{1F9EE}"},marketQuality:{label:"Market Quality",icon:"\u{1F4CA}"},lpStability:{label:"LP Stability",icon:"\u{1F3E6}"},issuerRisk:{label:"Issuer Risk",icon:"\u{1F6E1}"}};function ui(e){return e>=75?"#50fa7b":e>=50?"#f1fa8c":e>=25?"#ffb86c":"#ff5555"}function Cb(e){return e>=75?"STRONG":e>=50?"MODERATE":e>=25?"WEAK":"VERY WEAK"}function Pb(){if(Qe.loading)return`<div class="xpd-section-head"><h2>\u{1F52C} Project Intelligence</h2></div>
      ...
      <div class="xpd-error">${g(Qe.error)}</div>`;let e=Qe.data;if(!e)return"";let t=e.strength,n=ui(t.overall),s=t.dimensionsLive.map(a=>{let o=Tb[a],i=t.subScores[a],r=Qe.expandedSubScore===a;return`
      <div class="pi-subscore ${r?"pi-subscore--open":""}" onclick="toggleProjectIntelSubScore('${a}')">
        ...
        ${r?`<div class="pi-subscore-detail">${Object.entries(i.inputs).map(([l,c])=>`
          <div class="pi-kv"><span>${g($d(l))}</span><span class="mono">${g(Lb(c))}</span></div>
        `).join("")}</div>`:""}
      </div>`}).join("");return`
    <div class="xpd-section-head">
    ...
    <div class="pi-cards-grid">
      ...
      ${Eb(e.issuerRisk)}
    </div>`}function $d(e){return e.replace(/([A-Z])/g," $1").replace(/^./,t=>t.toUpperCase()).trim()}function Lb(e){return e==null?"\u2014":typeof e=="boolean"?e?"Yes":"No":typeof e=="number"?Number.isInteger(e)?String(e):R(e,2):String(e)}function Mb(e){var i;let t=e.amm,n=e.orderBook,s=t!=null&&t.exists?`
  ...
  `:'<div class="inspect-empty-note">No live order book found for this pair.</div>';return`
    ...
    <div class="finding finding--${s?a?"ok":"warn":"ok"}">
      ...
      <div class="finding-body">
        <div class="finding-label">${$d(n)}</div>
    ...
    </div>`}function _b(e){return e?ft.loading?'<div class="xpd-amm-card"><h3>Your liquidity positions</h3><div class="xpd-loading">Loading account_objects and LP balances...</div></div>':ft.error?`<div class="xpd-amm-card"><h3>Your liquidity positions</h3><div class="xpd-error">${g(ft.error)}</div></div>`:ft.pools.length?`<div class="xpd-amm-card"><h3>Your liquidity positions</h3><div class="xpd-pool-list">${ft.pools.map(t=>`
  ...
  </div>`}function Ge(e){return Number.isFinite(Number(e))?new Intl.NumberFormat("en-US",{notation:"compact",maximumFractionDigits:2}).format(Number(e)):"\u2014"}function ds(e){return typeof e=="string"?`${R(Number(e)/1e6,4)} XRP`:e&&typeof e=="object"?`${R(Number(e.value||0),4)} ${e.currency||"UNK"}`:"\u2014"}function Ib(e){if(!e||typeof e!="string")return"";try{let t=e.trim();if(!/^[0-9A-Fa-f]+$/.test(t)||t.length%2!==0)return t;let n=new Uint8Array(t.length/2);for(let s=0;s<t.length;s+=2)n[s/2]=parseInt(t.slice(s,s+2),16);return new TextDecoder().decode(n)}catch{return""}}function qc(e){return e?e.startsWith("ipfs://")?`https://ipfs.io/ipfs/${e.slice(7)}`:e.startsWith("ar://")?`https://arweave.net/${e.slice(5)}`:e:""}async function Fb(e){let t=qc(Ib(e.URI||e.uri||""));if(!t)return"";if(/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(t))return t;try{let n=await fetch(t,{method:"GET"});if(!n.ok)return"";let s=await n.json(),a=(s==null?void 0:s.image)||(s==null?void 0:s.image_url)||(s==null?void 0:s.thumbnail);return qc(a||"")}catch{return""}}async function $t(e,{timeoutMs:t=9e3,allowProxy:n=!0}={}){let s=async a=>{let o=typeof(AbortSignal==null?void 0:AbortSignal.timeout)=="function"?AbortSignal.timeout(t):void 0,i=await fetch(a,{method:"GET",mode:"cors",cache:"no-store",signal:o});if(!i.ok)throw new Error(`HTTP ${i.status}`);return await i.json()};try{return await s(e)}catch(a){if(!n)throw a;for(let o of Xv)try{return await s(o(e))}catch{}throw a}}async function Sd(){var e;Te.loading=!0,Te.error="",q();try{let t=await $t("https://api.exchange.coinbase.com/products/XRP-USD/ticker"),n=await $t("https://api.exchange.coinbase.com/products/XRP-USD/candles?granularity=86400"),s=Array.isArray(n)&&n.length?n[0]:null,a=Number((t==null?void 0:t.price)||0),o=s?Number(s[3]||0):a,i=s?Number(s[2]||a):a,r=s?Number(s[1]||a):a,l=s?Number(s[5]||0):0;Te.data={priceUsd:a,change24h:o?(a-o)/o*100:0,volume24h:l*a,marketCap:Number(((e=Te.data)==null?void 0:e.marketCap)||0),high24h:i,low24h:r}}catch(t){Te.error=(t==null?void 0:t.message)||"Could not load market data right now.",Te.data=null}finally{Te.loading=!1,q()}}async function Td(e){if(mt.loading=!0,mt.error="",mt.items=[],q(),!e){mt.loading=!1,q();return}try{let t,n=[];do{let a=await Ze({method:"account_nfts",params:[{account:e,limit:100,...t?{marker:t}:{}}]});n.push(...(a==null?void 0:a.account_nfts)||[]),t=a==null?void 0:a.marker}while(t);let s=await Promise.all(n.slice(0,80).map(async a=>({id:a.NFTokenID||a.nf_token_id||"Unknown",image:await Fb(a)})));mt.items=s}catch(t){mt.error=(t==null?void 0:t.message)||"Could not load NFTs for this wallet."}finally{mt.loading=!1,q()}}async function Cd(e){if(ft.loading=!0,ft.error="",ft.pools=[],q(),!e){ft.loading=!1,q();return}try{let t=[],n;do{let o=await Ze({method:"account_objects",params:[{account:e,type:"amm",limit:200,...n?{marker:n}:{}}]});((o==null?void 0:o.account_objects)||[]).forEach(r=>{let l=ds(r.Asset||r.amount),c=ds(r.Asset2||r.amount2),d=l.split(" ").pop()||"AssetA",u=c.split(" ").pop()||"AssetB";t.push({pair:`${d}/${u}`,lpBalance:r.LPTokenBalance?ds(r.LPTokenBalance):"Not reported",estimatedValue:"Estimate unavailable",tradingFee:r.TradingFee!=null?`${r.TradingFee} bps`:"\u2014",tvl:"Unavailable"})}),n=o==null?void 0:o.marker}while(n);let s=await Ze({method:"account_lines",params:[{account:e,limit:200}]});((s==null?void 0:s.lines)||[]).filter(o=>typeof o.currency=="string"&&o.currency.length>=16&&Number(o.balance)>0).forEach(o=>{t.push({pair:`LP Token ${o.currency.slice(0,8)}...`,lpBalance:`${R(Number(o.balance),4)} ${o.currency.slice(0,8)}...`,estimatedValue:"Estimate unavailable",tradingFee:"\u2014",tvl:"Unavailable"})}),ft.pools=t}catch(t){ft.error=(t==null?void 0:t.message)||"Could not load account AMM objects."}finally{ft.loading=!1,q()}}async function Pd(e){try{let t=await Ze({method:"amm_info",params:[{asset:e.asset,asset2:e.asset2}]}),n=t==null?void 0:t.amm;return n?{label:e.label,reserveA:ds(n.amount),reserveB:ds(n.amount2),tradingFee:String(n.trading_fee??"\u2014"),totalLp:ds(n.lp_token||n.lp_token_balance),tvl:Ob(n.amount,n.amount2)}:null}catch{return null}}function Ob(e,t){let n=a=>typeof a=="string"?Number(a)/1e6:a&&typeof a=="object"?Number(a.value||0):0,s=n(e)+n(t);return!Number.isFinite(s)||s<=0?"Unavailable":`${R(s,4)} (asset units)`}async function Ld(){kt.loading=!0,kt.error="",q();try{let e=await Promise.all(rb.map(Pd));kt.pools=e.filter(Boolean)}catch(e){kt.error=(e==null?void 0:e.message)||"Could not load AMM explorer data.",kt.pools=[]}finally{kt.loading=!1,q()}}async function Md(){var r,l,c,d;let e=(((r=document.getElementById("xpd-asset1-currency"))==null?void 0:r.value)||"").trim().toUpperCase(),t=(((l=document.getElementById("xpd-asset1-issuer"))==null?void 0:l.value)||"").trim(),n=(((c=document.getElementById("xpd-asset2-currency"))==null?void 0:c.value)||"").trim().toUpperCase(),s=(((d=document.getElementById("xpd-asset2-issuer"))==null?void 0:d.value)||"").trim();if(!e||!n){Re.error="Enter both asset currency codes first.",q();return}let a=e==="XRP"?{currency:"XRP"}:{currency:e,issuer:t},o=n==="XRP"?{currency:"XRP"}:{currency:n,issuer:s};if(e!=="XRP"&&!t||n!=="XRP"&&!s){Re.error="Issuer is required for non-XRP assets.",q();return}Re.loading=!0,Re.error="",Re.pool=null,q();let i=await Pd({label:`${e}/${n}`,asset:a,asset2:o});Re.loading=!1,i?Re.pool=i:Re.error="Pool not found or unavailable on this network.",q()}function Wa(){return _n.find(e=>e.id===C.pair)||_n[0]}function Ii(e){if(e==="D")return 1440;if(e==="W")return 10080;if(e==="M")return 43200;let t=Number(e);return Number.isFinite(t)&&t>0?t:60}function Bb(e){if(e==="M")return"W";if(e==="D"||e==="W")return e;let t=Ii(e);for(let n of[5,15,60,240])if(t<=n)return String(n);return"D"}async function Xb(e,t,n,s=200){var d;let a=t?`${e}.${t}`:e,o=Bb(n),i=Math.min(2e3,Math.max(20,Number(s)||200)),r=`https://api.onthedex.live/public/v1/ohlc?base=${encodeURIComponent(a)}&quote=XRP&interval=${o}&bars=${i}`,l=await $t(r,{timeoutMs:1e4});return(Array.isArray((d=l==null?void 0:l.data)==null?void 0:d.ohlc)?l.data.ohlc:[]).map(u=>({time:Number(u.t),open:Number(u.o),high:Number(u.h),low:Number(u.l),close:Number(u.c),volume:Number(u.vb||0)})).filter(u=>Number.isFinite(u.time)&&[u.open,u.high,u.low,u.close].every(Number.isFinite)).sort((u,p)=>u.time-p.time)}function Ad(e){return{xrpusd:"XRP-USD",XRPUSDT:"XRP-USD",ETHUSDT:"ETH-USD",BTCUSDT:"BTC-USD",SOLUSDT:"SOL-USD"}[e]||null}function Hb(e){return{XRP:"XRP-USD",BTC:"BTC-USD",ETH:"ETH-USD",SOL:"SOL-USD",USDC:"USDC-USD"}[String(e||"").toUpperCase()]||null}var mi=new Map;async function zb(e,t){if(!e)return null;let n=`${He(e)}:${t}`,s=mi.get(n);if(s)return s;let a=Ub(e,t);mi.set(n,a);try{return await a}finally{mi.delete(n)}}async function Ub(e,t){var o,i;let n=String(e.symbol||"").toUpperCase();if(!n||n==="XRP")return null;let s=Hb(n);if(s)try{let r=_d(t),l=r===86400?rd:ld,{candles:c,reachedStart:d}=await $i(s,r,l);if(c.length){let u=d?`${new Date(c[0].time*1e3).getUTCFullYear()}\u2013present`:`last ${c.length} bars`;return{candles:c,source:`Coinbase direct (${s}, ${u})`,mode:"token-direct"}}}catch{}let a=String(e.issuer||"").trim();if(a)try{let r=await Xb(n,a,t,2e3);if(r.length){let l=Number(((o=C.stats)==null?void 0:o.xrplSpot)||((i=C.stats)==null?void 0:i.price)||0);return{candles:l>0?r.map(d=>({time:d.time,open:d.open*l,high:d.high*l,low:d.low*l,close:d.close*l,volume:d.volume})):r,source:`OnTheDex XRPL DEX (${n}/XRP)`,mode:"token-direct"}}}catch{}return null}function jb(e,{mode:t="pair",source:n="Source pending",tokenKey:s=""}={}){let a=Array.isArray(e)?e:[];if(!a.length){C.chartMeta={tokenKey:s||C.tokenFocusKey||"XRP|",symbol:s&&s.split("|")[0]||"XRP",source:n,last:null,high:null,low:null,mode:t};return}let o=a[a.length-1],i=a.map(c=>Number(c.high)).filter(c=>Number.isFinite(c)),r=a.map(c=>Number(c.low)).filter(c=>Number.isFinite(c)),l=s||C.tokenFocusKey||"XRP|";C.chartMeta={tokenKey:l,symbol:l.split("|")[0]||"XRP",source:n,last:Number(o.close),high:i.length?Math.max(...i):Number(o.high),low:r.length?Math.min(...r):Number(o.low),mode:t}}async function Gs(){var t;let e=Wa();C.loading=!0,C.error="";try{let n=Ad(e.ticker);if(n){let s=await $t(`https://api.exchange.coinbase.com/products/${n}/ticker`),a=await $t(`https://api.exchange.coinbase.com/products/${n}/candles?granularity=86400`),o=Array.isArray(a)&&a.length?a[0]:null,i=Number(s.price||0),r=o?Number(o[3]):i;C.stats={price:i,high:o?Number(o[2]):i,low:o?Number(o[1]):i,changePct:r?(i-r)/r*100:0,baseSource:"Coinbase",source:"Coinbase"},await Ed(),!Te.data&&C.stats.price&&(Te.data={priceUsd:C.stats.price,change24h:C.stats.changePct,volume24h:o?Number(o[5]||0)*C.stats.price:0,marketCap:Number(((t=Te.data)==null?void 0:t.marketCap)||0)});return}throw new Error("Chart stats unavailable for selected pair.")}catch(n){C.error=(n==null?void 0:n.message)||"DEX chart stats unavailable right now."}finally{C.loading=!1}}async function Ed(){var e,t,n;try{let s=await Ze({method:"book_offers",params:[{taker_gets:{currency:"USD",issuer:"rhub8VRN55s94qWKDv6jmDy1pUykJzF3wq"},taker_pays:{currency:"XRP"},limit:1}]}),a=(e=s==null?void 0:s.offers)==null?void 0:e[0];if(!a)return;let o=Number(((t=a.TakerGets)==null?void 0:t.value)||0),i=typeof a.TakerPays=="string"?Number(a.TakerPays)/1e6:Number(((n=a.TakerPays)==null?void 0:n.value)||0);if(o>0&&i>0){let r=o/i;if(id=Date.now(),C.stats||(C.stats={}),C.stats.xrplSpot=r,String(Wa().symbol||"XRP").toUpperCase()==="XRP"){C.stats.price=r;let l=C.stats.baseSource||C.stats.source||"Market feed";C.stats.source=`${l} + XRPL live`,Te.data&&(Te.data.priceUsd=r)}}}catch{}}async function Wb(){var e,t;return(e=window.LightweightCharts)!=null&&e.createChart?!0:(Ia||(Ia=new Promise((n,s)=>{let a=document.querySelector('script[data-lw-chart="1"]');if(a){a.addEventListener("load",()=>n(!0),{once:!0}),a.addEventListener("error",()=>s(new Error("Chart library failed to load.")),{once:!0});return}let o=document.createElement("script");o.src="https://unpkg.com/lightweight-charts@4.2.2/dist/lightweight-charts.standalone.production.js",o.async=!0,o.defer=!0,o.dataset.lwChart="1",o.onload=()=>n(!0),o.onerror=()=>s(new Error("Chart library failed to load.")),document.head.appendChild(o)}).finally(()=>{var n;(n=window.LightweightCharts)!=null&&n.createChart||(Ia=null)})),await Ia,!!((t=window.LightweightCharts)!=null&&t.createChart))}async function qb(){var e,t;return(e=window.THREE)!=null&&e.Scene?!0:(Fa||(Fa=new Promise((n,s)=>{let a=document.querySelector('script[data-three-chart="1"]');if(a){a.addEventListener("load",()=>n(!0),{once:!0}),a.addEventListener("error",()=>s(new Error("Three.js failed to load.")),{once:!0});return}let o=document.createElement("script");o.src="https://unpkg.com/three@0.166.1/build/three.min.js",o.async=!0,o.defer=!0,o.dataset.threeChart="1",o.onload=()=>n(!0),o.onerror=()=>s(new Error("Three.js failed to load.")),document.head.appendChild(o)}).finally(()=>{var n;(n=window.THREE)!=null&&n.Scene||(Fa=null)})),await Fa,!!((t=window.THREE)!=null&&t.Scene))}function Ks(){var e,t;if(Ue.resizeHandler)try{window.removeEventListener("resize",Ue.resizeHandler)}catch{}if(Ue.raf&&cancelAnimationFrame(Ue.raf),(t=(e=Ue.renderer)==null?void 0:e.domElement)!=null&&t.parentElement)try{Ue.renderer.domElement.parentElement.removeChild(Ue.renderer.domElement)}catch{}if(Ue.renderer)try{Ue.renderer.dispose()}catch{}Ue={renderer:null,scene:null,camera:null,points:null,raf:0,host:null,resizeHandler:null}}async function Vb(){var t;let e=document.getElementById("xpd-chart-atmosphere");if(e&&!(Ue.renderer&&e.contains(Ue.renderer.domElement))&&!((navigator.hardwareConcurrency||4)<=3))try{if(await qb(),!((t=window.THREE)!=null&&t.Scene))return;Ks();let n=window.THREE,s=Math.max(1,e.clientWidth||640),a=Math.max(1,e.clientHeight||460),o=new n.Scene,i=new n.PerspectiveCamera(52,s/a,.1,1e3);i.position.z=46;let r=new n.WebGLRenderer({alpha:!0,antialias:!0});r.setPixelRatio(Math.min(2,window.devicePixelRatio||1)),r.setSize(s,a),e.appendChild(r.domElement);let l=900,c=new Float32Array(l*3);for(let w=0;w<l;w+=1){let k=w*3;c[k]=(Math.random()-.5)*90,c[k+1]=(Math.random()-.5)*40,c[k+2]=(Math.random()-.5)*24}let d=new n.BufferGeometry;d.setAttribute("position",new n.BufferAttribute(c,3));let u=new n.PointsMaterial({color:5101823,size:.25,transparent:!0,opacity:.26,depthWrite:!1}),p=new n.Points(d,u);o.add(p);let m=0,f=0,v=()=>{var S;m+=1;let w=Number(((S=C.stats)==null?void 0:S.changePct)||0),k=Math.min(4,Math.max(.35,Math.abs(w)/2.2)),b=w>0?16758874:16739999;u.color.setHex(Math.abs(w)>1.5?b:5101823),p.rotation.y+=9e-4*k,p.rotation.x=Math.sin(m*.0015*k)*.08,p.position.y=Math.sin(m*.003*k)*.7,(!document.hidden||Date.now()-f>350)&&(r.render(o,i),f=Date.now()),Ue.raf=requestAnimationFrame(v)},h=()=>{let w=Math.max(1,e.clientWidth||640),k=Math.max(1,e.clientHeight||460);i.aspect=w/k,i.updateProjectionMatrix(),r.setSize(w,k)};window.addEventListener("resize",h,{passive:!0}),Ue={renderer:r,scene:o,camera:i,points:p,raf:0,host:e,resizeHandler:h},v()}catch{}}function Nd(e){return Ii(e)*60}function _d(e){let t=Ii(e);return t<=1?60:t<=5?300:t<=15?900:t<=60?3600:t<=360?21600:86400}function Rd(){let e=String(C.tokenFocusKey||"");if(!e)return null;let t=e.includes("|")?e.split("|")[0]:e;return j.tokens.find(n=>He(n)===e)||j.tokens.find(n=>String(n.symbol||"").toUpperCase()===String(t||"").toUpperCase())||null}function Dd(){var a,o;let e=Rd(),t=Number((e==null?void 0:e.price)||0);if(Number.isFinite(t)&&t>0)return t;let n=Number(((a=C.stats)==null?void 0:a.xrplSpot)||0);if(Number.isFinite(n)&&n>0)return n;let s=Number(((o=C.stats)==null?void 0:o.price)||0);return Number.isFinite(s)&&s>0?s:1}function Id(e,t=1,n=220){let s=Math.max(60,Nd(e)),a=Math.floor(Date.now()/1e3),o=Math.floor(a/s)*s-n*s,i=Math.max(1e-6,Number(t)||1),r=[];for(let l=0;l<n;l+=1){let c=o+l*s,d=i,u=Math.sin((l+1)/9)*.004+Math.cos((l+3)/17)*.002;i=Math.max(1e-6,d*(1+u));let p=Math.max(d,i)*(1+Math.abs(u)*.55+.0015),m=Math.min(d,i)*Math.max(1e-6,1-(Math.abs(u)*.55+.0015));r.push({time:c,open:Number(d),high:Number(p),low:Number(m),close:Number(i),volume:Math.max(0,Math.round(Math.abs(u)*12e3+l%13*55))})}return r}function wi(e){if(!e.length)return e;let t=[];for(let n=0;n<e.length;n+=1){let s=e[n],a=(s.open+s.high+s.low+s.close)/4,o=t[n-1],i=o?(o.open+o.close)/2:(s.open+s.close)/2,r=Math.max(s.high,i,a),l=Math.min(s.low,i,a);t.push({...s,open:i,high:r,low:l,close:a})}return t}function xi(e,t){let n=[],s=0;for(let a=0;a<e.length;a+=1)s+=e[a].close,a>=t&&(s-=e[a-t].close),a>=t-1&&n.push({time:e[a].time,value:s/t});return n}function qs(e,t){let n=[];if(!e.length)return n;let s=2/(t+1),a=e[0].close;for(let o=0;o<e.length;o+=1)a=o===0?e[o].close:e[o].close*s+a*(1-s),o>=t-1&&n.push({time:e[o].time,value:a});return n}function Gb(e,t){let n=[],s=t*(t+1)/2;for(let a=t-1;a<e.length;a+=1){let o=0;for(let i=0;i<t;i+=1)o+=e[a-i].close*(t-i);n.push({time:e[a].time,value:o/s})}return n}function Kb(e,t=20,n=2){let s=xi(e,t),a=[],o=[];for(let i=t-1;i<e.length;i+=1){let r=e.slice(i-t+1,i+1),l=s[i-(t-1)].value,c=r.reduce((u,p)=>u+Math.pow(p.close-l,2),0)/t,d=Math.sqrt(c);a.push({time:e[i].time,value:l+d*n}),o.push({time:e[i].time,value:l-d*n})}return{upper:a,lower:o}}function Jb(e){let t=[],n=0,s=0;for(let a=0;a<e.length;a+=1){let o=(e[a].high+e[a].low+e[a].close)/3;n+=o*(e[a].volume||0),s+=e[a].volume||0,s>0&&t.push({time:e[a].time,value:n/s})}return t}function Yb(e,t=14){let n=[];if(e.length<=t)return n;let s=0,a=0;for(let r=1;r<=t;r+=1){let l=e[r].close-e[r-1].close;s+=l>0?l:0,a+=l<0?-l:0}let o=s/t,i=a/t;for(let r=t+1;r<e.length;r+=1){let l=e[r].close-e[r-1].close;o=(o*(t-1)+(l>0?l:0))/t,i=(i*(t-1)+(l<0?-l:0))/t;let c=i>0?o/i:100;n.push({time:e[r].time,value:100-100/(1+c)})}return n}function Fi(e,t=14){let n=[];for(let o=0;o<e.length;o+=1){let i=o>0?e[o-1].close:e[o].close;n.push(Math.max(e[o].high-e[o].low,Math.abs(e[o].high-i),Math.abs(e[o].low-i)))}let s=[],a=n.slice(0,t).reduce((o,i)=>o+i,0)/Math.max(1,t);for(let o=t;o<e.length;o+=1)a=(a*(t-1)+n[o])/t,s.push({time:e[o].time,value:a});return s}function Qb(e,t=14,n=3){let s=[];for(let o=t-1;o<e.length;o+=1){let i=e.slice(o-t+1,o+1),r=Math.max(...i.map(d=>d.high)),l=Math.min(...i.map(d=>d.low)),c=r!==l?(e[o].close-l)/(r-l)*100:50;s.push({time:e[o].time,value:c})}let a=[];for(let o=n-1;o<s.length;o+=1){let i=s.slice(o-n+1,o+1).reduce((r,l)=>r+l.value,0)/n;a.push({time:s[o].time,value:i})}return{k:s,d:a}}function Zb(e,t=12,n=26,s=9){let a=qs(e,t),o=qs(e,n),i=new Map(o.map(u=>[u.time,u.value])),r=a.filter(u=>i.has(u.time)).map(u=>({time:u.time,value:u.value-i.get(u.time)})),l=[];if(r.length){let u=2/(s+1),p=r[0].value;for(let m=0;m<r.length;m+=1)p=m===0?r[m].value:r[m].value*u+p*(1-u),m>=s-1&&l.push({time:r[m].time,value:p})}let c=new Map(l.map(u=>[u.time,u.value])),d=r.filter(u=>c.has(u.time)).map(u=>({time:u.time,value:u.value-c.get(u.time)}));return{line:r,signal:l,hist:d}}function Fd(e,t=9,n=26,s=52){let a=u=>{let p=[];for(let m=u-1;m<e.length;m+=1){let f=e.slice(m-u+1,m+1);p.push({time:e[m].time,value:(Math.max(...f.map(v=>v.high))+Math.min(...f.map(v=>v.low)))/2})}return p},o=a(t),i=a(n),r=new Map(i.map(u=>[u.time,u.value])),l=o.filter(u=>r.has(u.time)).map(u=>({time:u.time,value:(u.value+r.get(u.time))/2})),c=a(s),d=e.map(u=>({time:u.time,value:u.close}));return{tenkan:o,kijun:i,senkouA:l,senkouB:c,chikou:d}}function ey(e,t=20){let n=[],s=[],a=[];for(let o=t-1;o<e.length;o+=1){let i=e.slice(o-t+1,o+1),r=Math.max(...i.map(c=>c.high)),l=Math.min(...i.map(c=>c.low));n.push({time:e[o].time,value:r}),s.push({time:e[o].time,value:l}),a.push({time:e[o].time,value:(r+l)/2})}return{upper:n,lower:s,mid:a}}function ty(e,t=20,n=2){let s=qs(e,t),a=Fi(e,t),o=new Map(a.map(l=>[l.time,l.value])),i=[],r=[];return s.forEach(l=>{if(o.has(l.time)){let c=o.get(l.time)*n;i.push({time:l.time,value:l.value+c}),r.push({time:l.time,value:l.value-c})}}),{upper:i,lower:r,mid:s}}function ny(e){let t=[],n=[],s=0,a=0;for(let o=0;o<e.length;o+=1){if(o>0){let l=e[o-1];e[o].close>l.close&&(s+=e[o].volume||0),e[o].close<l.close&&(s-=e[o].volume||0)}let i=e[o].high-e[o].low||1,r=(e[o].close-e[o].low-(e[o].high-e[o].close))/i;a+=r*(e[o].volume||0),t.push({time:e[o].time,value:s}),n.push({time:e[o].time,value:a})}return{obv:t,adline:n}}function sy(e,t=14){if(e.length<t+2)return{adx:[],plusDi:[],minusDi:[]};let n=[],s=[],a=[];for(let p=1;p<e.length;p+=1){let m=e[p].high-e[p-1].high,f=e[p-1].low-e[p].low;s.push(m>f&&m>0?m:0),a.push(f>m&&f>0?f:0),n.push(Math.max(e[p].high-e[p].low,Math.abs(e[p].high-e[p-1].close),Math.abs(e[p].low-e[p-1].close)))}let o=n.slice(0,t).reduce((p,m)=>p+m,0),i=s.slice(0,t).reduce((p,m)=>p+m,0),r=a.slice(0,t).reduce((p,m)=>p+m,0),l=[],c=[],d=[];for(let p=t;p<n.length;p+=1){o=o-o/t+n[p],i=i-i/t+s[p],r=r-r/t+a[p];let m=o>0?100*i/o:0,f=o>0?100*r/o:0,v=m+f>0?100*Math.abs(m-f)/(m+f):0,h=e[p+1].time;l.push({time:h,value:m}),c.push({time:h,value:f}),d.push({time:h,value:v})}let u=[];if(d.length>=t){let p=d.slice(0,t).reduce((m,f)=>m+f.value,0)/t;for(let m=t;m<d.length;m+=1)p=(p*(t-1)+d[m].value)/t,u.push({time:d[m].time,value:p})}return{adx:u,plusDi:l,minusDi:c}}function ay(e,t=14){let n=[],s=[];for(let a=t-1;a<e.length;a+=1){let o=e.slice(a-t+1,a+1),i=0,r=0;for(let d=1;d<o.length;d+=1)o[d].high>=o[i].high&&(i=d),o[d].low<=o[r].low&&(r=d);let l=t-1-i,c=t-1-r;n.push({time:e[a].time,value:(t-l)/t*100}),s.push({time:e[a].time,value:(t-c)/t*100})}return{up:n,down:s}}function oy(e,t=14){let n=[],s=[];if(e.length<t+2)return{plus:n,minus:s};for(let a=t;a<e.length;a+=1){let o=0,i=0,r=0;for(let l=a-t+1;l<=a;l+=1){let c=e[l-1],d=e[l];o+=Math.max(d.high-d.low,Math.abs(d.high-c.close),Math.abs(d.low-c.close)),i+=Math.abs(d.high-c.low),r+=Math.abs(d.low-c.high)}n.push({time:e[a].time,value:o>0?i/o:0}),s.push({time:e[a].time,value:o>0?r/o:0})}return{plus:n,minus:s}}function iy(e,t=10,n=3){let s=Fi(e,t),a=new Map(s.map(c=>[c.time,c.value])),o=[],i=0,r=0,l=!0;return e.forEach((c,d)=>{if(d===0||!a.has(c.time))return;let u=(c.high+c.low)/2,p=a.get(c.time),m=u+n*p,f=u-n*p;d>1&&(m>i&&e[d-1].close<=i&&(m=i),f<r&&e[d-1].close>=r&&(f=r));let v=c.close>m?!0:c.close<f?!1:l,h=v?f:m;o.push({time:c.time,value:h,trendUp:v?1:0}),i=m,r=f,l=v}),o}function ry(e,t=14){let n=[];if(e.length<=t)return n;let s=e.map((a,o)=>{let i=(a.high+a.low+a.close)/3,r=o>0?(e[o-1].high+e[o-1].low+e[o-1].close)/3:i,l=i*(a.volume||0);return{time:a.time,pos:i>=r?l:0,neg:i<r?l:0}});for(let a=t;a<s.length;a+=1){let o=s.slice(a-t+1,a+1),i=o.reduce((c,d)=>c+d.pos,0),r=o.reduce((c,d)=>c+d.neg,0),l=r>0?i/r:100;n.push({time:s[a].time,value:100-100/(1+l)})}return n}function ly(e,t=14){let n=[];for(let s=t-1;s<e.length;s+=1){let a=e.slice(s-t+1,s+1),o=Math.max(...a.map(l=>l.high)),i=Math.min(...a.map(l=>l.low)),r=o!==i?(o-e[s].close)/(o-i)*-100:-50;n.push({time:e[s].time,value:r})}return n}function cy(e,t=20){let n=[],s=e.map(a=>({time:a.time,value:(a.high+a.low+a.close)/3}));for(let a=t-1;a<s.length;a+=1){let o=s.slice(a-t+1,a+1),i=o.reduce((c,d)=>c+d.value,0)/t,r=o.reduce((c,d)=>c+Math.abs(d.value-i),0)/t,l=r>0?(s[a].value-i)/(.015*r):0;n.push({time:s[a].time,value:l})}return n}function dy(e){if(e.length<30)return[];let t=[],n=[];for(let o=1;o<e.length;o+=1){let i=e[o-1].close,r=Math.min(e[o].low,i),l=Math.max(e[o].high,i);t.push({time:e[o].time,value:e[o].close-r}),n.push({time:e[o].time,value:l-r})}let s=(o,i,r)=>o.slice(i-r+1,i+1).reduce((l,c)=>l+c.value,0),a=[];for(let o=27;o<t.length;o+=1){let i=s(t,o,7)/Math.max(1e-9,s(n,o,7)),r=s(t,o,14)/Math.max(1e-9,s(n,o,14)),l=s(t,o,28)/Math.max(1e-9,s(n,o,28));a.push({time:t[o].time,value:100*(4*i+2*r+l)/7})}return a}function py(e,t=20){let n=[],s=e.map(a=>{let o=a.high-a.low||1,i=(a.close-a.low-(a.high-a.close))/o;return{time:a.time,value:i*(a.volume||0),volume:a.volume||0}});for(let a=t-1;a<s.length;a+=1){let o=s.slice(a-t+1,a+1),i=o.reduce((l,c)=>l+c.value,0),r=o.reduce((l,c)=>l+c.volume,0);n.push({time:s[a].time,value:r>0?i/r:0})}return n}function uy(e){return(Array.isArray(e)?e:[]).map(t=>({time:Number(t[0]),low:Number(t[1]),high:Number(t[2]),open:Number(t[3]),close:Number(t[4]),volume:Number(t[5])})).filter(t=>Number.isFinite(t.time)&&[t.open,t.high,t.low,t.close].every(Number.isFinite))}function ki(e,t){let n=new Map;for(let s of e)n.set(s.time,s);for(let s of t)n.set(s.time,s);return Array.from(n.values()).sort((s,a)=>s.time-a.time)}async function $i(e,t,n){let s=[],a=Math.floor(Date.now()/1e3),o=!1;for(let i=0;i<n;i+=1){let r=a-t*300,l=`https://api.exchange.coinbase.com/products/${e}/candles?granularity=${t}&start=${new Date(r*1e3).toISOString()}&end=${new Date(a*1e3).toISOString()}`,c;try{c=await $t(l,{timeoutMs:1e4})}catch{break}let d=uy(c);if(!d.length){o=!0;break}s.push(...d),a=r,i<n-1&&await nb(tb)}return{candles:ki(s,[]),reachedStart:o}}async function my(e,t,n){if(!e||!Number.isFinite(t)||!Number.isFinite(n)||n<=t)return[];try{let s=`https://api.coingecko.com/api/v3/coins/${e}/market_chart/range?vs_currency=usd&from=${t}&to=${n}`,a=await $t(s,{timeoutMs:15e3});return(Array.isArray(a==null?void 0:a.prices)?a.prices:[]).map(([i,r])=>({time:Math.floor(Number(i)/1e3/86400)*86400,open:Number(r),high:Number(r),low:Number(r),close:Number(r),volume:0})).filter(i=>Number.isFinite(i.time)&&Number.isFinite(i.close)&&i.close>0).sort((i,r)=>i.time-r.time)}catch{return[]}}async function fy(e,t,n){var u;let s=_d(n),a=s===86400,o=`${eb}${e.id}:${s}`,i=be(Z(o));if((u=i==null?void 0:i.candles)!=null&&u.length&&i.reachedStart){let p=await $i(t,s,1),m=ki(i.candles,p.candles);return te(o,JSON.stringify({candles:m,reachedStart:!0,source:i.source,cachedAt:Date.now()})),{candles:m,source:i.source}}let l=await $i(t,s,a?rd:ld),c=l.candles,d=c.length?`Coinbase (${new Date(c[0].time*1e3).getUTCFullYear()}\u2013present, ${c.length} bars)`:"Coinbase";if(a&&l.reachedStart&&e.coingeckoId&&c.length){let p=c[0].time,m=await my(e.coingeckoId,0,p);m.length&&(c=ki(m,c),d=`CoinGecko (${new Date(c[0].time*1e3).getUTCFullYear()}+) + Coinbase (${new Date(p*1e3).getUTCFullYear()}\u2013present)`)}else!a&&!l.reachedStart&&(d=`Coinbase (last ${c.length} bars \u2014 full inception history is daily/weekly/monthly only)`);return c.length&&te(o,JSON.stringify({candles:c,reachedStart:l.reachedStart,source:d,cachedAt:Date.now()})),{candles:c,source:d}}var fi=new Map;async function Od(e,t){let n=`${e.id}:${t}`,s=zs.get(n);if(s&&Date.now()-s.ts<6e4&&Array.isArray(s.data)&&s.data.length)return s.data;let a=fi.get(n);if(a)return a;let o=(async()=>{let i=[],r="",l=Ad(e.ticker);if(l)try{let c=await fy(e,l,t);i=c.candles,r=c.source}catch{i=[]}if(!Array.isArray(i)||!i.length){let c=Id(t,Dd());return zs.set(n,{ts:Date.now(),data:c,sourceLabel:""}),c}return zs.set(n,{ts:Date.now(),data:i,sourceLabel:r}),i})();fi.set(n,o);try{return await o}finally{fi.delete(n)}}async function hy(){var u,p,m,f,v,h,w,k;let e=Wa(),t=await Od(e,C.interval),s=((u=zs.get(`${e.id}:${C.interval}`))==null?void 0:u.sourceLabel)||String(((p=C.stats)==null?void 0:p.source)||"Coinbase + XRPL live"),a="pair",o=Rd(),i=String((o==null?void 0:o.symbol)||"").toUpperCase(),r=o?He(o):`${e.symbol||"XRP"}|`;if(o&&i&&i!=="XRP"){let b=await zb(o,C.interval);(m=b==null?void 0:b.candles)!=null&&m.length&&(t=b.candles,s=b.source,a=b.mode)}let l=String(e.symbol||"XRP").toUpperCase()==="XRP",c=Number((l?(f=C.stats)==null?void 0:f.xrplSpot:null)??((v=C.stats)==null?void 0:v.price)??0);if(a==="pair"&&Number.isFinite(c)&&c>0&&t.length){let b=Math.max(60,Nd(C.interval)),x=Math.floor(Date.now()/1e3),S=Math.floor(x/b)*b,T=t[t.length-1];T.time===S?(T.high=Math.max(T.high,c),T.low=Math.min(T.low,c),T.close=c):T.time<S&&t.push({time:S,open:T.close,high:Math.max(T.close,c),low:Math.min(T.close,c),close:c,volume:0})}let d=Number((o==null?void 0:o.price)||0);if(a==="pair"&&o&&i&&i!=="XRP"&&Number.isFinite(d)&&d>0&&t.length){let b=Number((l?(h=C.stats)==null?void 0:h.xrplSpot:null)??((w=C.stats)==null?void 0:w.price)??((k=t[t.length-1])==null?void 0:k.close)??0);if(Number.isFinite(b)&&b>0){let x=d/b;Number.isFinite(x)&&x>0&&(t=t.map(S=>({...S,open:S.open*x,high:S.high*x,low:S.low*x,close:S.close*x})),s="Coinbase pair + token spot proxy",a="token-proxy")}}return t.length||(t=Id(C.interval,Dd()),s=o&&i!=="XRP"?"Synthetic fallback (token-focused)":"Synthetic fallback",a="fallback"),o&&i!=="XRP"&&(a==="pair"||a==="fallback")?pi!==r&&(pi=r,he(`No market data found for ${o.symbol} yet \u2014 showing ${e.symbol||"XRP"} for reference.`)):r&&(pi=""),C.chartType==="heikin_ashi"&&(t=wi(t)),jb(t,{mode:a,source:s,tokenKey:r||"XRP|"}),t}function Vc(e){if(!Array.isArray(e))return[];let t=e.map(s=>({time:Number(s.time),open:Number(s.open),high:Number(s.high),low:Number(s.low),close:Number(s.close),volume:Number(s.volume||0)})).filter(s=>Number.isFinite(s.time)&&Number.isFinite(s.open)&&Number.isFinite(s.high)&&Number.isFinite(s.low)&&Number.isFinite(s.close)&&s.time>0).sort((s,a)=>s.time-a.time),n=[];for(let s=0;s<t.length;s+=1){let a=t[s],o=n[n.length-1];o&&o.time===a.time?n[n.length-1]=a:n.push(a)}return n}function gy(e,t,n){var i;let s=Number(((i=e==null?void 0:e[e.length-1])==null?void 0:i.close)||0);if(!Number.isFinite(s)||s<=0)return 6;let o=Math.max(1e-12,Math.abs(Number(n)-Number(t)))/Math.max(1e-12,Math.abs(s));return s>=1e3?o<.01?4:2:s>=1?o<.01?6:4:s>=.01?o<.03?8:6:8}function vy(){if(ee.resizeObserver)try{ee.resizeObserver.disconnect()}catch{}ee.chart&&(ee.chart.remove(),ee={chart:null,volumeSeries:null,activeSeries:null,compareSeries:null,indicatorSeries:[],indicatorPriceLines:[],priceLines:[],resizeObserver:null,chartType:"",configKey:"",legendEl:null,renderLegend:null,ichimokuData:null,indicatorLegendItems:[],alertPriceLines:[]})}function Gc(e,t){var d;let{addOverlay:n,addOsc:s,addOscHist:a,addPriceLevel:o}=t,i=(u,p)=>{var f,v;let m=Number((v=(f=C.indicatorSettings)==null?void 0:f[u])==null?void 0:v.length);return Number.isFinite(m)&&m>1?Math.min(500,Math.max(2,m)):p},r=(u,p,m)=>{var h,w;let f=(w=(h=C.indicatorSettings)==null?void 0:h[u])==null?void 0:w[p];if(f==null||f==="")return m;let v=(Ei[u]||[]).find(k=>k.id===p);if((v==null?void 0:v.type)==="number"){let k=Number(f);return Number.isFinite(k)&&k>1?Math.min(500,Math.max(2,k)):m}return f},l=C.indicators;if(l.sma20&&n(xi(e,i("sma20",20)),r("sma20","color","#f1c40f"),1.2,!1,"SMA"),l.ema20&&n(qs(e,i("ema20",20)),r("ema20","color","#ffb86c"),1.2,!1,"EMA"),l.wma20&&n(Gb(e,i("wma20",20)),r("wma20","color","#bd93f9"),1.2,!1,"WMA"),l.vwap&&n(Jb(e),r("vwap","color","#80ffea"),1.2,!1,"VWAP"),l.bb20){let u=r("bb20","color","#ff79c6"),p=Kb(e,i("bb20",20),2);n(p.upper,u,1,!0,"BB Upper"),n(p.lower,u,1,!0,"BB Lower")}if(l.ichimoku){let u=Fd(e,r("ichimoku","tenkanLen",9),r("ichimoku","kijunLen",26),r("ichimoku","senkouBLen",52));n(u.tenkan,r("ichimoku","tenkanColor","#ffde59"),1.2,!1,"Tenkan"),n(u.kijun,r("ichimoku","kijunColor","#6ecbff"),1.2,!1,"Kijun"),n(u.senkouA,"rgba(70,255,160,0.8)",1,!0,"Senkou A"),n(u.senkouB,"rgba(255,120,120,0.8)",1,!0,"Senkou B"),n(u.chikou,"rgba(220,220,255,0.6)",.9,!0,"Chikou")}if(l.donchian){let u=r("donchian","color","#9cfb8c"),p=ey(e,i("donchian",20));n(p.upper,u,1,!0,"Donchian Upper"),n(p.lower,u,1,!0,"Donchian Lower"),n(p.mid,"rgba(156,251,140,0.6)",1.2,!1,"Donchian Mid")}if(l.keltner){let u=r("keltner","color","#7ee7ff"),p=ty(e,i("keltner",20),2);n(p.upper,u,1,!0,"Keltner Upper"),n(p.lower,u,1,!0,"Keltner Lower"),n(p.mid,"rgba(126,231,255,0.66)",1.2,!1,"Keltner Mid")}if(l.pivots&&e.length>=2){let u=e[e.length-2],p=(u.high+u.low+u.close)/3,m=2*p-u.low,f=2*p-u.high,v=p+(u.high-u.low),h=p-(u.high-u.low);o(p,"#f6f6f6","P"),o(m,"#61ffb0","R1"),o(f,"#ff8f8f","S1"),o(v,"rgba(97,255,176,0.6)","R2"),o(h,"rgba(255,143,143,0.6)","S2")}if(l.supertrend||l.sar||l.elderRay){let u=qs(e,14);if(l.supertrend&&n(iy(e,10,3),r("supertrend","color","#8bffde"),1.3,!1,"Supertrend"),l.sar&&n(u.map(p=>({time:p.time,value:p.value*.998})),r("sar","color","#ffaf7a"),1,!0,"SAR"),l.elderRay){let p=new Map(u.map(v=>[v.time,v.value])),m=e.filter(v=>p.has(v.time)).map(v=>({time:v.time,value:v.high-p.get(v.time)})),f=e.filter(v=>p.has(v.time)).map(v=>({time:v.time,value:v.low-p.get(v.time)}));s(m,r("elderRay","bullColor","#5fff9d"),"Elder Bull"),s(f,r("elderRay","bearColor","#ff9d9d"),"Elder Bear")}}if(l.rsi&&s(Yb(e,i("rsi",14)),r("rsi","color","#a6ff4d"),"RSI"),l.atr&&s(Fi(e,i("atr",14)),r("atr","color","#ffb86c"),"ATR"),l.stdev){let u=xi(e,20),p=new Map(u.map(f=>[f.time,f.value])),m=e.filter(f=>p.has(f.time)).map(f=>({time:f.time,value:Math.abs(f.close-p.get(f.time))}));s(m,r("stdev","color","#b2a3ff"),"StdDev")}if(l.stoch){let u=Qb(e,i("stoch",14),3);s(u.k,r("stoch","kColor","#9ee8ff"),"%K"),s(u.d,r("stoch","dColor","#ffd86b"),"%D")}if(l.macd){let u=Zb(e,r("macd","fastLen",12),r("macd","slowLen",26),r("macd","signalLen",9));s(u.line,r("macd","lineColor","#8fd9ff"),"MACD"),s(u.signal,r("macd","signalColor","#ffcf8e"),"Signal"),(d=u.hist)!=null&&d.length&&a(u.hist)}let c=ny(e);if(l.obv&&s(c.obv,r("obv","color","#8cf9ff"),"OBV"),l.adline&&s(c.adline,r("adline","color","#ffb7ff"),"A/D"),l.cmf&&s(py(e,i("cmf",20)),r("cmf","color","#f8ff87"),"CMF"),l.williamsr&&s(ly(e,i("williamsr",14)),r("williamsr","color","#ff9adf"),"Williams %R"),l.cci&&s(cy(e,i("cci",20)),r("cci","color","#b8ff8e"),"CCI"),l.mfi&&s(ry(e,i("mfi",14)),r("mfi","color","#7bffd2"),"MFI"),l.uo&&s(dy(e),r("uo","color","#ffd36f"),"UO"),l.adx){let u=sy(e,i("adx",14));s(u.adx,r("adx","adxColor","#9fd8ff"),"ADX"),s(u.plusDi,r("adx","plusColor","#73ffc0"),"+DI"),s(u.minusDi,r("adx","minusColor","#ff9797"),"-DI")}if(l.aroon){let u=ay(e,i("aroon",14));s(u.up,r("aroon","upColor","#6cffb0"),"Aroon Up"),s(u.down,r("aroon","downColor","#ff8f8f"),"Aroon Down")}if(l.vortex){let u=oy(e,i("vortex",14));s(u.plus.map(p=>({time:p.time,value:p.value*100})),r("vortex","plusColor","#d6a8ff"),"VI+"),s(u.minus.map(p=>({time:p.time,value:p.value*100})),r("vortex","minusColor","#ffb0f3"),"VI-")}}function by(e){let t=e.querySelector(":scope > svg.xpd-ichimoku-cloud-svg");return t||(t=document.createElementNS("http://www.w3.org/2000/svg","svg"),t.setAttribute("class","xpd-ichimoku-cloud-svg"),e.appendChild(t)),t}function yy(e){var t;(t=e==null?void 0:e.querySelector(":scope > svg.xpd-ichimoku-cloud-svg"))==null||t.remove()}function Bd(e,t,n,s,a){var h;if(!e||!t||!n)return;let o=n.clientWidth||0,i=n.clientHeight||0;if(!o||!i||!(s!=null&&s.length)||!(a!=null&&a.length))return;let r=by(n);r.setAttribute("width",o),r.setAttribute("height",i),r.setAttribute("viewBox",`0 0 ${o} ${i}`);let l=((h=C.indicatorSettings)==null?void 0:h.ichimoku)||{},c=l.bullColor||"#46ffa0",d=l.bearColor||"#ff7878",u=new Map(a.map(w=>[w.time,w.value])),p=e.timeScale(),m=[];for(let w of s){let k=u.get(w.time);if(k==null)continue;let b=p.timeToCoordinate(w.time),x=t.priceToCoordinate(w.value),S=t.priceToCoordinate(k);b==null||x==null||S==null||m.push({x:b,ya:x,yb:S,bullish:w.value>=k})}let f="",v=0;for(let w=1;w<=m.length;w+=1){if(!(w===m.length||m[w].bullish!==m[v].bullish))continue;let b=m.slice(v,w);if(b.length>=2){let x=b.map($=>`${$.x.toFixed(1)},${$.ya.toFixed(1)}`).join(" "),S=b.slice().reverse().map($=>`${$.x.toFixed(1)},${$.yb.toFixed(1)}`).join(" "),T=b[0].bullish?c:d;f+=`<polygon points="${x} ${S}" fill="${g(T)}" opacity="0.22" />`}v=w}r.innerHTML=f}function Kc(e,t,n,s){var i;if(!C.indicators.ichimoku){yy(s),ee.ichimokuData=null;return}let a=((i=C.indicatorSettings)==null?void 0:i.ichimoku)||{},o=Fd(e,Number(a.tenkanLen)||9,Number(a.kijunLen)||26,Number(a.senkouBLen)||52);ee.ichimokuData={senkouA:o.senkouA,senkouB:o.senkouB},Bd(t,n,s,o.senkouA,o.senkouB)}function Si(){let e=ee.ichimokuData,t=document.getElementById("xpd-tv-widget");e&&ee.chart&&ee.activeSeries&&t&&Bd(ee.chart,ee.activeSeries,t,e.senkouA,e.senkouB)}async function wy(){var n,s,a,o;let e=++Mn,t=document.getElementById("xpd-tv-widget");if(t){ee.chart||(t.innerHTML='<div class="xpd-loading">Loading full price history\u2026</div>');try{C.threeEnabled?Vb():Ks();let i=await hy(),r=Vc(i);if(e!==Mn)return;if(!r.length)throw new Error("No chart bars returned for selected pair/timeframe.");ky(r);let l=JSON.stringify({pair:C.pair,interval:C.interval,chartType:C.chartType,comparePair:C.comparePair,indicators:C.indicators,indicatorSettings:C.indicatorSettings,threeEnabled:C.threeEnabled});if(!C.comparePair&&!(C.drawings||[]).length&&ee.chart&&ee.configKey===l){let B=C.chartType==="heikin_ashi"?wi(r):r;C.chartType==="line"||C.chartType==="area"?ee.activeSeries.setData(B.map(ue=>({time:ue.time,value:ue.close}))):ee.activeSeries.setData(B),ee.volumeSeries.setData(r.map(ue=>({time:ue.time,value:ue.volume||0,color:ue.close>=ue.open?"rgba(38,166,154,0.5)":"rgba(239,83,80,0.5)"})));let J=0,re=0,pe=[];Gc(r,{addOverlay:(ue,H,Y,me,De)=>{let Be=ee.indicatorSeries[J++];ue!=null&&ue.length&&(Be==null||Be.setData(ue),De&&Be&&pe.push({label:De,color:H,series:Be,points:ue}))},addOsc:(ue,H,Y)=>{let me=ee.indicatorSeries[J++];ue!=null&&ue.length&&(me==null||me.setData(ue),Y&&me&&pe.push({label:Y,color:H,series:me,points:ue}))},addOscHist:ue=>{let H=ee.indicatorSeries[J++];ue!=null&&ue.length&&(H==null||H.setData(ue.map(Y=>({time:Y.time,value:Y.value,color:Y.value>=0?"rgba(99,255,157,0.45)":"rgba(255,126,126,0.45)"}))))},addPriceLevel:ue=>{let H=ee.indicatorPriceLines[re++];Number.isFinite(ue)&&(H==null||H.applyOptions({price:ue}))}}),ee.indicatorLegendItems=pe,Kc(r,ee.chart,ee.activeSeries,t),Jc(ee.activeSeries);let Ce=r[r.length-1];(n=ee.renderLegend)==null||n.call(ee,Ce.open,Ce.high,Ce.low,Ce.close,Ce.volume);return}if(!await Wb())throw new Error("Chart library failed to load.");if(e!==Mn)return;let d=document.getElementById("xpd-tv-widget");if(!d||e!==Mn)return;let u=null;try{u=((s=ee.chart)==null?void 0:s.timeScale().getVisibleLogicalRange())||null}catch{}vy(),d.innerHTML="";let p=window.LightweightCharts,m=Math.max(320,d.clientWidth||((a=d.parentElement)==null?void 0:a.clientWidth)||320),f=Math.max(360,d.clientHeight||460),v=r.map(B=>B.low),h=r.map(B=>B.high),w=gy(r,Math.min(...v),Math.max(...h)),k={type:"price",precision:w,minMove:Math.pow(10,-w)},b="#26a69a",x="#ef5350",S=p.createChart(d,{width:m,height:f,layout:{background:{type:p.ColorType.Solid,color:"#131722"},textColor:"#d1d4dc"},grid:{vertLines:{color:"#242832"},horzLines:{color:"#242832"}},rightPriceScale:{borderColor:"rgba(197,203,206,0.3)"},leftPriceScale:{visible:!1,borderColor:"rgba(197,203,206,0.3)"},timeScale:{borderColor:"rgba(197,203,206,0.3)",timeVisible:!0,secondsVisible:!1},crosshair:{mode:p.CrosshairMode.Normal},handleScroll:!0,handleScale:!0}),T=C.chartType==="heikin_ashi"?wi(r):r,$;if(C.chartType==="line")$=S.addLineSeries({color:"#2962ff",lineWidth:2,priceFormat:k}),$.setData(T.map(B=>({time:B.time,value:B.close})));else if(C.chartType==="area")$=S.addAreaSeries({lineColor:"#2962ff",topColor:"rgba(41,98,255,0.36)",bottomColor:"rgba(41,98,255,0.04)",lineWidth:2,priceFormat:k}),$.setData(T.map(B=>({time:B.time,value:B.close})));else if(C.chartType==="bars")$=S.addBarSeries({upColor:b,downColor:x,priceFormat:k}),$.setData(T);else{let B=C.chartType==="hollow_candles";$=S.addCandlestickSeries({upColor:B?"rgba(0,0,0,0)":b,downColor:x,borderUpColor:b,borderDownColor:x,wickUpColor:b,wickDownColor:x,borderVisible:!0,priceFormat:k}),$.setData(T)}let P=S.addHistogramSeries({priceFormat:{type:"volume"},priceScaleId:"vol"});S.priceScale("vol").applyOptions({scaleMargins:{top:.85,bottom:0}}),P.setData(r.map(B=>({time:B.time,value:B.volume||0,color:B.close>=B.open?"rgba(38,166,154,0.5)":"rgba(239,83,80,0.5)"})));let M=document.createElement("div");M.className="xpd-chart-legend",d.appendChild(M);let N=B=>Number.isFinite(B)?B.toFixed(w):"\u2014",_=(B,J,re,pe,Se,ve)=>{let rt=pe>=B,lt=(ve||[]).map(Ce=>`<span style="color:${g(Ce.color)}">${g(Ce.label)} <b>${N(Ce.value)}</b></span>`).join("");M.innerHTML=`
        <span>O <b>${N(B)}</b></span>
        <span>H <b>${N(J)}</b></span>
        <span>L <b>${N(re)}</b></span>
        <span class="${rt?"xpd-legend-up":"xpd-legend-down"}">C <b>${N(pe)}</b></span>
        ${Se!=null?`<span>Vol <b>${Ge(Se)}</b></span>`:""}
        ${lt}
      `},X=T[T.length-1],I=(o=r[r.length-1])==null?void 0:o.volume,D=()=>{if(!X)return;let B=X.open??X.value,J=X.high??X.value,re=X.low??X.value,pe=X.close??X.value,Se=(ee.indicatorLegendItems||[]).map(ve=>{var rt,lt;return{label:ve.label,color:ve.color,value:(lt=(rt=ve.points)==null?void 0:rt[ve.points.length-1])==null?void 0:lt.value}}).filter(ve=>Number.isFinite(ve.value));_(B,J,re,pe,I,Se)};D(),S.subscribeCrosshairMove(B=>{var Ce,ue;if(!B.time){D();return}let J=(Ce=B.seriesData)==null?void 0:Ce.get($);if(!J){D();return}let re=(ue=B.seriesData)==null?void 0:ue.get(P),pe=J.open??J.value,Se=J.high??J.value,ve=J.low??J.value,rt=J.close??J.value,lt=(ee.indicatorLegendItems||[]).map(H=>{var Y,me;return{label:H.label,color:H.color,value:(me=(Y=B.seriesData)==null?void 0:Y.get(H.series))==null?void 0:me.value}}).filter(H=>Number.isFinite(H.value));_(pe,Se,ve,rt,re==null?void 0:re.value,lt)});let F=!1,W=()=>{F||(F=!0,S.priceScale("osc").applyOptions({scaleMargins:{top:.65,bottom:.18}}))},E=[],U=[],Q=[];Gc(r,{addOverlay:(B,J,re=1.2,pe=!1,Se="")=>{if(!(B!=null&&B.length))return;let ve=S.addLineSeries({color:J,lineWidth:re,lineStyle:pe?p.LineStyle.Dashed:p.LineStyle.Solid,priceLineVisible:!1,lastValueVisible:!1});ve.setData(B),E.push(ve),Se&&Q.push({label:Se,color:J,series:ve,points:B})},addOsc:(B,J,re="")=>{if(!(B!=null&&B.length))return;W();let pe=S.addLineSeries({color:J,lineWidth:1.2,priceScaleId:"osc",title:re,priceLineVisible:!1,lastValueVisible:!1});pe.setData(B),E.push(pe),re&&Q.push({label:re,color:J,series:pe,points:B})},addOscHist:B=>{if(!(B!=null&&B.length))return;W();let J=S.addHistogramSeries({priceScaleId:"osc",priceLineVisible:!1,lastValueVisible:!1});J.setData(B.map(re=>({time:re.time,value:re.value,color:re.value>=0?"rgba(99,255,157,0.45)":"rgba(255,126,126,0.45)"}))),E.push(J)},addPriceLevel:(B,J,re="")=>{Number.isFinite(B)&&U.push($.createPriceLine({price:B,color:J,lineWidth:1,lineStyle:p.LineStyle.Dashed,axisLabelVisible:!0,title:re}))}}),Kc(r,S,$,d),S.timeScale().subscribeVisibleLogicalRangeChange(Si);let fe=null;if(C.comparePair){let B=_n.find(J=>J.id===C.comparePair);if(B){let J=Vc(await Od(B,C.interval));if(e===Mn&&J.length){let re=J[0].close||1;fe=S.addLineSeries({color:"#ffd166",lineWidth:1.4,priceScaleId:"left",priceLineVisible:!1,lastValueVisible:!0,priceFormat:{type:"custom",formatter:pe=>`${pe>=0?"+":""}${pe.toFixed(2)}%`,minMove:.01}}),S.priceScale("left").applyOptions({visible:!0,borderColor:"rgba(255,209,102,0.35)"}),fe.setData(J.map(pe=>({time:pe.time,value:(pe.close/re-1)*100})))}}}else S.priceScale("left").applyOptions({visible:!1});if(e!==Mn){try{S.remove()}catch{}return}let K=(C.drawings||[]).filter(B=>Number.isFinite(B==null?void 0:B.price)).map(B=>$.createPriceLine({price:B.price,color:"#78e5ff",lineWidth:2,lineStyle:p.LineStyle.Dashed,axisLabelVisible:!0,title:"Line"}));if(S.subscribeClick(B=>{if(C.drawingTool!=="hline"||!B.point)return;let J=$.coordinateToPrice(B.point.y);Number.isFinite(J)&&(C.drawings=[...C.drawings||[],{price:J}].slice(-50),K.push($.createPriceLine({price:J,color:"#78e5ff",lineWidth:2,lineStyle:p.LineStyle.Dashed,axisLabelVisible:!0,title:"Line"})))}),u&&Number.isFinite(u.from)&&Number.isFinite(u.to))try{S.timeScale().setVisibleLogicalRange(u)}catch{S.timeScale().fitContent()}else S.timeScale().fitContent();let xe=new ResizeObserver(()=>{S.applyOptions({width:Math.max(320,d.clientWidth||320)}),Si()});xe.observe(d),ee={chart:S,volumeSeries:P,activeSeries:$,compareSeries:fe,indicatorSeries:E,indicatorPriceLines:U,priceLines:K,resizeObserver:xe,chartType:C.chartType,configKey:l,legendEl:M,renderLegend:_,indicatorLegendItems:Q,alertPriceLines:[]},Jc($)}catch(i){if(e!==Mn)return;if(C.error=(i==null?void 0:i.message)||"Could not initialize chart widget.",ee.chart){he(C.error);return}let r=document.getElementById("xpd-tv-widget");r&&(r.innerHTML=`<div class="xpd-error">${g(C.error)}</div>`)}}}function Xd(){let e=Z(vi)==="1"?"0":"1";te(vi,e),q()}async function Hd(e){_n.some(t=>t.id===e)&&(C.pair=e,await Gs(),St(),q())}async function zd(e){C.interval=e,await Gs(),St(),q()}function Ud(e){C.chartType=e,St(),q()}async function jd(){await Gs(),q()}function Wd(e,t){e in C.indicators&&(C.indicators[e]=!!t,t&&(C.selectedIndicator=e,C.selectedEducationTab="indicator"),St(),q())}function qd(){C.indicatorMenuOpen=!C.indicatorMenuOpen,q()}function Vd(){C.moreMenuOpen=!C.moreMenuOpen,q()}function Gd(e){C.indicatorQuery=String(e||""),C.indicatorMenuOpen||(C.indicatorMenuOpen=!0),q()}function Kd(e){let t=String(e||"").trim();t in C.indicators&&(C.indicators[t]=!0,C.selectedIndicator=t,C.selectedEducationTab="indicator",St(),q())}function Jd(e){let t=String(e||"").trim();t in C.indicators&&(C.indicators[t]=!1,St(),q())}function xy(e){var s,a;if(C.settingsOpenFor!==e)return"";let t=C.indicatorSettings[e]||{},n=Ei[e]||[];return`
    ...
    </div>`}function Yd(e){let t=String(e||"").trim();t in C.indicators&&(C.settingsOpenFor=C.settingsOpenFor===t?null:t,q())}function Qd(){C.settingsOpenFor=null,q()}function Zd(e){let t=String(e||"").trim();if(!(t in C.indicators))return;let n=C.indicatorSettings[t]||{},s={...n};for(let a of Ei[t]||[]){let o=document.getElementById(`xpd-ind-${a.id}-${t}`);o&&(a.type==="number"?s[a.id]=Math.max(2,Math.min(500,Number(o.value)||n[a.id]||a.default)):s[a.id]=o.value)}C.indicatorSettings[t]=s,C.settingsOpenFor=null,q()}function ep(e){let t=String(e||"").trim();delete C.indicatorSettings[t],C.settingsOpenFor=null,q()}function tp(){St(),Gi(window.location.href),ne("Chart link copied.")}function np(){C.threeEnabled=!C.threeEnabled,te(Ai,C.threeEnabled?"1":"0"),C.threeEnabled||Ks(),q()}function sp(e){let t=!!e;C.threeEnabled!==t&&(C.threeEnabled=t,te(Ai,C.threeEnabled?"1":"0"),C.threeEnabled||Ks(),q())}function ap(e){C.comparePair=e||"",St(),q()}function op(e){let t=String(e||"").trim();!t||!(t in C.indicators)||(C.indicators[t]=!0,C.selectedIndicator=t,q())}function ip(e){let t=String(e||"none");md.some(n=>n.key===t)&&(C.drawingTool=t,C.educationHint=ib[t]||"",q())}function qa(){var e;return((e=C.chartMeta)==null?void 0:e.tokenKey)||C.tokenFocusKey||`${Wa().symbol||"XRP"}|`}function rp(){var s,a;let e=Number((s=C.chartMeta)==null?void 0:s.last),t=prompt(`Alert me when ${((a=C.chartMeta)==null?void 0:a.symbol)||"price"} crosses:`,Number.isFinite(e)?e.toFixed(6):"");if(t==null)return;let n=Number(t);if(!Number.isFinite(n)||n<=0){he("Enter a valid price.");return}C.alerts=[...C.alerts||[],{id:`alert_${Date.now()}`,tokenKey:qa(),price:n,createdAt:new Date().toISOString()}],_i(),q(),ne(`Alert set at $${R(n,6)}`)}function lp(e){C.alerts=(C.alerts||[]).filter(t=>t.id!==e),_i(),q()}function Jc(e){if(!e)return;let t=window.LightweightCharts;for(let s of ee.alertPriceLines||[])try{e.removePriceLine(s)}catch{}let n=(C.alerts||[]).filter(s=>s.tokenKey===qa());ee.alertPriceLines=n.map(s=>{var a;return e.createPriceLine({price:s.price,color:"#ffb703",lineWidth:2,lineStyle:((a=t==null?void 0:t.LineStyle)==null?void 0:a.Dotted)??2,axisLabelVisible:!0,title:"\u{1F514} Alert"})})}function ky(e){var r,l;if(e.length<2)return;let t=qa(),n=(C.alerts||[]).filter(c=>c.tokenKey===t);if(!n.length)return;let s=e[e.length-2].close,a=e[e.length-1].close,o=[],i=!1;for(let c of n){let d=s<c.price&&a>=c.price,u=s>c.price&&a<=c.price;d||u?(ne(`\u{1F514} ${((r=C.chartMeta)==null?void 0:r.symbol)||"Price"} crossed $${R(c.price,6)} (${d?"\u2191":"\u2193"} now $${R(a,6)})`),i=!0,(l=document.querySelector(`.xpd-indicator-chip[data-alert-id="${c.id}"]`))==null||l.remove()):o.push(c)}i&&(C.alerts=[...(C.alerts||[]).filter(c=>c.tokenKey!==t),...o],_i())}function cp(){let e=ee.activeSeries;if(e)for(let t of ee.priceLines||[])try{e.removePriceLine(t)}catch{}ee.priceLines=[],C.drawings=[]}function Va(e,t=0){var i;let n=(i=ee.chart)==null?void 0:i.timeScale();if(!n)return;let s=n.getVisibleLogicalRange();if(!s)return;let a=(s.to-s.from)*e,o=(s.to+s.from)/2+(s.to-s.from)*t;n.setVisibleLogicalRange({from:o-a/2,to:o+a/2})}function dp(){Va(.8)}function pp(){Va(1.25)}function up(){Va(1,-.2)}function mp(){Va(1,.2)}function fp(){C.educationCollapsed=!C.educationCollapsed,q()}function hp(e){["indicator","psychology","practice"].includes(e)&&(C.selectedEducationTab=e,q())}function gp(){let e=document.body.classList.contains("theme-gold");hn(e?"cosmic":"gold"),q()}var Yc=!1;function $y(){let e=document.getElementById("xpd-tv-widget");e&&ee.chart&&(ee.chart.applyOptions({width:Math.max(320,e.clientWidth||320),height:Math.max(320,e.clientHeight||460)}),Si())}function vp(){var t;let e=document.querySelector(".xpd-chart-wrap");e&&(Yc||(Yc=!0,document.addEventListener("fullscreenchange",()=>{requestAnimationFrame(()=>requestAnimationFrame($y))})),document.fullscreenElement?document.exitFullscreen():(t=e.requestFullscreen)==null||t.call(e))}function bp(){let e=ee.chart;if(!e){he("Chart image is not ready yet.");return}let t=e.takeScreenshot(),n=document.createElement("a");n.download=`xrpl-chart-${Date.now()}.png`,n.href=t.toDataURL("image/png"),n.click()}function yp(){return C.tokenFocusKey||C.pair||"default"}function wp(){var n;let e=be(Z(bi))||{},t=yp();e[t]={interval:C.interval,chartType:C.chartType,comparePair:C.comparePair,indicators:C.indicators,indicatorSettings:C.indicatorSettings},te(bi,JSON.stringify(e)),ne(`Chart layout saved for ${((n=C.chartMeta)==null?void 0:n.symbol)||t}.`)}function xp(){var s;let e=be(Z(bi))||{},t=yp(),n=e[t];if(!n){he(`No saved layout for ${((s=C.chartMeta)==null?void 0:s.symbol)||t} yet.`);return}C.interval=n.interval||C.interval,C.chartType=n.chartType||C.chartType,C.comparePair=n.comparePair||"",C.indicators={...C.indicators,...n.indicators||{}},C.indicatorSettings={...C.indicatorSettings,...n.indicatorSettings||{}},St(),q()}function Ga(e){let t=String(e||"").trim();if(/^[0-9A-F]{40}$/i.test(t)){let n=t.replace(/(00)+$/g,"");if(/^[0-9A-F]+$/i.test(n)&&n.length%2===0)try{let s=new Uint8Array(n.length/2);for(let o=0;o<n.length;o+=2)s[o/2]=parseInt(n.slice(o,o+2),16);let a=new TextDecoder().decode(s).replace(/\0/g,"").trim();if(a)return a.toUpperCase()}catch{}}return t.toUpperCase()}function He(e){return`${e.symbol}|${e.issuer||""}`}function Bs(e){return encodeURIComponent(e).replace(/'/g,"%27")}function Sy(e){var l,c,d,u,p,m,f,v,h,w;let t=Ga(e.code||e.currency||""),n=String(e.issuer||((l=e.IssuingAccount)==null?void 0:l.account)||"").trim(),s=Number(e.price??((c=e.metrics)==null?void 0:c.price)??0),a=Number(e.marketcap??((d=e.metrics)==null?void 0:d.marketcap)??0),o=Number(((u=e.metrics)==null?void 0:u.volume_24h)??0),i=((m=(p=e.meta)==null?void 0:p.token)==null?void 0:m.name)||((f=e.IssuingAccount)==null?void 0:f.name)||t||"Unknown Token",r=Number(e.holders??((v=e.metrics)==null?void 0:v.holders)??((h=e.metrics)==null?void 0:h.trustlines)??0);return{symbol:t,name:i,issuer:n,tokenId:e.id||`${t}.${n}`,price:Number.isFinite(s)&&s>0?s:null,marketCap:Number.isFinite(a)&&a>0?a:null,volume24h:Number.isFinite(o)?o:null,holders:Number.isFinite(r)?r:null,change24h:null,verified:!!((w=e.IssuingAccount)!=null&&w.verified),score:Number(e.score||0)||null}}function Ty(e){let t=Ga((e==null?void 0:e.symbol)||""),n=String((e==null?void 0:e.name)||t||"Unknown Token"),s=Number((e==null?void 0:e.current_price)||0),a=Number((e==null?void 0:e.market_cap)||0),o=Number((e==null?void 0:e.total_volume)||0),i=Number((e==null?void 0:e.price_change_percentage_24h)||0);return{symbol:t,name:n,issuer:"",tokenId:`cg:${(e==null?void 0:e.id)||t}`,price:Number.isFinite(s)&&s>0?s:null,marketCap:Number.isFinite(a)&&a>0?a:null,volume24h:Number.isFinite(o)&&o>0?o:null,holders:null,change24h:Number.isFinite(i)?i:null,verified:!1,score:null}}function Cy(e){var r;let t=Ga((e==null?void 0:e.currency)||(e==null?void 0:e.token)||""),n=String((e==null?void 0:e.issuer)||(e==null?void 0:e.account)||"").trim(),s=String((e==null?void 0:e.name)||t||"Unknown Token"),a=Number((e==null?void 0:e.price)||((r=e==null?void 0:e.market)==null?void 0:r.price)||0),o=Number((e==null?void 0:e.marketcap)||(e==null?void 0:e.marketCap)||0),i=Number((e==null?void 0:e.volume24h)||(e==null?void 0:e.volume)||0);return{symbol:t,name:s,issuer:n,tokenId:`bithomp:${t}.${n||"na"}`,price:Number.isFinite(a)&&a>0?a:null,marketCap:Number.isFinite(o)&&o>0?o:null,volume24h:Number.isFinite(i)&&i>0?i:null,holders:Number.isFinite(Number((e==null?void 0:e.holders)||0))?Number((e==null?void 0:e.holders)||0):null,change24h:Number.isFinite(Number((e==null?void 0:e.change24h)||0))?Number((e==null?void 0:e.change24h)||0):null,verified:!!(e!=null&&e.verified),score:null}}function Py(e){let t=Ga((e==null?void 0:e.symbol)||(e==null?void 0:e.currency)||""),n=String((e==null?void 0:e.issuer)||(e==null?void 0:e.issuerAddress)||"").trim(),s=String((e==null?void 0:e.name)||t||"Unknown Token"),a=Number((e==null?void 0:e.price)||0),o=Number((e==null?void 0:e.volume24h)||0);return{symbol:t,name:s,issuer:n,tokenId:`xrplto:${t}.${n||"na"}`,price:Number.isFinite(a)&&a>0?a:null,marketCap:Number.isFinite(Number((e==null?void 0:e.marketCap)||0))?Number((e==null?void 0:e.marketCap)||0):null,volume24h:Number.isFinite(o)&&o>0?o:null,holders:Number.isFinite(Number((e==null?void 0:e.holders)||0))?Number((e==null?void 0:e.holders)||0):null,change24h:Number.isFinite(Number((e==null?void 0:e.change24h)||0))?Number((e==null?void 0:e.change24h)||0):null,verified:!!(e!=null&&e.verified),score:null}}async function kp(){var e,t,n,s,a,o;if(za()){j.loading=!0,j.error="";try{let i=Date.now(),r=Bc.get("tokens");if(r&&i-r.ts<6e4){j.tokens=r.data,j.total=r.total||r.data.length,j.lastSyncAt=r.ts,j.filtered=us(j.query,r.data),j.trending=Qc(r.data),j.loading=!1;return}let l=500,c=12,d=[];for(let T=1;T<=c;T+=1){let $=await $t(`${zv}?page=${T}&limit=${l}`,{allowProxy:!1,timeoutMs:12e3});if(!Array.isArray($)||!$.length||(d.push(...$),$.length<l))break}let u=d.map(Sy).filter(T=>!!T.symbol&&!!T.issuer),p=[$t(`${Uv}?vs_currency=usd&order=market_cap_desc&per_page=250&page=1&sparkline=false`,{timeoutMs:12e3}).catch(()=>[]),Date.now()<(zc.xrplto||0)?Promise.resolve([]):$t(Wv,{timeoutMs:12e3,allowProxy:!1}).catch(T=>(String((T==null?void 0:T.message)||"").includes("429")&&(zc.xrplto=Date.now()+5*60*1e3),[]))];qv&&p.push($t(jv,{timeoutMs:12e3,allowProxy:!1}).catch(()=>[]));let[m,f,v=[]]=await Promise.all(p),h=Array.isArray(m)?m.map(Ty).filter(T=>!!T.symbol):[],w=Array.isArray(v)?v.map(Cy).filter(T=>!!T.symbol):[],k=Array.isArray(f)?f.map(Py).filter(T=>!!T.symbol):[],b={symbol:"XRP",name:"XRP Ledger Native",issuer:"",tokenId:"XRP",price:((e=Te.data)==null?void 0:e.priceUsd)||((t=C.stats)==null?void 0:t.price)||null,marketCap:((n=Te.data)==null?void 0:n.marketCap)||null,volume24h:((s=Te.data)==null?void 0:s.volume24h)||null,holders:null,change24h:((a=Te.data)==null?void 0:a.change24h)||((o=C.stats)==null?void 0:o.changePct)||null,verified:!0,score:1},x=new Map;[b,...u,...w,...k,...h].forEach(T=>{let $=He(T);x.has($)||x.set($,T)});let S=[...x.values()];Bc.set("tokens",{ts:i,data:S,total:S.length}),j.tokens=S,j.total=S.length,j.lastSyncAt=i,j.filtered=us(j.query,S),j.trending=Qc(S)}catch(i){j.error=(i==null?void 0:i.message)||"Could not load XRPL token discovery data."}finally{j.loading=!1}}}function us(e,t){let n=String(e||"").trim().toLowerCase(),s=j.filters||{type:"all",minCap:0,minVol:0,hasDex:!1};return t.filter(a=>{if(n&&!`${a.symbol} ${a.name} ${a.issuer} ${a.tokenId||""}`.toLowerCase().includes(n)||Number(s.minCap||0)>0&&Number(a.marketCap||0)<Number(s.minCap||0)||Number(s.minVol||0)>0&&Number(a.volume24h||0)<Number(s.minVol||0)||s.hasDex&&!(Number(a.volume24h||0)>0||Number(a.holders||0)>100))return!1;if(s.type&&s.type!=="all"){let o=String(a.symbol||"").toLowerCase(),i=String(a.name||"").toLowerCase(),r=/usd|usdc|usdt|rlusd|eur|gbp/.test(o)||/stable/.test(i),l=/meme|dog|cat|frog|shib|pepe/.test(o)||/meme/.test(i),c=/mpt/.test(o)||/multi-purpose/.test(i);if(s.type==="stablecoin"&&!r||s.type==="meme"&&!l||s.type==="mpt"&&!c||s.type==="standard"&&(r||l||c))return!1}return!0})}function Qc(e){return[...e].sort((t,n)=>{let s=Number(t.volume24h||0),a=Number(n.volume24h||0);if(a!==s)return a-s;let o=Number(t.marketCap||0);return Number(n.marketCap||0)-o}).slice(0,24)}async function $p(e){if(xt.loading=!0,xt.error="",xt.items=[],!e){xt.loading=!1;return}try{let t=await Ui(e,20);xt.items=t||[]}catch(t){xt.error=(t==null?void 0:t.message)||"Could not load recent transactions."}finally{xt.loading=!1}}function Sp(e){j.query=e,Hs&&clearTimeout(Hs),Hs=setTimeout(()=>{j.listLimit=240,j.filtered=us(j.query,j.tokens),q()},300)}function Tp(e,t){j.filters={...j.filters,[e]:t},j.listLimit=240,j.filtered=us(j.query,j.tokens),q()}function Cp(){Hs&&clearTimeout(Hs),j.query="",j.filters={type:"all",minCap:0,minVol:0,hasDex:!1},j.listLimit=240,j.filtered=us("",j.tokens),q()}function Pp(){j.listLimit=Math.min(5e3,Number(j.listLimit||240)+240),q()}function Lp(){let e=j.filtered;j.listLimit=Math.min(5e3,Math.max(240,e.length)),q()}function Mp(){j.listLimit=240,q()}function Ap(e){j.selectedTokenKey=e||"",q()}function Ep(e){let t=String(e||"").trim();if(!t)return;let n=Ri();n.includes(t)||n.push(t),yd(n),q()}function Np(e){let t=String(e||"").trim(),n=Ri().filter(s=>s!==t);yd(n),q()}async function Ka(e){let t=String(e||"").trim();if(!t)return;let n=String(t.includes("|")?t.split("|")[0]:t).toUpperCase(),s=t.includes("|")?j.tokens.find(o=>He(o)===t):j.tokens.find(o=>String(o.symbol||"").toUpperCase()===String(n||"").toUpperCase());j.selectedTokenKey=s?He(s):t.includes("|")?t:j.selectedTokenKey,C.tokenFocusKey=j.selectedTokenKey||t;let a={BTC:"BINANCE:BTCUSDT",ETH:"BINANCE:ETHUSDT",SOL:"BINANCE:SOLUSDT"};C.pair=a[n]||"BITSTAMP:XRPUSD",St(),q(),setTimeout(()=>db(),20),await Gs(),St(),q()}async function _p(e){return Ka(e)}async function Rp(){await kp(),q()}async function Oi(){let e=document.getElementById("xpd-lookup-currency"),t=document.getElementById("xpd-lookup-issuer"),n=String((e==null?void 0:e.value)||"").trim().toUpperCase(),s=String((t==null?void 0:t.value)||"").trim();if(!n){he("Enter a currency code first.");return}if(!s||!Fe(s)){he("Enter a valid XRPL issuer address.");return}let a=`${n}|${s}`,o=j.tokens.find(i=>He(i)===a);o||(o={symbol:n,name:n,issuer:s,price:null,marketCap:null,volume24h:null,holders:null,tokenId:""},j.tokens=[...j.tokens,o],j.filtered=us(j.query,j.tokens)),j.selectedTokenKey=a,e&&(e.value=""),t&&(t.value=""),await Ka(a)}async function Dp(){var s,a;let e=String(((s=document.getElementById("xpd-lookup-currency"))==null?void 0:s.value)||"").trim().toUpperCase(),t=String(((a=document.getElementById("xpd-lookup-issuer"))==null?void 0:a.value)||"").trim();if(!e){he("Enter a currency code first.");return}if(!t||!Fe(t)){he("Enter a valid XRPL issuer address.");return}let n=`${e}|${t}`;await Oi(),await Di(n)}async function Ip(){var t;let e=((t=Tt())==null?void 0:t.address)||"";await $p(e),q()}async function cs({silent:e=!1,force:t=!1}={}){var s;if(!t&&!za())return;let n=((s=Tt())==null?void 0:s.address)||"";e||ne("Refreshing XRPL dashboard data..."),await Promise.allSettled([Gs(),Sd(),kp(),$p(n),Td(n),Cd(n),Ld()]),q()}function Fp(){return Sd()}function Op(){var e;return Td(((e=Tt())==null?void 0:e.address)||"")}function Bp(){var e;return Cd(((e=Tt())==null?void 0:e.address)||"")}function Xp(){return Ld()}function Hp(e){ne(`NFT ${e.slice(0,12)}... selected. Send flow can be wired to NFTokenCreateOffer.`)}function Vs(){let e=y("profile-metrics-row");if(!e)return;let t=!1,n=Object.values(Ke).reduce((u,p)=>u+((p==null?void 0:p.xrp)||0),0),s=zp(),a=Object.values(Ke).flatMap(u=>(u==null?void 0:u.tokens)||[]),o=Tt(),i=o?Xs[o.address]:null,r=(i==null?void 0:i.ownerCount)||0,l=Ba+r*Xa,c=o!=null&&o.createdAt?My(new Date(o.createdAt)):"\u2014",d=(i==null?void 0:i.sequence)!=null?i.sequence:"\u2014";e.innerHTML=`
    ...
    <div class="onboarding-card">
      ...
      <div class="onb-items">
        ${Oa("\u{1F48E}","Generate your first XRPL wallet","Encrypted with AES-256-GCM, never leaves this device.",e,"openWalletCreator()")}
        ${Oa("\u{1F517}","Connect a social account","Link Discord, X, GitHub, or any platform.",t,"switchProfileTab('social')")}
        ${Oa("\u270F\uFE0F","Add a bio","Tell people who you are.",n,"openProfileEditor()")}
        ${Oa("\u{1F4BE}","Export an encrypted backup","Protect against device loss.",s,"exportVaultBackup()")}
      </div>
    </div>`}function Oa(e,t,n,s,a){return`<div class="onb-item ${s?"onb-item--done":""}" ${s?"":` onclick="${a}"`}>
  ...
  </div>`}function Bi(){let e=y("profile-tab-social");if(!e)return;let t=Object.values(je).filter(Boolean).length;e.innerHTML=`
    ...
    ${t?`<div class="social-preview-row">
    ...
    </div>`:""}`,Ua("stat-socials-val",t)}function Up(e){let t=ps.find(l=>l.id===e);if(!t)return;let n=y("social-modal");if(!n)return;let s=y("social-modal-icon"),a=y("social-modal-title"),o=y("social-modal-sub"),i=y("social-modal-input"),r=y("social-modal-delete");s&&(s.className=`social-platform-icon-lg social-icon ${t.id}`,s.textContent=t.icon),a&&(a.textContent=`Connect ${t.label}`),o&&(o.textContent=`Enter your ${t.label} ${t.id==="discord"?"user ID or username":"username"}.`),i&&(i.value=je[e]||"",i.placeholder=`Your ${t.label} handle`),r&&(r.style.display=je[e]?"":"none"),n.dataset.platform=e,n.classList.add("show"),setTimeout(()=>i==null?void 0:i.focus(),80)}function jp(){let e=y("social-modal"),t=e==null?void 0:e.dataset.platform,n=y("social-modal-input");if(!t||!n)return;let s=n.value.trim().replace(/^@/,"");s?je[t]=s:delete je[t],gd(),Bi(),ms();let a=ps.find(o=>o.id===t);d0(),gt("social_connected",`${(a==null?void 0:a.label)||t} @${s||"(removed)"}`),ne(`${a==null?void 0:a.label} updated`)}function Wp(){var t;let e=(t=y("social-modal"))==null?void 0:t.dataset.platform;e&&(delete je[e],gd(),Bi(),ms(),gt("social_removed",e),ne("Social connection removed"))}function qp(e){let t=ps.find(n=>n.id===e);t&&je[e]&&window.open(`${t.prefix}${je[e]}`,"_blank","noopener")}function ms(){var e;(e=y("social-modal"))==null||e.classList.remove("show")}function vt(){let e=y("profile-tab-wallets");if(!e)return;if(oe.length===0){e.innerHTML=Ay()+`
      ...
      </div>`,Ua("stat-wallets-val",0);return}let t=oe.length>3?`
    ...
    </div>`,Ua("stat-wallets-val",oe.length)}function Ey(e,t){let n=e.id===it,s=!!e.watchOnly,a=Ke[e.address],o=Xs[e.address],r=!0||s,l=(o==null?void 0:o.ownerCount)||0,c=Ba+l*Xa,d=r?a?R(a.xrp,2):"\u2014":"\u2022\u2022\u2022\u2022",u=a&&r?Math.max(0,a.xrp-c):null,p=(a==null?void 0:a.tokens)||[],m=a!=null&&a.fetchedAt?vd(a.fetchedAt):null,f=e.address.slice(0,8)+"\u2026"+e.address.slice(-6),v=zi(e.address);return`
  <div class="wcard ${n?"wcard--active":""} ${s?"wcard--watch":""}" id="wallet-item-${e.id}" style="--i:${t}">
    <div class="wcard-top">
      ...
      <div class="wcard-balance-col">
        ${v.length>=2?`<div class="wcard-sparkline">${nu(v,70,22,e.color||"#00fff0")}</div>`:""}
    ...
    ${o?`<div class="wcard-reserve-row">
      ...
      <span class="wcard-reserve-sub">${l} object${l!==1?"s":""} \xB7 base ${Ba} + ${l}\xD7${Xa}</span>
    </div>`:""}
    ...
    <div class="wcard-actions">
      ...
      <button class="wcard-btn wcard-btn--expand ${An===e.id?"wcard-btn--expand-open":""}" onclick="toggleWalletDrawer('${e.id}')">${An===e.id?"\u25B2 Hide":"\u25BC Details"}</button>
  ...
  </div>`}function Vp(e){var n;let t=oe[e];t&&(oe.splice(e,1),Ws(),it===t.id&&(it=((n=oe[0])==null?void 0:n.id)||null,it&&te(Us,it)),vt(),Nt(),gt("wallet_removed",t.label),Ny(`Wallet "${t.label}" removed`,()=>{oe.splice(e,0,t),Ws(),it||(it=t.id,te(Us,t.id)),vt(),Nt(),gt("wallet_created",t.label+" (restored)")},()=>{t.address&&dt(Ci+t.address)}))}function Ny(e,t,n){let s=document.getElementById("undo-toast");s&&s.remove();let a=document.createElement("div");a.id="undo-toast",a.className="undo-toast",a.innerHTML=`<span class="undo-msg">${g(e)}</span><button class="undo-btn">Undo</button>`,document.body.appendChild(a),requestAnimationFrame(()=>a.classList.add("show"));let o=setTimeout(()=>{a.classList.remove("show"),setTimeout(()=>a.remove(),300),n==null||n()},5e3);a.querySelector(".undo-btn").addEventListener("click",()=>{clearTimeout(o),t(),a.classList.remove("show"),setTimeout(()=>a.remove(),300),ne("Wallet restored")})}function Gp(e){var n,s;let t=y("inspect-addr");t&&(t.value=e),(n=window.switchTab)==null||n.call(window,document.querySelector('[data-tab="inspector"]'),"inspector"),(s=window.showDashboard)==null||s.call(window)}function Kp(e){An=An===e?null:e,An&&!cn[e]&&(cn[e]="txns"),vt(),An&&setTimeout(()=>Xi(e,cn[e]),60)}function Jp(e,t){cn[e]=t;let n=document.getElementById(`wcard-drawer-${e}`);n&&(n.querySelectorAll(".wdt-btn").forEach(s=>s.classList.toggle("active",s.textContent.toLowerCase().includes(t==="txns"?"trans":t==="nfts"?"nft":t==="orders"?"dex":"amm"))),Xi(e,t))}async function Xi(e,t){var a,o,i;let n=oe.find(r=>r.id===e),s=document.getElementById(`wcard-drawer-body-${e}`);if(!(!n||!s)){s.innerHTML='<div class="wdd-loading"><div class="spinner"></div> Loading\u2026</div>';try{t==="txns"?s.innerHTML=_y(((a=Li[n.address])==null?void 0:a.txns)||await Ui(n.address),n.address):t==="nfts"?s.innerHTML=Ry(((o=ad[n.address])==null?void 0:o.nfts)||await Vy(n.address),n.address):t==="orders"?s.innerHTML=Dy(((i=Mi[n.address])==null?void 0:i.offers)||await Gy(n.address),n.id,n.address):t==="amm"&&(s.innerHTML=await Iy(n.address))}catch(r){s.innerHTML=`<div class="wdd-error">\u26A0\uFE0F ${g(r.message)}</div>`}}}function Yp(e){return{Payment:"\u{1F4B8}",OfferCreate:"\u{1F4CA}",OfferCancel:"\u2715",TrustSet:"\u{1F517}",NFTokenMint:"\u{1F3A8}",NFTokenBurn:"\u{1F525}",NFTokenCreateOffer:"\u{1F3AF}",NFTokenAcceptOffer:"\u2705",AMMCreate:"\u{1F30A}",AMMDeposit:"\u{1F4E5}",AMMWithdraw:"\u{1F4E4}",AMMVote:"\u{1F5F3}",AMMBid:"\u{1F4A1}",EscrowCreate:"\u23F3",EscrowFinish:"\u2705",EscrowCancel:"\u2715",AccountSet:"\u2699",SetRegularKey:"\u{1F511}",SignerListSet:"\u{1F4CB}"}[e]||"\u{1F4C4}"}function Ti(e){return e?typeof e=="string"?`${R(Number(e)/1e6,4)} XRP`:`${R(parseFloat(e.value||0),4)} ${(e.currency||"?").length>4?e.currency.slice(0,4)+"\u2026":e.currency}`:"\u2014"}function _y(e,t){return e!=null&&e.length?`<div class="wdd-tx-list">
    ${e.slice(0,25).map(n=>{var c,d;let s=n.TransactionType||"?",a=n.Account===t,o=!(((c=n.metaData)==null?void 0:c.TransactionResult)||((d=n.meta)==null?void 0:d.TransactionResult)||"").match(/^tec|^tem|^tef|^tel/),i=n.date?(n.date+946684800)*1e3:0,r=i?new Date(i).toLocaleDateString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"\u2014",l=n.hash||n.tx_hash||"";return`<div class="wdd-tx-row ${o?"":"wdd-tx-failed"}">
        <div class="wdd-tx-icon">${Yp(s)}</div>
    ...
    </div>`:'<div class="wdd-empty"><div class="wdd-empty-icon">\u{1F4CA}</div><div>No open DEX orders.</div></div>'}async function Iy(e){try{let n=(Pi[e]||[]).filter(s=>{var a;return((a=s.currency)==null?void 0:a.length)===40});return n.length?`<div class="wdd-amm-list">
    ...
    <div class="act-section-row">
      <div class="act-section">
        ...
        ${t.length?`<div class="act-timeline">${t.slice(0,20).map(s=>`
            <div class="act-entry">
              <div class="act-entry-icon">${lb[s.type]||"\u25CF"}</div>
              <div class="act-entry-body">
                <div class="act-entry-detail">${g(s.detail)}</div>
                <div class="act-entry-time">${vd(s.ts)}</div>
              </div>
    ...
    </div>`}function pn(){var o,i;let e=y("profile-tab-settings");if(!e)return;let t=["gold","cosmic","starry","hawaiian"],n=Z("nalulf_pref_currency")||"XRP",s=Z("nalulf_pref_network")||"mainnet",a=Z("nalulf_pref_autolock")||"30";e.innerHTML=`<div class="settings-grid">
  ...
  </div>`}var Hi=!1;function Oy(){var s;let e=((s=Ne.vault)==null?void 0:s.ai)||{},t=!!e.apiKey;return Hi||!t?`
    <div class="settings-card">
      ...
      <select id="ai-settings-model" class="xrpl-input">
        ${Hv.map(a=>`<option value="${a.id}" ${(e.model||gi)===a.id?"selected":""}>${g(a.label)}</option>`).join("")}
      </select>
    ...
    <div class="settings-card">
    ...
    </div>`}function Zp(e=!0){Hi=e,pn()}async function eu(){var s,a,o;let e=(s=y("ai-settings-key"))==null?void 0:s.value.trim(),t=(a=y("ai-settings-proxy"))==null?void 0:a.value.trim(),n=((o=y("ai-settings-model"))==null?void 0:o.value.trim())||gi;if(!e||!t){he("Enter both the API key and the proxy URL.");return}if(!/^https?:\/\//.test(t)){he("Proxy URL must start with http:// or https://");return}await Ne.update(i=>{i.ai={apiKey:e,proxyUrl:t,model:n}}),Hi=!1,ne("AI settings saved."),pn()}async function tu(){await Ne.update(e=>{delete e.ai}),ne("AI settings removed."),pn()}async function By(){var t;let e=y("profile-tab-analytics");if(e){e.innerHTML='<div class="analytics-grid"><div class="skeleton-card analytics-card--wide" style="height:80px"></div><div class="skeleton-card" style="height:160px"></div><div class="skeleton-card" style="height:160px"></div></div>';try{let n=Tt(),s=Object.values(Ke).reduce((r,l)=>r+((l==null?void 0:l.xrp)||0),0),a=zp(),o=Object.values(Ke).flatMap(r=>(r==null?void 0:r.tokens)||[]),i=[];if(n)try{i=((t=Li[n.address])==null?void 0:t.txns)||await Ui(n.address,100)}catch{}e.innerHTML=`<div class="analytics-grid">
      <div class="analytics-card analytics-card--wide">
        ...
        <div class="portfolio-summary-row">
          ${oe.length?oe.map(r=>{let l=Ke[r.address],c=l?R(l.xrp,2):"\u2014",d=l&&a?`$${R(l.xrp*a,2)}`:"",u=zi(r.address);return`<div class="portfolio-wallet-row">
                ...
                <div class="pwr-sparkline">${nu(u,80,28,r.color||"#00fff0")}</div>
      ...
      <div class="analytics-card analytics-card--wide">
        ...
        ${n?Hy(i):'<div class="analytics-empty">Activate a wallet to see activity.</div>'}
      </div>
      ...
      <div class="analytics-card">
        ...
        ${i.length?zy(i):'<div class="analytics-empty-chart"><div class="aec-icon">\u{1F4CA}</div><div>Transaction breakdown builds up as this wallet transacts.</div></div>'}
      </div>
      ...
      <div class="analytics-card">
        ...
        ${n&&i.length?jy(i,n.address):`<div class="analytics-empty-chart"><div class="aec-icon">\u{1F4B0}</div><div>${n?"XRP flow needs at least one transaction to estimate.":"Activate a wallet to see XRP flow."}</div></div>`}
      </div>
      ...
      <div class="analytics-card">
        ...
        ${o.length?Uy(o):'<div class="analytics-empty-chart"><div class="aec-icon">\u{1FA99}</div><div>No token trustlines held by any wallet yet.</div></div>'}
      </div>
    </div>`}catch(n){ub(e,"analytics",n)}}}function nu(e,t,n,s){if(e.length<2)return`<svg width="${t}" height="${n}"><line x1="0" y1="${n/2}" x2="${t}" y2="${n/2}" stroke="${s}" stroke-opacity=".2" stroke-width="1" stroke-dasharray="3 2"/></svg>`;let a=e.map(u=>u.xrp),o=Math.min(...a),i=Math.max(...a),r=i-o||1,l=a.map((u,p)=>`${3+p/(a.length-1)*(t-6)},${3+(1-(u-o)/r)*(n-6)}`),[c,d]=l[l.length-1].split(",");return`<svg width="${t}" height="${n}" viewBox="0 0 ${t} ${n}">
  ...
  </svg>`}function Xy(e){let t=zi(e);if(t.length<2)return`<div class="analytics-empty-chart"><div class="aec-icon">\u{1F4CA}</div><div>Balance history builds up as you refresh your wallet over time.</div><div class="aec-sub">${t.length} snapshot${t.length!==1?"s":""} recorded.</div></div>`;let n=560,s=130,a=52,o=12,i=14,r=30,l=t.map(_=>_.xrp),c=t.map(_=>_.ts),d=Math.min(...l),u=Math.max(...l),p=u-d||1,m=c[0],f=c[c.length-1],v=f-m||1,h=_=>a+(_-m)/v*(n-a-o),w=_=>i+(1-(_-d)/p)*(s-i-r),k=t.map(_=>`${h(_.ts).toFixed(1)},${w(_.xrp).toFixed(1)}`),b=h(c[0]),x=h(c[c.length-1]),S=l[l.length-1]-l[0],T=S>=0,$=l[0]?Math.abs(S/l[0]*100).toFixed(2):"0.00",P=T?"#00d4ff":"#ff5555",M=[d,(d+u)/2,u].map(_=>({v:_,y:w(_),l:R(_,2)})),N=[0,.5,1].map(_=>({x:a+_*(n-a-o),l:new Date(m+_*v).toLocaleDateString("en-US",{month:"short",day:"numeric"})}));return`
  ...
  </svg></div>`}function zy(e){let t=new Map;e.forEach(a=>t.set(a.TransactionType||"?",(t.get(a.TransactionType||"?")||0)+1));let n=[...t.entries()].sort((a,o)=>o[1]-a[1]),s=e.length;return`<div class="tx-breakdown-list">${n.slice(0,8).map(([a,o])=>`<div class="txb-row"><div class="txb-icon">${Yp(a)}</div><div class="txb-type">${a}</div><div class="txb-bar-wrap"><div class="txb-bar" style="width:${(o/s*100).toFixed(0)}%"></div></div><div class="txb-count">${o}</div></div>`).join("")}</div>`}function Uy(e){let t=new Map;e.forEach(o=>{let i=Math.abs(parseFloat(o.balance||0));t.set(o.currency,(t.get(o.currency)||0)+i)});let n=[...t.entries()].sort((o,i)=>i[1]-o[1]).slice(0,8),s=n.reduce((o,[,i])=>o+i,0)||1,a=["#00fff0","#00d4ff","#bd93f9","#50fa7b","#ffb86c","#ff79c6","#f1fa8c","#ff5555"];return`<div class="token-alloc-list">${n.map(([o,i],r)=>{let l=(i/s*100).toFixed(1),c=a[r%a.length],d=o.length>4?o.slice(0,4)+"\u2026":o;return`<div class="ta-row"><div class="ta-swatch" style="background:${c}"></div><div class="ta-cur mono">${d}</div><div class="ta-bar-wrap"><div class="ta-bar" style="width:${l}%;background:${c}20;border-color:${c}55"></div></div><div class="ta-pct">${l}%</div></div>`}).join("")}</div>`}function jy(e,t){let n=0,s=0;e.forEach(i=>{var c,d;if(i.TransactionType!=="Payment"||!((((c=i.metaData)==null?void 0:c.TransactionResult)||((d=i.meta)==null?void 0:d.TransactionResult))==="tesSUCCESS")||typeof i.Amount!="string")return;let l=Number(i.Amount)/1e6;i.Destination===t&&(n+=l),i.Account===t&&(s+=l)});let a=n-s,o=a>=0;return`<div class="xrp-flow-grid">
  ...
  <div class="xrf-note">Based on ${e.length} fetched Payment TXs. Excludes fees and DEX fills.</div>`}async function Ze(e){var n,s;try{if(((n=O.wsConn)==null?void 0:n.readyState)===1){let{wsSend:a}=await import("./xrpl-6AERZ5KD.js"),o={command:e==null?void 0:e.method,...((s=e==null?void 0:e.params)==null?void 0:s[0])||{}},i=await a(o);if((i==null?void 0:i.status)==="error")throw new Error(i.error_message||i.error||"XRPL RPC error");return(i==null?void 0:i.result)||null}}catch{}let t=async a=>{let o=await fetch(a,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e),mode:"cors"});if(!o.ok)throw new Error(`HTTP ${o.status}`);return(await o.json()).result};try{return await t(Ov)}catch{return await t(Bv)}}async function Rn(e){var t;try{let n=[],s;do{let l=await Ze({method:"account_lines",params:[{account:e,ledger_index:"current",limit:400,...s?{marker:s}:{}}]});if(!l||l.error)break;n.push(...l.lines||[]),s=l.marker}while(s);let a=await Ze({method:"account_info",params:[{account:e,ledger_index:"current"}]});if(a!=null&&a.error)return null;let o=Number(a.account_data.Balance)/1e6,i=n.map(l=>({currency:l.currency,issuer:l.account,balance:l.balance,limit:l.limit}));Ke[e]={xrp:o,tokens:i,fetchedAt:Date.now()},Pi[e]=i,qy(e,o);let r=document.getElementById("awb-balance");return r&&e===((t=Tt())==null?void 0:t.address)&&p0(r.querySelector(".awb-xrp-num")||r,o,2,600),Ke[e]}catch{return null}}async function Wy(){await Promise.allSettled(oe.map(e=>Rn(e.address))),vt(),Nt(),Vs()}function qy(e,t){let n=Ci+e,s=be(Z(n))||[],a=Date.now();s.length&&a-s[s.length-1].ts<5*6e4?s[s.length-1]={xrp:t,ts:a}:s.push({xrp:t,ts:a}),s.length>90&&s.splice(0,s.length-90),te(n,JSON.stringify(s))}function zi(e){return be(Z(Ci+e))||[]}async function Ui(e,t=25){let n=await Ze({method:"account_tx",params:[{account:e,limit:t,ledger_index_min:-1,ledger_index_max:-1}]}),s=((n==null?void 0:n.transactions)||[]).map(a=>a.tx||a.transaction||a);return Li[e]={txns:s,fetchedAt:Date.now()},s}async function Vy(e){let t=await Ze({method:"account_nfts",params:[{account:e,limit:50}]}),n=(t==null?void 0:t.account_nfts)||[];return ad[e]={nfts:n,fetchedAt:Date.now()},n}async function Gy(e){let t=await Ze({method:"account_offers",params:[{account:e,limit:50}]}),n=(t==null?void 0:t.offers)||[];return Mi[e]={offers:n,fetchedAt:Date.now()},n}async function Ky(e){let t=await Ze({method:"account_info",params:[{account:e,ledger_index:"current"}]});return(t==null?void 0:t.account_data)||null}async function Jy(){let e=await Ze({method:"ledger",params:[{ledger_index:"current"}]});return(e==null?void 0:e.ledger_current_index)||0}function Yy(e){let t=(e==null?void 0:e.engine_result)||"";return t==="tesSUCCESS"||t.startsWith("tes")||(e==null?void 0:e.engine_result_code)===0}function Qy(e){let t=(e==null?void 0:e.engine_result)||"";return Jv[t]||(e==null?void 0:e.engine_result_message)||t||"Unknown error"}async function Zy(e,t,n){if(await ja(),!window.xrpl)throw new Error("xrpl.js library not loaded. Cannot sign transactions.");let s=oe.find(i=>i.id===e);if(!s)throw new Error("Wallet not found.");if(s.watchOnly)throw new Error("Watch-only wallets cannot sign transactions.");let a=await Zv(s,n),o;try{o=window.xrpl.Wallet.fromSeed(a,{algorithm:s.algo==="secp256k1"?"secp256k1":"ed25519"})}catch(i){throw new Error("Invalid seed phrase: "+i.message)}if(o.classicAddress!==s.address)throw new Error("Seed does not match this wallet address.");try{let[i,r]=await Promise.all([Ky(s.address),Jy()]);if(!i)throw new Error("Account not found on-chain. Fund with at least 10 XRP first (base reserve requirement).");let l={...t,Account:s.address,Fee:"12",Sequence:i.Sequence,LastLedgerSequence:r+20},{tx_blob:c,hash:d}=o.sign(l);return{...await Ze({method:"submit",params:[{tx_blob:c}]}),tx_hash:d}}finally{}}async function e0(e,t,n){return Zy(e,{TransactionType:"OfferCancel",OfferSequence:parseInt(t)},n)}function t0(){if(y("send-modal-overlay"))return;let e=document.createElement("div");e.innerHTML=`
  <!-- Send -->
  ...
  <div class="generic-modal-overlay" id="token-details-modal">
  ...
  </div>`,document.body.appendChild(e),["send-modal-overlay","receive-modal-overlay","trustline-modal-overlay","import-address-modal","import-seed-modal","token-details-modal"].forEach(t=>{let n=document.getElementById(t);n==null||n.addEventListener("click",s=>{s.target===n&&(n.classList.remove("show"),n.style.display="")})})}function su(){var o;["displayName","handle","bio","location","website"].forEach(i=>{let r=y(`edit-${i}`);r&&(r.value=le[i]||"")});let e=y("editor-avatar-preview");if(e){let i=localStorage.getItem(dn);e.innerHTML=i?`<img src="${i}" class="profile-avatar-img"/>`:le.avatar||"\u{1F30A}"}let t=y("avatar-remove-btn");t&&(t.style.display=localStorage.getItem(dn)?"":"none");let n=y("editor-banner-preview");if(n){let i=localStorage.getItem(Nn);n.style.backgroundImage=i?`url(${i})`:"",n.style.backgroundSize="cover",n.style.backgroundPosition="center",js.forEach(r=>n.classList.remove(r)),i||n.classList.add(le.banner||"banner-ocean")}let s=y("avatar-picker-grid");s&&(s.innerHTML=Vv.map(i=>`<div class="avatar-option ${le.avatar===i?"active":""}" onclick="selectAvatar('${i}')">${i}</div>`).join(""));let a=y("banner-picker-grid");a&&(a.innerHTML=js.map(i=>`<div class="banner-option ${i} ${le.banner===i?"active":""}" onclick="selectBanner('${i}')"></div>`).join("")),(o=y("profile-editor-modal"))==null||o.classList.add("show")}function Js(){var e;(e=y("profile-editor-modal"))==null||e.classList.remove("show")}function au(){var e,t,n,s,a;le.displayName=((e=y("edit-displayName"))==null?void 0:e.value.trim())||le.displayName,le.handle=(((t=y("edit-handle"))==null?void 0:t.value.trim())||le.handle).replace(/^@/,"").replace(/\s+/g,"_").toLowerCase(),le.bio=((n=y("edit-bio"))==null?void 0:n.value.trim())||"",le.location=((s=y("edit-location"))==null?void 0:s.value.trim())||"",le.website=((a=y("edit-website"))==null?void 0:a.value.trim())||"",hd(),gt("profile_saved","Profile details updated"),q(),Js(),ne("Profile saved")}function ou(e){localStorage.removeItem(dn),le.avatar=e,Ie(".avatar-option").forEach(s=>s.classList.toggle("active",s.textContent===e));let t=y("editor-avatar-preview");t&&(t.innerHTML=e);let n=y("avatar-remove-btn");n&&(n.style.display="none")}function iu(e){localStorage.removeItem(Nn),le.banner=e,Ie(".banner-option").forEach(n=>n.classList.toggle("active",n.classList.contains(e)));let t=y("editor-banner-preview");t&&(t.style.backgroundImage="",js.forEach(n=>t.classList.remove(n)),t.classList.add(e)),q()}function ru(e){var s;let t=(s=e==null?void 0:e.files)==null?void 0:s[0];if(!t)return;if(t.size>2*1024*1024){he("Image too large \u2014 max 2 MB");return}let n=new FileReader;n.onload=a=>{let o=new Image;o.onload=()=>{let i=document.createElement("canvas");i.width=i.height=200;let r=i.getContext("2d"),l=Math.min(o.width,o.height);r.drawImage(o,(o.width-l)/2,(o.height-l)/2,l,l,0,0,200,200);let c=i.toDataURL("image/jpeg",.85);localStorage.setItem(dn,c);let d=y("editor-avatar-preview");d&&(d.innerHTML=`<img src="${c}" class="profile-avatar-img"/>`);let u=y("avatar-remove-btn");u&&(u.style.display=""),q(),ne("Profile photo updated")},o.src=a.target.result},n.readAsDataURL(t),e.value=""}function lu(){localStorage.removeItem(dn);let e=y("editor-avatar-preview");e&&(e.innerHTML=le.avatar||"\u{1F30A}");let t=y("avatar-remove-btn");t&&(t.style.display="none"),q()}function cu(e){var s;let t=(s=e==null?void 0:e.files)==null?void 0:s[0];if(!t)return;if(t.size>5*1024*1024){he("Image too large \u2014 max 5 MB");return}let n=new FileReader;n.onload=a=>{let o=new Image;o.onload=()=>{let i=document.createElement("canvas");i.width=900,i.height=180;let r=i.getContext("2d"),l=Math.max(900/o.width,180/o.height);r.drawImage(o,(900-o.width*l)/2,(180-o.height*l)/2,o.width*l,o.height*l);let c=i.toDataURL("image/jpeg",.88);localStorage.setItem(Nn,c);let d=y("editor-banner-preview");d&&(d.style.backgroundImage=`url(${c})`,d.style.backgroundSize="cover",d.style.backgroundPosition="center",js.forEach(p=>d.classList.remove(p)));let u=y("banner-remove-btn");u&&(u.style.display=""),q(),ne("Banner updated")},o.src=a.target.result},n.readAsDataURL(t),e.value=""}function du(){localStorage.removeItem(Nn);let e=y("editor-banner-preview");e&&(e.style.backgroundImage="",js.forEach(n=>e.classList.remove(n)),e.classList.add(le.banner||"banner-ocean"));let t=y("banner-remove-btn");t&&(t.style.display="none"),q()}function n0(){let e=oe.map(({id:n,label:s,address:a,algo:o,emoji:i,color:r,testnet:l,watchOnly:c,createdAt:d})=>({id:n,label:s,address:a,algo:o,emoji:i,color:r,testnet:l,watchOnly:c,createdAt:d})),t=document.createElement("a");t.href="data:application/json;charset=utf-8,"+encodeURIComponent(JSON.stringify(e,null,2)),t.download=`nalulf-wallets-${new Date().toISOString().slice(0,10)}.json`,t.click(),gt("backup_exported","Wallet addresses exported"),ne("Wallet addresses exported")}function pu(){n0()}function ji(){var e;ht=1,we={algo:"ed25519",label:"",emoji:"\u{1F48E}",color:"#50fa7b",seed:"",address:"",passphrase:""},En.clear(),Wi(1),a0(),s0(),(e=y("wallet-creator-overlay"))==null||e.classList.add("show"),setTimeout(()=>{var t;return(t=y("wallet-label-input"))==null?void 0:t.focus()},80)}function Ys(){var e;(e=y("wallet-creator-overlay"))==null||e.classList.remove("show"),we.seed=we.address=we.passphrase=""}function s0(){let e=y("wizard-security-banner");e&&(e.innerHTML=`<div class="wsb-icon">\u{1F510}</div>
    ...
    </div>`).join("")}async function r0(){var n;let e=await sd(we.seed,we.passphrase),t={id:crypto.randomUUID(),label:we.label,address:we.address,algo:we.algo,emoji:we.emoji,color:we.color,testnet:((n=y("wallet-testnet-check"))==null?void 0:n.checked)||!1,watchOnly:!1,encSeed:e,createdAt:new Date().toISOString()};oe.push(t),Ws(),it||(it=t.id,te(Us,t.id)),vt(),Nt(),Ua("wallet-success-address",we.address),setTimeout(()=>{we.seed="",we.address="",we.passphrase=""},100),gt("wallet_created",we.label||"New XRPL Wallet"),ne("Wallet created and encrypted locally"),Rn(t.address).then(()=>vt())}function fu(e){we.algo=e,Ie(".algo-card").forEach(t=>t.classList.toggle("active",t.dataset.algo===e))}function hu(e){we.emoji=e,Ie(".wallet-emoji-opt").forEach(t=>t.classList.toggle("active",t.textContent===e))}function gu(e){we.color=e,Ie(".color-swatch").forEach(t=>t.classList.toggle("active",t.style.background===e||t.dataset.color===e))}function vu(e){let t=document.querySelector(`.security-check-${e}`);if(!t)return;let n=t.querySelector(".check-box");En.has(e)?(En.delete(e),t.classList.remove("checked"),n&&(n.textContent="")):(En.add(e),t.classList.add("checked"),n&&(n.textContent="\u2713")),bu()}function bu(){let e=y("wizard-next-btn");e&&ht===2&&(e.disabled=En.size<4)}function yu(){var t;(t=y("wizard-seed-value"))==null||t.classList.remove("blur");let e=y("seed-reveal-hint");e&&(e.style.display="none"),setTimeout(()=>{var n;return(n=y("wizard-seed-value"))==null?void 0:n.classList.add("blur")},3e4)}function wu(){let e=y("wizard-seed-value");if(!e)return;Ki(e.textContent,3e4);let t=y("btn-copy-seed");t&&(t.textContent="Copied!",t.classList.add("copied"),setTimeout(()=>{t.textContent="Copy Seed",t.classList.remove("copied")},2e3))}function xu(){let e=y("wizard-address-value")||y("wallet-success-address");if(!e)return;Ki(e.textContent);let t=y("btn-copy-addr");t&&(t.textContent="Copied!",t.classList.add("copied"),setTimeout(()=>{t.textContent="Copy",t.classList.remove("copied")},2e3))}function ku(){let e=y("import-address-modal");if(!e)return;e.querySelector("#inp-import-address").value="",e.querySelector("#inp-import-label").value="";let t=e.querySelector("#import-address-error");t&&(t.textContent=""),e.classList.add("show"),setTimeout(()=>{var n;return(n=e.querySelector("#inp-import-address"))==null?void 0:n.focus()},80)}function qi(){var e;(e=y("import-address-modal"))==null||e.classList.remove("show")}function $u(){var s,a;let e=(((s=y("inp-import-address"))==null?void 0:s.value)||"").trim(),t=(((a=y("inp-import-label"))==null?void 0:a.value)||"").trim()||"Watch Wallet",n=y("import-address-error");if(!Fe(e)){n&&(n.textContent="Enter a valid XRPL address (starts with r\u2026)");return}if(oe.find(o=>o.address===e)){n&&(n.textContent="This address is already in your list.");return}oe.push({id:"watch_"+Date.now(),label:t,address:e,algo:"\u2014",emoji:"\u{1F441}",color:"#8be9fd",testnet:!1,createdAt:new Date().toISOString(),watchOnly:!0}),Ws(),gt("watch_added",`${t} (${e.slice(0,8)}\u2026)`),qi(),vt(),Nt(),Vs(),Rn(e).then(()=>{vt(),Vs()}),ne(`\u{1F441} Watch-only wallet added: ${t}`)}function Su(){let e=y("import-seed-modal");if(!e)return;e.querySelector("#inp-import-seed").value="",e.querySelector("#inp-import-seed-pass").value="",e.querySelector("#inp-import-seed-pass-confirm").value="",e.querySelector("#inp-import-seed-label").value="";let t=e.querySelector("#import-seed-error");t&&(t.textContent=""),e.classList.add("show"),setTimeout(()=>{var n;return(n=e.querySelector("#inp-import-seed"))==null?void 0:n.focus()},80)}function Vi(){var e;(e=y("import-seed-modal"))==null||e.classList.remove("show")}async function Tu(){var r,l,c,d,u;let e=(((r=y("inp-import-seed"))==null?void 0:r.value)||"").trim(),t=(((l=y("inp-import-seed-label"))==null?void 0:l.value)||"").trim()||"Imported Wallet",n=(((c=y("inp-import-seed-pass"))==null?void 0:c.value)||"").trim(),s=(((d=y("inp-import-seed-pass-confirm"))==null?void 0:d.value)||"").trim(),a=y("import-seed-error"),o=y("import-seed-btn"),i=p=>{a&&(a.textContent=p)};if(i(""),!e)return i("Enter your seed phrase.");if(n.length<10)return i("Use a wallet password with at least 10 characters.");if(n!==s)return i("Wallet password confirmation does not match.");if(await ja(),!window.xrpl)return i("xrpl.js not loaded \u2014 cannot derive address from seed.");o&&(o.disabled=!0,o.textContent="Importing\u2026");try{let p=window.xrpl.Wallet.fromSeed(e),m=p.address,f=(u=p.algorithm)!=null&&u.toLowerCase().includes("ed")?"ed25519":"secp256k1";if(oe.find(b=>b.address===m))return i("This address is already in your vault.");let v="imp_"+Date.now(),h="\u{1F511}",w="#bd93f9",k=await sd(e,n);oe.push({id:v,label:t,address:m,algo:f,emoji:h,color:w,testnet:!1,watchOnly:!1,encSeed:k,createdAt:new Date().toISOString()}),Ws(),gt("wallet_imported",`${t} (${m.slice(0,8)}\u2026)`),Vi(),vt(),Nt(),Rn(m).then(()=>{vt(),Vs()}),ne(`\u{1F511} Wallet imported: ${t}`)}catch(p){i("Invalid seed: "+(p.message||"Could not derive wallet."))}finally{o&&(o.disabled=!1,o.textContent="Import Wallet \u2192");let p=document.getElementById("inp-import-seed");p&&(p.value="");let m=document.getElementById("inp-import-seed-pass");m&&(m.value="");let f=document.getElementById("inp-import-seed-pass-confirm");f&&(f.value="")}}function Cu(e,t,n){var d;let s=y("token-details-modal");if(!s)return;let a=s.querySelector(".generic-modal");if(!a)return;let o=Ke[n],i=(d=o==null?void 0:o.tokens)==null?void 0:d.find(u=>u.currency===e&&u.issuer===t),r=i?R(parseFloat(i.balance||0),6):"\u2014",l=i!=null&&i.limit?R(parseFloat(i.limit),2):"Unlimited",c=e.length>4&&Ji(e)||e;a.innerHTML=`
    <div class="tdm-hdr">
    ...
    </div>`,s.classList.add("show")}function Pu(){let e=y("token-details-modal");e&&(e.classList.remove("show"),e.style.display="")}function Lu(){var s;(s=document.getElementById("pub-profile-overlay"))==null||s.remove();let e=localStorage.getItem(dn),t=ps.filter(a=>je[a.id]),n=document.createElement("div");n.id="pub-profile-overlay",n.className="pub-profile-overlay",n.innerHTML=`
    <div class="pub-profile-modal">
      <div class="pub-banner ${le.banner||"banner-ocean"}" ${localStorage.getItem(Nn)?`style="background-image:url(${localStorage.getItem(Nn)});background-size:cover;background-position:center;"`:""}>
      </div>
      <div class="pub-hdr">
    ...
    </div>`,document.body.appendChild(n),requestAnimationFrame(()=>requestAnimationFrame(()=>n.classList.add("show"))),n.addEventListener("click",a=>{a.target===n&&n.remove()})}function Mu(e){hn(e),pn(),gt("theme_changed",e)}function Au(e){te("nalulf_pref_currency",e),pn(),ne(`Display currency: ${e}`)}function Eu(e){te("nalulf_pref_network",e),pn(),ne(`Default network: ${e}`)}function Nu(e){te("nalulf_pref_autolock",e),pn(),ne(`Auto-lock: ${e} minutes`)}window._profileWipeAllData=()=>{confirm("Clear all profile, wallet list, social, and activity data? Encrypted wallet seeds saved on this device will be deleted.")&&(["nalulf_profile","nalulf_wallets","nalulf_social","nalulf_activity_log","nalulf_avatar_img","nalulf_banner_img","naluxrp_active_wallet"].forEach(e=>localStorage.removeItem(e)),oe=[],je={},it=null,Ke={},Pi={},yi(),q(),Ni("wallets"),ne("Local data cleared"))};function l0(){var e,t,n;(e=y("profile-editor-modal"))==null||e.addEventListener("click",s=>{s.target===s.currentTarget&&Js()}),(t=y("wallet-creator-overlay"))==null||t.addEventListener("click",s=>{s.target===s.currentTarget&&Ys()}),(n=y("social-modal"))==null||n.addEventListener("click",s=>{s.target===s.currentTarget&&ms()})}function c0(){let e=[{done:!!le.displayName&&le.displayName!=="Anonymous",label:"Display name"},{done:!!le.bio,label:"Bio"},{done:le.avatar!=="\u{1F30A}"||!!localStorage.getItem(dn),label:"Custom avatar"},{done:!!localStorage.getItem(Nn),label:"Custom banner"},{done:oe.length>0,label:"Wallet added"},{done:Object.keys(je).length>=1,label:"Social connected"},{done:!!le.location,label:"Location set"},{done:!!le.website,label:"Website added"}],t=e.filter(n=>n.done).length;return{pct:Math.round(t/e.length*100),done:t,total:e.length,checks:e}}function d0(){let e=document.getElementById("profile-completeness");if(!e)return;let{pct:t,checks:n}=c0(),s=t===100?"#50fa7b":t>=60?"#00fff0":"#ffb86c",a=2*Math.PI*16,o=t/100*a,i=n.filter(r=>!r.done).map(r=>r.label);e.title=t===100?"Profile complete \u2713":`${t}% \u2014 Missing: ${i.join(", ")}`,e.innerHTML=`
    <div class="pc-wrap">
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&Qs()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&e.classList.contains("show")&&Qs()})}function v0(e){let t=u0[e];if(!t)return;g0();let n=document.getElementById("learnModalOverlay"),s=document.getElementById("learnModalTitle"),a=document.getElementById("learnModalSub"),o=document.getElementById("learnModalBody"),i=document.getElementById("learnModalLinks"),r=document.getElementById("learnModalCtas");!n||!s||!a||!o||!i||!r||(s.textContent=t.title,a.textContent=t.subtitle||"",o.innerHTML=(t.sections||[]).map(l=>`
  ...
  `).join(""),n.classList.add("show"),document.body.classList.add("modal-open"))}function Qs(){var e;(e=document.getElementById("learnModalOverlay"))==null||e.classList.remove("show"),document.body.classList.remove("modal-open")}function b0(e){var t,n,s;if(e==="modal:close")return Qs();if(e==="auth:signup"){Qs(),(t=window.openAuth)==null||t.call(window,"signup");return}if(e==="auth:login"){Qs(),(n=window.openAuth)==null||n.call(window,"login");return}if(e.startsWith("topic:"))return v0(e.split(":")[1]);if(e.startsWith("scroll:")){let a=e.split(":")[1];(s=document.getElementById(a))==null||s.scrollIntoView({behavior:"smooth",block:"start"})}}function y0(){_u||(_u=!0,document.addEventListener("click",e=>{var s,a;let t=(a=(s=e.target).closest)==null?void 0:a.call(s,"[data-action]");if(!t)return;let n=t.getAttribute("data-action")||"";n&&(e.preventDefault(),b0(n))}))}function Ru(){var l,c,d,u,p;y0(),(l=document.querySelector(".landing-features"))==null||l.setAttribute("id","learn"),(c=document.querySelector(".landing-simple-value"))==null||c.setAttribute("id","use-cases"),(d=document.querySelector(".landing-cta"))==null||d.setAttribute("id","about"),(u=document.querySelector(".landing-tagline"))==null||u.classList.add("tagline-pill");let e=document.querySelector(".landing-stats-strip");e&&(e.classList.add("landing-learn-strip"),e.innerHTML=m0.map(m=>`
    ...
    `).join(""));let t=y("features-grid");t&&(t.innerHTML=f0.map(m=>`
    ...
    `).join(""));let n=document.querySelector(".landing-simple-value"),s=n==null?void 0:n.querySelector("h2");s&&(s.textContent="What you can do with NaluLF");let a=y("value-grid");a&&(a.innerHTML=h0.map(m=>`
    ...
    `)}function Du(){let e=new IntersectionObserver(t=>t.forEach(n=>{n.isIntersecting&&n.target.classList.add("visible")}),{threshold:.12});document.querySelectorAll(".reveal").forEach(t=>e.observe(t))}function Iu(){let e=document.getElementById("particle-canvas");if(!e)return;let t=e.getContext("2d"),n,s,a=[],o=80,i=120;function r(){n=e.width=window.innerWidth,s=e.height=window.innerHeight}function l(){return{x:Math.random()*n,y:Math.random()*s,vx:(Math.random()-.5)*.4,vy:(Math.random()-.5)*.4,r:Math.random()*1.5+.5,alpha:Math.random()*.5+.1}}function c(){r(),a=Array.from({length:o},l)}function d(){return getComputedStyle(document.body).getPropertyValue("--accent-secondary").trim()||"#ffd700"}function u(){t.clearRect(0,0,n,s);let v=d();for(let h=0;h<a.length;h++)for(let w=h+1;w<a.length;w++){let k=a[h].x-a[w].x,b=a[h].y-a[w].y,x=Math.sqrt(k*k+b*b);x<i&&(t.beginPath(),t.moveTo(a[h].x,a[h].y),t.lineTo(a[w].x,a[w].y),t.strokeStyle=`rgba(255,255,255,${(1-x/i)*.06})`,t.lineWidth=1,t.stroke())}a.forEach(h=>{t.beginPath(),t.arc(h.x,h.y,h.r,0,Math.PI*2),t.fillStyle=`rgba(255,255,255,${h.alpha*.6})`,t.fill()})}function p(){a.forEach(v=>{v.x+=v.vx,v.y+=v.vy,(v.x<0||v.x>n)&&(v.vx*=-1),(v.y<0||v.y>s)&&(v.vy*=-1)})}let m;function f(){p(),u(),m=requestAnimationFrame(f)}c(),f(),document.addEventListener("visibilitychange",()=>{document.hidden?(m&&cancelAnimationFrame(m),m=null):!m&&O.currentPage==="landing"&&f()}),window.addEventListener("naluxrp:pagechange",v=>{var h;((h=v==null?void 0:v.detail)==null?void 0:h.pageId)!=="landing"?(m&&cancelAnimationFrame(m),m=null):m||f()}),window.addEventListener("resize",()=>{r(),a.forEach(v=>{v.x=Math.min(v.x,n),v.y=Math.min(v.y,s)})}),e.style.cssText=`
    position: fixed; inset: 0;
    width: 100%; height: 100%;
    pointer-events: none;
    z-index: 0; opacity: 0.4;
  `}function fs(e){var t;(t=window.showProfile)==null||t.call(window),setTimeout(()=>e==null?void 0:e(),250)}var Yi=[{label:"\u{1F30A} Live Stream",hint:"Dashboard \u2192 Stream tab",action:()=>bn(document.querySelector('[data-tab="stream"]'),"stream")},{label:"\u{1F50D} Inspector",hint:"Dashboard \u2192 Inspector tab",action:()=>bn(document.querySelector('[data-tab="inspector"]'),"inspector")},{label:"\u{1F4E1} Network Health",hint:"Dashboard \u2192 Network tab",action:()=>bn(document.querySelector('[data-tab="network"]'),"network")},{label:"\u{1F464} Profile",hint:"View your profile",action:()=>{var e;return(e=window.showProfile)==null?void 0:e.call(window)}},{label:"\u{1F48E} Wallets",hint:"Profile \u2192 Wallets",action:()=>fs(()=>{var e;return(e=window.switchProfileTab)==null?void 0:e.call(window,"wallets")})},{label:"\u{1F4CA} Portfolio Analytics",hint:"Profile \u2192 Balance history, heatmap, flow",action:()=>fs(()=>{var e;return(e=window.switchProfileTab)==null?void 0:e.call(window,"analytics")})},{label:"\u{1F517} Social Links",hint:"Profile \u2192 Connect Discord, X, GitHub\u2026",action:()=>fs(()=>{var e;return(e=window.switchProfileTab)==null?void 0:e.call(window,"social")})},{label:"\u{1F4DC} Activity Log",hint:"Profile \u2192 Recent account activity",action:()=>fs(()=>{var e;return(e=window.switchProfileTab)==null?void 0:e.call(window,"activity")})},{label:"\u2699\uFE0F Settings",hint:"Profile \u2192 Preferences & AI Explanations",action:()=>fs(()=>{var e;return(e=window.switchProfileTab)==null?void 0:e.call(window,"settings")})},{label:"\u{1F52C} Analyze a Project",hint:"Liquidity depth, holder/LP concentration, issuer risk",action:()=>fs(()=>{var e;return(e=window.jumpToProjectIntelLookup)==null?void 0:e.call(window)})},{label:"\u{1F511} Sign In",hint:"Open auth",action:()=>{var e;return(e=window._openAuth)==null?void 0:e.call(window,"login")}},{label:"\u2728 Sign Up",hint:"Create account",action:()=>{var e;return(e=window._openAuth)==null?void 0:e.call(window,"signup")}},{label:"\u{1F3E0} Landing Page",hint:"Go home",action:()=>{var e;return(e=window._goHome)==null?void 0:e.call(window)}},{label:"\u{1F3A8} Cycle Theme",hint:"gold \u2192 cosmic \u2192 starry \u2192",action:()=>{var e;return(e=window._cycleTheme)==null?void 0:e.call(window)}}];function w0(e){let t=e.trim();return Fe(t)?[{label:`\u{1F50D} Inspect ${t.slice(0,8)}\u2026${t.slice(-6)}`,hint:"Run the full account inspection on this address",action:()=>{bn(document.querySelector('[data-tab="inspector"]'),"inspector"),setTimeout(()=>{var s;let n=document.getElementById("inspect-addr");n&&(n.value=t),(s=window.runInspect)==null||s.call(window)},200)}}]:[]}var Dn=0,Ja=[...Yi];function Qi(e=""){let t=y("cmdkOverlay"),n=y("cmdkInput");!t||!n||(t.classList.add("show"),n.value=e||"",Bu(e),n.focus())}function hs(){var e;(e=y("cmdkOverlay"))==null||e.classList.remove("show")}function Ou(){let e=y("cmdkOverlay"),t=y("cmdkInput");!e||!t||(t.addEventListener("input",()=>Bu(t.value)),t.addEventListener("keydown",n=>{n.key==="ArrowDown"&&(n.preventDefault(),Fu(1)),n.key==="ArrowUp"&&(n.preventDefault(),Fu(-1)),n.key==="Enter"&&(n.preventDefault(),x0()),n.key==="Escape"&&hs()}),e.addEventListener("click",n=>{n.target===e&&hs()}))}function Bu(e=""){let t=y("cmdkList");if(!t)return;let n=e.toLowerCase().trim(),s=w0(e);Ja=n?[...s,...Yi.filter(a=>a.label.toLowerCase().includes(n)||a.hint.toLowerCase().includes(n))]:[...s,...Yi],Dn=0,t.innerHTML=Ja.length?Ja.map((a,o)=>`
        <button class="cmdk-item${o===0?" is-active":""}" data-index="${o}">
          <span class="cmdk-label">${g(a.label)}</span>
          <span class="cmdk-hint2">${g(a.hint)}</span>
        </button>`).join(""):'<div class="cmdk-section-label">No results</div>',t.querySelectorAll(".cmdk-item").forEach(a=>{a.addEventListener("click",()=>Xu(Number(a.dataset.index)))})}function Fu(e){var n,s,a;let t=Ie("#cmdkList .cmdk-item");t.length&&((n=t[Dn])==null||n.classList.remove("is-active"),Dn=(Dn+e+t.length)%t.length,(s=t[Dn])==null||s.classList.add("is-active"),(a=t[Dn])==null||a.scrollIntoView({block:"nearest"}))}function Xu(e){let t=Ja[e];t&&(hs(),t.action())}function x0(){Xu(Dn)}var Ya=!1;function Zi(){let e=O.currentPage,t=O.currentTab,n=e==="dashboard"&&t==="stream",s=e==="dashboard"&&t==="inspector",a=e==="dashboard"&&t==="network";Pl(n),Kl(s),Ac(a)}function Hu(){Ya||(Cl(),Gl(),Mc(),fd(),Zi(),Ya=!0)}window.openAuth=e=>po(e);window.closeAuth=()=>Vt();window.showAuthView=e=>qt(e);window.authKeydown=e=>Br(e);window.submitSignIn=()=>mo();window.submitSignUp=()=>fo();window.refreshCaptcha=()=>Tr();window.showForgotView=()=>Ar();window.forgotRestoreFromFile=()=>Er();window.forgotWipeConfirm=()=>Nr();window.forgotWipeExecute=()=>_r();window.forgotBackToOptions=()=>Rr();window.submitSyncImport=()=>ho();window.exportVaultSyncCode=()=>Dr();window.syncImportFromFile=()=>Ir();window.signupNext=()=>uo();window.signupBack=()=>Sr();window.logout=()=>Fr();window.goHome=()=>gn();window.showLandingPage=()=>gn();window.showDashboard=()=>vn();window.showProfile=()=>ao();window.switchTab=(e,t)=>bn(e,t);window.runInspect=()=>ts();window.closeCommandPalette=()=>hs();window.setTheme=e=>hn(e);window.cycleTheme=()=>no();window.measureLatency=()=>Is();window.switchProfileTab=e=>Ni(e);window.openProfileEditor=()=>su();window.closeProfileEditor=()=>Js();window.saveProfileEditor=()=>au();window.selectAvatar=e=>ou(e);window.selectBanner=e=>iu(e);window.uploadAvatarImage=e=>ru(e);window.removeAvatarImage=()=>lu();window.uploadBannerImage=e=>cu(e);window.removeBannerImage=()=>du();window.prefSetTheme=e=>Mu(e);window.setPrefCurrency=e=>Au(e);window.setPrefNetwork=e=>Eu(e);window.setPrefAutoLock=e=>Nu(e);window.openPublicProfilePreview=()=>Lu();window.exportVaultBackup=()=>pu();window.logActivity=(e,t)=>gt(e,t);window.toggleWalletDrawer=e=>Kp(e);window.switchWalletDrawerTab=(e,t)=>Jp(e,t);window.cancelOffer=(e,t,n)=>Qp(e,t,n);window.fetchBalance=e=>Rn(e);window.setActiveWallet=e=>bd(e);window.openImportAddressModal=()=>ku();window.closeImportAddressModal=()=>qi();window.importWatchOnlyWallet=()=>$u();window.openImportSeedModal=()=>Su();window.closeImportSeedModal=()=>Vi();window.executeImportFromSeed=()=>Tu();window.openTokenDetailsModal=(e,t,n)=>Cu(e,t,n);window.closeTokenDetailsModal=()=>Pu();window.refreshXrplDashboard=()=>cs();window.refreshMarketData=()=>Fp();window.refreshNftGallery=()=>Op();window.refreshAmmPools=()=>Bp();window.refreshPoolExplorer=()=>Xp();window.loadCustomAmmPool=()=>Md();window.sendNft=e=>Hp(e);window.toggleSeedBackupStatus=()=>Xd();window.setDexPair=e=>Hd(e);window.setDexInterval=e=>zd(e);window.setDexChartType=e=>Ud(e);window.refreshDexChart=()=>jd();window.setComparePair=e=>ap(e);window.toggleIndicator=(e,t)=>Wd(e,t);window.toggleTerminalTheme=()=>gp();window.toggleChartFullscreen=()=>vp();window.exportChartPng=()=>bp();window.saveChartLayoutPreset=()=>wp();window.loadChartLayoutPreset=()=>xp();window.setIndicatorFromDropdown=e=>op(e);window.toggleIndicatorMenu=()=>qd();window.toggleChartMoreMenu=()=>Vd();window.setIndicatorQuery=e=>Gd(e);window.addIndicatorFromMenu=e=>Kd(e);window.removeIndicator=e=>Jd(e);window.openIndicatorSettings=e=>Yd(e);window.closeIndicatorSettings=()=>Qd();window.applyIndicatorSettings=e=>Zd(e);window.resetIndicatorSettings=e=>ep(e);window.copyChartLink=()=>tp();window.toggleThreeEffects=()=>np();window.setThreeEffects=e=>sp(e);window.editAiSettings=e=>Zp(e);window.saveAiSettings=()=>eu();window.clearAiSettings=()=>tu();window.setDrawingTool=e=>ip(e);window.clearAllDrawings=()=>cp();window.addPriceAlert=()=>rp();window.removePriceAlert=e=>lp(e);window.zoomChartIn=()=>dp();window.zoomChartOut=()=>pp();window.panChartLeft=()=>up();window.panChartRight=()=>mp();window.toggleEducationPanel=()=>fp();window.selectEducationTab=e=>hp(e);window.searchTokens=e=>Sp(e);window.lookupIssuedAsset=()=>Oi();window.lookupAndAnalyzeProject=()=>Dp();window.addTokenToWatchlist=e=>Ep(e);window.removeTokenFromWatchlist=e=>Np(e);window.openTokenOnChart=e=>Ka(e);window.loadToken=e=>_p(e);window.refreshTokenDiscovery=()=>Rp();window.refreshRecentTransactions=()=>Ip();window.setTokenFilter=(e,t)=>Tp(e,t);window.clearTokenFilters=()=>Cp();window.selectTokenDetails=e=>Ap(e);window.openProjectIntel=e=>Di(e);window.jumpToProjectIntelLookup=()=>wd();window.toggleProjectIntelSubScore=e=>xd(e);window.closeProjectIntel=()=>kd();window.showMoreIssuedTokens=()=>Pp();window.showAllIssuedTokens=()=>Lp();window.resetIssuedTokenLimit=()=>Mp();window.openWalletCreator=()=>ji();window.closeWalletCreator=()=>Ys();window.wizardNext=()=>uu();window.wizardBack=()=>mu();window.selectAlgo=e=>fu(e);window.selectWalletEmoji=e=>hu(e);window.selectWalletColor=e=>gu(e);window.toggleSecurityCheck=e=>vu(e);window.revealSeed=()=>yu();window.copySeed=()=>wu();window.copyAddress=()=>xu();window.copyToClipboard=e=>Gi(e);window.deleteWallet=e=>Vp(e);window.inspectWalletAddr=e=>Gp(e);window.openSocialModal=e=>Up(e);window.closeSocialModal=()=>ms();window.saveSocialModal=()=>jp();window.deleteSocial=()=>Wp();window.viewSocial=e=>qp(e);window._openAuth=po;window._goHome=gn;window._cycleTheme=no;window._showProfile=ao;document.addEventListener("DOMContentLoaded",()=>{var e;console.log("\u{1F30A} NaluLF: booting\u2026"),xr(),gn(),Ru(),Du(),Iu(),Ou(),k0(),window.addEventListener("naluxrp:pagechange",t=>{var s;let n=(s=t==null?void 0:t.detail)==null?void 0:s.pageId;(n==="dashboard"||n==="inspector"||n==="profile")&&Hu(),Ya&&Zi()}),window.addEventListener("naluxrp:tabchange",()=>{Ya&&Zi()}),document.addEventListener("keydown",t=>{var s;let n=["INPUT","TEXTAREA"].includes((s=document.activeElement)==null?void 0:s.tagName);if((t.ctrlKey||t.metaKey)&&t.key==="k"){t.preventDefault(),Qi();return}if(t.key==="/"&&!n){t.preventDefault(),Qi();return}t.key==="Escape"&&(hs(),Vt(),Js(),Ys(),ms())}),(e=document.getElementById("auth-overlay"))==null||e.addEventListener("click",t=>{t.target===t.currentTarget&&Vt()}),Or()&&(Hu(),vn(),import("./xrpl-6AERZ5KD.js").then(({connectXRPL:t})=>t())),console.log("\u2705 NaluLF: ready")});function k0(){Uu(),setInterval(()=>{document.hidden||Uu()},3e4)}async function zu(e){let t=[s=>`https://corsproxy.io/?${encodeURIComponent(s)}`,s=>`https://api.allorigins.win/raw?url=${encodeURIComponent(s)}`],n=async s=>{let a=typeof(AbortSignal==null?void 0:AbortSignal.timeout)=="function"?AbortSignal.timeout(7e3):void 0,o=await fetch(s,{mode:"cors",cache:"no-store",signal:a});if(!o.ok)throw new Error(`HTTP ${o.status}`);return await o.json()};try{return await n(e)}catch{for(let s of t)try{return await n(s(e))}catch{}throw new Error("Price feed unreachable")}}async function Uu(){try{let e=await zu("https://api.exchange.coinbase.com/products/XRP-USD/ticker"),t=await zu("https://api.exchange.coinbase.com/products/XRP-USD/candles?granularity=86400");if(e!=null&&e.price){let n=Number(e.price),s=Array.isArray(t)&&t.length?t[0]:null,a=s?Number(s[3]||n):n,o=a?(n-a)/a*100:0,i=document.getElementById("xrpPrice"),r=document.getElementById("xrpChange");if(i&&(i.textContent=`$${Number(n).toFixed(3)}`),r){let l=o>=0;r.textContent=`${l?"+":""}${Number(o).toFixed(2)}%`,r.className=`xrp-price-change ${Math.abs(o)<.1?"flat":l?"up":"down"}`}}}catch{}}
